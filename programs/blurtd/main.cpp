...
void info() {
  ...
  std::cerr << "------------------------------------------------------\n";
  auto initminer_private_key =
      blurt::utilities::key_to_wif(BLURT_INIT_PRIVATE_KEY);
...
int main(int argc, char **argv) {
  try {
    ...
    options.add_options()("backtrace",
                          bpo::value<string>()->default_value("yes"),
    ...
    bool initialized =
        appbase::app()
            .initialize<blurt::plugins::chain::chain_plugin,
                        blurt::plugins::p2p::p2p_plugin,
                        blurt::plugins::webserver::webserver_plugin>(argc,
                                                                     argv);
    ...
    try {
      fc::optional<fc::logging_config> logging_config =
          blurt::utilities::load_logging_config(args,
                                                appbase::app().data_dir());
      if (logging_config)
