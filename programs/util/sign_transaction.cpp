  ...
  if (argc > 1 && !strncmp(argv[1], CHAIN_ID_PARAM, chainIdLen)) {
    ...
    if (*strChainId == '=') {
      ...
      if (*strChainId == '\0') {
        if (argc == 2) {
          /// --chain-id=
          std::cerr << "Missing parameter for `--chain-id' option. Option "
                       "ignored, default chain-id used."
                    << std::endl;
          std::cerr << "Usage: sign_transaction [--chain-id=<hex-chain-id>]"
                    << std::endl;
        } else {
          /// --chain-id= XXXXX
      ...
      std::cerr << "Missing parameter for `--chain-id' option. Option ignored, "
                   "default chain-id used."
                << std::endl;
      std::cerr << "Usage: sign_transaction [--chain-id=<hex-chain-id>]"
                << std::endl;
    }
    ...
    if (*strChainId != '\0') {
      try {
        ...
        chainId = parsedId;
        std::cerr << "Using explicit chain-id: `" << strChainId << "'"
                  << std::endl;
      } catch (const fc::exception &e) {
        std::cerr << "Specified explicit chain-id : `" << strChainId
                  << "' is invalid. Option ignored, default chain-id used."
                  << std::endl;
  ...
  while (std::cin) {
    ...
    try {
      ...
      if (priv_key) {
        ...
        if (sreq.wif.empty())
          std::cerr
              << "Missing Wallet Import Format in the input JSON structure"
              << std::endl;
        else
          std::cerr << "Passed JSON points to invalid data according to Wallet "
