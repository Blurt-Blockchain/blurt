...
typedef multi_index_container<
    book,
    indexed_by<
        ordered_non_unique<BOOST_MULTI_INDEX_MEMBER(book, shared_string,
                                                    author)>,
...
int main(int argc, char **argv, char **envp) {
  try {
    ...
    b.deq.push_back( shared_string( "hello world", basic_string_allocator(
    seg.get_segment_manager() )  ) ); idump((b));
    */
#ifndef ENABLE_MIRA
    book_container *pbc =
        seg.find_or_construct<book_container>("book container")(
...
#ifndef ENABLE_MIRA
    t_deque<book> *deq = seg.find_or_construct<chainbase::t_deque<book>>(
        "book deque")(allocator<book>(seg.get_segment_manager()));
#else
