...
void check_variant(const std::string &s,
                   const blurt::protocol::fixed_string_impl<Storage> &fs) {
  fc::variant vs, vfs;
  fc::to_variant(s, vs);
  fc::to_variant(fs, vfs);
  if (!(vs.is_string() && vfs.is_string() && (vs.get_string() == s) &&
        (vfs.get_string() == s))) {
...
template <typename Storage>
void check_pack(const std::string &s,
                const blurt::protocol::fixed_string_impl<Storage> &fs) {
...
int main(int argc, char **argv, char **envp) {
  ...
  */
  ...
  std::cout << "checking conversions, size(), comparison operators"
            << std::endl;
  ...
  std::cout << "test_fixed_string_16 found " << errors << " errors"
            << std::endl;
  ...
  std::cout << "test_fixed_string_24 found " << errors << " errors"
            << std::endl;
  ...
  std::cout << "test_fixed_string_32 found " << errors << " errors"
            << std::endl;
