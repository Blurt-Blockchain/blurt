  ...
  for (const auto &witness : idx) {
    db.modify(witness, [&](witness_object &wobj) {
      ...
      wobj.virtual_scheduled_time =
          BLURT_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wobj.votes.value + 1);
    });
  }
}
...
void update_median_witness_props(database &db) {
  ...
  asset median_account_creation_fee =
      active[active.size() / 2]->props.account_creation_fee;
  ...
  uint32_t median_maximum_block_size =
      active[active.size() / 2]->props.maximum_block_size;

  /// sort them by account_subsidy_budget
  std::sort(active.begin(), active.end(),
            [&](const witness_object *a, const witness_object *b) {
              return a->props.account_subsidy_budget <
                     b->props.account_subsidy_budget;
            });
  int32_t median_account_subsidy_budget =
      active[active.size() / 2]->props.account_subsidy_budget;

  /// sort them by account_subsidy_decay
  std::sort(active.begin(), active.end(),
            [&](const witness_object *a, const witness_object *b) {
              return a->props.account_subsidy_decay <
                     b->props.account_subsidy_decay;
            });
  uint32_t median_account_subsidy_decay =
      active[active.size() / 2]->props.account_subsidy_decay;

  /// sort them by operation_flat_fee
  std::sort(active.begin(), active.end(),
            [&](const witness_object *a, const witness_object *b) {
              return a->props.operation_flat_fee.amount <
                     b->props.operation_flat_fee.amount;
            });
  asset median_operation_flat_fee =
      active[active.size() / 2]->props.operation_flat_fee;

  /// sort them by bandwidth_kbytes_fee
  std::sort(active.begin(), active.end(),
            [&](const witness_object *a, const witness_object *b) {
              return a->props.bandwidth_kbytes_fee.amount <
                     b->props.bandwidth_kbytes_fee.amount;
            });
  asset median_bandwidth_kbytes_fee =
      active[active.size() / 2]->props.bandwidth_kbytes_fee;

  /// sort them by pool level
  std::sort(active.begin(), active.end(),
            [&](const witness_object *a, const witness_object *b) {
              return a->available_witness_account_subsidies <
                     b->available_witness_account_subsidies;
            });
  int64_t median_available_witness_account_subsidies =
      active[active.size() / 2]->available_witness_account_subsidies;
  ...
  account_subsidy_user_params.budget_per_time_unit =
      median_account_subsidy_budget;
  account_subsidy_user_params.decay_per_time_unit =
      median_account_subsidy_decay;
  ...
  // Should never fail, as validate_user_params() is checked and median of valid
  // params should always be valid
  ...
  db.modify(wso, [&](witness_schedule_object &_wso) {
    ...
    rd_setup_dynamics_params(account_subsidy_user_params,
                             account_subsidy_system_params,
                             _wso.account_subsidy_rd);
    rd_setup_dynamics_params(account_subsidy_per_witness_user_params,
                             account_subsidy_system_params,
                             _wso.account_subsidy_witness_rd);

    int64_t median_decay =
        rd_compute_pool_decay(_wso.account_subsidy_witness_rd.decay_params,
                              median_available_witness_account_subsidies, 1);
    median_decay = std::max(median_decay, int64_t(0));
    int64_t min_decay = (fc::uint128(median_decay) *
                         BLURT_DECAY_BACKSTOP_PERCENT / BLURT_100_PERCENT)
                            .to_int64();
...
void update_witness_schedule(database &db) {
  if ((db.head_block_num() % BLURT_MAX_WITNESSES) != 0)
    return;
  ...
  const auto &widx =
      db.get_index<witness_index>().indices().get<by_vote_name>();
  for (auto itr = widx.begin();
    ...
    db.modify(*itr, [&](witness_object &wo) {
      wo.schedule = witness_object::elected;
    });
  }
  ...
  const auto &schedule_idx =
      db.get_index<witness_index>().indices().get<by_schedule_time>();
  ...
  for (auto witness_count = selected_voted.size();
       ...
       ++sitr) {
    new_virtual_time = sitr->virtual_scheduled_time; /// everyone advances to at
                                                     /// least this time
  ...
  for (auto itr = processed_witnesses.begin(); itr != processed_witnesses.end();
       ++itr) {
    auto new_virtual_scheduled_time =
        new_virtual_time +
        BLURT_VIRTUAL_SCHEDULE_LAP_LENGTH2 / ((*itr)->votes.value + 1);
  ...
  size_t expected_active_witnesses =
      std::min(size_t(BLURT_MAX_WITNESSES), widx.size());
  FC_ASSERT(active_witnesses.size() == expected_active_witnesses,
            "number of active witnesses does not equal "
            "expected_active_witnesses=${expected_active_witnesses}",
            ("active_witnesses.size()", active_witnesses.size())(
                "BLURT_MAX_WITNESSES", BLURT_MAX_WITNESSES)(
                "expected_active_witnesses", expected_active_witnesses));
...

  {
    ...
    flat_map<std::tuple<hardfork_version, time_point_sec>, uint32_t>
    ...
    for (uint32_t i = 0; i < wso.num_scheduled_witnesses; i++) {
      ...
      auto version_vote = std::make_tuple(witness.hardfork_version_vote,
    ...
    // The map should be sorted highest version to smallest, so we iterate until
    // we hit the majority of witnesses on at least this version
  ...
  db.modify(wso, [&](witness_schedule_object &_wso) {
    ...
    _wso.num_scheduled_witnesses =
        std::max<uint8_t>(active_witnesses.size(), 1);
