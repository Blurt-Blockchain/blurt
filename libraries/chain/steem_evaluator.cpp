...
#ifndef IS_LOW_MEM
FC_TODO(
    "After we vendor fc, also vendor diff_match_patch and fix these warnings")
...
inline void validate_permlink_0_1(const string &permlink) {
  FC_ASSERT(permlink.size() > BLURT_MIN_PERMLINK_LENGTH &&
                permlink.size() < BLURT_MAX_PERMLINK_LENGTH,
  ...
  for (const auto &c : permlink) {
    ...
    case '-':
      break;
    default:
      FC_ASSERT(false, "Invalid permlink character: ${s}",
                ("s", std::string() + c));
    }
  }
}
...
void copy_legacy_chain_properties(chain_properties &dest,
                                  const legacy_chain_properties &src) {
...
void witness_update_evaluator::do_apply(const witness_update_operation &o) {
...

  {
    ...
    FC_ASSERT(o.props.account_creation_fee.amount <=
                  BLURT_MAX_ACCOUNT_CREATION_FEE,
              "account_creation_fee greater than maximum account creation fee");
  }

  FC_ASSERT(o.props.maximum_block_size <= BLURT_SOFT_MAX_BLOCK_SIZE,
            "Max block size cannot be more than 2MiB");

  const auto &by_witness_name_idx =
      _db.get_index<witness_index>().indices().get<by_name>();
...
void witness_set_properties_evaluator::do_apply(
    const witness_set_properties_operation &o) {
  const auto &witness =
      _db.get<witness_object, by_name>(o.owner); // verifies witness exists;
  ...
  // This existence of 'key' is checked in
  // witness_set_properties_operation::validate
  fc::raw::unpack_from_vector(itr->second, signing_key);
  FC_ASSERT(signing_key == witness.signing_key,
            "'key' does not match witness signing key.",
  ...
  if (flags.account_creation_changed) {
...

    {
      FC_TODO("Move to validate() after HF20");
      FC_ASSERT(
          props.account_creation_fee.amount <= BLURT_MAX_ACCOUNT_CREATION_FEE,
          "account_creation_fee greater than maximum account creation fee");
    }
  }
  ...
  if (flags.operation_flat_fee_changed) {
    ...
    FC_ASSERT(props.operation_flat_fee.amount <= 100000,
              "operation_flat_fee too high!");
  }
  ...
  if (flags.bandwidth_kbytes_fee_changed) {
    ...
    FC_ASSERT(props.bandwidth_kbytes_fee.amount <= 100000,
              "bandwidth_kbytes_fee too high!");
  }
...
void verify_authority_accounts_exist(const database &db, const authority &auth,
                                     const account_name_type &auth_account,
                                     authority::classification auth_class) {
  for (const std::pair<account_name_type, weight_type> &aw :
       auth.account_auths) {
    const account_object *a = db.find_account(aw.first);
    FC_ASSERT(a != nullptr,
              "New ${ac} authority on account ${aa} references non-existing "
              "account ${aref}",
              ("aref", aw.first)("ac", auth_class)("aa", auth_account));
  }
}

void initialize_account_object(
    account_object &acc, const account_name_type &name,
    const public_key_type &key, const dynamic_global_property_object &props,
    bool mined, const account_name_type &recovery_account, uint32_t hardfork) {
  acc.name = name;
  acc.memo_key = key;
  ...
  FC_TODO("If after HF 20, there are no temp account creations, the HF check "
          "can be removed.")
...
void account_create_evaluator::do_apply(const account_create_operation &o) {
  ...
  FC_ASSERT(o.fee == wso.median_props.account_creation_fee,
            ...
            ("p", o.fee)("f", wso.median_props.account_creation_fee));

  {
    verify_authority_accounts_exist(_db, o.owner, o.new_account_name,
                                    authority::owner);
    verify_authority_accounts_exist(_db, o.active, o.new_account_name,
                                    authority::active);
    verify_authority_accounts_exist(_db, o.posting, o.new_account_name,
                                    authority::posting);
  }

  _db.adjust_balance(creator, -o.fee);
  _db.adjust_balance(_db.get<account_object, by_name>(BLURT_NULL_ACCOUNT),
                     o.fee);
...
void account_update_evaluator::do_apply(const account_update_operation &o) {
  ...
  const auto &account_auth =
      _db.get<account_authority_object, by_account>(o.account);
  ...
  if (o.owner) {
#ifndef IS_TEST_NET
    FC_ASSERT(_db.head_block_time() - account_auth.last_owner_update >
                  BLURT_OWNER_UPDATE_LIMIT,
  ...
  if (o.active)
    verify_authority_accounts_exist(_db, *o.active, o.account,
                                    authority::active);
  if (o.posting)
    verify_authority_accounts_exist(_db, *o.posting, o.account,
                                    authority::posting);
  ...
  if (o.json_metadata.size() > 0 || o.posting_json_metadata.size() > 0) {
    _db.modify(_db.get<account_metadata_object, by_account>(account.id),
               [&](account_metadata_object &meta) {
                 if (o.json_metadata.size() > 0)
                   from_string(meta.json_metadata, o.json_metadata);

                 if (o.posting_json_metadata.size() > 0)
                   from_string(meta.posting_json_metadata,
                               o.posting_json_metadata);
               });
  }
#endif
...
/**
 *  Because net_rshares is 0 there is no need to update any pending payout
 * calculations or parent posts.
 */
void delete_comment_evaluator::do_apply(const delete_comment_operation &o) {
  ...
  FC_ASSERT(comment.net_rshares <= 0,
  ...
  /// this loop can be skiped for validate-only nodes as it is merely gathering
  /// stats for indicies
  if (comment.parent_author != BLURT_ROOT_POST_PARENT) {
    auto parent =
        &_db.get_comment(comment.parent_author, comment.parent_permlink);
    ...
    while (parent) {
      ...
      if (parent->parent_author != BLURT_ROOT_POST_PARENT)
        parent =
            &_db.get_comment(parent->parent_author, parent->parent_permlink);
      else
#endif
        parent = nullptr;
    }
  ...
  void operator()(const comment_payout_beneficiaries &cpb) const {
    ...
    _db.modify(_c, [&](comment_object &c) {
      for (auto &b : cpb.beneficiaries) {
        ...
        FC_ASSERT(acc != nullptr, "Beneficiary \"${a}\" must exist.",
                  ("a", b.account));
...
void comment_options_evaluator::do_apply(const comment_options_operation &o) {
  ...
  if (!o.allow_curation_rewards || !o.allow_votes ||
    ...
    FC_ASSERT(comment.abs_rshares == 0,
              "One of the included comment options requires the comment to "
              "have no rshares allocated to it.");
  ...
  FC_ASSERT(comment.percent_blurt >= o.percent_blurt,
            "A comment cannot accept a greater percent BLURT.");
...
void comment_evaluator::do_apply(const comment_operation &o) {
  try {
    ...
    const auto &by_permlink_idx =
        _db.get_index<comment_index>().indices().get<by_permlink>();
    ...
    if (o.parent_author != BLURT_ROOT_POST_PARENT) {
      ...
      FC_ASSERT(parent->depth < BLURT_MAX_COMMENT_DEPTH,
                "Comment is nested ${x} posts deep, maximum depth is ${y}.",
                ("x", parent->depth)("y", BLURT_MAX_COMMENT_DEPTH));
    }
    ...
    if (itr == by_permlink_idx.end()) {
      ...
      FC_TODO("Cleanup this logic after HF 20. Old ops don't need to check "
              "pre-hf20 times.") {
        if (o.parent_author == BLURT_ROOT_POST_PARENT)
          FC_ASSERT((now - auth.last_root_post) >
                        BLURT_MIN_ROOT_COMMENT_INTERVAL,
                    "You may only post once every 5 minutes.",
                    ("now", now)("last_root_post", auth.last_root_post));
        else
          FC_ASSERT((now - auth.last_post) >= BLURT_MIN_REPLY_INTERVAL_HF20,
      ...
      /// this loop can be skiped for validate-only nodes as it is merely
      /// gathering stats for indicies
      //      auto now = _db.head_block_time();
      while (parent) {
        _db.modify(*parent, [&](comment_object &p) {
          p.children++;
    ...
    } else // start edit case
    {
      ...
      FC_ASSERT(now - auth.last_post_edit >= BLURT_MIN_COMMENT_EDIT_INTERVAL,
                "Can only perform one comment edit per block.");
      ...
      _db.modify(comment, [&](comment_object &com) {
        ...
        equal = [](const shared_string &a, const string &b) -> bool {
      ...
      _db.modify(auth, [&](account_object &a) { a.last_post_edit = now; });
#ifndef IS_LOW_MEM
      if (!filtered) {
        _db.modify(_db.get<comment_content_object, by_comment>(comment.id),
                   [&](comment_content_object &con) {
                     if (o.title.size())
                       from_string(con.title, o.title);
                     ...
                     if (o.body.size()) {
                       try {
                         ...
                         auto patch =
                             dmp.patch_fromText(utf8_to_wstring(o.body));
                         if (patch.size()) {
                           auto result = dmp.patch_apply(
                               patch, utf8_to_wstring(to_string(con.body)));
                           auto patched_body = wstring_to_utf8(result.first);
                           if (!fc::is_utf8(patched_body)) {
                             idump(("invalid utf8")(patched_body));
                             from_string(con.body,
                                         fc::prune_invalid_utf8(patched_body));
                           } else {
...
void escrow_transfer_evaluator::do_apply(const escrow_transfer_operation &o) {
  try {
    ...
    FC_ASSERT(
        o.ratification_deadline > _db.head_block_time(),
...
void escrow_approve_evaluator::do_apply(const escrow_approve_operation &o) {
  try {
    ...
    FC_ASSERT(escrow.ratification_deadline >= _db.head_block_time(),
              "The escrow ratification deadline has passed. Escrow can no "
              "longer be ratified.");
    ...
    if (o.who == o.agent) {
      ...
      if (!reject_escrow) {
        _db.modify(escrow,
                   [&](escrow_object &esc) { esc.agent_approved = true; });
      }
    }
    ...
    if (reject_escrow) {
      ...
      _db.modify(escrow,
                 [&](escrow_object &esc) { esc.pending_fee.amount = 0; });
    }
  }
  FC_CAPTURE_AND_RETHROW((o))
}
...
void escrow_dispute_evaluator::do_apply(const escrow_dispute_operation &o) {
  try {
    ...
    FC_ASSERT(e.to_approved && e.agent_approved,
              "The escrow must be approved by all parties before a dispute can "
              "be raised.");
...
void escrow_release_evaluator::do_apply(const escrow_release_operation &o) {
  try {
    ...
    const auto &e = _db.get_escrow(o.from, o.escrow_id);
    FC_ASSERT(
        e.blurt_balance >= o.blurt_amount,
        "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}",
        ("a", o.blurt_amount)("b", e.blurt_balance));
    FC_ASSERT(e.to == o.to,
              "Operation 'to' (${o}) does not match escrow 'to' (${e}).",
              ("o", o.to)("e", e.to));
    FC_ASSERT(e.agent == o.agent,
    ...
    // If there is a dispute regardless of expiration, the agent can release
    ...
    if (e.disputed) {
      ...
      FC_ASSERT(o.who == e.from || o.who == e.to,
                "Only 'from' (${f}) and 'to' (${t}) can release funds from a "
                "non-disputed escrow",
                ("f", e.from)("t", e.to));
    ...
    // If escrow expires and there is no dispute, either party can release funds
    // to either party.

    _db.adjust_balance(o.receiver, o.blurt_amount);
    _db.modify(
        e, [&](escrow_object &esc) { esc.blurt_balance -= o.blurt_amount; });
...
void transfer_to_vesting_evaluator::do_apply(
    const transfer_to_vesting_operation &o) {
  const auto &from_account = _db.get_account(o.from);
...
void withdraw_vesting_evaluator::do_apply(const withdraw_vesting_operation &o) {
  ...
  FC_ASSERT(
      ...
      "Cannot withdraw negative VESTS. account: ${account}, vests:${vests}",
  ...
  if (o.vesting_shares.amount == 0) {
    ...
    int vesting_withdraw_intervals =
        BLURT_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
    ...
    _db.modify(account, [&](account_object &a) {
      ...
      if (new_vesting_withdraw_rate.amount * vesting_withdraw_intervals <
...
void set_withdraw_vesting_route_evaluator::do_apply(
  ...
  try {
    ...
    const auto &wd_idx = _db.get_index<withdraw_vesting_route_index>()
                             .indices()
                             .get<by_withdraw_route>();
    auto itr =
        wd_idx.find(boost::make_tuple(from_account.name, to_account.name));
    ...
    if (itr == wd_idx.end()) {
      ...
      FC_ASSERT(from_account.withdraw_routes < BLURT_MAX_WITHDRAW_ROUTES,
                "Account already has the maximum number of routes.");

      _db.create<withdraw_vesting_route_object>(
    ...
    FC_ASSERT(
        total_percent <= BLURT_100_PERCENT,
        "More than 100% of vesting withdrawals allocated to destinations.");
  }
  FC_CAPTURE_AND_RETHROW()
}

void account_witness_proxy_evaluator::do_apply(
  ...
  if (o.proxy.size()) {
    ...
    /// check for proxy loops and fail to update the proxy if it would create a
    /// loop
    ...
    while (cprox->proxy.size() != 0) {
      ...
      cprox = &next_proxy;
      FC_ASSERT(proxy_chain.size() <= BLURT_MAX_PROXY_RECURSION_DEPTH,
    ...
    for (int i = 0; i <= BLURT_MAX_PROXY_RECURSION_DEPTH; ++i)
      delta[i] = -delta[i];
    _db.adjust_proxied_witness_votes(account, delta);
  } else { /// we are clearing the proxy which means we simply update the
           /// account
    _db.modify(account, [&](account_object &a) { a.proxy = o.proxy; });
  }
}

void account_witness_vote_evaluator::do_apply(
    const account_witness_vote_operation &o) {
  const auto &voter = _db.get_account(o.account);
  FC_ASSERT(voter.proxy.size() == 0, "A proxy is currently set, please clear "
                                     "the proxy before voting for a witness.");
  ...
  auto itr =
      by_account_witness_idx.find(boost::make_tuple(voter.name, witness.owner));
  ...
  if (itr == by_account_witness_idx.end()) {
    ...
    {
      FC_ASSERT(
          voter.witnesses_voted_for < BLURT_MAX_ACCOUNT_WITNESS_VOTES,
          "Account has voted for too many witnesses."); // TODO: Remove after
                                                        // hardfork 2
    ...
    FC_ASSERT(!o.approve, "Vote currently exists, user must indicate a desire "
                          "to reject witness.");
...
void vote_evaluator::do_apply(const vote_operation &o) {
  try {
    ...
    FC_ASSERT(o.weight >= 0,
              "Downvote is not allowed"); /// TODO: move this to validate?
    ...
    if (_db.calculate_discussion_payout_time(comment) ==
      ...
      FC_ASSERT(_db.head_block_time() < comment.cashout_time,
                "Comment is actively being rewarded. Cannot vote on comment.");
    }

    const auto &comment_vote_idx =
        _db.get_index<comment_vote_index, by_comment_voter>();
    auto itr = comment_vote_idx.find(boost::make_tuple(comment.id, voter.id));

    // Lazily delete vote
    if (itr != comment_vote_idx.end() && itr->num_changes == -1) {
      FC_TODO("This looks suspicious. We might not be deleting vote objects "
              "that we should be on nodes that are configured to clear votes");
    ...
    auto now = _db.head_block_time();
    FC_ASSERT((now - voter.last_vote_time).to_seconds() >=
                  BLURT_MIN_VOTE_INTERVAL_SEC,
    ...
    FC_ASSERT(voter.voting_manabar.current_mana >= 0,
              "Account does not have enough mana to vote.");

    int16_t abs_weight = abs(o.weight);
    uint128_t used_mana = (uint128_t(voter.voting_manabar.current_mana) *
                           abs_weight * 60 * 60 * 24) /
                          BLURT_100_PERCENT;
    int64_t max_vote_denom =
        dgpo.vote_power_reserve_rate * BLURT_VOTING_MANA_REGENERATION_SECONDS;
    ...
    uint32_t cashout_delta =
        (comment.cashout_time - _db.head_block_time()).to_seconds();

    if (cashout_delta < BLURT_UPVOTE_LOCKOUT_SECONDS) {
      abs_rshares = (int64_t)((uint128_t(abs_rshares) * cashout_delta) /
    ...
    if (itr == comment_vote_idx.end()) {
      ...
      /// if the current net_rshares is less than 0, the post is getting 0
      /// rewards so it is not factored into total rshares^2
      //      fc::uint128_t old_rshares = std::max(comment.net_rshares.value,
      //      int64_t(0));
      ...
      //      fc::uint128_t new_rshares = std::max( comment.net_rshares.value,
      //      int64_t(0) );
      //
       ...
       *
       *  cv.weight / c.total_vote_weight ==> % of rshares increase that is
       *accounted for by the vote
       *
       *  W(R) = B * R / ( R + 2S )
       *  W(R) is bounded above by B. B is fixed at 2^64 - 1, so all weights fit
       *in a 64 bit integer.
       *
       *  The equation for an individual vote is:
       *    W(R_N) - W(R_N-1), which is the delta increase of proportional
       *weight
       *
       *  c.total_vote_weight =
       *    W(R_1) - W(R_0) +
       ...
       *
       *  Since W(R_0) = 0, c.total_vote_weight is also bounded above by B and
       *will always fit in a 64 bit integer.
       *
       **/
      _db.create<comment_vote_object>([&](comment_vote_object &cv) {
        ...
        bool curation_reward_eligible =
            rshares > 0 && (comment.last_payout == fc::time_point_sec()) &&
            comment.allow_curation_rewards;

        if (curation_reward_eligible) {
          curation_reward_eligible =
              _db.get_curation_rewards_percent(comment) > 0;
        }
        ...
        if (curation_reward_eligible) {
          ...
          uint64_t old_weight =
              util::evaluate_reward_curve(old_vote_rshares.value, curve,
                                          reward_fund.content_constant)
                  .to_uint64();
          uint64_t new_weight =
              util::evaluate_reward_curve(comment.vote_rshares.value, curve,
                                          reward_fund.content_constant)
                  .to_uint64();

          if (old_weight >=
              new_weight) // old_weight > new_weight should never happen
          {
            cv.weight = 0;
          } else {
            ...
            uint64_t delta_t = std::min(
                ...
                uint64_t(dgpo.reverse_auction_seconds));
      ...
      FC_ASSERT(
          itr->num_changes < BLURT_MAX_VOTE_CHANGES,
...
void custom_evaluator::do_apply(const custom_operation &o) {
  ...
  if (d.is_producing())
    FC_ASSERT(o.data.size() <= BLURT_CUSTOM_OP_DATA_MAX_LENGTH,
              "Operation data must be less than ${bytes} bytes.",
              ("bytes", BLURT_CUSTOM_OP_DATA_MAX_LENGTH));

  FC_ASSERT(
      o.required_auths.size() <= BLURT_MAX_AUTHORITY_MEMBERSHIP,
      "Authority membership exceeded. Max: ${max} Current: ${n}",
      ("max", BLURT_MAX_AUTHORITY_MEMBERSHIP)("n", o.required_auths.size()));
}
...
void custom_json_evaluator::do_apply(const custom_json_operation &o) {
  ...
  if (d.is_producing())
    FC_ASSERT(o.json.length() <= BLURT_CUSTOM_OP_DATA_MAX_LENGTH,
              "Operation JSON must be less than ${bytes} bytes.",
              ("bytes", BLURT_CUSTOM_OP_DATA_MAX_LENGTH));

  {
    size_t num_auths =
        o.required_auths.size() + o.required_posting_auths.size();
    FC_ASSERT(num_auths <= BLURT_MAX_AUTHORITY_MEMBERSHIP,
              "Authority membership exceeded. Max: ${max} Current: ${n}",
              ("max", BLURT_MAX_AUTHORITY_MEMBERSHIP)("n", num_auths));
  }

  std::shared_ptr<custom_operation_interpreter> eval =
      d.get_custom_json_evaluator(o.id);
  if (!eval)
    return;
...
void custom_binary_evaluator::do_apply(const custom_binary_operation &o) {
  ...
  {
    size_t num_auths = o.required_owner_auths.size() +
                       o.required_active_auths.size() +
...
void claim_account_evaluator::do_apply(const claim_account_operation &o) {
  if (_db.has_hardfork(BLURT_HARDFORK_0_2))
    FC_ASSERT(false, "This operation is disable since hard fork 2.");
  ...
  if (o.fee.amount == 0) {
    ...
    // This block is a little weird. We want to enforce that only elected
    // witnesses can include the transaction, but we do not want to prevent the
    // transaction from propogating on the p2p network. Because we do not know
    // what type of witness will have produced the including block when the tx
    // is broadcast, we need to disregard this assertion when the tx is
    // propogating, but require it when applying the block.
    if (!_db.is_pending_tx()) {
      ...
      FC_ASSERT(current_witness.schedule == witness_object::elected,
                "Subsidized accounts can only be claimed by elected witnesses. "
                "current_witness:${w} witness_type:${t}",
    ...
    FC_ASSERT(gpo.available_account_subsidies >=
                  BLURT_ACCOUNT_SUBSIDY_PRECISION,
    ...
    FC_ASSERT(
        o.fee == wso.median_props.account_creation_fee,
        "Cannot pay more than account creation fee. paid: ${p} fee: ${f}",
        ("p", o.fee.amount.value)("f", wso.median_props.account_creation_fee));
  }
...
void create_claimed_account_evaluator::do_apply(
    const create_claimed_account_operation &o) {
  if (_db.has_hardfork(BLURT_HARDFORK_0_2))
    FC_ASSERT(false, "This operation is disable since hard fork 2.");
  ...
  FC_ASSERT(creator.pending_claimed_accounts > 0,
            "${creator} has no claimed accounts to create",
            ("creator", o.creator));
  ...
  _db.modify(creator, [&](account_object &a) { a.pending_claimed_accounts--; });
...
void request_account_recovery_evaluator::do_apply(
  ...
  if (request == recovery_request_idx.end()) // New Request
  {
    ...
    _db.create<account_recovery_request_object>(
        [&](account_recovery_request_object &req) {
...
void recover_account_evaluator::do_apply(const recover_account_operation &o) {
  ...
  FC_ASSERT(_db.head_block_time() - account.last_account_recovery >
                BLURT_OWNER_UPDATE_LIMIT,
            "Owner authority can only be updated once an hour.");
  ...
  const auto &recent_auth_idx = _db.get_index<owner_authority_history_index>()
                                    .indices()
                                    .get<by_account>();
  ...
  _db.remove(
      *request); // Remove first, update_owner_authority may invalidate iterator
...
void change_recovery_account_evaluator::do_apply(
  ...
  const auto &change_recovery_idx =
      _db.get_index<change_recovery_account_request_index>()
          .indices()
          .get<by_account>();
  ...
  if (request == change_recovery_idx.end()) // New request
  {
    _db.create<change_recovery_account_request_object>(
        [&](change_recovery_account_request_object &req) {
          ...
          req.effective_on =
              _db.head_block_time() + BLURT_OWNER_AUTH_RECOVERY_PERIOD;
        });
...
void transfer_to_savings_evaluator::do_apply(
    const transfer_to_savings_operation &op) {
  const auto &from = _db.get_account(op.from);
  const auto &to = _db.get_account(op.to);

  FC_ASSERT(op.to != BLURT_TREASURY_ACCOUNT, "Cannot transfer savings to ${s}",
            ("s", BLURT_TREASURY_ACCOUNT));

  _db.adjust_balance(from, -op.amount);
  _db.adjust_savings_balance(to, op.amount);
}

void transfer_from_savings_evaluator::do_apply(
    const transfer_from_savings_operation &op) {
  const auto &from = _db.get_account(op.from);
...
void cancel_transfer_from_savings_evaluator::do_apply(
...
void decline_voting_rights_evaluator::do_apply(
    const decline_voting_rights_operation &o) {
  const auto &account = _db.get_account(o.account);
  const auto &request_idx = _db.get_index<decline_voting_rights_request_index>()
                                .indices()
                                .get<by_account>();
  auto itr = request_idx.find(account.name);

  if (o.decline) {
    FC_ASSERT(itr == request_idx.end(),
              "Cannot create new request because one already exists.");

    _db.create<decline_voting_rights_request_object>(
    ...
    FC_ASSERT(itr != request_idx.end(),
              "Cannot cancel the request because it does not exist.");
    _db.remove(*itr);
  }
}
...
void reset_account_evaluator::do_apply(const reset_account_operation &op) {
  ...
  /*
     ...
     if( band != nullptr )
        FC_ASSERT( ( _db.head_block_time() - band->last_bandwidth_update ) >
     fc::days(60), "Account must be inactive for 60 days to be eligible for
     reset" ); FC_ASSERT( acnt.reset_account == op.reset_account, "Reset account
     does not match reset account on account." );

     _db.update_owner_authority( acnt, op.new_owner_authority );
  */
}

void set_reset_account_evaluator::do_apply(
    const set_reset_account_operation &op) {
  FC_ASSERT(false, "Set Reset Account Operation is currently disabled.");
  /*
     const auto& acnt = _db.get_account( op.account );
     _db.get_account( op.reset_account );

     FC_ASSERT( acnt.reset_account == op.current_reset_account, "Current reset
     account does not match reset account on account." ); FC_ASSERT(
     acnt.reset_account != op.reset_account, "Reset account must change" );
...
void claim_reward_balance_evaluator::do_apply(
    const claim_reward_balance_operation &op) {
  const auto &acnt = _db.get_account(op.account);

  FC_ASSERT(op.reward_blurt <= acnt.reward_blurt_balance,
            "Cannot claim that much BLURT. Claim: ${c} Actual: ${a}",
  ...
  if (op.reward_vests == acnt.reward_vesting_balance)
    ...
    reward_vesting_blurt_to_move =
        asset(((uint128_t(op.reward_vests.amount.value) *
                uint128_t(acnt.reward_vesting_blurt.amount.value)) /
...
void delegate_vesting_shares_evaluator::do_apply(
    const delegate_vesting_shares_operation &op) {
#pragma message(                                                               \
    "TODO: Update get_effective_vesting_shares when modifying this operation to support SMTs.")
...

  {
    ...
    available_shares =
        asset(delegator.voting_manabar.current_mana, VESTS_SYMBOL);

    // Assume delegated VESTS are used first when consuming mana. You cannot
    // delegate received vesting shares
    available_shares.amount =
        std::min(available_shares.amount,
                 max_mana - delegator.received_vesting_shares.amount);

    if (delegator.next_vesting_withdrawal < fc::time_point_sec::maximum() &&
        delegator.to_withdraw - delegator.withdrawn >
            delegator.vesting_withdraw_rate.amount) {
      /*
      ...
      The remaining withdrawal needs to be added in but then the current week is
      double counted.
      */

      auto weekly_withdraw = asset(
          std::min(
              delegator.vesting_withdraw_rate.amount.value, // Weekly amount
              delegator.to_withdraw.value -
                  delegator.withdrawn.value // Or remainder
              ),
          VESTS_SYMBOL);
  ...
  auto min_delegation =
      asset(wso.median_props.account_creation_fee.amount / 3, BLURT_SYMBOL) *
      gpo.get_vesting_share_price();
  auto min_update =
      asset(wso.median_props.account_creation_fee.amount / 30, BLURT_SYMBOL) *
      gpo.get_vesting_share_price();

  // If delegation doesn't exist, create it
  if (delegation == nullptr) {
    FC_ASSERT(
        available_shares >= op.vesting_shares,
        "Account ${acc} does not have enough mana to delegate. required: ${r} "
        "available: ${a}",
  ...
  // Else if the delegation is increasing
    ...
    FC_ASSERT(delta >= min_update,
              "Steem Power increase is not enough of a difference. min_update: "
              "${min}",
              ("min", min_update));
    FC_ASSERT(available_shares >= delta,
              "Account ${acc} does not have enough mana to delegate. required: "
              "${r} available: ${a}",
  ...
  else /* delegation->vesting_shares > op.vesting_shares */
  {
    ...
    if (op.vesting_shares.amount > 0) {
      FC_ASSERT(delta >= min_update,
                "Steem Power decrease is not enough of a difference. "
                "min_update: ${min}",
                ("min", min_update));
      FC_ASSERT(op.vesting_shares >= min_delegation,
                "Delegation must be removed or leave minimum delegation amount "
                "of ${v}",
                ("v", min_delegation));
    } else {
      FC_ASSERT(delegation->vesting_shares.amount > 0,
                "Delegation would set vesting_shares to zero, but it is "
                "already zero");
    }

    _db.create<vesting_delegation_expiration_object>(
        [&](vesting_delegation_expiration_object &obj) {
          obj.delegator = op.delegator;
          obj.vesting_shares = delta;
          obj.expiration =
              std::max(_db.head_block_time() + gpo.delegation_return_period,
