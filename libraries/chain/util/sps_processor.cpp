...
bool sps_processor::is_maintenance_period(
    const time_point_sec &head_time) const {
...
void sps_processor::remove_proposals(const time_point_sec &head_time) {
  ...
  while (itr != found) {
    itr = sps_helper::remove_proposal<by_end_date>(
        itr, proposalIndex, votesIndex, byVoterIdx, obj_perf);
    if (obj_perf.done)
      break;
  }
}

void sps_processor::find_active_proposals(const time_point_sec &head_time,
                                          t_proposals &proposals) {
  const auto &pidx =
      db.get_index<proposal_index>().indices().get<by_start_date>();
...
void sps_processor::calculate_votes(const t_proposals &proposals) {
  for (auto &item : proposals) {
    ...
    db.modify(_item,
              [&](auto &proposal) { proposal.total_votes = total_votes; });
  }
}

void sps_processor::sort_by_votes(t_proposals &proposals) {
  std::sort(proposals.begin(), proposals.end(),
            [](const proposal_object &a, const proposal_object &b) {
...
asset sps_processor::calculate_maintenance_budget(
    const time_point_sec &head_time) {
  // Get funds from 'treasury' account ( treasury_fund )
  asset treasury_fund = get_treasury_fund();

  //   FC_ASSERT( treasury_fund.symbol == daily_inflation.symbol, "symbols must
  //   be the same" );

  // Calculate budget for given maintenance period
  uint32_t passed_time_seconds =
      (head_time - db.get_dynamic_global_properties().last_budget_time)
          .to_seconds();

  // Calculate daily_budget_limit
  int64_t daily_budget_limit =
      treasury_fund.amount.value / total_amount_divider;

  daily_budget_limit =
      ((uint128_t(passed_time_seconds) * daily_budget_limit) / daily_seconds)
          .to_uint64();

  return asset(daily_budget_limit, treasury_fund.symbol);
}

void sps_processor::transfer_payments(const time_point_sec &head_time,
  ...
  auto processing = [this, &treasury_account](const proposal_object &_item,
                                              const asset &payment) {
    const auto &receiver_account = db.get_account(_item.receiver);

    operation vop =
        proposal_pay_operation(_item.receiver, payment, db.get_current_trx(),
                               db.get_current_op_in_trx());
    /// Push vop to be recorded by other parts (like AH plugin etc.)
    db.push_virtual_operation(vop);
    /// Virtual ops have no evaluators, so operation must be immediately
    /// "evaluated"
  ...
  for (auto &item : proposals) {
    ...
    asset period_pay = asset(
        (ratio * _item.daily_pay.amount.value).to_uint64() / BLURT_100_PERCENT,
        _item.daily_pay.symbol);
...
void sps_processor::update_settings(const time_point_sec &head_time) {
  db.modify(db.get_dynamic_global_properties(),
            [&](dynamic_global_property_object &_dgpo) {
              _dgpo.next_maintenance_time =
                  head_time + fc::seconds(BLURT_PROPOSAL_MAINTENANCE_PERIOD);
...
void sps_processor::make_payments(const block_notification &note) {
  ...
  // Find all active proposals, where actual_time >= start_date and actual_time
  // <= end_date
...
const std::string &sps_processor::get_removing_name() { return removing_name; }
