...
void sps_helper::remove_proposals(database &db,
  ...
  for (auto id : proposal_ids) {
    ...
    FC_ASSERT(foundPosI->creator == proposal_owner,
              "Only proposal owner can remove it...");

    remove_proposal<by_proposal_id>(foundPosI, proposalIndex, votesIndex,
                                    byVoterIdx, obj_perf);
    if (obj_perf.done)
      break;
  }
