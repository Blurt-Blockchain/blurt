...
std::string advanced_benchmark_dumper::virtual_operation_name =
    "virtual_operation";
...
advanced_benchmark_dumper::~advanced_benchmark_dumper() { dump(); }
...
void advanced_benchmark_dumper::end(const std::string &str) {
  uint64_t time = std::chrono::duration_cast<std::chrono::milliseconds>(
                  ...
                  time_begin;
  auto res =
      info.emplace(APPLY_CONTEXT ? (apply_context_name + str) : str, time);
...
void advanced_benchmark_dumper::dump_impl(const total_info<COLLECTION> &src,
...
void advanced_benchmark_dumper::dump() {
  ...
  std::for_each(info.items.begin(), info.items.end(),
                [&rinfo](const item &obj) {
