...
void block_log::open(const fc::path &file) {
   ...
   * index file can be in relation to eachother.
   *
   *                          Block Log
   *                     Exists       Is New
   *                 +------------+------------+
...
uint64_t block_log::append(const signed_block &b) {
  try {
    ...
    FC_ASSERT(static_cast<uint64_t>(my->index_stream.tellp()) ==
...
std::pair<signed_block, uint64_t>
block_log::read_block_helper(uint64_t pos) const {
  try {
    my->check_block_read();
...
void block_log::set_locking(bool use_locking) { my->use_locking = true; }
} // namespace chain
} // namespace blurt
