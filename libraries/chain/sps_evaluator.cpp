...
void create_proposal_evaluator::do_apply(const create_proposal_operation &o) {
  try {
    /** start date can be earlier than head_block_time - otherwise creating a
       proposal can be difficult, since passed date should be adjusted by
       potential transaction execution delay (i.e. 3 sec as a time for processed
       next block).
    */
    FC_ASSERT(o.end_date > _db.head_block_time(),
              "Can't create inactive proposals...");
    ...
    FC_ASSERT(receiver_account != nullptr,
              "Specified receiver account: ${r} must exist in the blockchain",
              ("r", o.receiver));
    ...
    if (commentObject == nullptr) {
      ...
      FC_ASSERT(commentObject != nullptr,
                "Proposal permlink must point to the article posted by creator "
                "or receiver");
    }
...
void update_proposal_votes_evaluator::do_apply(
    const update_proposal_votes_operation &o) {
  try {
    const auto &pidx =
        _db.get_index<proposal_index>().indices().get<by_proposal_id>();
    const auto &pvidx =
        _db.get_index<proposal_vote_index>().indices().get<by_voter_proposal>();
    ...
    for (const auto id : o.proposal_ids) {
      ...
      if (o.approve) {
        if (found == pvidx.end())
          _db.create<proposal_vote_object>(
              [&](proposal_vote_object &proposal_vote) {
...
void remove_proposal_evaluator::do_apply(const remove_proposal_operation &op) {
  try {
    ...
    /*
       Because of performance removing proposals are restricted due to the
       `sps_remove_threshold` threshold. Therefore all proposals are marked with
       flag `removed` and `end_date` is moved beyond 'head_time +
       BLURT_PROPOSAL_MAINTENANCE_CLEANUP` flag `removed` - it's information for
       'sps_api' plugin moving `end_date` - triggers the algorithm in
       `sps_processor::remove_proposals`
    ...
    */
    for (const auto id : op.proposal_ids) {
      ...
      _db.modify(*found_id, [&](proposal_object &proposal) {
        ...
        auto new_end_date =
            head_date - fc::seconds(BLURT_PROPOSAL_MAINTENANCE_CLEANUP);
