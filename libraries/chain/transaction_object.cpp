...
const object *
transaction_index::create(const std::function<void(object *)> &constructor,
                          object_id_type) {
...
void transaction_index::add(unique_ptr<object> o) {
  ...
  FC_ASSERT(result.second,
            "Could not insert transaction_object! Uniqueness Constraint?. "
            "error from: blurt/chain/transaction_object.cpp    auto result = "
            "_index.insert(std::move(*trx));");
}
