...
void fork_database::_push_block(const item_ptr &item) {
  ...
  if (_head && item->previous_id() != block_id_type()) {
    ...
    BLURT_ASSERT(itr != index.end(), unlinkable_block_exception,
...
fork_database::walk_main_branch_to_num(uint32_t block_num) const {
...
shared_ptr<fork_item>
fork_database::fetch_block_on_main_branch_by_number(uint32_t block_num) const {
