...
struct pending_transactions_restorer {
  pending_transactions_restorer(
      database &db, std::vector<signed_transaction> &&pending_transactions)
  ...
  ~pending_transactions_restorer() {
    ...
    for (const auto &tx : _db._popped_tx) {
      if (apply_trxs && fc::time_point::now() - start >
    ...
    for (const signed_transaction &tx : _pending_transactions) {
      ...
      if (apply_trxs) {
        try {
          ...
          dlog("Pending transaction became invalid after switching to block "
               "${b} ${n} ${t}",
               ("b", _db.head_block_id())("n", _db.head_block_num())(
                   "t", _db.head_block_time()));
          dlog("The invalid transaction caused exception ${e}",
          ...
          dlog( "Pending transaction became invalid after switching to block
          ${b} ${n} ${t}",
             ("b", _db.head_block_id())("n", _db.head_block_num())("t",
          _db.head_block_time()) ); dlog( "The invalid pending transaction
          caused exception ${e}", ("e", e.to_detail_string() ) ); dlog( "${t}",
          ("t", tx) );
          */
        }
      } else {
