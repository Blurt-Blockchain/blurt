...
struct get_legacy_custom_operation_name {
  string &name;
  get_legacy_custom_operation_name(string &dv) : name(dv) {}
...
struct get_custom_operation_name {
  string &name;
  get_custom_operation_name(string &n) : name(n) {}
...
void custom_op_from_variant(const fc::variant &var, CustomOperationType &vo) {
  ...
  if (var.is_array()) // legacy serialization
  {
    ...
    if (ar[0].is_uint64())
      ...
      FC_ASSERT(itr != to_legacy_tag.end(), "Invalid operation name: ${n}",
                ("n", ar[0]));
  ...
  } else // new serialization
  {
    ...
    if (v_object["type"].is_integer()) {
      ...
      FC_ASSERT(itr != to_tag.end(), "Invalid object name: ${n}",
                ("n", v_object["type"]));
      which = itr->second;
    }
...
public:
  ...
  void apply_operations(const vector<CustomOperationType> &custom_operations,
    ...
    operation_get_required_authorities(outer_o, outer_active, outer_owner,
                                       outer_posting, outer_other);

    for (const CustomOperationType &inner_o : custom_operations) {
      operation_validate(inner_o);
      operation_get_required_authorities(inner_o, inner_active, inner_owner,
                                         inner_posting, inner_other);
    }
    ...
    for (const CustomOperationType &inner_o : custom_operations) {
      ...
      // error message is "declarations in dependent base are not found by
      // unqualified lookup"
  ...
  virtual void
  apply(const protocol::custom_binary_operation &outer_o) override {
    try {
      vector<CustomOperationType> custom_operations;

      try {
        custom_operations =
            fc::raw::unpack_from_vector<vector<CustomOperationType>>(
                outer_o.data);
      } catch (fc::exception &) {
        custom_operations.push_back(
            fc::raw::unpack_from_vector<CustomOperationType>(outer_o.data));
      }
  ...
  virtual std::shared_ptr<blurt::schema::abstract_schema>
  get_operation_schema() override {
    return blurt::schema::get_schema_for_type<CustomOperationType>();
  }

  virtual custom_id_type get_custom_id() override { return custom_id; }
