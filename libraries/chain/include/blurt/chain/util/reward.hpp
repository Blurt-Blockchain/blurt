...
struct comment_reward_context
{
   ...
   uint128_t  content_constant = BLURT_CONTENT_CONSTANT;
};

uint64_t get_rshare_reward( const comment_reward_context& ctx, const dynamic_global_property_object& gpo, uint32_t hardfork );

uint128_t evaluate_reward_curve( const uint128_t& rshares, const protocol::curve_id& curve = protocol::quadratic, const uint128_t& var1 = BLURT_CONTENT_CONSTANT );
