...
struct manabar {
  ...
  void regenerate_mana(const manabar_params &params, uint32_t now) {
    ...
    uint128_t max_mana_dt =
        uint64_t(params.max_mana >= 0 ? params.max_mana : 0);
    max_mana_dt *= dt;
    uint64_t u_regen = (max_mana_dt / params.regen_time).to_uint64();
    FC_ASSERT(u_regen <= std::numeric_limits<int64_t>::max());
    int64_t new_current_mana =
        fc::signed_sat_add(current_mana, int64_t(u_regen));
    current_mana = (new_current_mana > params.max_mana) ? params.max_mana
                                                        : new_current_mana;
  ...
  void use_mana(int64_t mana_used,
                int64_t min_mana = std::numeric_limits<uint64_t>::min()) {
...
template <typename T> int64_t get_effective_vesting_shares(const T &account) {
  ...
  // If there is a power down occuring, also reduce effective vesting shares by
  // this week's power down amount
  if (account.next_vesting_withdrawal != fc::time_point_sec::maximum()) {
    effective_vesting_shares -= std::min(
...
void update_manabar(const PropType &gpo, AccountType &account,
                    bool downvote_mana = false, bool check_overflow = true,
                    int64_t new_mana = 0) {
  auto effective_vests = util::get_effective_vesting_shares(account);
  try {
    manabar_params params(effective_vests,
  ...
  //   FC_TODO( "This hardfork check should not be needed. Remove after HF21 if
  //   that is the case." );
  //   // This is used as a hardfork check. Can be replaced with if(
  //   gpo.downvote_pool_percent ). Leaving as a hard check to be safe until
  //   after HF 21 try{ if( downvote_mana )
  //   {
  ...
  //      if( check_overflow )
  //      {
  //         params.max_mana = ( ( uint128_t( effective_vests ) *
  //         gpo.downvote_pool_percent ) / BLURT_100_PERCENT ).to_int64();
  //      }
  //      else
  //      {
  //         FC_TODO( "Cleanup once we have verified the overflow has not
  //         permanently made it in to the chain" ); uint128_t numerator =
  //         effective_vests * gpo.downvote_pool_percent; if( numerator.hi != 0
  //         )
  //            elog( "NOTIFYALERT! max mana overflow made it in to the chain"
  //            );
  //
  //         params.max_mana = ( effective_vests * gpo.downvote_pool_percent ) /
  //         BLURT_100_PERCENT;
  //      }
  //
  //      account.downvote_manabar.regenerate_mana( params, gpo.time );
  //      account.downvote_manabar.use_mana( ( -new_mana *
  //      gpo.downvote_pool_percent ) / BLURT_100_PERCENT );
  //   }
  //   } FC_CAPTURE_LOG_AND_RETHROW( (account)(effective_vests) )
}
...
FC_REFLECT(blurt::chain::util::manabar, (current_mana)(last_update_time))
