...
public:
  evaluator_registry(database &d) : _db(d) {
  ...
  void register_evaluator(Args... args) {
    _op_evaluators[OperationType::template tag<
                       typename EvaluatorType::operation_type>::value]
        .reset(new EvaluatorType(_db, args...));
  }

private:
  template <bool CHECK>
  unique_ptr<evaluator<OperationType>> &
  get_evaluator_impl(const OperationType &op) {
