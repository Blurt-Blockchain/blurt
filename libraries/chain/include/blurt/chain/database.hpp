...
#ifdef ENABLE_MIRA
using set_index_type_func =
    std::function<void(database &, mira::index_type,
                       const boost::filesystem::path &, const boost::any &)>;
#endif
...
public:
  ...
  bool is_processing_block() const {
    return _currently_processing_block_id.valid();
  }
  ...
  enum validation_steps {
    ...
    skip_validate = 1 << 10, ///< used prior to checkpoint, skips validate()
                             ///< call on transaction
    skip_validate_invariants =
        1 << 11, ///< used to skip database invariant check on block application
    skip_undo_block = 1 << 12, ///< used to skip undo db on reindex
    skip_block_log = 1 << 13   ///< used to skip block logging on reindex
  };

  typedef std::function<void(uint32_t, const abstract_index_cntr_t &)>
      TBenchmarkMidReport;
  ...
  struct open_args {
    ...
    TBenchmark benchmark =
        TBenchmark(0, [](uint32_t, const abstract_index_cntr_t &) {});
  };

  /**
   * @brief Open a database, creating a new one if necessary
   *
   * Opens a database in the specified directory. If no initialized database is
   * found the database will be initialized with the default state.
   *
   ...
   *
   * This method may be called after or instead of @ref database::open, and will
   * rebuild the object graph by replaying blockchain history. When this method
   * exits successfully, the database will be open.
   *
  ...
  /**
   * @brief wipe Delete database from disk, and potentially the raw chain as
   * well.
   * @param include_blocks If true, delete the raw chain as well as the
   * database.
   *
   * Will close the database before wiping. Database will be closed when this
   * function returns.
   */
  void wipe(const fc::path &data_dir, const fc::path &shared_mem_dir,
            bool include_blocks);
  ...
  /** Allows to visit all stored blocks until processor returns true. Caller is
   * responsible for block disasembling const signed_block_header& - header of
   * previous block const signed_block& - block to be processed currently
   */
  void foreach_block(
      std::function<bool(const signed_block_header &, const signed_block &)>
          processor) const;

  /// Allows to process all blocks visit all transactions held there until
  /// processor returns true.
  void foreach_tx(
      std::function<bool(const signed_block_header &, const signed_block &,
                         const signed_transaction &, uint32_t)>
          processor) const;
  /// Allows to process all operations held in blocks and transactions until
  /// processor returns true.
  void foreach_operation(
      std::function<bool(const signed_block_header &, const signed_block &,
                         const signed_transaction &, uint32_t,
                         const operation &, uint16_t)>
          processor) const;
  ...
  const comment_object *find_comment(const account_name_type &author,
                                     const shared_string &permlink) const;
  ...
  const escrow_object &get_escrow(const account_name_type &name,
                                  uint32_t escrow_id) const;
  const escrow_object *find_escrow(const account_name_type &name,
                                   uint32_t escrow_id) const;

  const savings_withdraw_object &
  get_savings_withdraw(const account_name_type &owner,
                       uint32_t request_id) const;
  const savings_withdraw_object *
  find_savings_withdraw(const account_name_type &owner,
                        uint32_t request_id) const;
  ...
  calculate_discussion_payout_time(const comment_object &comment) const;
  const reward_fund_object &get_reward_fund(const comment_object &c) const;

  asset get_effective_vesting_shares(const account_object &account,
                                     asset_symbol_type vested_symbol) const;
  ...
  const set<account_name_type> get_spam_accounts() const {
    return _spam_accounts;
  }
  ...
  /**
   *  This method is used to track applied operations during the evaluation of a
   * block, these operations should include any operation actually included in a
   * transaction as well as any implied/virtual operations that resulted, such
   * as filling an order. The applied operations are cleared after
  ...
  using apply_operation_handler_t =
      std::function<void(const operation_notification &)>;
  using apply_transaction_handler_t =
      std::function<void(const transaction_notification &)>;
...
private:
  template <typename TSignal, typename TNotification = std::function<
                                  typename TSignal::signature_type>>
  boost::signals2::connection
  connect_impl(TSignal &signal, const TNotification &func,
               const abstract_plugin &plugin, int32_t group,
               const std::string &item_name = "");

  template <bool IS_PRE_OPERATION>
  boost::signals2::connection
  any_apply_operation_handler_impl(const apply_operation_handler_t &func,
                                   const abstract_plugin &plugin,
                                   int32_t group);

public:
  boost::signals2::connection
  add_pre_apply_operation_handler(const apply_operation_handler_t &func,
                                  const abstract_plugin &plugin,
                                  int32_t group = -1);
  boost::signals2::connection
  add_post_apply_operation_handler(const apply_operation_handler_t &func,
                                   const abstract_plugin &plugin,
                                   int32_t group = -1);
  boost::signals2::connection
  add_pre_apply_transaction_handler(const apply_transaction_handler_t &func,
                                    const abstract_plugin &plugin,
                                    int32_t group = -1);
  boost::signals2::connection
  add_post_apply_transaction_handler(const apply_transaction_handler_t &func,
                                     const abstract_plugin &plugin,
                                     int32_t group = -1);
  boost::signals2::connection
  add_pre_apply_block_handler(const apply_block_handler_t &func,
                              const abstract_plugin &plugin,
                              int32_t group = -1);
  boost::signals2::connection
  add_post_apply_block_handler(const apply_block_handler_t &func,
                               const abstract_plugin &plugin,
                               int32_t group = -1);
  boost::signals2::connection
  add_irreversible_block_handler(const irreversible_block_handler_t &func,
                                 const abstract_plugin &plugin,
                                 int32_t group = -1);
  boost::signals2::connection
  add_pre_reindex_handler(const reindex_handler_t &func,
                          const abstract_plugin &plugin, int32_t group = -1);
  boost::signals2::connection
  add_post_reindex_handler(const reindex_handler_t &func,
                           const abstract_plugin &plugin, int32_t group = -1);
  ...
  asset create_vesting(const account_object &to_account, asset liquid,
                       bool to_reward_balance = false);
  ...
  void adjust_reward_balance(const account_name_type &name,
                             const asset &value_delta,
                             const asset &share_delta = asset(0, VESTS_SYMBOL));
  void adjust_supply(const asset &delta, bool adjust_vesting = false);
  void adjust_rshares2(const comment_object &comment,
                       fc::uint128_t old_rshares2, fc::uint128_t new_rshares2);
  void update_owner_authority(const account_object &account,
                              const authority &owner_authority);

  asset get_balance(const account_object &a, asset_symbol_type symbol) const;
  asset get_savings_balance(const account_object &a,
                            asset_symbol_type symbol) const;
  asset get_balance(const account_name_type &aname,
                    asset_symbol_type symbol) const;

  /** this updates the votes for witnesses as a result of account voting proxy
   * changing */
  ...
  /** this updates the votes for all witnesses as a result of account VESTS
   * changing */
  void adjust_proxied_witness_votes(const account_object &a, share_type delta,
                                    int depth = 0);

  /** this is called by `adjust_proxied_witness_votes` when account proxy to
   * self */
  void adjust_witness_votes(const account_object &a, share_type delta);

  /** this updates the vote of a single witness as a result of a vote being
   * added or removed*/
  ...
  share_type cashout_comment_helper(util::comment_reward_context &ctx,
  ...
  /** when popping a block, the transactions that were removed get cached here
   * so they can be reapplied at the proper time */
...
protected:
  ...
  // it should call pop_block() instead void pop_undo() {
...
private:
  ...
  void update_signing_witness(const witness_object &signing_witness,
  ...
  void modify_balance(const account_object &a, const asset &delta,
                      bool check_balance);
  void modify_reward_balance(const account_object &a, const asset &value_delta,
                             const asset &share_delta, bool check_balance);

  operation_notification
  create_operation_notification(const operation &op) const {
...
public:
  ...
  int16_t get_sps_remove_threshold() const { return _sps_remove_threshold; }

  void set_sps_remove_threshold(int16_t val) { _sps_remove_threshold = val; }
...
private:
  ...
  flat_map<custom_id_type, std::shared_ptr<custom_operation_interpreter>>
  ...
  /**
   *  This signal is emitted for plugins to process every operation after it has
   * been fully applied.
   */
   ...
   * to the chain state.
   */
  fc::signal<void(const transaction_notification &)>
      _pre_apply_transaction_signal;
   ...
   */
  fc::signal<void(const transaction_notification &)>
      _post_apply_transaction_signal;
