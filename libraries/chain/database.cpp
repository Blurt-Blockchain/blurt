...
FC_REFLECT(blurt::chain::db_schema,
           (types)(object_types)(operation_type)(custom_operation_types))
FC_REFLECT(blurt::chain::account_snapshot,
           (name)(owner)(active)(posting)(memo)(balance)(power))
...
void database::open(const open_args &args) {
  try {
    init_schema();
    chainbase::database::open(args.shared_mem_dir, args.chainbase_flags,
                              args.shared_file_size, args.database_cfg);

    initialize_indexes();
    initialize_evaluators();

    if (!find<dynamic_global_property_object>())
      with_write_lock([&]() { init_genesis(args); });
    ...
    // Rewind all undo state. This should return us to the state at the last
    // irreversible block.
    with_write_lock([&]() {
#ifndef ENABLE_MIRA
      undo_all();
    ...
    if (head_block_num()) {
      ...
      FC_ASSERT(
          head_block.valid() && head_block->id() == head_block_id(),
    ...
    //
    //          // change witness key
    //          for (auto itr = witness_idx.begin(); itr != witness_idx.end();
    //          ++itr) {
    //            modify(*itr, [&](witness_object& w) {
    //              w.signing_key =
    //              blurt::protocol::public_key_type("BLT875YGJ2rXwEhUr4hRXduZguwJKEJufsS4oYT6ehHWiDhev7hah");
    //            });
    //          }
    //        }
  ...
  FC_CAPTURE_LOG_AND_RETHROW(
      (args.data_dir)(args.shared_mem_dir)(args.shared_file_size))
}

#ifdef ENABLE_MIRA
void reindex_set_index_helper(database &db, mira::index_type type,
  ...
  if (indices.size() > 0) {
    for (auto &index_name : indices) {
      if (db.has_index_delegate(index_name))
        ...
        wlog("Encountered an unknown index name '${name}'.",
             ("name", index_name));
    }
  } else {
  ...
  for (auto const &delegate : delegates) {
    ilog("Converting index '${name}' to ${type} type.",
         ("name", delegate.first)("type", type_str));
...
uint32_t database::reindex(const open_args &args) {
  ...
  try {
    ...
    if (args.replay_in_memory) {
      ...
      reindex_set_index_helper(*this, mira::index_type::bmic,
                               args.shared_mem_dir, args.database_cfg,
                               args.replay_memory_indices);
    }
#endif

    _fork_db
        .reset(); // override effect of _fork_db.start_block() call in open()

    auto start = fc::time_point::now();
    BLURT_ASSERT(_block_log.head(), block_log_exception,
    ...
    with_write_lock([&]() {
      ...
      while (itr.first.block_num() != last_block_num) {
        ...
        if (cur_block_num % 100000 == 0) {
          std::cerr << "   " << double(cur_block_num * 100) / last_block_num
                    ...
                    << "   (" <<
#ifdef ENABLE_MIRA
              get_cache_size() << " objects cached using "
                    << (get_cache_usage() >> 20) << "M"
#else
      ...
      if ((args.benchmark.first > 0) &&
          (note.last_block_number % args.benchmark.first == 0))
        args.benchmark.second(note.last_block_number,
                              get_abstract_index_cntr());
      set_revision(head_block_num());
    ...
    if (args.replay_in_memory) {
      ...
      reindex_set_index_helper(*this, mira::index_type::mira,
                               args.shared_mem_dir, args.database_cfg,
                               args.replay_memory_indices);
    }
#endif
 ...
 * this method is called with a VERY old transaction we will return false, they
 * should query things by blocks if they are that old.
 */
bool database::is_known_transaction(const transaction_id_type &id) const {
  try {
    const auto &trx_idx =
        get_index<transaction_index>().indices().get<by_trx_id>();
...
block_id_type database::find_block_id_for_num(uint32_t block_num) const {
  try {
    ...
    shared_ptr<fork_item> fitem =
        _fork_db.fetch_block_on_main_branch_by_number(block_num);
    if (fitem)
      return fitem->id;
...
database::fetch_block_by_id(const block_id_type &id) const {
  try {
    auto b = _fork_db.fetch_block(id);
    if (!b) {
      auto tmp =
          _block_log.read_block_by_num(protocol::block_header::num_from_id(id));
...
optional<signed_block>
database::fetch_block_by_number(uint32_t block_num) const {
  try {
...
chain_id_type database::get_chain_id() const { return blurt_chain_id; }
...
void database::foreach_block(
    std::function<bool(const signed_block_header &, const signed_block &)>
        processor) const {
  if (!_block_log.head())
    return;
...
void database::foreach_tx(
    std::function<bool(const signed_block_header &, const signed_block &,
                       const signed_transaction &, uint32_t)>
        processor) const {
...
void database::foreach_operation(
    std::function<bool(const signed_block_header &, const signed_block &,
                       const signed_transaction &, uint32_t, const operation &,
                       uint16_t)>
        processor) const {
  foreach_tx([&processor](const signed_block_header &prevBlockHeader,
                          const signed_block &block,
                          const signed_transaction &tx,
                          uint32_t txInBlock) -> bool {
    uint16_t opInTx = 0;
...
database::get_witness(const account_name_type &name) const {
  try {
...
const witness_object *
database::find_witness(const account_name_type &name) const {
  return find<witness_object, by_name>(name);
}

const account_object &
database::get_account(const account_name_type &name) const {
  try {
...
const account_object *
database::find_account(const account_name_type &name) const {
  return find<account_object, by_name>(name);
}

const comment_object &
database::get_comment(const account_name_type &author,
...
const comment_object *
database::find_comment(const account_name_type &author,
                       const shared_string &permlink) const {
...
database::get_savings_withdraw(const account_name_type &owner,
...
const savings_withdraw_object *
database::find_savings_withdraw(const account_name_type &owner,
                                uint32_t request_id) const {
...
database::get_dynamic_global_properties() const {
  try {
...
const time_point_sec database::calculate_discussion_payout_time(
    const comment_object &comment) const {
  return comment.cashout_time;
}

const reward_fund_object &
database::get_reward_fund(const comment_object &c) const {
  return get<reward_fund_object, by_name>(BLURT_POST_REWARD_FUND_NAME);
}

asset database::get_effective_vesting_shares(
    const account_object &account, asset_symbol_type vested_symbol) const {
  if (vested_symbol == VESTS_SYMBOL)
    return account.vesting_shares - account.delegated_vesting_shares +
...
bool database::push_block(const signed_block &new_block, uint32_t skip) {
  ...
  if (_checkpoints.size() && _checkpoints.rbegin()->second != block_id_type()) {
    ...
    if (itr != _checkpoints.end())
      FC_ASSERT(new_block.id() == itr->second, "Block did not match checkpoint",
                ("checkpoint", *itr)("block_id", new_block.id()));

    if (_checkpoints.rbegin()->first >= block_num)
      skip = skip_witness_signature | skip_transaction_signatures |
             skip_transaction_dupe_check
             /*| skip_fork_db Fork db cannot be skipped or else blocks will not
                be written out to block log */
             | skip_block_size_check | skip_tapos_check |
             skip_authority_check
             /* | skip_merkle_check While blockchain is being downloaded, txs
                need to be validated against block headers */
             | skip_undo_history_check | skip_witness_schedule_check |
             skip_validate | skip_validate_invariants;
  }
  ...
  //   ilog( "push_block ${b} took ${t} microseconds", ("b",
  //   new_block.block_num())("t", dt.count()) );
  return result;
}
...
void database::_maybe_warn_multiple_production(uint32_t height) const {
  ...
  if (blocks.size() > 1) {
    ...
    ilog("Encountered block num collision at block ${n} due to a fork, "
         "witnesses are: ${w}",
  ...
  try {
    ...
    if (!(skip & skip_fork_db)) {
      ...
      if (new_head->data.previous != head_block_id()) {
        ...
        // back in new_head Only switch forks if new_head is actually higher than
        // head
 ...
 * transaction to be pushed even if it causes the pending block size to exceed
 * the maximum block size. Although the transaction will probably not propagate
 * further now, as the peers are likely to have their pending queues full as
 * well, it will be kept in the queue to be propagated later when a new block
 * flushes out the pending queues.
 */
...
void database::_push_transaction(const signed_transaction &trx) {
  ...
  // undo session. This allows us to quickly rewind to the clean state of the
  // head block, in case a new block arrives.
...
void database::pop_block() {
  try {
    ...
    BLURT_ASSERT(head_block.valid(), pop_empty_chain,
...
struct action_validate_visitor {
  ...
  template <typename Action> void operator()(const Action &a) const {
    a.validate();
  }
};
...
void database::notify_pre_apply_transaction(
    const transaction_notification &note) {
  BLURT_TRY_NOTIFY(_pre_apply_transaction_signal, note)
}

void database::notify_post_apply_transaction(
    const transaction_notification &note){
...
account_name_type database::get_scheduled_witness(uint32_t slot_num) const {
  ...
  return wso
      .current_shuffled_witnesses[current_aslot % wso.num_scheduled_witnesses];
}
...
// Create vesting, then a caller-supplied callback after determining how many
// shares to create, but before we modify the database. This allows us to
// implement virtual op pre-notifications in the Before function.
template <typename Before>
asset create_vesting2(database &db, const account_object &to_account,
                      asset liquid, bool to_reward_balance,
                      Before &&before_vesting_callback) {
  try {
    auto calculate_new_vesting = [liquid](price vesting_share_price) -> asset {
      /**
       *  The ratio of total_vesting_shares / total_vesting_fund_blurt should
       * not change as the result of the user adding funds
       *
       *  V / C  = (V+Vn) / (C+Cn)
       *
       *  Simplifies to Vn = (V * Cn ) / C
       *
       *  If Cn equals o.amount, then we must solve for Vn to know how many new
       * vesting shares the user should receive.
       *
       *  128 bit math is requred due to multiplying of 64 bit numbers. This is
       * done in asset and price.
       */
    ...
    // ^ A novelty, needed but risky in case someone managed to slip SBD/TESTS
    // here in blockchain history. Get share price.
    ...
    if (to_reward_balance) {
      ...
      db.modify(to_account, [&](account_object &a) {
        util::update_manabar(cprops, a, true, true, new_vesting.amount.value);
      });
 ...
 */
asset database::create_vesting(const account_object &to_account, asset liquid,
                               bool to_reward_balance) {
  return create_vesting2(*this, to_account, liquid, to_reward_balance,
                         [](asset vests_created) {});
}
...
void database::adjust_proxied_witness_votes(const account_object &a,
  ...
  if (a.proxy != BLURT_PROXY_TO_SELF_ACCOUNT) {
    ...
    modify(proxy,
           [&](account_object &a) { a.proxied_vsf_votes[depth] += delta; });
...
void database::adjust_witness_vote(const witness_object &witness,
                                   share_type delta) {
  const witness_schedule_object &wso = get_witness_schedule_object();
  modify(witness, [&](witness_object &w) {
    auto delta_pos =
        w.votes.value * (wso.current_virtual_time - w.virtual_last_update);
    w.virtual_position += delta_pos;

    w.virtual_last_update = wso.current_virtual_time;
    w.votes += delta;
    FC_ASSERT(
        w.votes <= get_dynamic_global_properties().total_vesting_shares.amount,
        "",
        ("w.votes", w.votes)(
    ...
    w.virtual_scheduled_time =
        ...
        (BLURT_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position) /
            (w.votes.value + 1);

    /** witnesses with a low number of votes could overflow the time field and
     * end up with a scheduled time in the past */
...
void database::clear_null_account_balance() {
  ...
  if (null_account.vesting_shares.amount > 0) {
    ...
    vesting_shares_steem_value =
        null_account.vesting_shares * gpo.get_vesting_share_price();
  ...
  clear_null_account_balance_operation &vop =
      vop_op.get<clear_null_account_balance_operation>();
  ...
  if (null_account.vesting_shares.amount > 0) {
    ...
    modify(null_account,
           [&](account_object &a) { a.vesting_shares.amount = 0; });
  }
...
void database::process_regent_account() {
  if ((head_block_num() % 864000) == 0) { // 864000 ~ 1 month
    ...
    if (new_regent_vesting_shares >= 0) {
      auto delta =
          dgpo.regent_vesting_shares.amount - new_regent_vesting_shares;
      const auto &regent_account = this->get_account(BLURT_REGENT_ACCOUNT);

      modify(dgpo, [&](dynamic_global_property_object &p) {
        p.regent_vesting_shares =
            asset(new_regent_vesting_shares, VESTS_SYMBOL);
...
/**
 * This method updates total_reward_shares2 on DGPO, and children_rshares2 on
 * comments, when a comment's rshares2 changes from old_rshares2 to
 * new_rshares2.  Maintaining invariants that children_rshares2 is the sum of
 * all descendants' rshares2, and dgpo.total_reward_shares2 is the total number
 * of rshares2 outstanding.
 */
void database::adjust_rshares2(const comment_object &c,
...
void database::process_vesting_withdrawals() {
  ...
  const auto &didx =
      get_index<withdraw_vesting_route_index, by_withdraw_route>();
  ...
  while (current != widx.end() &&
    ...
    // Do two passes, the first for vests, the second for steem. Try to maintain
    // as much accuracy for vests as possible.
    for (auto itr = didx.upper_bound(
             boost::make_tuple(from_account.name, account_name_type()));
         itr != didx.end() && itr->from_account == from_account.name; ++itr) {
      if (itr->auto_vest) {
        ...
        if (to_deposit > 0) {
          const auto &to_account =
              get<account_object, by_name>(itr->to_account);

          operation vop = fill_vesting_withdraw_operation(
    ...
    for (auto itr = didx.upper_bound(
      ...
      if (!itr->auto_vest) {
        ...
        if (to_deposit > 0) {
          ...
          modify(to_account,
                 [&](account_object &a) { a.balance += converted_steem; });
    ...
    share_type to_convert =
        to_withdraw - vests_deposited_as_steem - vests_deposited_as_vests;
    ...
    operation vop = fill_vesting_withdraw_operation(
        from_account.name, from_account.name, asset(to_convert, VESTS_SYMBOL),
        converted_steem);
    pre_push_virtual_operation(vop);
    ...
    modify(from_account, [&](account_object &a) {
      ...
      if (a.withdrawn >= a.to_withdraw || a.vesting_shares.amount == 0) {
        ...
        a.next_vesting_withdrawal +=
            fc::seconds(BLURT_VESTING_WITHDRAW_INTERVAL_SECONDS);
      }
    });
...
void database::adjust_total_payout(const comment_object &cur,
                                   const asset &total_payout_value,
                                   const asset &curator_payout_value,
                                   const asset &beneficiary_payout_value) {
 ...
 */
share_type database::pay_curators(const comment_object &c,
                                  share_type &max_rewards) {
  struct cmp {
    bool operator()(const comment_vote_object *obj,
                    const comment_vote_object *obj2) const {
    ...
    if (!c.allow_curation_rewards) {
      ...
      for (auto &item : proxy_set) {
        try {
          ...
          auto claim =
              ((max_rewards.value * weight) / total_weight).to_uint64();
          if (claim > 0) // min_amt is non-zero satoshis
          {
            unclaimed_rewards -= claim;
            const auto &voter = get(item->voter);
            operation vop =
                curation_reward_operation(voter.name, asset(0, VESTS_SYMBOL),
                                          c.author, to_string(c.permlink));
            create_vesting2(*this, voter, asset(claim, BLURT_SYMBOL), true,
                            [&](const asset &reward) {
                              vop.get<curation_reward_operation>().reward =
                                  reward;
                              pre_push_virtual_operation(vop);
                            });
...
#ifndef IS_LOW_MEM
            modify(voter,
                   [&](account_object &a) { a.curation_rewards += claim; });
#endif
            post_push_virtual_operation(vop);
          }
        }
...
share_type database::cashout_comment_helper(util::comment_reward_context &ctx,
                                            const comment_object &comment,
                                            bool forward_curation_remainder) {
  try {
      ...
      if (reward_tokens > 0) {
        ...
        for (auto &b : comment.beneficiaries) {
          auto benefactor_tokens =
              (author_tokens * b.weight) / BLURT_100_PERCENT;
          auto benefactor_vesting_blurt = benefactor_tokens;
          auto vop = comment_benefactor_reward_operation(
              b.account, comment.author, to_string(comment.permlink),
              asset(0, BLURT_SYMBOL), asset(0, VESTS_SYMBOL));

          if (b.account == BLURT_TREASURY_ACCOUNT) {
            benefactor_vesting_blurt = 0;
            vop.blurt_payout = asset(benefactor_tokens, BLURT_SYMBOL);
            adjust_balance(get_account(BLURT_TREASURY_ACCOUNT),
                           vop.blurt_payout);
          } else {
            benefactor_vesting_blurt = benefactor_tokens;
          }

          create_vesting2(*this, get_account(b.account),
                          asset(benefactor_vesting_blurt, BLURT_SYMBOL), true,
                          [&](const asset &reward) {
        ...
        auto author_blurt_tokens =
            (author_tokens * comment.percent_blurt) / (4 * BLURT_100_PERCENT);
        ...
        operation vop =
            author_reward_operation(comment.author, to_string(comment.permlink),
                                    blurt_payout, asset(0, VESTS_SYMBOL));

        create_vesting2(*this, author, asset(vesting_blurt, BLURT_SYMBOL), true,
                        [&](const asset &vesting_payout) {
                          vop.get<author_reward_operation>().vesting_payout =
                              vesting_payout;
                          pre_push_virtual_operation(vop);
                        });

        adjust_total_payout(comment,
                            blurt_payout + asset(vesting_blurt, BLURT_SYMBOL),
                            asset(curation_tokens, BLURT_SYMBOL),
                            asset(total_beneficiary, BLURT_SYMBOL));
        post_push_virtual_operation(vop);
        vop = comment_reward_operation(comment.author,
                                       to_string(comment.permlink),
                                       asset(claimed_reward, BLURT_SYMBOL));
        pre_push_virtual_operation(vop);
        post_push_virtual_operation(vop);

#ifndef IS_LOW_MEM
        modify(comment,
               [&](comment_object &c) { c.author_rewards += author_tokens; });
    ...
    modify(comment, [&](comment_object &c) {
       ...
       * around for the next time this post might get an upvote.
       */
      if (c.net_rshares > 0)
        c.net_rshares = 0;
    ...
    push_virtual_operation(comment_payout_update_operation(
        comment.author, to_string(comment.permlink)));
    ...
    while (vote_itr != vote_idx.end() && vote_itr->comment == comment.id) {
      ...
      ++vote_itr;
      if (calculate_discussion_payout_time(comment) !=
          fc::time_point_sec::maximum()) {
        modify(cur_vote,
               [&](comment_vote_object &cvo) { cvo.num_changes = -1; });
      } else {
#ifdef CLEAR_VOTES
        remove(cur_vote);
#endif
...
void database::process_comment_cashout() {
  ...
  for (auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr) {
    ...
    modify(*itr, [&](reward_fund_object &rfo) {
      ...
      int64_t delta_seconds =
          (head_block_time() - rfo.last_update).to_seconds();
      if (delta_seconds < decay_time.to_seconds()) {
        rfo.recent_claims -=
            (rfo.recent_claims * delta_seconds) / decay_time.to_seconds();
      }
    ...
    // The index is by ID, so the ID should be the current size of the vector
    // (0, 1, 2, etc...)
  ...
  const auto &cidx =
      get_index<comment_index>().indices().get<by_cashout_time>();
  auto current = cidx.begin();
  //  add all rshares about to be cashed out to the reward funds. This ensures
  //  equal satoshi per rshare payment
  {
    while (current != cidx.end() &&
      ...
      if (current->net_rshares > 0) {
        ...
        funds[rf.id._id].recent_claims += util::evaluate_reward_curve(
            current->net_rshares.value, rf.author_reward_curve,
            rf.content_constant);
      }
   ...
   * Cashout comment helper does not know about the reward fund it is paying
   * from. The helper only does token allocation based on curation rewards and
   * the SBD global %, etc.
   *
   * Each context is used by get_rshare_reward to determine what part of each
   * budget the comment is entitled to. Prior to hardfork 17, all payouts are
   * done against the global state updated each payout. After the hardfork, each
   * payout is done against a reward fund state that is snapshotted before all
   * payouts in the block.
   */
  while (current != cidx.end() && current->cashout_time <= head_block_time()) {
    ...
    funds[fund_id].blurt_awarded +=
        cashout_comment_helper(ctx, *current, false);
    ...
    for (size_t i = 0; i < funds.size(); i++) {
      modify(get<reward_fund_object, by_id>(reward_fund_id_type(i)),
             [&](reward_fund_object &rfo) {
               ...
               rfo.reward_balance -=
                   asset(funds[i].blurt_awarded, BLURT_SYMBOL);
             });
    }
  }
}
 ...
 *
 *  This method pays out vesting and reward shares every block, and liquidity
 * shares once per day. This method does not pay out witnesses.
 */
void database::process_funds() {
  ...
  /**
   * At block 7,000,000 have a 9.5% instantaneous inflation rate, decreasing to
   * 0.95% at a rate of 0.01% every 250k blocks. This narrowing will take
   * approximately 20.5 years and will complete on block 220,750,000
   */
  int64_t start_inflation_rate = int64_t(BLURT_INFLATION_RATE_START_PERCENT);
  int64_t inflation_rate_adjustment =
      int64_t(head_block_num() / BLURT_INFLATION_NARROWING_PERIOD);
  int64_t inflation_rate_floor = int64_t(BLURT_INFLATION_RATE_STOP_PERCENT);

  // below subtraction cannot underflow int64_t because
  // inflation_rate_adjustment is <2^32
  int64_t current_inflation_rate = std::max(
      start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor);

  auto new_steem =
      (props.current_supply.amount * current_inflation_rate) /
      (int64_t(BLURT_100_PERCENT) * int64_t(BLURT_BLOCKS_PER_YEAR));
  auto content_reward =
      (new_steem * props.content_reward_percent) / BLURT_100_PERCENT;
  content_reward = pay_reward_funds(content_reward);
  auto vesting_reward =
      (new_steem * props.vesting_reward_percent) / BLURT_100_PERCENT;
  ...
  create_vesting2(
      *this, get_account(cwit.owner), asset(witness_reward, BLURT_SYMBOL),
      false, [&](const asset &vesting_shares) {
...
void database::process_savings_withdraws() {
  ...
  while (itr != idx.end()) {
    ...
    push_virtual_operation(fill_transfer_from_savings_operation(
        itr->from, itr->to, itr->amount, itr->request_id,
        to_string(itr->memo)));
...
void database::process_subsidized_accounts() {
  ...
  // Update global pool.
  modify(gpo, [&](dynamic_global_property_object &g) {
    g.available_account_subsidies =
        rd_apply(wso.account_subsidy_rd, g.available_account_subsidies);
  });
  ...
  if (current_witness.schedule == witness_object::elected) {
    modify(current_witness, [&](witness_object &w) {
      w.available_witness_account_subsidies =
          rd_apply(wso.account_subsidy_witness_rd,
                   w.available_witness_account_subsidies);
    });
  }
}
...
share_type database::pay_reward_funds(share_type reward) {
  ...
  for (auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr) {
    // reward is a per block reward and the percents are 16-bit. This should
    // never overflow
    ...
    // Sanity check to ensure we aren't printing more BLURT than has been
    // allocated through inflation
...
void database::account_recovery_processing() {
  ...
  const auto &rec_req_idx = get_index<account_recovery_request_index>()
                                .indices()
                                .get<by_expiration>();
  ...
  const auto &hist_idx =
      get_index<owner_authority_history_index>().indices(); // by id
  auto hist = hist_idx.begin();

  while (hist != hist_idx.end() &&
         time_point_sec(hist->last_valid_time +
                        BLURT_OWNER_AUTH_RECOVERY_PERIOD) < head_block_time()) {
    remove(*hist);
    hist = hist_idx.begin();
  }

  // Apply effective recovery_account changes
  const auto &change_req_idx =
      get_index<change_recovery_account_request_index>()
          .indices()
...
void database::initialize_evaluators() {
  ...
  _my->_evaluator_registry
      .register_evaluator<create_claimed_account_evaluator>();
  _my->_evaluator_registry
  ...
  _my->_evaluator_registry
      .register_evaluator<transfer_from_savings_evaluator>();
  _my->_evaluator_registry
      .register_evaluator<cancel_transfer_from_savings_evaluator>();
  _my->_evaluator_registry
      .register_evaluator<decline_voting_rights_evaluator>();
  ...
  _my->_evaluator_registry
      .register_evaluator<witness_set_properties_evaluator>();
  _my->_evaluator_registry.register_evaluator<create_proposal_evaluator>();
  _my->_evaluator_registry
      .register_evaluator<update_proposal_votes_evaluator>();
  _my->_evaluator_registry.register_evaluator<remove_proposal_evaluator>();
}

void database::register_custom_operation_interpreter(
    std::shared_ptr<custom_operation_interpreter> interpreter) {
  FC_ASSERT(interpreter);
  bool inserted = _custom_operation_interpreters
                      .emplace(interpreter->get_custom_id(), interpreter)
                      .second;
  // This assert triggering means we're mis-configured (multiple registrations
  // of custom JSON evaluator for same ID)
  FC_ASSERT(inserted);
}

std::shared_ptr<custom_operation_interpreter>
database::get_custom_json_evaluator(const custom_id_type &id) {
...
void database::init_schema() {
  ...
  std::shared_ptr< abstract_schema > operation_schema = get_schema_for_type<
  operation >(); operation_schema->get_name( ds.operation_type );
  schema_list.push_back( operation_schema );

  for( const std::pair< std::string, std::shared_ptr<
  custom_operation_interpreter > >& p : _custom_operation_interpreters )
  {
  ...
  for( std::shared_ptr< abstract_schema >& s : schema_list )
  {
     std::string tname;
     s->get_name( tname );
     FC_ASSERT( ds.types.find( tname ) == ds.types.end(), "types with different
  ID's found for name ${tname}", ("tname", tname) ); std::string ss;
     s->get_str_schema( ss );
  ...
  try {
    ...
    { // BLURT_MINER_ACCOUNT
      create<account_object>(
          [&](account_object &a) { a.name = BLURT_MINER_ACCOUNT; });
    ...
    { // BLURT_NULL_ACCOUNT
      create<account_object>(
          [&](account_object &a) { a.name = BLURT_NULL_ACCOUNT; });
      create<account_authority_object>([&](account_authority_object &auth) {
    ...
    { // BLURT_TEMP_ACCOUNT
      create<account_object>(
          [&](account_object &a) { a.name = BLURT_TEMP_ACCOUNT; });
      create<account_authority_object>([&](account_authority_object &auth) {
    ...
    { // BLURT_INIT_MINER
      for (int i = 0; i < BLURT_MAX_WITNESSES; ++i) {
        create<account_object>([&](account_object &a) {
          a.name =
              BLURT_INIT_MINER_NAME + (i ? fc::to_string(i) : std::string());
          a.memo_key = init_public_key;
          a.balance = asset(
              i ? 0 : args.initial_supply - BLURT_INIT_POST_REWARD_BALANCE,
              BLURT_SYMBOL);
        });
        ...
        create<witness_object>([&](witness_object &w) {
          ...
          w.schedule = (i < BLURT_MAX_VOTED_WITNESSES_HF17)
                           ? witness_object::elected
                           : witness_object::timeshare;
        });
      }
    }
    ...
    create<dynamic_global_property_object>(
        [&](dynamic_global_property_object &p) {
          ...
          p.regent_init_vesting_shares =
              asset(args.initial_supply / 2, BLURT_SYMBOL) *
              p.get_vesting_share_price(); // 50% of the init_supply
    ...
    create<witness_schedule_object>([&](witness_schedule_object &wso) {
      ...
      account_subsidy_system_params.resource_unit =
          BLURT_ACCOUNT_SUBSIDY_PRECISION;
      account_subsidy_system_params.decay_per_time_unit_denom_shift =
          BLURT_RD_DECAY_DENOM_SHIFT;
      util::rd_user_params account_subsidy_user_params;
      account_subsidy_user_params.budget_per_time_unit =
          wso.median_props.account_subsidy_budget;
      account_subsidy_user_params.decay_per_time_unit =
          wso.median_props.account_subsidy_decay;

      util::rd_user_params account_subsidy_per_witness_user_params;
      int64_t w_budget = wso.median_props.account_subsidy_budget;
      w_budget =
          (w_budget * BLURT_WITNESS_SUBSIDY_BUDGET_PERCENT) / BLURT_100_PERCENT;
      w_budget =
          std::min(w_budget, int64_t(std::numeric_limits<int32_t>::max()));
      uint64_t w_decay = wso.median_props.account_subsidy_decay;
      w_decay =
          (w_decay * BLURT_WITNESS_SUBSIDY_DECAY_PERCENT) / BLURT_100_PERCENT;
      w_decay =
          std::min(w_decay, uint64_t(std::numeric_limits<uint32_t>::max()));

      account_subsidy_per_witness_user_params.budget_per_time_unit =
          int32_t(w_budget);
      account_subsidy_per_witness_user_params.decay_per_time_unit =
          uint32_t(w_decay);

      util::rd_setup_dynamics_params(account_subsidy_user_params,
                                     account_subsidy_system_params,
                                     wso.account_subsidy_rd);
      util::rd_setup_dynamics_params(account_subsidy_per_witness_user_params,
                                     account_subsidy_system_params,
                                     wso.account_subsidy_witness_rd);
    });
    ...
    { // IMPORT snapshot.json
      ...
      auto snapshot_path =
          args.data_dir.string() + std::string("/../snapshot.json");
      FC_ASSERT(boost::filesystem::exists(snapshot_path),
                "Snapshot '${path}' was not found.", ("path", snapshot_path));
      ...
      while (std::getline(snapshot, line)) {
        if (line.length() > 0) {
          account_snapshot ss_account =
              fc::json::from_string(line).as<account_snapshot>();
          ...
          if (counter % 100000 == 0)
            ilog("creating account ${i}...", ("i", counter));
          counter++;
        }
      }
      ...
      while (std::getline(snapshot, line)) {
        if (line.length() > 0) {
          ...
          if (counter % 100000 == 0)
            ilog("creating authority ${i}...", ("i", counter));
          counter++;
        }
      }
      ...
      modify(get_dynamic_global_properties(),
             [&](dynamic_global_property_object &_dgpo) {
               _dgpo.total_vesting_fund_blurt =
                   snapshot_total_vesting_fund_blurt;
               _dgpo.total_vesting_shares = snapshot_total_vesting_shares;
             });

      modify(get_account(BLURT_INIT_MINER_NAME), [&](account_object &a) {
        a.balance -=
            (snapshot_total_balance + snapshot_total_vesting_fund_blurt);
      });
      ...
      { // create account_metadata_object for all accounts
        ...
        counter = 0;
        const auto &acc_idx =
            get_index<chain::account_index>().indices().get<chain::by_name>();
        ...
        while (itr != acc_idx.end()) {
          ...
          if (counter % 100000 == 0)
            ilog("creating account_metadata_object ${i}...", ("i", counter));
          counter++;
        }
#endif
      }
    ...
    { // pre-apply HF 1 to 22 here
      ...
      { // BLURT_HARDFORK_0_17:
        static_assert(BLURT_MAX_VOTED_WITNESSES_HF17 +
                              BLURT_MAX_RUNNER_WITNESSES_HF17 ==
                          BLURT_MAX_WITNESSES,
        ...
        auto post_rf = create<reward_fund_object>([&](reward_fund_object &rfo) {
          ...
          rfo.reward_balance =
              asset(BLURT_INIT_POST_REWARD_BALANCE, BLURT_SYMBOL);
#ifndef IS_TEST_NET
        ...
        // The IDs must be assigned this way. The assertion is a dummy check to
        // ensure this happens.
      ...
      { // BLURT_HARDFORK_0_21:
        //           auto rec_req = find< account_recovery_request_object,
        //           by_account >( BLURT_TREASURY_ACCOUNT ); if( rec_req )
        //              remove( *rec_req );
        //
        //           auto change_request = find<
        //           change_recovery_account_request_object, by_account >(
        //           BLURT_TREASURY_ACCOUNT ); if( change_request )
...
void database::notify_changed_objects() {
  try {
    ...
    changed_ids.reserve(head_undo.old_values.size()); for( const auto& item :
    head_undo.old_values ) changed_ids.push_back(item.first); for( const auto&
    item : head_undo.new_ids ) changed_ids.push_back(item); vector<const
    object*> removed; removed.reserve( head_undo.removed.size() ); for( const
    auto& item : head_undo.removed )
       {
  ...
  try {
    ...
    if (_flush_blocks != 0) {
      if (_next_flush_block == 0) {
        ...
        // use time_point::now() as RNG source to pick block randomly between
        // lep and rep
        uint32_t span = rep - lep;
        uint32_t x = lep;
        if (span > 0) {
          uint64_t now =
              uint64_t(fc::time_point::now().time_since_epoch().count());
          x += now % span;
        }
        _next_flush_block = x;
      ...
      if (_next_flush_block == block_num) {
        _next_flush_block = 0;
        // ilog( "Flushing database shared memory at block ${b}", ("b",
        // block_num) );
        chainbase::database::flush();
      }
    }
...
void database::check_free_memory(bool force_print, uint32_t current_block_num) {
  ...
  if (BOOST_UNLIKELY(
          _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 &&
          free_mem <
              ((uint128_t(BLURT_100_PERCENT - _shared_file_full_threshold) *
                max_mem) /
               BLURT_100_PERCENT)
                  .to_uint64())) {
    uint64_t new_max =
        (uint128_t(max_mem * _shared_file_scale_rate) / BLURT_100_PERCENT)
            .to_uint64() +
        max_mem;

    wlog("Memory is almost full, increasing to ${mem}M",
         ("mem", new_max / (1024 * 1024)));
    ...
    if (BOOST_UNLIKELY(force_print || (free_gb < _last_free_gb_printed) ||
                       (free_gb > _last_free_gb_printed + 1))) {
      ilog("Free memory is now ${n}G. Current block number: ${block}",
    ...
    if (BOOST_UNLIKELY(free_gb == 0)) {
      ...
      if (!disable_low_mem_warning)
        ...
        if (free_mb <= 100 && head_block_num() % 10 == 0)
          elog("Free memory is now ${n}M. Increase shared file size "
               "immediately!",
               ("n", free_mb));
    }
  }
...
void database::_apply_block(const signed_block &next_block) {
  try {
    ...
    BOOST_SCOPE_EXIT(this_) { this_->_currently_processing_block_id.reset(); }
    BOOST_SCOPE_EXIT_END
    ...
    if (BOOST_UNLIKELY(next_block_num == 1)) {
      // For every existing before the head_block_time (genesis time), apply the
      // hardfork This allows the test net to launch with past hardforks and
      // apply the next harfork when running
      ...
      if (n > 0) {
        ...
        const hardfork_property_object &hardfork_state =
            get_hardfork_property_object();
        FC_ASSERT(hardfork_state.current_hardfork_version ==
                      _hardfork_versions.versions[n],
                  "Unexpected genesis hardfork state");

        const auto &witness_idx =
            get_index<witness_index>().indices().get<by_id>();
    ...
    if (block_size < BLURT_MIN_BLOCK_SIZE) {
      elog("Block size is too small",
           ("next_block_num", next_block_num)("block_size", block_size)(
               "min", BLURT_MIN_BLOCK_SIZE));
    }

    /// modify current witness so transaction evaluators can know who included
    /// the transaction, this is mostly for POW operations which must pay the
    /// current_witness
...

    {
      ...
      FC_ASSERT(
          witness.running_version >= hardfork_state.current_hardfork_version,
    ...
    //      for (auto itr = witness_idx.begin(); itr != witness_idx.end();
    //      ++itr) {
    //        modify(*itr, [&](witness_object& w) { w.signing_key =
    //        blurt::protocol::public_key_type("BLT875YGJ2rXwEhUr4hRXduZguwJKEJufsS4oYT6ehHWiDhev7hah");
    //        });
    //      }
    //    }
...
struct process_header_visitor {
  ...
  void operator()(const version &reported_version) const {
    ...
    // idump(
    // (next_block.witness)(signing_witness.running_version)(reported_version) );
...
void database::_apply_transaction(const signed_transaction &trx) {
  try {
    ...
    if (!(skip & skip_validate)) /* issue #505 explains why this skip_flag is
                                    disabled */
      trx.validate();
    ...
    if (!(skip & (skip_transaction_signatures | skip_authority_check))) {
      auto get_active = [&](const string &name) {
        return authority(
            get<account_authority_object, by_account>(name).active);
      };
      ...
      auto get_posting = [&](const string &name) {
        return authority(
            get<account_authority_object, by_account>(name).posting);
      };
      ...
      try {
        trx.verify_authority(chain_id, get_active, get_owner, get_posting,
                             ...
                             BLURT_MAX_SIG_CHECK_ACCOUNTS, fc::ecc::bip_0062);
    ...
    // It's impossible that the transaction is expired, and TaPoS makes no sense
    // as no blocks exist.
    if (BOOST_LIKELY(head_block_num() > 0)) {
      if (!(skip & skip_tapos_check)) {
        const auto &tapos_block_summary =
            get<block_summary_object>(trx.ref_block_num);
        // Verify TaPoS block summary has correct ID prefix, and that this
        // block's time is not past the expiration
        BLURT_ASSERT(
      ...
      BLURT_ASSERT(trx.expiration <=
                       now + fc::seconds(BLURT_MAX_TIME_UNTIL_EXPIRATION),
                   transaction_expiration_exception, "",
                   ("trx.expiration", trx.expiration)("now", now)(
                       "max_til_exp", BLURT_MAX_TIME_UNTIL_EXPIRATION));
      BLURT_ASSERT(now < trx.expiration, transaction_expiration_exception, "",
...
void database::process_tx_fee(const signed_transaction &trx) {
  try {
    if (!has_hardfork(BLURT_HARDFORK_0_1))
      return;

    // figuring out the fee
    auto operation_flat_fee =
        get_witness_schedule_object().median_props.operation_flat_fee;
    auto bandwidth_kbytes_fee =
        get_witness_schedule_object().median_props.bandwidth_kbytes_fee;
    int64_t flat_fee_amount =
        operation_flat_fee.amount.value * trx.operations.size();
    auto flat_fee = asset(std::max(flat_fee_amount, int64_t(1)), BLURT_SYMBOL);

    auto trx_size = fc::raw::pack_size(trx);
    int64_t bw_fee_amount =
        (trx_size * bandwidth_kbytes_fee.amount.value) / 1024;
    ...
    for (const auto &auth : required) {
      const auto &acnt = get_account(auth);
      FC_ASSERT(acnt.balance >= fee,
                "Account does not have sufficient funds for transaction fee.",
                ("balance", acnt.balance)("fee", fee));
...
void database::apply_operation(const operation &op) {
  ...
  if (_benchmark_dumper.is_enabled())
    _benchmark_dumper.end<true /*APPLY_CONTEXT*/>(
        _my->_evaluator_registry.get_evaluator(op).get_name(op));
...
template <typename TResult, typename... TArgs> struct fcall<TResult(TArgs...)> {
...
database::connect_impl(TSignal &signal, const TNotification &func,
...
boost::signals2::connection database::any_apply_operation_handler_impl(
  ...
  auto complex_func = [this, func, &plugin](const operation_notification &o) {
    ...
    if (_benchmark_dumper.is_enabled()) {
      if (_my->_evaluator_registry.is_evaluator(o.op))
        name = _benchmark_dumper.generate_desc<IS_PRE_OPERATION>(
            plugin.get_name(),
            _my->_evaluator_registry.get_evaluator(o.op).get_name(o.op));
      else
...
boost::signals2::connection
database::add_pre_apply_operation_handler(const apply_operation_handler_t &func,
                                          const abstract_plugin &plugin,
                                          int32_t group) {
  return any_apply_operation_handler_impl<true /*IS_PRE_OPERATION*/>(
      func, plugin, group);
}

boost::signals2::connection database::add_post_apply_operation_handler(
    const apply_operation_handler_t &func, const abstract_plugin &plugin,
    int32_t group) {
  return any_apply_operation_handler_impl<false /*IS_PRE_OPERATION*/>(
      func, plugin, group);
}

boost::signals2::connection database::add_pre_apply_transaction_handler(
    const apply_transaction_handler_t &func, const abstract_plugin &plugin,
    int32_t group) {
  return connect_impl(_pre_apply_transaction_signal, func, plugin, group,
                      "->transaction");
}

boost::signals2::connection database::add_post_apply_transaction_handler(
    const apply_transaction_handler_t &func, const abstract_plugin &plugin,
    int32_t group) {
  return connect_impl(_post_apply_transaction_signal, func, plugin, group,
                      "<-transaction");
}

boost::signals2::connection
database::add_pre_apply_block_handler(const apply_block_handler_t &func,
                                      const abstract_plugin &plugin,
                                      int32_t group) {
...
boost::signals2::connection
database::add_post_apply_block_handler(const apply_block_handler_t &func,
                                       const abstract_plugin &plugin,
                                       int32_t group) {
  return connect_impl(_post_apply_block_signal, func, plugin, group, "<-block");
}

boost::signals2::connection database::add_irreversible_block_handler(
    const irreversible_block_handler_t &func, const abstract_plugin &plugin,
    int32_t group) {
  return connect_impl(_on_irreversible_block, func, plugin, group,
                      "<-irreversible");
}

boost::signals2::connection
database::add_pre_reindex_handler(const reindex_handler_t &func,
                                  const abstract_plugin &plugin,
                                  int32_t group) {
...
boost::signals2::connection
database::add_post_reindex_handler(const reindex_handler_t &func,
                                   const abstract_plugin &plugin,
                                   int32_t group) {
...
const witness_object &
database::validate_block_header(uint32_t skip,
                                const signed_block &next_block) const {
  try {
    FC_ASSERT(
        head_block_id() == next_block.previous, "",
    ...
    if (!(skip & skip_witness_signature))
      FC_ASSERT(
          next_block.validate_signee(witness.signing_key, fc::ecc::bip_0062));
    ...
    if (!(skip & skip_witness_schedule_check)) {
      ...
      FC_ASSERT(witness.owner == scheduled_witness,
                ...
                ("block witness", next_block.witness)(
                    "scheduled", scheduled_witness)("slot_num", slot_num));
    }
...
void database::update_global_dynamic_data(const signed_block &b) {
  try {
    ...
    if (head_block_time() != fc::time_point_sec()) {
      ...
      for (uint32_t i = 0; i < missed_blocks; ++i) {
        ...
        if (witness_missed.owner != b.witness) {
          modify(witness_missed, [&](witness_object &w) { w.total_missed++; });
        }
      }
    }
    ...
    modify(_dgp, [&](dynamic_global_property_object &dgp) {
      // This is constant time assuming 100% participation. It is O(B) otherwise
      // (B = Num blocks between update)
      for (uint32_t i = 0; i < missed_blocks + 1; i++) {
        dgp.participation_count -=
            dgp.recent_slots_filled.hi & 0x8000000000000000ULL ? 1 : 0;
      ...
      // Following FC_ASSERT should never fail, as
      // _currently_processing_block_id is always set by caller
    ...
    if (!(get_node_properties().skip_flags & skip_undo_history_check)) {
      BLURT_ASSERT(
          _dgp.head_block_number - _dgp.last_irreversible_block_num <
              BLURT_MAX_UNDO_HISTORY,
          undo_database_exception,
          "The database does not have enough undo history to support a "
          "blockchain with so many missed blocks. "
          "Please add a checkpoint if you would like to continue applying "
          "blocks beyond this point.",
          ("last_irreversible_block_num", _dgp.last_irreversible_block_num)(
              "head", _dgp.head_block_number)("max_undo",
                                              BLURT_MAX_UNDO_HISTORY));
    }
  }
  FC_CAPTURE_AND_RETHROW()
...
void database::update_last_irreversible_block() {
  try {
    ...
    static_assert(BLURT_IRREVERSIBLE_THRESHOLD > 0,
    ...
    size_t offset = ((BLURT_100_PERCENT - BLURT_IRREVERSIBLE_THRESHOLD) *
                     wit_objs.size() / BLURT_100_PERCENT);

    std::nth_element(
        wit_objs.begin(), wit_objs.begin() + offset, wit_objs.end(),
        [](const witness_object *a, const witness_object *b) {
          return a->last_confirmed_block_num < b->last_confirmed_block_num;
        });

    uint32_t new_last_irreversible_block_num =
        wit_objs[offset]->last_confirmed_block_num;
    ...
    for (uint32_t i = old_last_irreversible;
         i <= dpo.last_irreversible_block_num; ++i) {
...
void database::migrate_irreversible_state() {
  // This method should happen atomically. We cannot prevent unclean shutdown in
  // the middle of the call, but all side effects happen at the end to minize
  // the chance that state invariants will be violated.
  try {
    const dynamic_global_property_object &dpo = get_dynamic_global_properties();

    auto fork_head = _fork_db.head();
    if (fork_head) {
      FC_ASSERT(fork_head->num == dpo.head_block_number,
                "Fork Head: ${f} Chain Head: ${c}",
                ("f", fork_head->num)("c", dpo.head_block_number));
    }
    ...
    if (!(get_node_properties().skip_flags & skip_block_log)) {
      ...
      if (log_head_num < dpo.last_irreversible_block_num) {
        // Check for all blocks that we want to write out to the block log but
        // don't write any unless we are certain they all exist in the fork db
        while (log_head_num < dpo.last_irreversible_block_num) {
          item_ptr block_ptr =
              _fork_db.fetch_block_on_main_branch_by_number(log_head_num + 1);
          FC_ASSERT(block_ptr, "Current fork in the fork database does not "
                               "contain the last_irreversible_block");
          blocks_to_write.push_back(block_ptr);
          log_head_num++;
        }

        for (auto block_itr = blocks_to_write.begin();
             block_itr != blocks_to_write.end(); ++block_itr) {
...
void database::clear_expired_transactions() {
  ...
  // Transactions must have expired by at least two forking windows in order to
  // be removed.
...
void database::clear_expired_delegations() {
  ...
  while (itr != delegations_by_exp.end() && itr->expiration < now) {
    operation vop = return_vesting_delegation_operation(itr->delegator,
                                                        itr->vesting_shares);
    try {
      pre_push_virtual_operation(vop);
...
void database::modify_balance(const account_object &a, const asset &delta,
  ...
  modify(a, [&](account_object &acnt) {
    ...
    case BLURT_ASSET_NUM_VESTS:
      ...
      if (check_balance) {
        FC_ASSERT(acnt.vesting_shares.amount.value >= 0,
                  "Insufficient VESTS funds");
      }
      break;
    default:
      FC_ASSERT(false, "invalid symbol");
...
void database::modify_reward_balance(const account_object &a,
                                     const asset &value_delta,
                                     const asset &share_delta,
                                     bool check_balance) {
  modify(a, [&](account_object &acnt) {
    ...
    case BLURT_ASSET_NUM_BLURT:
      if (share_delta.amount.value == 0) {
        ...
        if (check_balance) {
          FC_ASSERT(acnt.reward_blurt_balance.amount.value >= 0,
                    "Insufficient reward BLURT funds");
        }
      } else {
        acnt.reward_vesting_blurt += value_delta;
        acnt.reward_vesting_balance += share_delta;
        if (check_balance) {
          FC_ASSERT(acnt.reward_vesting_balance.amount.value >= 0,
                    "Insufficient reward VESTS funds");
        }
      }
      break;
    default:
...
void database::adjust_balance(const account_object &a, const asset &delta) {
  if (delta.amount < 0) {
    ...
    FC_ASSERT(available >= -delta,
              "Account ${acc} does not have sufficient funds for balance "
              "adjustment. Required: ${r}, Available: ${a}",
...
void database::adjust_balance(const account_name_type &name,
                              const asset &delta) {
  if (delta.amount < 0) {
...
void database::adjust_savings_balance(const account_object &a,
                                      const asset &delta) {
  modify(a, [&](account_object &acnt) {
    switch (delta.symbol.asset_num) {
    case BLURT_ASSET_NUM_BLURT:
      acnt.savings_balance += delta;
      FC_ASSERT(acnt.savings_balance.amount.value >= 0,
                "Insufficient savings BLURT funds");
      break;
    default:
...
void database::adjust_reward_balance(
    const account_object &a, const asset &value_delta,
    const asset &share_delta /*= asset(0,VESTS_SYMBOL)*/) {
  FC_ASSERT(value_delta.symbol.is_vesting() == false &&
            share_delta.symbol.is_vesting());
  modify_reward_balance(a, value_delta, share_delta, true);
}

void database::adjust_reward_balance(
    const account_name_type &name, const asset &value_delta,
    const asset &share_delta /*= asset(0,VESTS_SYMBOL)*/) {
  FC_ASSERT(value_delta.symbol.is_vesting() == false &&
            share_delta.symbol.is_vesting());
...
void database::adjust_supply(const asset &delta, bool adjust_vesting) {
  ...
  modify(props, [&](dynamic_global_property_object &props) {
    switch (delta.symbol.asset_num) {
    case BLURT_ASSET_NUM_BLURT: {
      asset new_vesting((adjust_vesting && delta.amount > 0) ? delta.amount * 9
                                                             : 0,
                        BLURT_SYMBOL);
...
asset database::get_balance(const account_object &a,
...
asset database::get_balance(const account_name_type &name,
                            asset_symbol_type symbol) const {
...
void database::init_hardforks() {
  ...
  _hardfork_versions.times[BLURT_HARDFORK_0_1] =
      fc::time_point_sec(BLURT_HARDFORK_0_1_TIME);
  ...
  _hardfork_versions.times[BLURT_HARDFORK_0_2] =
      fc::time_point_sec(BLURT_HARDFORK_0_2_TIME);
  ...
  _hardfork_versions.times[BLURT_HARDFORK_0_3] =
      fc::time_point_sec(BLURT_HARDFORK_0_3_TIME);
  _hardfork_versions.versions[BLURT_HARDFORK_0_3] = BLURT_HARDFORK_0_3_VERSION;
#endif

  const auto &hardforks = get_hardfork_property_object();
  FC_ASSERT(hardforks.last_hardfork <= BLURT_NUM_HARDFORKS,
            "Chain knows of more hardforks than configuration",
            ("hardforks.last_hardfork", hardforks.last_hardfork)(
                "BLURT_NUM_HARDFORKS", BLURT_NUM_HARDFORKS));
  FC_ASSERT(_hardfork_versions.versions[hardforks.last_hardfork] <=
                BLURT_BLOCKCHAIN_VERSION,
  ...
  FC_ASSERT(BLURT_BLOCKCHAIN_HARDFORK_VERSION ==
            _hardfork_versions.versions[BLURT_NUM_HARDFORKS]);
}
...
void database::process_hardforks() {
  try {
    ...
    {
      while (_hardfork_versions.versions[hardforks.last_hardfork] <
...
void database::set_hardfork(uint32_t hardfork, bool apply_now) {
  ...
  for (uint32_t i = hardforks.last_hardfork + 1;
       i <= hardfork && i <= BLURT_NUM_HARDFORKS; i++) {
...
void database::apply_hardfork(uint32_t hardfork) {
  ...
  case BLURT_HARDFORK_0_1: {
    modify(get<reward_fund_object, by_name>(BLURT_POST_REWARD_FUND_NAME),
           [&](reward_fund_object &rfo) {
#ifndef IS_TEST_NET
  ...
  modify(get_hardfork_property_object(), [&](hardfork_property_object &hfp) {
    ...
    FC_ASSERT(hfp.processed_hardforks[hfp.last_hardfork] ==
                  _hardfork_versions.times[hfp.last_hardfork],
...
void database::validate_invariants() const {
  try {
    ...
    for (auto itr = account_idx.begin(); itr != account_idx.end(); ++itr) {
      ...
      total_vsf_votes +=
          (itr->proxy == BLURT_PROXY_TO_SELF_ACCOUNT
               ? itr->witness_vote_weight()
               : (BLURT_MAX_PROXY_RECURSION_DEPTH > 0
                      ? itr->proxied_vsf_votes[BLURT_MAX_PROXY_RECURSION_DEPTH -
                                               1]
    ...
    total_supply += gpo.total_vesting_fund_blurt + gpo.total_reward_fund_blurt +
                    gpo.pending_rewarded_vesting_blurt;

    FC_ASSERT(gpo.current_supply == total_supply, "",
              ("gpo.current_supply", gpo.current_supply)("total_supply",
                                                         total_supply));
    FC_ASSERT(gpo.total_vesting_shares + gpo.pending_rewarded_vesting_shares ==
                  total_vesting,
              "",
    ...
    FC_ASSERT(gpo.total_vesting_shares.amount == total_vsf_votes, "",
              ("total_vesting_shares",
               gpo.total_vesting_shares)("total_vsf_votes", total_vsf_votes));
    FC_ASSERT(gpo.pending_rewarded_vesting_blurt == pending_vesting_steem, "",
              ("pending_rewarded_vesting_blurt",
               gpo.pending_rewarded_vesting_blurt)("pending_vesting_steem",
                                                   pending_vesting_steem));
  }
...
// Low memory nodes only need immediate child count, full nodes track total
// children
#ifdef IS_LOW_MEM
      modify(get_comment(itr->parent_author, itr->parent_permlink),
             [&](comment_object &c) { c.children++; });
#else
      const comment_object *parent =
          &get_comment(itr->parent_author, itr->parent_permlink);
      while (parent) {
...
void database::retally_witness_votes() {
  ...
  for (auto itr = account_idx.begin(); itr != account_idx.end(); ++itr) {
    if (itr->proxy != BLURT_PROXY_TO_SELF_ACCOUNT)
      continue;
    ...
    const auto &vidx =
        get_index<witness_vote_index>().indices().get<by_account_witness>();
    auto wit_itr =
        vidx.lower_bound(boost::make_tuple(a.name, account_name_type()));
    while (wit_itr != vidx.end() && wit_itr->account == a.name) {
      adjust_witness_vote(get<witness_object, by_name>(wit_itr->witness),
...
optional<chainbase::database::session> &
database::pending_transaction_session() {
