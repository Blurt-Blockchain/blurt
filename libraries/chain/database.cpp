   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         ...
         case STEEM_ASSET_NUM_SBD:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.sbd_balance.amount.value >= 0, "Insufficient BBD funds" );
            }
            break;
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( value_delta.symbol.asset_num )
      {
         ...
         case STEEM_ASSET_NUM_SBD:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.reward_sbd_balance.amount.value >= 0, "Insufficient reward BBD funds" );
            }
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         ...
         case STEEM_ASSET_NUM_SBD:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_sbd_balance.amount.value >= 0, "Insufficient savings BBD funds" );
            }
            break;
         default:
   ...
   try
   {
      ...
      for( auto itr = escrow_idx.begin(); itr != escrow_idx.end(); ++itr )
      {
         ...
         if( itr->pending_fee.symbol == STEEM_SYMBOL )
            ...
            FC_ASSERT( false, "found escrow pending fee that is not BBD or BLURT" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == STEEM_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not BBD or BLURT" );
      }
