...
struct witness_properties {
  ...
  legacy_asset operation_flat_fee =
      legacy_asset::from_asset(asset(50, BLURT_SYMBOL));
  legacy_asset bandwidth_kbytes_fee =
      legacy_asset::from_asset(asset(10, BLURT_SYMBOL));
};
...
public:
   ...
   */
  vector<database_api::api_withdraw_vesting_route_object> get_withdraw_routes(
      string account,
      condenser_api::withdraw_route_type type = condenser_api::all) const;
   ...
   * retrieve all accounts, start by setting \c lowerbound to the empty string
   * \c "", and then each iteration, pass the last account name returned as the
   * \c lowerbound for the next \c list_accounts() call.
   *
   * @param lowerbound the name of the first account to return.  If the named
   * account does not exist, the list will start at the account that comes after
   * \c lowerbound
   * @param limit the maximum number of accounts to return (max: 1000)
   * @returns a list of accounts mapping account names to account ids
   */
  vector<account_name_type> list_accounts(const string &lowerbound,
                                          uint32_t limit);
   ...
   */
  condenser_api::extended_dynamic_global_properties
   ...
   *  @return public key corresponding to generated private key, and private key
   * in WIF format.
   */
  ...
  get_transaction(transaction_id_type trx_id) const;
   ...
   * future writes, so think of this function as 'Save a Copy As...' instead of
   ...
   */
  condenser_api::legacy_signed_transaction
  create_account(string creator, string new_account_name, string json_meta,
                 bool broadcast);
   ...
   */
  condenser_api::legacy_signed_transaction
  create_account_with_keys(string creator, string newname, string json_meta,
   ...
   */
  condenser_api::legacy_signed_transaction
  update_account(string accountname, string json_meta, public_key_type owner,
   ...
   * @param weight The weight the key should have in the authority. A weight of
   * 0 indicates the removal of the key.
   * @param broadcast true if you wish to broadcast the transaction.
   */
  condenser_api::legacy_signed_transaction
  update_account_auth_key(string account_name, authority_type type,
   ...
   * @param weight The weight the account should have in the authority. A weight
   * of 0 indicates the removal of the account.
   * @param broadcast true if you wish to broadcast the transaction.
   */
  condenser_api::legacy_signed_transaction
  update_account_auth_account(string account_name, authority_type type,
   ...
   */
  condenser_api::legacy_signed_transaction
  update_account_auth_threshold(string account_name, authority_type type,
   ...
   */
  condenser_api::legacy_signed_transaction
  update_account_memo_key(string account_name, public_key_type key,
                          bool broadcast);
   ...
   */
  condenser_api::legacy_signed_transaction
  delegate_vesting_shares(string delegator, string delegatee,
   ...
   * \c "", and then each iteration, pass the last witness name returned as the
   * \c lowerbound for the next \c list_witnesss() call.
   *
   * @param lowerbound the name of the first witness to return.  If the named
   * witness does not exist, the list will start at the witness that comes after
   * \c lowerbound
   * @param limit the maximum number of witnesss to return (max: 1000)
   * @returns a list of witnesss mapping witness names to witness ids
   */
  vector<account_name_type> list_witnesses(const string &lowerbound,
                                           uint32_t limit);

  /** Returns information about the given witness.
   * @param owner_account the name or id of the witness account owner, or the id
   * of the witness
   ...
   * @param url A URL containing some information about the witness.  The empty
   * string makes it remain the same.
   * @param block_signing_key The new block signing public key.  The empty
   * string disables block production.
   * @param props The chain properties the witness is voting on.
   * @param broadcast true if you wish to broadcast the transaction.
   */
  condenser_api::legacy_signed_transaction update_witness(
      string witness_name, string url, public_key_type block_signing_key,
      const condenser_api::api_chain_properties &props, bool broadcast = false);

  /**
   * witness_set_properties operation
   */
  condenser_api::legacy_signed_transaction
  update_witness_properties(string witness_name,
   ...
   */
  condenser_api::legacy_signed_transaction
  set_voting_proxy(string account_to_modify, string proxy,
                   bool broadcast = false);
   ...
   * @param approve true if the account is voting for the account to be able to
   * be a block produce
   * @param broadcast true if you wish to broadcast the transaction
   */
  condenser_api::legacy_signed_transaction
  vote_for_witness(string account_to_vote_with, string witness_to_vote_for,
   ...
   */
  condenser_api::legacy_signed_transaction
  transfer(string from, string to, condenser_api::legacy_asset amount,
           string memo, bool broadcast = false);

  /**
   * Transfer funds from one account to another using escrow. BLURT can be
   * transferred.
   *
   * @param from The account the funds are coming from
   * @param to The account the funds are going to
   ...
   * the escrow transfer
   * @param escrow_expiration The expiration of the escrow transfer, after which
   * either party can claim the funds
   ...
   */
  condenser_api::legacy_signed_transaction
  escrow_approve(string from, string to, string agent, string who,
   ...
   */
  condenser_api::legacy_signed_transaction
  escrow_dispute(string from, string to, string agent, string who,
   ...
   */
  condenser_api::legacy_signed_transaction
  escrow_release(string from, string to, string agent, string who,
                 string receiver, uint32_t escrow_id,
                 condenser_api::legacy_asset blurt_amount,
                 bool broadcast = false);

  /**
   * Transfer BLURT into a vesting fund represented by vesting shares (VESTS).
   ...
   * against dilution up until 90% of BLURT is vesting.
   *
   ...
   */
  condenser_api::legacy_signed_transaction
  transfer_to_vesting(string from, string to,
   ...
   */
  condenser_api::legacy_signed_transaction
  transfer_to_savings(string from, string to,
                      condenser_api::legacy_asset amount, string memo,
                      bool broadcast = false);

  /**
   *  @param from       - the account that initiated the transfer
   *  @param request_id - an unique ID assigned by from account, the id is used
   * to cancel the operation and can be reused after the transfer completes
   ...
   */
  condenser_api::legacy_signed_transaction
  transfer_from_savings(string from, uint32_t request_id, string to,
                        condenser_api::legacy_asset amount, string memo,
                        bool broadcast = false);
   ...
   */
  condenser_api::legacy_signed_transaction
  cancel_transfer_from_savings(string from, uint32_t request_id,
                               bool broadcast = false);
   ...
   * @param vesting_shares The amount of VESTS to withdraw over the next two
   * years. Each week (amount/104) shares are withdrawn and deposited back as
   * BLURT. i.e. "10.000000 VESTS"
   * @param broadcast true if you wish to broadcast the transaction
   */
  condenser_api::legacy_signed_transaction
  withdraw_vesting(string from, condenser_api::legacy_asset vesting_shares,
                   bool broadcast = false);
   ...
   * @param percent The percent of the withdraw to go to the 'to' account. This
   * is denoted in hundreths of a percent. i.e. 100 is 1% and 10000 is 100%.
   * This value must be between 1 and 100000
   * @param auto_vest Set to true if the from account should receive the VESTS
   * as VESTS, or false if it should receive them as BLURT.
   * @param broadcast true if you wish to broadcast the transaction.
   */
  condenser_api::legacy_signed_transaction
  set_withdraw_vesting_route(string from, string to, uint16_t percent,
   ...
   */
  condenser_api::legacy_signed_transaction
  sign_transaction(condenser_api::legacy_signed_transaction tx,
                   bool broadcast = false);
   ...
   * builder's \c add_operation_to_builder_transaction() , but to do that from
   ...
   */
  condenser_api::legacy_signed_transaction
  post_comment(string author, string permlink, string parent_author,
   ...
   * @param permlink The permlink of the comment to be voted on. (author,
   * permlink) is a unique pair
   * @param weight The weight [-100,100] of the vote
   * @param broadcast true if you wish to broadcast the transaction
   */
  condenser_api::legacy_signed_transaction vote(string voter, string author,
                                                string permlink, int16_t weight,
                                                bool broadcast);
   ...
   */
  condenser_api::legacy_signed_transaction
  request_account_recovery(string recovery_account, string account_to_recover,
   ...
   */
  condenser_api::legacy_signed_transaction
  recover_account(string account_to_recover, authority recent_authority,
                  authority new_authority, bool broadcast);
   ...
   */
  condenser_api::legacy_signed_transaction
  change_recovery_account(string owner, string new_recovery_account,
                          bool broadcast);

  vector<database_api::api_owner_authority_history_object>
  get_owner_history(string account) const;
   ...
   */
  void check_memo(const string &memo,
  ...
  condenser_api::legacy_signed_transaction
  claim_reward_balance(string account, condenser_api::legacy_asset reward_blurt,
   ...
   * @param daily_pay  - the amount of BLURT_SYMBOL that is being requested to
   * be paid out daily,
   * @param subject    - briefly description of proposal of its title,
   * @param url        - link to page with description of proposal.
   */
  condenser_api::legacy_signed_transaction
  create_proposal(account_name_type creator, account_name_type receiver,
                  time_point_sec start_date, time_point_sec end_date,
                  condenser_api::legacy_asset daily_pay, string subject,
                  string url, bool broadcast);
  /**
   ...
   */
  condenser_api::legacy_signed_transaction
  update_proposal_votes(account_name_type voter, flat_set<int64_t> proposals,
   ...
   * @param status     - list only results with given status (all, inactive,
   * active, expired, votable),
   */
  condenser_api::list_proposals_return list_proposals(
      fc::variant start, uint32_t limit, database_api::sort_order_type order_by,
  ...
  find_proposals(vector<int64_t> proposal_ids);
   ...
   */
  condenser_api::legacy_signed_transaction
  remove_proposal(account_name_type deleter, flat_set<int64_t> ids,
                  bool broadcast);
};
...
FC_REFLECT(blurt::wallet::wallet_data,
           (cipher_keys)(ws_server)(ws_user)(ws_password))

FC_REFLECT(blurt::wallet::brain_key_info,
...
FC_API(blurt::wallet::wallet_api,
       /// wallet api
       (help)(gethelp)(about)(is_new)(is_locked)(lock)(unlock)(set_password)(
           load_wallet_file)(save_wallet_file)

       /// key api
       (import_key)(suggest_brain_key)(list_keys)(get_private_key)(
           get_private_key_from_password)(normalize_brain_key)

       /// query api
       (info)(list_my_accounts)(list_accounts)(list_witnesses)(get_witness)(
           get_account)(get_block)(get_ops_in_block)(get_account_history)(
           get_state)(get_withdraw_routes)

       /// transaction api
       (create_account)(create_account_with_keys)(update_account)(
           update_account_auth_key)(update_account_auth_account)(
           update_account_auth_threshold)(update_account_meta)(
           update_account_memo_key)(delegate_vesting_shares)(update_witness)(
           update_witness_properties)(set_voting_proxy)(vote_for_witness)(
           follow)(transfer)(escrow_transfer)(escrow_approve)(escrow_dispute)(
           escrow_release)(transfer_to_vesting)(withdraw_vesting)(
           set_withdraw_vesting_route)(post_comment)(vote)(
           set_transaction_expiration)(request_account_recovery)(
           recover_account)(change_recovery_account)(get_owner_history)(
           transfer_to_savings)(transfer_from_savings)(
           cancel_transfer_from_savings)(get_encrypted_memo)(decrypt_memo)(
       ...
       /// worker proposal api
       (create_proposal)(update_proposal_votes)(list_proposals)(find_proposals)(
           list_proposal_votes)(remove_proposal))

FC_REFLECT(blurt::wallet::memo_data, (from)(to)(nonce)(check)(encrypted))
FC_REFLECT(
    blurt::wallet::witness_properties,
    (key)(account_creation_fee)(maximum_block_size)(account_subsidy_budget)(
        account_subsidy_decay)(operation_flat_fee)(bandwidth_kbytes_fee))