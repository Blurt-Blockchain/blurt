...
string normalize_brain_key(string s) {
  ...
  while (i < n) {
    ...
    if (preceded_by_whitespace && non_empty) result.push_back(' ');
    result.push_back(c);
...
class wallet_api_impl {
 ...
 public:
  ...
  wallet_api_impl(wallet_api &s, const wallet_data &initial_data, fc::api_connection &con)
      : self(s),
  ...
  bool copy_wallet_file(string destination_filename) {
    ...
    if (!fc::exists(src_path)) return false;
    ...
    try {
      ...
      if (!fc::exists(dest_parent)) fc::create_directories(dest_parent);
  ...
  variant info() const {
    ...
    result["hardfork_version"] = fc::string(_remote_database_api->get_hardfork_version());
    ...
    result["post_reward_fund"] = fc::variant(_remote_database_api->get_reward_fund(BLURT_POST_REWARD_FUND_NAME)).get_object();
    return result;
  }
  ...
  variant_object about() const {
    ...
    if (pos != string::npos && client_version.size() > pos) client_version = client_version.substr(pos + 1);
  ...
  optional<fc::ecc::private_key> try_get_private_key(const public_key_type &id) const {
    ...
    if (it != _keys.end()) return wif_to_key(it->second);
  ...
  fc::ecc::private_key get_private_key_for_account(const account_api_obj &account) const {
    ...
    if (active_keys.size() != 1) FC_THROW("Expecting a simple authority with one active key");
  ...
  bool import_key(string wif_key) {
    ...
    if (!optional_private_key) FC_THROW("Invalid private key");
  ...
  bool load_wallet_file(string wallet_filename = "") {
    ...
    if (wallet_filename == "") wallet_filename = _wallet_filename;

    if (!fc::exists(wallet_filename)) return false;
    ...
    for (int key_index = 0;; ++key_index) {
      ...
      if (_keys.find(derived_public_key) == _keys.end()) {
        if (number_of_consecutive_unused_keys) {
          ...
          if (number_of_consecutive_unused_keys > 5) return first_unused_index;
        } else {
  ...
  signed_transaction create_account_with_private_key(fc::ecc::private_key owner_privkey, string account_name, string creator_account_name,
    ...
    try {
      ...
      if (save_wallet) save_wallet_file();
      if (broadcast) {
  ...
  optional<witness_api_obj> get_witness(string owner_account) { return _remote_database_api->get_witness_by_account(owner_account); }
  ...
  annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false) {
    ...
    for (const auto &auth : other_auths)
      for (const auto &a : auth.account_auths) req_active_approvals.insert(a.first);

    // std::merge lets us de-duplicate account_id's that occur in both
    //   sets, and dump them into a vector (as required by remote_db api)
    //   at the same time
    vector<string> v_approving_account_names;
    std::merge(req_active_approvals.begin(), req_active_approvals.end(), req_owner_approvals.begin(), req_owner_approvals.end(),
               std::back_inserter(v_approving_account_names));

    for (const auto &a : req_posting_approvals) v_approving_account_names.push_back(a);
    ...
    for (const optional<account_api_obj> &approving_acct : approving_account_objects) {
      if (!approving_acct.valid()) {
        wlog("operation_get_required_auths said approval of non-existing account ${name} was needed", ("name", v_approving_account_names[i]));
        i++;
        continue;
      }
    ...
    for (account_name_type &acct_name : req_active_approvals) {
      ...
      if (it == approving_account_lut.end()) continue;
    ...
    auto minimal_signing_keys = tx.minimize_required_signatures(
        BLURT_CHAIN_ID, available_keys, [&](const string &account_name) -> const authority & { return (get_account_from_lut(account_name).active); },
        [&](const string &account_name) -> const authority & { return (get_account_from_lut(account_name).owner); },
        [&](const string &account_name) -> const authority & { return (get_account_from_lut(account_name).posting); }, BLURT_MAX_SIG_CHECK_DEPTH);
    ...
    for (const public_key_type &k : minimal_signing_keys) {
      ...
      tx.sign(it->second, BLURT_CHAIN_ID);
    }
  ...
  std::map<string, std::function<string(fc::variant, const fc::variants &)>> get_result_formatters() const {
    ...
    m["list_my_accounts"] = [](variant result, const fc::variants &a) {
      ...
      for (const auto &a : accounts) {
        ...
        out << std::left << std::setw(17) << std::string(a.name) << std::right << std::setw(18) << fc::variant(a.balance).as_string() << " " << std::right
            << std::setw(26) << fc::variant(a.vesting_shares).as_string() << "\n";
      }
      out << "-------------------------------------------------------------------------\n";
      out << std::left << std::setw(17) << "TOTAL" << std::right << std::setw(18) << fc::variant(total_steem).as_string() << " " << std::right << std::setw(26)
  ...
  operation get_prototype_operation(string operation_name) {
    ...
    if (it == _prototype_ops.end()) FC_THROW("Unsupported operation: \"${operation_name}\"", ("operation_name", operation_name));
    return it->second;
  }
...
wallet_api::wallet_api(const wallet_data &initial_data, fc::api_connection &con) : my(new detail::wallet_api_impl(*this, initial_data, con)) {}

wallet_api::~wallet_api() {}

bool wallet_api::copy_wallet_file(string destination_filename) { return my->copy_wallet_file(destination_filename); }

optional<signed_block_api_obj> wallet_api::get_block(uint32_t num) { return my->_remote_database_api->get_block(num); }
...
vector<account_api_obj> wallet_api::list_my_accounts() {
  ...
  for (const auto &item : my->_keys) pub_keys.push_back(item.first);

  auto refs = my->_remote_account_by_key_api->get_key_references(pub_keys);
  set<string> names;
  for (const auto &item : refs)
    for (const auto &name : item) names.insert(name);

  result.reserve(names.size());
  for (const auto &name : names) result.emplace_back(get_account(name));

  return result;
}

set<string> wallet_api::list_accounts(const string &lowerbound, uint32_t limit) { return my->_remote_database_api->lookup_accounts(lowerbound, limit); }

std::vector<account_name_type> wallet_api::get_active_witnesses() const { return my->_remote_database_api->get_active_witnesses(); }
...
std::map<string, std::function<string(fc::variant, const fc::variants &)>> wallet_api::get_result_formatters() const { return my->get_result_formatters(); }
...
void wallet_api::lock() {
  try {
    FC_ASSERT(!is_locked());
    encrypt_keys();
    for (auto key : my->_keys) key.second = key_to_wif(fc::ecc::private_key());
    my->_keys.clear();
...
void wallet_api::set_password(string password) {
  if (!is_new()) FC_ASSERT(!is_locked(), "The wallet must be unlocked before the password can be set");
 ...
 * wallet.
 */
annotated_signed_transaction wallet_api::create_account_with_keys(string creator, string new_account_name, string json_meta, public_key_type owner,
...
annotated_signed_transaction wallet_api::update_account(string account_name, string json_meta, public_key_type owner, public_key_type active,
...
annotated_signed_transaction wallet_api::update_account_auth_key(string account_name, authority_type type, public_key_type key, weight_type weight,
                                                                 bool broadcast) {
  FC_ASSERT(!is_locked());
...
annotated_signed_transaction wallet_api::update_account_auth_account(string account_name, authority_type type, string auth_account, weight_type weight,
                                                                     bool broadcast) {
  FC_ASSERT(!is_locked());
...
annotated_signed_transaction wallet_api::account_forsale(string accountname, bool for_sale, string to, asset price, bool broadcast) {
  ...
  auto accounts = my->_remote_database_api->get_accounts({accountname});
  FC_ASSERT(accounts.size() == 1, "Account does not exist");
...
annotated_signed_transaction wallet_api::update_witness(string witness_account_name, string url, public_key_type block_signing_key,
                                                        const chain_properties &props, bool broadcast) {
  FC_ASSERT(!is_locked());

  witness_update_operation op;

  fc::optional<witness_api_obj> wit = my->_remote_database_api->get_witness_by_account(witness_account_name);
  if (!wit.valid()) {
    op.url = url;
  } else {
...
void wallet_api::check_memo(const string &memo, const account_api_obj &account) const {
  ...
  for (auto &key_weight_pair : account.owner.key_auths) {
    for (auto &key : keys) FC_ASSERT(key_weight_pair.first != key, "Detected private owner key in memo field. Cancelling transaction.");
  }

  for (auto &key_weight_pair : account.active.key_auths) {
    for (auto &key : keys) FC_ASSERT(key_weight_pair.first != key, "Detected private active key in memo field. Cancelling transaction.");
  }

  for (auto &key_weight_pair : account.posting.key_auths) {
    for (auto &key : keys) FC_ASSERT(key_weight_pair.first != key, "Detected private posting key in memo field. Cancelling transaction.");
  }

  const auto &memo_key = account.memo_key;
  for (auto &key : keys) FC_ASSERT(memo_key != key, "Detected private memo key in memo field. Cancelling transaction.");

  // Check against imported keys
  for (auto &key_pair : my->_keys) {
    for (auto &key : keys) FC_ASSERT(key != key_pair.first, "Detected imported private key in memo field. Cancelling trasanction.");
  }
}
