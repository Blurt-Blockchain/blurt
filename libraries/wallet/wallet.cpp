...
public:
  ...
  wallet_api_impl(wallet_api &s, const wallet_data &initial_data,
                  const blurt::protocol::chain_id_type &_blurt_chain_id,
  ...
  variant info() const {
    ...
    result["witness_majority_version"] =
        fc::string(_remote_api->get_witness_schedule().majority_version);
    result["hardfork_version"] =
        fc::string(_remote_api->get_hardfork_version());
    ...
    result["account_creation_fee"] =
        _remote_api->get_chain_properties().account_creation_fee;
    result["post_reward_fund"] =
        fc::variant(_remote_api->get_reward_fund(BLURT_POST_REWARD_FUND_NAME))
            .get_object();
    return result;
  }
  ...
  variant_object about() const {
    ...
    result["steem_revision_age"] = fc::get_approximate_relative_time_string(
        fc::time_point_sec(blurt::utilities::git_revision_unix_timestamp));
    ...
    result["boost_version"] =
        boost::replace_all_copy(std::string(BOOST_LIB_VERSION), "_", ".");
  ...
  bool import_key(string wif_key) {
    ...
    blurt::chain::public_key_type wif_pub_key =
  ...
  // This function generates derived keys starting with index 0 and keeps
  // incrementing the index until it finds a key that isn't registered in the
  // block chain.  To be safer, it continues checking for a few more keys to
  // make sure there wasn't a short gap caused by a failed registration or the
  // like.
  int find_first_unused_derived_key_index(
      const fc::ecc::private_key &parent_key) {
    int first_unused_index = 0;
    int number_of_consecutive_unused_keys = 0;
    for (int key_index = 0;; ++key_index) {
      fc::ecc::private_key derived_private_key =
          derive_private_key(key_to_wif(parent_key), key_index);
      blurt::chain::public_key_type derived_public_key =
          derived_private_key.get_public_key();
  ...
  signed_transaction create_account_with_private_key(
      fc::ecc::private_key owner_privkey, string account_name,
      string creator_account_name, bool broadcast = false,
      bool save_wallet = true) {
    try {
      int active_key_index = find_first_unused_derived_key_index(owner_privkey);
      fc::ecc::private_key active_privkey =
          derive_private_key(key_to_wif(owner_privkey), active_key_index);

      int memo_key_index = find_first_unused_derived_key_index(active_privkey);
      fc::ecc::private_key memo_privkey =
          derive_private_key(key_to_wif(active_privkey), memo_key_index);

      blurt::chain::public_key_type owner_pubkey =
          owner_privkey.get_public_key();
      blurt::chain::public_key_type active_pubkey =
          active_privkey.get_public_key();
  ...
  optional<condenser_api::api_witness_object>
  ...
  annotated_signed_transaction sign_transaction(signed_transaction tx,
                                                bool broadcast = false) {
    ...
    auto approving_account_objects =
        _remote_api->get_accounts(v_approving_account_names);
    ...
    size_t i = 0;
    for (const optional<extended_account> approving_acct :
         approving_account_objects) {
      if (!approving_acct.valid()) {
        wlog("operation_get_required_auths said approval of non-existing "
             "account ${name} was needed",
    ...
    auto get_account_from_lut =
        [&](const std::string &name) -> fc::optional<const extended_account *> {
      ...
      if (it != approving_account_lut.end()) {
        ...
        elog("Is it possible you are using an account authority? Signing with "
             "an account authority is currently not supported.");
      }
    ...
    auto minimal_signing_keys = tx.minimize_required_signatures(
        ...
        BLURT_MAX_SIG_CHECK_DEPTH, BLURT_MAX_AUTHORITY_MEMBERSHIP,
        BLURT_MAX_SIG_CHECK_ACCOUNTS, fc::ecc::fc_canonical);
  ...
  get_result_formatters() const {
    ...
    m["list_my_accounts"] = [](variant result, const fc::variants &a) {
      ...
      for (const auto &a : accounts) {
        ...
        out << std::left << std::setw(17) << std::string(a.name) << std::right
            << std::setw(18) << fc::variant(a.balance).as_string() << " "
            << std::right << std::setw(26)
            << fc::variant(a.vesting_shares).as_string() << "\n";
      }
      out << "-----------------------------------------------------------------"
             "--------\n";
      out << std::left << std::setw(17) << "TOTAL" << std::right
          << std::setw(18) << legacy_asset::from_asset(total_steem).to_string()
          << " " << std::right << std::setw(26)
          << legacy_asset::from_asset(total_vest).to_string() << "\n";
      return out.str();
    };
    ...
    m["get_withdraw_routes"] = [](variant result, const fc::variants &a) {
      auto routes =
          result.as<vector<database_api::api_withdraw_vesting_route_object>>();
      std::stringstream ss;
      ...
      for (auto &r : routes) {
        ...
        ss << ' ' << std::right << std::setw(8) << std::setprecision(2)
           << std::fixed << double(r.percent) / 100;
  ...
  static_variant_map _operation_which_map =
      create_static_variant_map<operation>();
...
wallet_api::wallet_api(const wallet_data &initial_data,
                       const blurt::protocol::chain_id_type &_blurt_chain_id,
                       fc::api<remote_node_api> rapi)
    : my(new detail::wallet_api_impl(*this, initial_data, _blurt_chain_id,
                                     rapi)) {}
...
optional<condenser_api::legacy_signed_block>
wallet_api::get_block(uint32_t num) {
...
vector<account_name_type> wallet_api::list_accounts(const string &lowerbound,
                                                    uint32_t limit) {
...
condenser_api::api_account_object
wallet_api::get_account(string account_name) const {
...
bool wallet_api::import_key(string wif_key) {
  ...
  if (!optional_private_key)
    FC_THROW("Invalid private key");
  //   string shorthash = detail::pubkey_to_shorthash(
  //   optional_private_key->get_public_key() ); copy_wallet_file(
  //   "before-import-key-" + shorthash );
...
/*
fc::ecc::private_key wallet_api::derive_private_key(const std::string&
prefix_string, int sequence_number) const
{
   return detail::derive_private_key( prefix_string, sequence_number );
}
*/

vector<account_name_type> wallet_api::list_witnesses(const string &lowerbound,
                                                     uint32_t limit) {
  return my->_remote_api->lookup_witness_accounts(lowerbound, limit);
}

optional<condenser_api::api_witness_object>
wallet_api::get_witness(string owner_account) {
  return my->get_witness(owner_account);
}

condenser_api::legacy_signed_transaction
wallet_api::set_voting_proxy(string account_to_modify, string voting_account,
                             bool broadcast /* = false */) {
...
condenser_api::legacy_signed_transaction
wallet_api::sign_transaction(condenser_api::legacy_signed_transaction tx,
                             bool broadcast /* = false */) {
  try {
    signed_transaction appbase_tx(tx);
    condenser_api::annotated_signed_transaction result =
        my->sign_transaction(appbase_tx, broadcast);
...
condenser_api::legacy_signed_transaction wallet_api::create_account_with_keys(
    ...
    public_key_type owner, public_key_type active, public_key_type posting,
  ...
  FC_CAPTURE_AND_RETHROW(
      (creator)(new_account_name)(json_meta)(owner)(active)(memo)(broadcast))
}

condenser_api::legacy_signed_transaction
wallet_api::request_account_recovery(string recovery_account,
...
condenser_api::legacy_signed_transaction
wallet_api::recover_account(string account_to_recover,
...
condenser_api::legacy_signed_transaction
wallet_api::change_recovery_account(string owner, string new_recovery_account,
                                    bool broadcast) {
  FC_ASSERT(!is_locked());
...
wallet_api::get_owner_history(string account) const {
  return my->_remote_api->get_owner_history(account);
}

condenser_api::legacy_signed_transaction
wallet_api::update_account(string account_name, string json_meta,
...
condenser_api::legacy_signed_transaction
wallet_api::update_account_auth_key(string account_name, authority_type type,
...
condenser_api::legacy_signed_transaction
wallet_api::update_account_auth_account(string account_name,
                                        authority_type type,
...
condenser_api::legacy_signed_transaction
wallet_api::update_account_auth_threshold(string account_name,
                                          authority_type type,
...
condenser_api::legacy_signed_transaction
wallet_api::update_account_meta(string account_name, string json_meta,
                                bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::update_account_memo_key(string account_name, public_key_type key,
                                    bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::delegate_vesting_shares(string delegator, string delegatee,
 ...
 * account which will be controlable by this wallet.
 */
condenser_api::legacy_signed_transaction
wallet_api::create_account(string creator, string new_account_name,
  ...
  try {
    ...
    return create_account_with_keys(creator, new_account_name, json_meta,
                                    owner.pub_key, active.pub_key,
...
condenser_api::legacy_signed_transaction wallet_api::update_witness(
    string witness_account_name, string url, public_key_type block_signing_key,
    const condenser_api::api_chain_properties &props, bool broadcast) {
  FC_ASSERT(!is_locked());

  witness_update_operation op;

  optional<condenser_api::api_witness_object> wit =
      my->_remote_api->get_witness_by_account(witness_account_name);
  if (!wit.valid()) {
    op.url = url;
  } else {
...
condenser_api::legacy_signed_transaction wallet_api::update_witness_properties(
    string witness_name, const witness_properties &props, bool broadcast) {
  FC_ASSERT(!is_locked());
  ...
  std::pair<string, vector<char>> key("key",
                                      fc::raw::pack_to_vector(props.key));
  std::pair<string, vector<char>> account_creation_fee(
      "account_creation_fee",
      fc::raw::pack_to_vector(props.account_creation_fee));
  std::pair<string, vector<char>> maximum_block_size(
      "maximum_block_size", fc::raw::pack_to_vector(props.maximum_block_size));
  std::pair<string, vector<char>> account_subsidy_budget(
      "account_subsidy_budget",
      fc::raw::pack_to_vector(props.account_subsidy_budget));
  std::pair<string, vector<char>> account_subsidy_decay(
      "account_subsidy_decay",
      fc::raw::pack_to_vector(props.account_subsidy_decay));
  std::pair<string, vector<char>> operation_flat_fee(
      "operation_flat_fee", fc::raw::pack_to_vector(props.operation_flat_fee));
  std::pair<string, vector<char>> bandwidth_kbytes_fee(
      "bandwidth_kbytes_fee",
      fc::raw::pack_to_vector(props.bandwidth_kbytes_fee));
  ...
  map_packed_props[account_subsidy_budget.first] =
      account_subsidy_budget.second;
...
condenser_api::legacy_signed_transaction
wallet_api::vote_for_witness(string voting_account, string witness_to_vote_for,
...
void wallet_api::check_memo(
  ...
  try {
    ...
    keys.push_back(
        fc::ecc::extended_private_key::from_base58(memo).get_public_key());
  ...
  keys.push_back(
      fc::ecc::private_key::regenerate(owner_secret).get_public_key());

  string active_seed = account.name + "active" + memo;
  auto active_secret =
      fc::sha256::hash(active_seed.c_str(), active_seed.size());
  keys.push_back(
      fc::ecc::private_key::regenerate(active_secret).get_public_key());

  string posting_seed = account.name + "posting" + memo;
  auto posting_secret =
      fc::sha256::hash(posting_seed.c_str(), posting_seed.size());
  keys.push_back(
      fc::ecc::private_key::regenerate(posting_secret).get_public_key());

  // Check keys against public keys in authorites
  for (auto &key_weight_pair : account.owner.key_auths) {
    for (auto &key : keys)
      FC_ASSERT(
          key_weight_pair.first != key,
  ...
  for (auto &key_weight_pair : account.posting.key_auths) {
    for (auto &key : keys)
      FC_ASSERT(key_weight_pair.first != key,
                "Detected private posting key in memo field. Cancelling "
                "transaction.");
  }
  ...
  for (auto &key_pair : my->_keys) {
    for (auto &key : keys)
      FC_ASSERT(key != key_pair.first, "Detected imported private key in memo "
                                       "field. Cancelling trasanction.");
  }
}
...
string wallet_api::get_encrypted_memo(string from, string to, string memo) {
  if (memo.size() > 0 && memo[0] == '#') {
    ...
    m.encrypted =
        fc::aes_encrypt(encrypt_key, fc::raw::pack_to_vector(memo.substr(1)));
...
condenser_api::legacy_signed_transaction
wallet_api::transfer(string from, string to, condenser_api::legacy_asset amount,
...
condenser_api::legacy_signed_transaction wallet_api::escrow_transfer(
    string from, string to, string agent, uint32_t escrow_id,
    condenser_api::legacy_asset blurt_amount, condenser_api::legacy_asset fee,
    time_point_sec ratification_deadline, time_point_sec escrow_expiration,
    string json_meta, bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::escrow_approve(string from, string to, string agent, string who,
...
condenser_api::legacy_signed_transaction
wallet_api::escrow_dispute(string from, string to, string agent, string who,
...
condenser_api::legacy_signed_transaction
wallet_api::escrow_release(string from, string to, string agent, string who,
...
/**
 *  Transfers into savings happen immediately, transfers from savings take 72
 * hours
 */
condenser_api::legacy_signed_transaction
wallet_api::transfer_to_savings(string from, string to,
                                condenser_api::legacy_asset amount, string memo,
                                bool broadcast) {
  FC_ASSERT(!is_locked());
  check_memo(memo, get_account(from));
...
/**
 * @param request_id - an unique ID assigned by from account, the id is used to
 * cancel the operation and can be reused after the transfer completes
 */
condenser_api::legacy_signed_transaction
wallet_api::transfer_from_savings(string from, uint32_t request_id, string to,
 ...
 */
condenser_api::legacy_signed_transaction
wallet_api::cancel_transfer_from_savings(string from, uint32_t request_id,
                                         bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::transfer_to_vesting(string from, string to,
                                condenser_api::legacy_asset amount,
                                bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction wallet_api::withdraw_vesting(
    string from, condenser_api::legacy_asset vesting_shares, bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::set_withdraw_vesting_route(string from, string to, uint16_t percent,
...
condenser_api::legacy_signed_transaction
wallet_api::decline_voting_rights(string account, bool decline,
                                  bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction wallet_api::claim_reward_balance(
    string account, condenser_api::legacy_asset reward_blurt,
    condenser_api::legacy_asset reward_vests, bool broadcast) {
  FC_ASSERT(!is_locked());
  ...
  if (!is_locked()) {
    for (auto &item : result) {
      if (item.second.op.which() ==
          condenser_api::legacy_operation::tag<
              condenser_api::legacy_transfer_operation>::value) {
        auto &top =
            item.second.op.get<condenser_api::legacy_transfer_operation>();
        top.memo = decrypt_memo(top.memo);
      } else if (item.second.op.which() ==
                 condenser_api::legacy_operation::tag<
                     condenser_api::legacy_transfer_from_savings_operation>::
                     value) {
        auto &top =
            item.second.op
                .get<condenser_api::legacy_transfer_from_savings_operation>();
        top.memo = decrypt_memo(top.memo);
      } else if (item.second.op.which() ==
                 condenser_api::legacy_operation::tag<
        ...
        auto &top =
            item.second.op
                .get<condenser_api::legacy_transfer_to_savings_operation>();
        top.memo = decrypt_memo(top.memo);
      }
    }
  }
...
wallet_api::get_withdraw_routes(string account,
                                condenser_api::withdraw_route_type type) const {
  return my->_remote_api->get_withdraw_routes(account, type);
}

condenser_api::legacy_signed_transaction
wallet_api::post_comment(string author, string permlink, string parent_author,
...
condenser_api::legacy_signed_transaction
wallet_api::vote(string voter, string author, string permlink, int16_t weight,
                 bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::get_transaction(transaction_id_type id) const {
  return my->_remote_api->get_transaction(id);
}

condenser_api::legacy_signed_transaction wallet_api::follow(string follower,
                                                            string following,
                                                            set<string> what,
                                                            bool broadcast) {
...
condenser_api::legacy_signed_transaction
wallet_api::create_proposal(account_name_type creator,
                            account_name_type receiver,
                            time_point_sec start_date, time_point_sec end_date,
                            condenser_api::legacy_asset daily_pay,
                            string subject, string permlink, bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::legacy_signed_transaction
wallet_api::update_proposal_votes(account_name_type voter,
                                  flat_set<int64_t> proposals, bool approve,
                                  bool broadcast) {
  FC_ASSERT(!is_locked());
...
condenser_api::list_proposals_return
wallet_api::list_proposals(fc::variant start, uint32_t limit,
                           database_api::sort_order_type order_by,
                           database_api::order_direction_type order_type,
                           database_api::proposal_status status) {
  return my->_remote_api->list_proposals(start, limit, order_by, order_type,
                                         status);
}

condenser_api::find_proposals_return
wallet_api::find_proposals(vector<int64_t> proposal_ids) {
  return my->_remote_api->find_proposals(proposal_ids);
}

condenser_api::list_proposal_votes_return
wallet_api::list_proposal_votes(fc::variant start, uint32_t limit,
                                database_api::sort_order_type order_by,
                                database_api::order_direction_type order_type,
                                database_api::proposal_status status) {
  return my->_remote_api->list_proposal_votes(start, limit, order_by,
                                              order_type, status);
}

condenser_api::legacy_signed_transaction
wallet_api::remove_proposal(account_name_type deleter, flat_set<int64_t> ids,
                            bool broadcast) {
  FC_ASSERT(!is_locked());
