...
public:
  blockchain_statistics_api_impl(blurt::app::application &app) : _app(app) {}
...
statistics
blockchain_statistics_api_impl::get_stats_for_time(fc::time_point_sec open,
                                                   uint32_t interval) const {
  statistics result;
  const auto &bucket_idx = _app.chain_database()
                               ->get_index<bucket_index>()
                               .indices()
                               .get<by_bucket>();
...
statistics blockchain_statistics_api_impl::get_stats_for_interval(
  ...
  statistics result;
  const auto &bucket_itr = _app.chain_database()
                               ->get_index<bucket_index>()
                               .indices()
                               .get<by_bucket>();
  const auto &sizes = _app.get_plugin<blockchain_statistics_plugin>(
                              BLURT_BLOCKCHAIN_STATISTICS_PLUGIN_NAME)
                          ->get_tracked_buckets();
  ...
  while (size_itr != sizes.rend() && time < end) {
    ...
    while (itr != bucket_itr.end() && itr->seconds == *size_itr &&
...
blockchain_statistics_api::blockchain_statistics_api(
    const blurt::app::api_context &ctx) {
...
statistics
blockchain_statistics_api::get_stats_for_time(fc::time_point_sec open,
                                              uint32_t interval) const {
  return my->_app.chain_database()->with_read_lock(
      [&]() { return my->get_stats_for_time(open, interval); });
}

statistics blockchain_statistics_api::get_stats_for_interval(
    fc::time_point_sec start, fc::time_point_sec end) const {
  return my->_app.chain_database()->with_read_lock(
...
statistics &statistics::operator+=(const bucket_object &b) {
  ...
  this->modified_vesting_withdrawal_requests +=
