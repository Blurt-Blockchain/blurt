...
public:
  ...
  flat_set<uint32_t> _tracked_buckets = {60,    3600,   21600,
...
void blockchain_statistics_plugin_impl::on_block(const signed_block &b) {
  ...
  if (b.block_num() == 1) {
    ...
    db.modify(db.get(bucket_id_type()),
              [&](bucket_object &bo) { bo.blocks++; });
  }
  ...
  const auto &bucket_idx =
      db.get_index<bucket_index>().indices().get<by_bucket>();
  ...
  for (const auto &bucket : _tracked_buckets) {
    auto open = fc::time_point_sec(
        (db.head_block_time().sec_since_epoch() / bucket) * bucket);
    ...
    if (itr == bucket_idx.end()) {
      ...
      if (_maximum_history_per_bucket_size > 0) {
        try {
          ...
          itr = bucket_idx.lower_bound(
              boost::make_tuple(bucket, fc::time_point_sec()));
...
void blockchain_statistics_plugin_impl::pre_operation(
  ...
  for (const auto &bucket_id : _current_buckets) {
    if (o.op.which() == operation::tag<delete_comment_operation>::value) {
      ...
      auto new_vesting_withdrawal_rate =
          op.vesting_shares.amount / BLURT_VESTING_WITHDRAW_INTERVALS;
      ...
      db.modify(bucket, [&](bucket_object &b) {
        ...
        // TODO: Figure out how to change delta when a vesting withdraw
        // finishes. Have until March 24th 2018 to figure that out...
...
void blockchain_statistics_plugin_impl::post_operation(
    const operation_notification &o) {
  try {
    auto &db = _self.database();
...
void blockchain_statistics_plugin::plugin_set_program_options(
  ...
  cli.add_options()(
      ...
      "the number of buckets (default: 100)");
  cfg.add(cli);
}
...
void blockchain_statistics_plugin::plugin_initialize(
  ...
  try {
    ...
    db.applied_block_proxy([&](const signed_block &b) { _my->on_block(b); },
                           *this);
    db.pre_apply_operation_proxy(
        [&](const operation_notification &o) { _my->pre_operation(o); }, *this);
    db.post_apply_operation_proxy(
    ...
    if (options.count("chain-stats-bucket-size")) {
      const std::string &buckets =
          options["chain-stats-bucket-size"].as<string>();
      _my->_tracked_buckets =
          fc::json::from_string(buckets).as<flat_set<uint32_t>>();
    }
...
const flat_set<uint32_t> &
blockchain_statistics_plugin::get_tracked_buckets() const {
...
BLURT_DEFINE_PLUGIN(blockchain_statistics,
                    blurt::blockchain_statistics::blockchain_statistics_plugin);
