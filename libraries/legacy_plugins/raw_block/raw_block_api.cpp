...
raw_block_api_impl::raw_block_api_impl(blurt::app::application &_app)
    : app(_app) {}

std::shared_ptr<blurt::plugin::raw_block::raw_block_plugin>
raw_block_api_impl::get_plugin() {
...
get_raw_block_result raw_block_api::get_raw_block(get_raw_block_args args) {
  ...
  fc::optional<chain::signed_block> block =
      db->fetch_block_by_number(args.block_num);
  if (!block.valid()) {
    return result;
  }
