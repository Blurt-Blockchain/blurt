...
public:
  ...
  static const std::string &name() {
    static std::string name = "plugin_a";
    return name;
  }

  virtual void set_program_options(options_description &cli,
                                   options_description &cfg) override {
    cfg.add_options()("dbsize", bpo::value<uint64_t>()->default_value(8 * 1024),
                      "Minimum size MB of database shared memory file");
    cli.add_options()("replay",
                      "clear plugin_a database and replay all blocks")(
...
public:
  ...
  static const std::string &name() {
    static std::string name = "plugin_b";
    return name;
  }
  ...
  virtual void set_program_options(options_description &cli,
    ...
    cfg.add_options()(
        "listen-endpoint",
        bpo::value<string>()->default_value("127.0.0.1:9876"),
        "The local IP address and port to listen for incoming connections.")(
        "remote-endpoint", bpo::value<vector<string>>()->composing(),
        "The IP address and port of a remote peer to sync with.")(
        "public-endpoint", bpo::value<string>()->default_value("0.0.0.0:9876"),
