...
fc::variant default_database_configuration() {
  ...
  config.global.statistics =
      false; // Incurs severe performance degradation when true

  // global::shared_cache
  config.global.shared_cache.capacity = std::to_string(GB(5));

  // global::write_buffer_manager
  config.global.write_buffer_manager.write_buffer_size =
      std::to_string(GB(1)); // Write buffer manager is within the shared cache
...
FC_REFLECT(blurt::utilities::database::configuration::shared_cache, (capacity));
...
FC_REFLECT(blurt::utilities::database::configuration::global,
           (shared_cache)(write_buffer_manager)(object_count)(statistics));
...
FC_REFLECT(blurt::utilities::database::configuration::block_based_table_options,
           (block_size)(cache_index_and_filter_blocks)(bloom_filter_policy));

FC_REFLECT(blurt::utilities::database::configuration::base_index,
           (optimize_level_style_compaction)(increase_parallelism)(
