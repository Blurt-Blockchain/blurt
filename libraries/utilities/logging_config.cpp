...
void appender_args::validate() {
  ...
  FC_ASSERT((file.length() > 0) ^ (stream.length() > 0),
            "Must specify either a file or a stream");
}
...
void set_logging_program_options(
  ...
  std::string str_default_appender =
      boost::algorithm::join(default_appender, " ");
  ...
  options.add_options()(
      ...
      boost::program_options::value<std::vector<std::string>>()
          ->composing()
          ->default_value(default_appender, str_default_appender),
      "Appender definition json: {\"appender\", \"stream\", \"file\"} Can only "
      "specify a file OR a stream")(
      ...
      boost::program_options::value<std::vector<std::string>>()
          ->composing()
          ->default_value(default_logger, str_default_logger),
...
load_logging_config(const boost::program_options::variables_map &args,
  ...
  try {
    ...
    if (args.count("log-appender")) {
      ...
      all_appenders.insert(all_appenders.end(), appenders.begin(),
                           appenders.end());
    }

    if (args.count("log-console-appender")) {
      std::vector<string> console_appenders =
          args["log-console-appender"].as<vector<string>>();
      all_appenders.insert(all_appenders.end(), console_appenders.begin(),
                           console_appenders.end());
    }

    if (args.count("log-file-appender")) {
      std::vector<string> file_appenders =
          args["log-file-appender"].as<vector<string>>();
      all_appenders.insert(all_appenders.end(), file_appenders.begin(),
                           file_appenders.end());
    }
    ...
    for (string &s : all_appenders) {
      ...
      while ((pos = s.find("{", pos)) != std::string::npos) {
        auto appender =
            fc::json::from_string(s.substr(pos++)).as<appender_args>();
        appender.validate();
        ...
        if (appender.stream.length()) {
          ...
          console_appender_config.level_colors.emplace_back(
              fc::console_appender::level_color(
                  fc::log_level::error, fc::console_appender::color::red));
          console_appender_config.stream =
              fc::variant(appender.stream)
                  .as<fc::console_appender::stream::type>();
          logging_config.appenders.push_back(
              fc::appender_config(appender.appender, "console",
        ...
        } else // validate ensures the is either a stream or file configured
        {
          ...
          // filename will be taken from ini file, everything else hard-coded
          // here
          ...
          logging_config.appenders.push_back(fc::appender_config(
              appender.appender, "file", fc::variant(file_appender_config)));
          found_logging_config = true;
        }
      }
    }
    ...
    if (args.count("log-logger")) {
      std::vector<string> loggers =
          args["log-logger"].as<std::vector<std::string>>();

      for (string &s : loggers) {
        std::size_t pos = 0;
        while ((pos = s.find("{", pos)) != std::string::npos) {
          auto logger =
              fc::json::from_string(s.substr(pos++)).as<logger_args>();
