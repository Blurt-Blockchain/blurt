 ...
 * The values of \see measure and \see dump are returned if you need further
 * processing (e.g. pretty print to console).
 */
class benchmark_dumper {
public:
  struct index_memory_details_t {
    index_memory_details_t(std::string &&name, size_t size, size_t i_sizeof,
                           size_t item_add_allocation,
                           size_t add_container_allocation)
  ...
  typedef std::function<void(index_memory_details_cntr_t &, bool)>
      get_indexes_memory_details_t;
  typedef std::function<void(database_object_sizeof_cntr_t &)>
      get_database_objects_sizeofs_t;
  ...
  const measurement &
  measure(uint32_t block_number,
          get_indexes_memory_details_t get_indexes_memory_details) {
    ...
    data.set(block_number,
             ...
             int((current_cpu_time - _last_cpu_time) * 1000 /
                 CLOCKS_PER_SEC), // cpu_ms
             current_virtual, peak_virtual);
    ...
    _all_data.total_measurement.set(
        _total_blocks, (_last_sys_time - _init_sys_time).count() / 1000,
  ...
  const measurement &
  dump(bool finalMeasure,
       get_indexes_memory_details_t get_indexes_memory_details) {
    if (finalMeasure) {
      /// Collect index data including dynamic container sizes, what can be time
      /// consuming
      ...
      std::sort(idxData.begin(), idxData.end(),
                [](const index_memory_details_t &info1,
                   const index_memory_details_t &info2) -> bool {
                  return info1.total_index_mem_usage >
                         info2.total_index_mem_usage;
                });
    }
...
FC_REFLECT(blurt::utilities::benchmark_dumper::measurement,
           (block_number)(real_ms)(cpu_ms)(current_mem)(peak_mem)(
