...
unsigned long long read_u64_value_from(FILE *input, const char *key,
                                       unsigned key_length,
                                       TScanErrorCallback error_callback) {
...
bool benchmark_dumper::read_mem(pid_t pid, uint64_t *current_virtual,
                                uint64_t *peak_virtual) {
  ...
  TScanErrorCallback error_callback = [procPath](const char *key) {
    elog("cannot read value of ${key} key in ${file}",
         ("key", key)("file", procPath));
  };
