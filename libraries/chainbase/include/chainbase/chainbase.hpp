...
#define CHAINBASE_REQUIRE_WRITE_LOCK(m, t)                                     \
  require_write_lock(m, typeid(t).name())
#else
...
struct index_statistic_info {
  ...
  /// Additional memory used for container internal structures (like tree
  /// nodes).
  size_t _additional_container_allocation = 0;
};

template <class IndexType>
void gather_index_static_data(const IndexType &index,
                              index_statistic_info *info) {
  info->_value_type_name =
      boost::core::demangle(typeid(typename IndexType::value_type).name());
...
template <class IndexType> class index_statistic_provider {
public:
  index_statistic_info gather_statistics(const IndexType &index,
                                         bool onlyStaticInfo) const {
    index_statistic_info info;
...
/** this class is ment to be specified to enable lookup of index type by object
 *type using the SET_INDEX_TYPE macro.
 **/
template <typename T> struct get_index_type {};

/**
 *  This macro must be used at global scope and OBJECT_TYPE and INDEX_TYPE must
 * be fully qualified
 */
#define CHAINBASE_SET_INDEX_TYPE(OBJECT_TYPE, INDEX_TYPE)                      \
  namespace chainbase {                                                        \
  template <> struct get_index_type<OBJECT_TYPE> { typedef INDEX_TYPE type; }; \
  }
...
public:
  ...
  typedef allocator<std::pair<const id_type, value_type>>
  ...
  typedef boost::interprocess::map<id_type, value_type, std::less<id_type>,
                                   id_value_allocator_type>
      id_value_type_map;
  typedef boost::interprocess::set<id_type, std::less<id_type>,
                                   id_allocator_type>
      id_type_set;
...
public:
  ...
  int_incrementer(int_incrementer &ii) : _target(ii._target) { ++_target; }
...
/**
 *  The value_type stored in the multiindex container must have a integer field
 * with the name 'id'.  This will be the primary key and it will be assigned and
 * managed by generic_index.
 *
...
public:
  ...
  generic_index(allocator<value_type> a, bfs::path p)
      : _stack(a), _indices(a, p),
        _size_of_value_type(sizeof(typename MultiIndexType::value_type)),
  ...
  void validate() const {
    if (sizeof(typename MultiIndexType::value_type) != _size_of_value_type ||
   ...
   * Set the ID to the next available ID, then increment _next_id and fire off
   * on_create().
   */
  template <typename Constructor> const value_type &emplace(Constructor &&c) {
    ...
    auto insert_result =
        _indices.emplace(constructor, _indices.get_allocator());

    if (!insert_result.second) {
      BOOST_THROW_EXCEPTION(
          std::logic_error("could not insert object, most likely a uniqueness "
  ...
  void modify(const value_type &obj, Modifier &&m) {
    ...
    if (!ok)
      BOOST_THROW_EXCEPTION(
          std::logic_error("Could not modify object, most likely a uniqueness "
                           "constraint was violated"));
  }
  ...
  template <typename ByIndex, typename IterType> IterType erase(IterType objI) {
    on_remove(*objI);
    return _indices.template mutable_get<ByIndex>().erase(objI);
  }
#else
  template <typename ByIndex>
  typename MultiIndexType::template index_iterator<ByIndex>::type
  erase(typename MultiIndexType::template index_iterator<ByIndex>::type objI) {
  ...
  /**
   *  Restores the state to how it was prior to the current session discarding
   * all changes made between the last revision and the current revision.
   */
  void undo() {
    if (!enabled())
      return;
    ...
    for (auto &item : head.old_values) {
      ...
      if (itr != _indices.end()) {
        ok = _indices.modify(
            itr, [&](value_type &v) { v = std::move(item.second); });
      } else {
      ...
      if (!ok)
        BOOST_THROW_EXCEPTION(
            std::logic_error("Could not modify object, most likely a "
    ...
    for (auto &item : head.removed_values) {
      ...
      if (!ok)
        BOOST_THROW_EXCEPTION(
            std::logic_error("Could not restore object, most likely a "
                             "uniqueness constraint was violated"));
    }
  ...
  /**
   *  This method works similar to git squash, it merges the change set from the
   * two most recent revision numbers into one revision number (reducing the
   * head revision number)
   *
   *  This method does not change the state of the index, only the state of the
   * undo buffer.
   */
  void squash() {
    if (!enabled())
    ...
    //
    // When merging A=prev_state and B=state we have a 4x4 matrix of all
    // possibilities:
    //
    //                   |--------------------- B ----------------------|
    //
    ...
    //
    // Type A means the composition of states contains the same entry as the
    // first of the two merged states for that object. Type B means the
    // composition of states contains the same entry as the second of the two
    // merged states for that object. Type C means the composition of states
    // contains an entry different from either of the merged states for that
    // object. Type N/A means the composition of states violates causal timing.
    // Type AB means both type A and type B simultaneously.
    //
    // The merge() operation is defined as modifying prev_state in-place to be
    // the state object which represents the composition of state A and B.
    //
    // Type A (and AB) can be implemented as a no-op; prev_state already
    // contains the correct value for the merged state. Type B (and AB) can be
    // implemented by copying from state to prev_state. Type C needs special
    // case-by-case logic. Type N/A can be ignored or assert(false) as it can
    // only occur if prev_state and state have illegal values (a serious logic
    // error which should never happen).
    //

    // We can only be outside type A/AB (the nop path) if B is not nop, so it
    // suffices to iterate through B's three containers.
    ...
    // *+new, but we assume the N/A cases don't happen, leaving type B nop+new
    // -> new
    ...
    for (auto &obj : state.removed_values) {
      ...
      prev_state.removed_values.emplace(
          std::move(obj)); //[obj.second->id] = std::move(obj.second);
    }
...
private:
  ...
  void on_modify(const value_type &v) {
    ...
    head.old_values.emplace(
        std::pair<typename value_type::id_type, const value_type &>(v.id, v));
  }
  ...
  boost::interprocess::deque<undo_state_type, allocator<undo_state_type>>
      _stack;

  /**
   *  Each new session increments the revision, a squash will decrement the
   * revision by combining the two most recent revisions into one revision.
   *
...
public:
...
#ifdef ENABLE_MIRA
  ~index_impl() { delete (BaseIndex *)abstract_index::_idx_ptr; }
#endif

  virtual unique_ptr<abstract_session> start_undo_session() override {
    return unique_ptr<abstract_session>(
        new session_impl<typename BaseIndex::session>(
            _base.start_undo_session()));
  }
  ...
  get_statistics(bool onlyStaticInfo) const override final {
  ...
  virtual size_t size() const override final { return _base.indicies().size(); }

  virtual void clear() override final { _base.clear(); }
  ...
  virtual void close() override final { _base.close(); }

  virtual void wipe(const bfs::path &dir) override final { _base.wipe(dir); }

  virtual void flush() override final { _base.flush(); }
  ...
  virtual void trim_cache() override final { _base.trim_cache(); }
...
public:
  void open(const bfs::path &dir, uint32_t flags = 0,
...
#ifdef CHAINBASE_CHECK_LOCKING
  void require_lock_fail(const char *method, const char *lock_type,
  ...
  public:
    session(session &&s)
        : _index_sessions(std::move(s._index_sessions)), _revision(s._revision),
  ...
  template <typename MultiIndexType> void add_index() {
    _index_types.push_back(
        unique_ptr<abstract_index_type>(new index_type_impl<MultiIndexType>()));
    _index_types.back()->add_index(*this);
  }

#ifndef ENABLE_MIRA
  auto get_segment_manager() -> decltype(
      ((bip::managed_mapped_file *)nullptr)->get_segment_manager()) {
  ...
  size_t get_max_memory() const { return _file_size; }
  ...
  const generic_index<MultiIndexType> &get_index() const {
    ...
    if (!has_index<MultiIndexType>()) {
      ...
      BOOST_THROW_EXCEPTION(std::runtime_error("unable to find index for " +
                                               type_name + " in database"));
    }
  ...
  auto get_index() const
      -> decltype(((generic_index<MultiIndexType> *)(nullptr))
                      ->indicies()
  ...
  generic_index<MultiIndexType> &get_mutable_index() {
    CHAINBASE_REQUIRE_WRITE_LOCK("get_mutable_index",
                                 typename MultiIndexType::value_type);
    typedef generic_index<MultiIndexType> index_type;
    typedef index_type *index_type_ptr;
  ...
  const ObjectType *find(CompatibleKey &&key) const {
    ...
    const auto &idx =
        get_index<index_type>().indicies().template get<IndexedByType>();
  ...
  const ObjectType &get(CompatibleKey &&key) const {
    ...
    auto obj =
        find<ObjectType, IndexedByType>(std::forward<CompatibleKey>(key));
    if (!obj) {
  ...
  const ObjectType &create(Constructor &&con) {
    ...
    typedef typename get_index_type<ObjectType>::type index_type;
    return get_mutable_index<index_type>().emplace(
        std::forward<Constructor>(con));
  }

  template <typename ObjectType> size_t count() const {
    typedef typename get_index_type<ObjectType>::type index_type;
  ...
  auto with_write_lock(Lambda &&callback, uint64_t wait_micro = 1000000)
    ...
    if (wait_micro) {
      while (
          !lock.timed_lock(boost::posix_time::microsec_clock::universal_time() +
                           boost::posix_time::microseconds(wait_micro))) {
  ...
  void for_each_index_extension(Lambda &&callback) const {
    for (const abstract_index *idx : _index_list) {
      ...
      for (const std::shared_ptr<index_extension> &e : exts) {
        std::shared_ptr<IndexExtensionType> e2 =
            std::dynamic_pointer_cast<IndexExtensionType>(e);
        if (e2)
          callback(e2);
      }
    }
...
private:
  template <typename MultiIndexType> void add_index_helper() {
    ...
    if (!(_index_map.size() <= type_id || _index_map[type_id] == nullptr)) {
      BOOST_THROW_EXCEPTION(
          std::logic_error(type_name + "::type_id is already in use"));
    }
...
#ifdef ENABLE_MIRA
    if (_is_open)
      new_index->open(_data_dir, _database_cfg);
#endif
  }
  ...
  /**
   * This is a sparse list of known indicies kept to accelerate creation of undo
   * sessions
   */
  abstract_index_cntr_t _index_list;

  /**
   * This is a full map (size 2^16) of all possible index designed for constant
   * time lookup
   */
