...
BOOST_AUTO_TEST_CASE(open_and_create) {
  ...
  try {
    ...
    BOOST_CHECK_THROW(db.open(temp),
                      std::runtime_error); /// temp does not exist
    ...
    BOOST_CHECK_THROW(db.add_index<book_index>(),
                      std::logic_error); /// cannot add same index twice
