...
struct get_str_schema_class_member_visitor {
  ...
  template <typename Member, class Class, Member(Class::*member)>
  void operator()(const char *name) const {
    std::shared_ptr<abstract_schema> member_schema =
        get_schema_for_type<Member>();
...
#define BLURT_SCHEMA_TEMPLATE_CLASS_BODY(CLASSNAME)                            \
  ...
  virtual ~CLASSNAME() {}                                                      \
                                                                               \
  virtual void get_deps(std::vector<std::shared_ptr<abstract_schema>> &deps)   \
      override;                                                                \
...
#define BLURT_SCHEMA_DEFINE_CLASS_METHODS(CLASSNAME)                           \
  CLASSNAME::CLASSNAME(int64_t id, const std::string &name)                    \
      : _id(id), _name(name) {}                                                \
  CLASSNAME::~CLASSNAME() {}                                                   \
  void CLASSNAME::get_name(std::string &name) { name = _name; }                \
  int64_t CLASSNAME::get_id() { return _id; }
...
template <typename ObjectType>
void schema_impl<ObjectType, false, false>::get_deps(
    std::vector<std::shared_ptr<abstract_schema>> &deps) {}

template <typename ObjectType>
void schema_impl<ObjectType, true, true>::get_deps(
    std::vector<std::shared_ptr<abstract_schema>> &deps) {}

template <typename ObjectType>
void schema_impl<ObjectType, true, false>::get_deps(
    std::vector<std::shared_ptr<abstract_schema>> &deps) {
...
std::shared_ptr<abstract_schema> create_schema(int64_t id) {
  return std::make_shared<
      typename schema_reflect<ObjectType>::schema_impl_type>(
