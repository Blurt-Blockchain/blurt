...
struct abstract_schema {
  virtual void
  get_deps(std::vector<std::shared_ptr<abstract_schema>> &deps) = 0;
...
std::shared_ptr<abstract_schema> get_schema_for_type() {
  static std::shared_ptr<abstract_schema> sch =
      std::shared_ptr<abstract_schema>();
  if (!sch) {
