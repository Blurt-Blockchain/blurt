...
void create_proposal_operation::validate() const {
  ...
  FC_ASSERT(daily_pay.symbol.asset_num == BLURT_ASSET_NUM_BLURT,
            "Daily pay should be expressed in BLURT_SYMBOL");

  FC_ASSERT(!subject.empty(), "subject is required");
  FC_ASSERT(subject.size() <= BLURT_PROPOSAL_SUBJECT_MAX_LENGTH,
            "Subject is too long");
