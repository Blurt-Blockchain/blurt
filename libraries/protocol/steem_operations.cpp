...
namespace steem { namespace protocol {
   ...
   void account_create_operation::validate() const
   {
      ...
      FC_ASSERT( is_asset_type( fee, STEEM_SYMBOL ), "Account creation fee must be BLURT" );
      owner.validate();
      active.validate();
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= STEEM_100_PERCENT, "Weight is not a BLURT percentage" );
      validate_permlink( permlink );
   }
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == STEEM_SYMBOL, "account_creation_fee must be in BLURT" );
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( sbd_exchange_rate.base, SBD_SYMBOL ) && is_asset_type( sbd_exchange_rate.quote, STEEM_SYMBOL ) ),
            "Price feed must be a BLURT/BBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, STEEM_SYMBOL ) && is_asset_type( min_to_receive, SBD_SYMBOL ) )
               ...
               "Limit order must be for the BLURT:BBD or SMT:(BLURT/BBD) market" );
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from BBD to BLURT, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, SBD_SYMBOL ), "Can only convert BBD to BLURT" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( (fee.symbol == STEEM_SYMBOL) || (fee.symbol == SBD_SYMBOL), "fee must be BLURT or BBD" );
      FC_ASSERT( sbd_amount.symbol == SBD_SYMBOL, "sbd amount must contain BBD" );
      FC_ASSERT( steem_amount.symbol == STEEM_SYMBOL, "steem amount must contain BLURT" );
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_steem, STEEM_SYMBOL ), "Reward Steem must be BLURT" );
      FC_ASSERT( is_asset_type( reward_sbd, SBD_SYMBOL ), "Reward Steem must be BBD" );
