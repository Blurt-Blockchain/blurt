...
void validate_auth_size(const authority &a) {
  ...
  FC_ASSERT(size <= BLURT_MAX_AUTHORITY_MEMBERSHIP,
            ...
            ("max", BLURT_MAX_AUTHORITY_MEMBERSHIP)("n", size));
}

void account_create_operation::validate() const {
  validate_account_name(new_account_name);
  FC_ASSERT(is_asset_type(fee, BLURT_SYMBOL),
            "Account creation fee must be BLURT");
  owner.validate();
  active.validate();
  ...
  FC_ASSERT(fee >= asset(0, BLURT_SYMBOL),
            "Account creation fee cannot be negative");
  FC_ASSERT(fee <= asset(BLURT_MAX_ACCOUNT_CREATION_FEE, BLURT_SYMBOL),
...
void account_update_operation::validate() const {
  ...
  if (posting_json_metadata.size() > 0) {
    FC_ASSERT(fc::is_utf8(posting_json_metadata),
              "JSON Metadata not formatted in UTF8");
    FC_ASSERT(fc::json::is_valid(posting_json_metadata),
              "JSON Metadata not valid JSON");
  }
}
...
void comment_operation::validate() const {
  FC_ASSERT(title.size() < BLURT_COMMENT_TITLE_LIMIT,
            "Title size limit exceeded. Max: ${max} Current: ${n}",
            ("max", BLURT_COMMENT_TITLE_LIMIT - 1)("n", title.size()));
...
void comment_payout_beneficiaries::validate() const {
  ...
  FC_ASSERT(beneficiaries.size() < BLURT_BENEFICIARY_LIMIT,
            "Cannot specify more than ${max} beneficiaries.",
            ("max", BLURT_BENEFICIARY_LIMIT -
                        1)); // Require size serializtion fits in one byte.

  validate_account_name(beneficiaries[0].account);
  FC_ASSERT(beneficiaries[0].weight <= BLURT_100_PERCENT,
            "Cannot allocate more than 100% of rewards to one account");
  sum += beneficiaries[0].weight;
  FC_ASSERT(
      sum <= BLURT_100_PERCENT,
      "Cannot allocate more than 100% of rewards to a comment"); // Have to
                                                                 // check
                                                                 // incrementally
                                                                 // to avoid
                                                                 // overflow
  ...
  for (size_t i = 1; i < beneficiaries.size(); i++) {
    ...
    FC_ASSERT(beneficiaries[i].weight <= BLURT_100_PERCENT,
              "Cannot allocate more than 100% of rewards to one account");
    ...
    FC_ASSERT(
        ...
        "Cannot allocate more than 100% of rewards to a comment"); // Have to
                                                                   ...
                                                                   // overflow
    FC_ASSERT(
        beneficiaries[i - 1] < beneficiaries[i],
...
void comment_options_operation::validate() const {
  validate_account_name(author);
  FC_ASSERT(max_accepted_payout.symbol == BLURT_SYMBOL,
            "Max accepted payout must be in BLURT_SYMBOL");
...
void vote_operation::validate() const {
  ...
  validate_account_name(author);
  FC_ASSERT(abs(weight) <= BLURT_100_PERCENT,
            "Weight is not a BLURTIT percentage");
  validate_permlink(permlink);
}
...
void transfer_operation::validate() const {
  try {
    ...
    FC_ASSERT(amount.symbol.is_vesting() == false,
...
void transfer_to_vesting_operation::validate() const {
  ...
  FC_ASSERT(amount.symbol == BLURT_SYMBOL ||
                (amount.symbol.space() == asset_symbol_type::smt_nai_space &&
                 amount.symbol.is_vesting() == false),
...
void set_withdraw_vesting_route_operation::validate() const {
  ...
  FC_ASSERT(0 <= percent && percent <= BLURT_100_PERCENT,
            "Percent must be valid percent");
}
...
void witness_set_properties_operation::validate() const {
  ...
  if (itr != props.end()) {
    ...
    FC_ASSERT(account_creation_fee.symbol == BLURT_SYMBOL,
              "account_creation_fee must be in BLURT");
  ...
  if (itr != props.end()) {
    ...
    fc::raw::unpack_from_vector(
        itr->second,
        account_subsidy_budget); // Checks that the value can be deserialized
    FC_ASSERT(account_subsidy_budget >= BLURT_RD_MIN_BUDGET,
              "Budget must be at least ${n}", ("n", BLURT_RD_MIN_BUDGET));
    FC_ASSERT(account_subsidy_budget <= BLURT_RD_MAX_BUDGET,
              "Budget must be at most ${n}", ("n", BLURT_RD_MAX_BUDGET));
  }
  ...
  if (itr != props.end()) {
    ...
    fc::raw::unpack_from_vector(
        itr->second,
        account_subsidy_decay); // Checks that the value can be deserialized
    FC_ASSERT(account_subsidy_decay >= BLURT_RD_MIN_DECAY,
              "Decay must be at least ${n}", ("n", BLURT_RD_MIN_DECAY));
    FC_ASSERT(account_subsidy_decay <= BLURT_RD_MAX_DECAY,
              "Decay must be at most ${n}", ("n", BLURT_RD_MAX_DECAY));
  }
  ...
  if (itr != props.end()) {
    ...
    FC_ASSERT(operation_flat_fee.symbol == BLURT_SYMBOL,
              "operation_flat_fee must be in BLURT");
  ...
  if (itr != props.end()) {
    ...
    FC_ASSERT(bandwidth_kbytes_fee.symbol == BLURT_SYMBOL,
              "bandwidth_kbytes_fee must be in BLURT");
...
void custom_json_operation::validate() const {
  ...
  FC_ASSERT(id.size() <= BLURT_CUSTOM_OP_ID_MAX_LENGTH,
            "Operation ID length exceeded. Max: ${max} Current: ${n}",
            ("max", BLURT_CUSTOM_OP_ID_MAX_LENGTH)("n", id.size()));
  FC_ASSERT(fc::is_utf8(json), "JSON Metadata not formatted in UTF8");
  FC_ASSERT(fc::json::is_valid(json), "JSON Metadata not valid JSON");
}
void custom_binary_operation::validate() const {
  /// required auth accounts are the ones whose bandwidth is consumed
  FC_ASSERT((required_owner_auths.size() + required_active_auths.size() +
             required_posting_auths.size()) > 0,
            "at least one account must be specified");
  FC_ASSERT(id.size() <= BLURT_CUSTOM_OP_ID_MAX_LENGTH,
            "Operation ID length exceeded. Max: ${max} Current: ${n}",
            ("max", BLURT_CUSTOM_OP_ID_MAX_LENGTH)("n", id.size()));
...
void claim_reward_balance_operation::validate() const {
  validate_account_name(account);
  FC_ASSERT(is_asset_type(reward_blurt, BLURT_SYMBOL),
            "Reward Steem must be BLURT");
  FC_ASSERT(is_asset_type(reward_vests, VESTS_SYMBOL),
            "Reward Steem must be VESTS");
  ...
  FC_ASSERT(reward_blurt.amount > 0 || reward_vests.amount > 0,
...
void delegate_vesting_shares_operation::validate() const {
  ...
  FC_ASSERT(is_asset_type(vesting_shares, VESTS_SYMBOL),
            "Delegation must be VESTS");
  FC_ASSERT(vesting_shares >= asset(0, VESTS_SYMBOL),
