...
blurt::protocol::signed_transaction::sign(const private_key_type &key,
...
signature_type blurt::protocol::signed_transaction::sign(
...
flat_set<public_key_type> signed_transaction::get_signature_keys(
    const chain_id_type &chain_id, canonical_signature_type canon_type) const {
  try {
...
set<public_key_type> signed_transaction::get_required_signatures(
    ...
    const authority_getter &get_posting, uint32_t max_recursion_depth,
    uint32_t max_membership, uint32_t max_account_auths,
    canonical_signature_type canon_type) const {
  ...
  get_required_authorities(required_active, required_owner, required_posting,
                           other);

  /** posting authority cannot be mixed with active authority in same
   * transaction */
  if (required_posting.size()) {
    sign_state s(get_signature_keys(chain_id, canon_type), get_posting,
                 available_keys);
  ...
  sign_state s(get_signature_keys(chain_id, canon_type), get_active,
               available_keys);
  s.max_recursion = max_recursion_depth;
  s.max_membership = max_membership;
  s.max_account_auths = max_account_auths;
...
set<public_key_type> signed_transaction::minimize_required_signatures(
    ...
    const flat_set<public_key_type> &available_keys,
    const authority_getter &get_active, const authority_getter &get_owner,
    const authority_getter &get_posting, uint32_t max_recursion,
    uint32_t max_membership, uint32_t max_account_auths,
    canonical_signature_type canon_type) const {
  set<public_key_type> s = get_required_signatures(
      chain_id, available_keys, get_active, get_owner, get_posting,
      max_recursion, max_membership, max_account_auths, canon_type);
  ...
  for (const public_key_type &k : s) {
    ...
    try {
      blurt::protocol::verify_authority(
          operations, result, get_active, get_owner, get_posting, max_recursion,
          max_membership, max_account_auths, false,
          flat_set<account_name_type>(), flat_set<account_name_type>(),
...
void signed_transaction::verify_authority(
    const chain_id_type &chain_id, const authority_getter &get_active,
    const authority_getter &get_owner, const authority_getter &get_posting,
    uint32_t max_recursion, uint32_t max_membership, uint32_t max_account_auths,
    canonical_signature_type canon_type) const {
  try {
    blurt::protocol::verify_authority(
        operations, get_signature_keys(chain_id, canon_type), get_active,
        get_owner, get_posting, max_recursion, max_membership,
        max_account_auths, false, flat_set<account_name_type>(),
        flat_set<account_name_type>(), flat_set<account_name_type>());
  }
