...
void verify_authority(const vector<AuthContainerType> &auth_containers,
  ...
  try {
    ...
    get_required_auth_visitor auth_visitor(required_active, required_owner,
                                           required_posting, other);

    for (const auto &a : auth_containers)
      auth_visitor(a);

    /**
     *  Transactions with operations required posting authority cannot be
     * combined with transactions requiring active or owner authority. This is
     * for ease of implementation. Future versions of authority verification may
     * be able to check for the merged authority of active and posting.
     */
    if (required_posting.size()) {
      ...
      for (const auto &id : required_posting) {
        BLURT_ASSERT(s.check_authority(id) ||
                     ...
                     ("id", id)("posting", get_posting(id))(
                         "active", get_active(id))("owner", get_owner(id)));
      }
      BLURT_ASSERT(!s.remove_unused_signatures(), tx_irrelevant_sig,
    ...
    for (const auto &auth : other) {
      BLURT_ASSERT(s.check_authority(auth), tx_missing_other_auth,
                   "Missing Authority", ("auth", auth)("sigs", sigs));
    }

    // fetch all of the top level authorities
    for (const auto &id : required_active) {
      BLURT_ASSERT(s.check_authority(id) || s.check_authority(get_owner(id)),
