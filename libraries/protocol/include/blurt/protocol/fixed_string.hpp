...
template <> inline uint128 endian_reverse(const uint128 &u) {
  return uint128(boost::endian::endian_reverse(u.hi),
                 boost::endian::endian_reverse(u.lo));
}
...
/**
 * This class is an in-place memory allocation of a fixed length character
 * string.
 *
...
template <typename _Storage> class fixed_string_impl {
public:
  ...
  friend std::string operator+(const fixed_string_impl &a,
                               const std::string &b) {
    return std::string(a) + b;
  }
  friend std::string operator+(const std::string &a,
                               const fixed_string_impl &b) {
    return a + std::string(b);
  }
  friend bool operator<(const fixed_string_impl &a,
                        const fixed_string_impl &b) {
...
// These storage types work with memory layout and should be used instead of a
// custom template.
...
//
//     BLURT_DEFINE_FIXED_STRING_IMPL( 32, fc::erpair< fc::uint128_t,
//     fc::uint128_t > )
//
...
//
//     BLURT_DEFINE_FIXED_STRING_IMPL( 32, BOOST_IDENTITY_TYPE((fc::erpair<
//     fc::uint128_t, fc::uint128_t >)) )
//
#define BLURT_DEFINE_FIXED_STRING_IMPL(SIZE, STORAGE_TYPE)                     \
  template <> struct fixed_string_impl_for_size<SIZE> {                        \
    typedef blurt::protocol::fixed_string_impl<STORAGE_TYPE> t;                \
  };                                                                           \
                                                                               \
  template <> struct fixed_string_size_for_impl<STORAGE_TYPE> {                \
    static const size_t size = SIZE;                                           \
  };

BLURT_DEFINE_FIXED_STRING_IMPL(16, BOOST_IDENTITY_TYPE((fc::uint128_t)))
BLURT_DEFINE_FIXED_STRING_IMPL(
    24, BOOST_IDENTITY_TYPE((fc::erpair<fc::uint128_t, uint64_t>)))
BLURT_DEFINE_FIXED_STRING_IMPL(
    32, BOOST_IDENTITY_TYPE((fc::erpair<fc::uint128_t, fc::uint128_t>)))
...
void to_variant(const blurt::protocol::fixed_string_impl<Storage> &s,
                variant &v) {
  v = std::string(s);
}
...
struct get_typename<blurt::protocol::fixed_string_impl<Storage>> {
  static const char *name() {
    static const std::string n =
        ...
        std::to_string(
            blurt::protocol::fixed_string_size_for_impl<Storage>::size) +
        std::string(">");
    return n.c_str();
  }
};
