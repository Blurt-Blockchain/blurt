...
struct account_update_operation : public base_operation {
  ...
  void get_required_active_authorities(flat_set<account_name_type> &a) const {
    if (!owner && (active || posting || memo_key || json_metadata.size() > 0))
      a.insert(account);
  }
...
struct beneficiary_route_type {
  ...
  // For use by std::sort such that the route is sorted first by name
  // (ascending)
...
/**
 *  Authors of posts may not want all of the benefits that come from creating a
 * post. This operation allows authors to update properties associated with
 * their post.
 *
...
struct comment_options_operation : public base_operation {
  ...
  asset max_accepted_payout =
      asset(1000000000, BLURT_SYMBOL); /// BLURT_SYMBOL value of the maximum
                                       /// payout this post will receive
  uint16_t percent_blurt = 0;
  bool allow_votes = true; /// allows a post to receive votes;
  bool allow_curation_rewards =
      true; /// allows voters to recieve curation rewards. Rewards return to
            /// reward fund.
 ...
 * contingently to another individual. The funds leave the *from* account and go
 * into a temporary balance where they are held until *from* releases it to *to*
 * or *to* refunds it to *from*.
 *
 *  In the event of a dispute the *agent* can divide the funds between the
 * to/from account. Disputes can be raised any time before or on the dispute
 * deadline time, after the escrow has been approved by all parties.
 *
 *  This operation only creates a proposed escrow transfer. Both the *agent* and
 * *to* must agree to the terms of the arrangement by approving the escrow.
 *
 *  The escrow agent is paid the fee on approval of all parties. It is up to the
 * escrow agent to determine the fee.
 *
 *  Escrow transactions are uniquely identified by 'from' and 'escrow_id', the
 * 'escrow_id' is defined by the sender.
 */
 ...
 * valid on the blockchain. Once a part approves the escrow, the cannot revoke
 * their approval. Subsequent escrow approve operations, regardless of the
 ...
 *  raise it for dispute. Once a payment is in dispute, the agent has authority
 * over who gets what.
 */
 ...
 *  If there is no dispute and escrow has not expired, either party can release
 * funds to the other. If escrow expires and there is no dispute, either party
 * can release funds to either party. If there is a dispute regardless of
 * expiration, the agent can release funds to either party following whichever
 * agreement was in place between the parties.
 */
struct escrow_release_operation : public base_operation {
  account_name_type from;
  account_name_type to; ///< the original 'to'
  account_name_type agent;
  account_name_type who;      ///< the account that is attempting to release the
                              ///< funds, determines valid 'receiver'
  account_name_type receiver; ///< the account that should receive funds (might
                              ///< be from, might be to)
...
/**
 *  This operation converts liquid token (BLURT or liquid SMT) into VFS (Vesting
 * Fund Shares, VESTS or vesting SMT) at the current exchange rate. With this
 * operation it is possible to give another account vesting shares so that
 * faucets can pre-fund new accounts with vesting shares.
 */
 ...
 * and well functioning network.  Any time @owner is in the active set of
 * witnesses these properties will be used to control the blockchain
 * configuration.
 */
struct legacy_chain_properties {
  /**
   *  This fee, paid in BLURT, is converted into VESTING SHARES for the new
   ...
   */
  legacy_blurt_asset account_creation_fee =
      legacy_blurt_asset::from_amount(BLURT_MIN_ACCOUNT_CREATION_FEE);

  /**
   *  This witnesses vote for the maximum_block_size which is used by the
   * network to tune rate limiting and capacity
   */
  uint32_t maximum_block_size = BLURT_MIN_BLOCK_SIZE_LIMIT * 2;

  template <bool force_canon> void validate() const {
    if (force_canon) {
...
struct witness_update_operation : public base_operation {
  ...
  asset fee; ///< the fee paid to register a new witness, should be 10x current
...
struct witness_set_properties_operation : public base_operation {
  ...
  void get_required_authorities(vector<authority> &a) const {
    ...
    if (key_itr != props.end()) {
      ...
      a.push_back(
          authority(1, BLURT_NULL_ACCOUNT,
                    1)); // The null account auth is impossible to satisfy
  }
};
...
/**
 * @brief provides a generic way to add higher level protocols on top of witness
 * consensus
 * @ingroup operations
 *
 * There is no validation for this operation other than that required auths are
 * valid
 */
 ...
 *posting authorities. Unlike custom_operation, this operation is designed to be
 ...
 * weight threshold of the new owner authority to 0, making it an open
 * authority.
 *
...
struct request_account_recovery_operation : public base_operation {
  account_name_type
      recovery_account; ///< The recovery account is listed as the recovery
                        ///< account on the account to recover.
  ...
  authority
      new_owner_authority; ///< The new owner authority the account to recover
                           ///< wishes to have. This is secret known by the
                           ///< account to recover and will be confirmed in a
 ...
 * verification of the recovery account as proof of identity. This operation can
 * only succeed if there was a recovery request sent by the account's recover
 * account.
 *
 ...
 * state. The operation must be signed by keys that satisfy both the new owner
 * authority and the recent owner authority. Failing either fails the operation
 * entirely.
 *
 ...
 * Additionally, an account cannot be permanently recovered to the wrong
 * account. While any owner authority from the past 30 days can be used,
 * including a compromised authority, the account can be continually recovered
 * until the recovery account is confident a combination of uncompromised
 * authorities were used to recover the account. The actual process of verifying
 * authority may become complicated, but that is an application level concern,
 * not the blockchain's concern.
 */
struct recover_account_operation : public base_operation {
  account_name_type account_to_recover; ///< The account to be recovered

  authority new_owner_authority; ///< The new owner authority as specified in
                                 ///< the request account recovery operation.

  authority
      recent_owner_authority; ///< A previous owner authority that the account
                              ///< holder will use to prove past ownership of
...
/**
 *  This operation allows recovery_accoutn to change account_to_reset's owner
 * authority to new_owner_authority after 60 days of inactivity.
 */
 ...
 * the account (The account that pays the creation fee and is a signer on the
 * transaction) or to the empty string if the account was mined. An account with
 * no recovery has the top voted witness as a recovery account, at the time the
 * recover request is created. Note: This does mean the effective recovery
 * account of an account with no listed recovery account can change at any time
 * as witness vote weights. The top voted witness is explicitly the most trusted
 * witness according to stake.
 */
struct change_recovery_account_operation : public base_operation {
  account_name_type account_to_recover; ///< The account that would be recovered
                                        ///< in case of compromise
  account_name_type
      new_recovery_account;   ///< The account that creates the recover request
  extensions_type extensions; ///< Extensions. Not currently used.
...
/**
 * Delegate vesting shares from one account to the other. The vesting shares are
 * still owned by the original account, but content voting rights and bandwidth
...
FC_REFLECT(blurt::protocol::transfer_to_savings_operation,
           (from)(to)(amount)(memo))
FC_REFLECT(blurt::protocol::transfer_from_savings_operation,
           (from)(request_id)(to)(amount)(memo))
FC_REFLECT(blurt::protocol::cancel_transfer_from_savings_operation,
           (from)(request_id))

FC_REFLECT(blurt::protocol::reset_account_operation,
           (reset_account)(account_to_reset)(new_owner_authority))
FC_REFLECT(blurt::protocol::set_reset_account_operation,
...
FC_REFLECT(blurt::protocol::account_create_operation,
           (fee)(creator)(new_account_name)(owner)(active)(posting)(memo_key)(
               json_metadata))
...
FC_REFLECT(blurt::protocol::withdraw_vesting_operation,
           (account)(vesting_shares))
FC_REFLECT(blurt::protocol::set_withdraw_vesting_route_operation,
           (from_account)(to_account)(percent)(auto_vest))
FC_REFLECT(blurt::protocol::witness_update_operation,
           (owner)(url)(block_signing_key)(props)(fee))
FC_REFLECT(blurt::protocol::witness_set_properties_operation,
           (owner)(props)(extensions))
FC_REFLECT(blurt::protocol::account_witness_vote_operation,
           (account)(witness)(approve))
FC_REFLECT(blurt::protocol::account_witness_proxy_operation, (account)(proxy))
FC_REFLECT(blurt::protocol::comment_operation,
           (parent_author)(parent_permlink)(author)(permlink)(title)(body)(
               json_metadata))
...
FC_REFLECT(blurt::protocol::custom_json_operation,
           (required_auths)(required_posting_auths)(id)(json))
FC_REFLECT(blurt::protocol::custom_binary_operation,
           (required_owner_auths)(required_active_auths)(
               required_posting_auths)(required_auths)(id)(data))
...
FC_REFLECT(blurt::protocol::comment_options_operation,
           (author)(permlink)(max_accepted_payout)(allow_votes)(
               allow_curation_rewards)(extensions))

FC_REFLECT(blurt::protocol::escrow_transfer_operation,
           (from)(to)(blurt_amount)(escrow_id)(agent)(fee)(json_meta)(
               ratification_deadline)(escrow_expiration));
FC_REFLECT(blurt::protocol::escrow_approve_operation,
           (from)(to)(agent)(who)(escrow_id)(approve));
FC_REFLECT(blurt::protocol::escrow_dispute_operation,
           (from)(to)(agent)(who)(escrow_id));
FC_REFLECT(blurt::protocol::escrow_release_operation,
           (from)(to)(agent)(who)(receiver)(escrow_id)(blurt_amount));
FC_REFLECT(blurt::protocol::claim_account_operation,
           (creator)(fee)(extensions));
FC_REFLECT(blurt::protocol::create_claimed_account_operation,
           (creator)(new_account_name)(owner)(active)(posting)(memo_key)(
               json_metadata)(extensions));
FC_REFLECT(
    blurt::protocol::request_account_recovery_operation,
    (recovery_account)(account_to_recover)(new_owner_authority)(extensions));
FC_REFLECT(blurt::protocol::recover_account_operation,
           (account_to_recover)(new_owner_authority)(recent_owner_authority)(
               extensions));
FC_REFLECT(blurt::protocol::change_recovery_account_operation,
           (account_to_recover)(new_recovery_account)(extensions));
FC_REFLECT(blurt::protocol::decline_voting_rights_operation,
           (account)(decline));
FC_REFLECT(blurt::protocol::claim_reward_balance_operation,
           (account)(reward_blurt)(reward_vests))
FC_REFLECT(blurt::protocol::delegate_vesting_shares_operation,
