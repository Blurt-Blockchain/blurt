...
void basic_test(const std::vector<uint64_t> &v,
                std::function<void(Object &)> call, chainbase::database &db) {
  ...
  db.modify(*(c.begin()), [](Object &o) { o.val = o.val + 1; });
...
void modify_test(const std::vector<uint64_t> &v,
                 ...
                 std::function<void(bool)> call5, chainbase::database &db) {
  const auto &c = db.get_index<IndexType, Order>();
  ...
  for (; it != end; ++it) {
    bool result = true;
    try {
      db.modify(*it, [&](Object &o) { o.val = first_val; });
...
template <typename IndexType, typename Object, typename SimpleIndex,
...
void misc_test(const std::vector<uint64_t> &v, chainbase::database &db) {
  ...
  db.modify(*(c.begin()),
            [](Object &o) { o.name = "completely_different_name"; });
  auto it1 = c.begin();
  ...
  auto it2 = it1;
  it2++;
  BOOST_CHECK_THROW(db.modify(*it2,
                              [val1](Object &obj) {
...
void misc_test3(const std::vector<uint64_t> &v, chainbase::database &db) {
  ...
  const auto &another_composite_ordered_idx =
      db.get_index<IndexType, AnotherComplexIndex>();
