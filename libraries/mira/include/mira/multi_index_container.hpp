...
public:
  ...
  typedef
      typename primary_index_type::reverse_iterator primary_reverse_iterator;
  ...
  multi_index_container() : super(ctor_args_list()), entry_count(0) {
  ...
  explicit multi_index_container(const boost::filesystem::path &p,
                                 const boost::any &cfg)
      : super(ctor_args_list()), entry_count(0) {
    std::vector<std::string> split_v;
    auto type = boost::core::demangle(typeid(Value).name());
    boost::split(split_v, type, boost::is_any_of(":"));
  ...
  explicit multi_index_container(InputIterator &first, InputIterator &last,
  ...
  multi_index_container(const multi_index_container &other)
      : super(other), _revision(other._revision), _name(other._name),
        _stats(other._stats), _wopts(other._wopts), _ropts(other._ropts),
        entry_count(other.entry_count) {}

  multi_index_container(multi_index_container &&other)
      : super(std::move(other)), _revision(other._revision),
        _name(std::move(other._name)), _stats(std::move(other._stats)),
        _wopts(std::move(other._wopts)), _ropts(std::move(other._ropts)),
        entry_count(other.entry_count) {}
  ...
  bool open(const boost::filesystem::path &p, const boost::any &cfg = nullptr) {
    ...
    // TODO: Move out of constructor becasuse throwing exceptions in a
    // constuctor is sad...
    ...
    try {
      detail::cache_manager::get()->set_object_threshold(
          configuration::get_object_count(cfg));

      opts = configuration::get_options(
          cfg, boost::core::demangle(typeid(Value).name()));
    ...
    ::rocksdb::Status s =
        ::rocksdb::DB::Open(opts, str_path, column_defs, &handles, &db);

    for (::rocksdb::ColumnFamilyHandle *h : handles) {
      super::_handles.push_back(
    ...
    if (s.ok()) {
      ...
      s = super::_db->Get(
          read_opts, &*super::_handles[DEFAULT_COLUMN],
          ::rocksdb::Slice(ser_count_key.data(), ser_count_key.size()),
          &value_slice);

      if (s.ok()) {
        entry_count = fc::raw::unpack_from_char_array<uint64_t>(
      ...
      if (s.ok()) {
        _revision = fc::raw::unpack_from_char_array<int64_t>(
            value_slice.data(), value_slice.size());
      }
    } else {
  ...
  void close() {
    if (super::_db && super::_db.unique()) {
      ...
      super::_db->Put(
          _wopts, &*(super::_handles[DEFAULT_COLUMN]),
          ::rocksdb::Slice(ser_count_key.data(), ser_count_key.size()),
  ...
  void wipe(const boost::filesystem::path &p) {
    ...
    auto s = rocksdb::DestroyDB((p / _name).string(), rocksdb::Options(),
                                column_defs);
  ...
  void trim_cache() { detail::cache_manager::get()->adjust_capacity(); }
  ...
  template <int N> struct nth_index {
    BOOST_STATIC_ASSERT(N >= 0 &&
                        N < boost::mpl::size<index_type_list>::type::value);
  ...
  template <typename Tag> struct index {
    typedef typename boost::mpl::find_if<index_type_list,
    ...
    BOOST_STATIC_CONSTANT(
        ...
        index_found =
            !(boost::is_same<
                iter, typename boost::mpl::end<index_type_list>::type>::value));
  ...
  int64_t set_revision(int64_t rev) {
    ...
    const static ::rocksdb::Slice rev_slice(ser_rev_key.data(),
                                            ser_rev_key.size());
  ...
  size_t get_cache_usage() const { return super::_cache->usage(); }

  size_t get_cache_size() const { return super::_cache->size(); }
  ...
  primary_iterator
  upper_bound(const typename primary_index_type::key_type &x) const {
  ...
  range(LowerBounder &lower,
        const typename primary_index_type::key_type upper) const {
  ...
  typename primary_index_type::const_reverse_iterator
  ...
  bool empty_() const { return entry_count == 0; }

  uint64_t size_() const { return entry_count; }

  uint64_t max_size_() const { return static_cast<uint64_t>(-1); }

  template <typename... Args> bool emplace_rocksdb_(Args &&...args) {
  ...
  template <typename Modifier> bool modify_(Modifier &mod, value_type &v) {
    bool status = false;
    ...
    if (super::modify_(mod, v, modified_indices)) {
      ...
      if (status) {
        ...
        std::lock_guard<std::mutex> lock(super::_cache->get_lock());
        super::_cache->get_index_cache(ID_INDEX)->update(
            (void *)&super::id(v), std::move(v), modified_indices);
        super::commit_first_key_update();
      } else {
        elog("${e}", ("e", retval.ToString()));
        super::reset_first_key_update();
  ...
  bool get_metadata(const MetaKey &k, MetaValue &v) {
    ...
    auto status =
        super::_db->Get(_ropts, &*super::_handles[0], key_slice, &value_slice);

    if (status.ok()) {
      unpack_from_slice(value_slice, v);
      // ilog( "Retrieved metdata for ${type}: ${key},${value}",
      // ("type",boost::core::demangle(typeid(Value).name()))("key",k)("value",v)
      // );
    } else {
      // ilog( "Failed to retrieve metadata for ${type}: ${key}",
      // ("type",boost::core::demangle(typeid(Value).name()))("key",k) );
    }
  ...
  bool put_metadata(const MetaKey &k, const MetaValue &v) {
    ...
    auto status =
        super::_db->Put(_wopts, &*super::_handles[0], key_slice, value_slice);

    if (status.ok()) {
      // ilog( "Stored metdata for ${type}: ${key},${value}",
      // ("type",boost::core::demangle(typeid(Value).name()))("key",k)("value",v)
      // );
    } else {
      // ilog( "Failed to store metadata for ${type}: ${key},${value}",
      // ("type",boost::core::demangle(typeid(Value).name()))("key",k)("value",v)
      // );
    }
...
private:
  ...
  bool maybe_create_schema(const std::string &str_path) {
    ...
    ::rocksdb::Status s = ::rocksdb::DB::OpenForReadOnly(
        opts, str_path, column_defs, &handles, &db);
    ...
    if (s.ok()) {
      ...
      if (s.ok()) {
        ...
        db->Put(_wopts, db->DefaultColumnFamily(),
                ::rocksdb::Slice(ser_rev_key.data(), ser_rev_key.size()),
                ::rocksdb::Slice(ser_rev_val.data(), ser_rev_val.size()));
...
template <typename MultiIndexContainer, int N> struct nth_index {
  BOOST_STATIC_CONSTANT(
      int, M = boost::mpl::size<
               typename MultiIndexContainer::index_type_list>::type::value);
  BOOST_STATIC_ASSERT(N >= 0 && N < M);
  typedef
      typename boost::mpl::at_c<typename MultiIndexContainer::index_type_list,
                                N>::type type;
};
...
template <typename MultiIndexContainer, typename Tag> struct index {
  ...
  typedef
      typename boost::mpl::find_if<index_type_list, detail::has_tag<Tag>>::type
          iter;
...
template <typename MultiIndexContainer, typename Tag> struct index_iterator {
  typedef typename ::mira::multi_index::index<MultiIndexContainer,
