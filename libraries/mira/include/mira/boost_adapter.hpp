...
class boost_multi_index_adapter
    : public boost::multi_index_container<Value, IndexSpecifierList,
                                          Allocator> {
private:
...
public:
  using boost::multi_index_container<Value, IndexSpecifierList,
  ...
  id_type next_id() { return _next_id; }

  void set_next_id(id_type id) { _next_id = id; }

  void close() {}
  void wipe(const boost::filesystem::path &p) {}
  void flush() {}
  bool open(const boost::filesystem::path &p, const boost::any &opts) {
    return true;
  }
  void trim_cache() {}
