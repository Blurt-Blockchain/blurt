...
template <>
struct is_static_length<fc::time_point_sec> : public boost::true_type {};
...
template <typename T>
struct is_static_length<fc::safe<T>> : public is_static_length<T> {};

template <>
struct is_static_length<boost::tuples::null_type> : public boost::true_type {};

template <typename HT, typename TT>
struct is_static_length<boost::tuples::cons<HT, TT>> {
  static const bool value =
      is_static_length<HT>::value && is_static_length<TT>::value;
};
...
template <typename T> void pack_to_slice(PinnableSlice &s, const T &t) {
  slice_packer<T>::pack(s, t);
}

template <typename T> void unpack_from_slice(const Slice &s, T &t) {
  slice_packer<T>::unpack(s, t);
}

template <typename T> T unpack_from_slice(const Slice &s) {
  T t;
