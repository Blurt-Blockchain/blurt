...
#define BOOST_MULTI_INDEX_CK_APPLY_METAFUNCTION_N(z, n, list)                  \
  BOOST_DEDUCED_TYPENAME BOOST_PP_LIST_AT(                                     \
      list, 0)<BOOST_PP_LIST_AT(list, 1), n>::type
...
template <typename CompositeKey, int N> struct nth_key_from_value {
  typedef typename CompositeKey::key_extractor_tuple key_extractor_tuple;
  typedef typename boost::mpl::eval_if_c <
      N<boost::tuples::length<key_extractor_tuple>::value,
        boost::tuples::element<N, key_extractor_tuple>,
        boost::mpl::identity<boost::tuples::null_type>>::type type;
};
...
#define BOOST_MULTI_INDEX_CK_NTH_COMPOSITE_KEY_FUNCTOR(name, functor)          \
  template <typename KeyFromValue> struct BOOST_PP_CAT(key_, name) {           \
    typedef functor<typename KeyFromValue::result_type> type;                  \
  };                                                                           \
                                                                               \
  template <> struct BOOST_PP_CAT(key_, name)<boost::tuples::null_type> {      \
...
typedef boost::tuple<BOOST_MULTI_INDEX_CK_ENUM(
    BOOST_MULTI_INDEX_CK_IDENTITY_ENUM_MACRO, detail::generic_operator_equal)>
...
typedef boost::tuple<BOOST_MULTI_INDEX_CK_ENUM(
    BOOST_MULTI_INDEX_CK_IDENTITY_ENUM_MACRO, detail::generic_operator_less)>
...
struct equal_key_key_terminal {
  static bool compare(const KeyCons1 &, const KeyCons2 &, const EqualCons &) {
    return true;
  }
};
...
struct equal_key_key_normal {
  static bool compare(const KeyCons1 &k1, const KeyCons2 &k2,
                      const EqualCons &eq) {
    if (!eq.get_head()(k1.get_head(), k2.get_head()))
      return false;
    return equal_key_key<typename KeyCons1::tail_type,
                         typename KeyCons2::tail_type,
                         typename EqualCons::tail_type>::compare(k1.get_tail(),
                                                                 k2.get_tail(),
                                                                 eq.get_tail());
  }
};
...
struct equal_key_key
    : boost::mpl::if_<
          boost::mpl::or_<boost::is_same<KeyCons1, boost::tuples::null_type>,
                          boost::is_same<KeyCons2, boost::tuples::null_type>>,
          equal_key_key_terminal<KeyCons1, KeyCons2, EqualCons>,
          equal_key_key_normal<KeyCons1, KeyCons2, EqualCons>>::type {};
...
struct compare_key_key_normal {
  static bool compare(const KeyCons1 &k1, const KeyCons2 &k2,
    ...
    if (comp.get_head()(k1.get_head(), k2.get_head()))
      return true;
    if (comp.get_head()(k2.get_head(), k1.get_head()))
      return false;
    return compare_key_key<
        typename KeyCons1::tail_type, typename KeyCons2::tail_type,
        typename CompareCons::tail_type>::compare(k1.get_tail(), k2.get_tail(),
                                                  comp.get_tail());
  }
};
...
struct compare_key_key
    : boost::mpl::if_<
          ...
          compare_key_key_normal<KeyCons1, KeyCons2, CompareCons>>::type {};
...
struct hash_ckey_normal {
  static std::size_t hash(const KeyCons &c, const Value &v, const HashCons &h,
    ...
    return hash_ckey<
        ...
        BOOST_DEDUCED_TYPENAME HashCons::tail_type>::hash(c.get_tail(), v,
                                                          h.get_tail(), carry);
  }
};
...
template <typename KeyCons, typename Value, typename HashCons>
struct hash_ckey
    : boost::mpl::if_<boost::is_same<KeyCons, boost::tuples::null_type>,
                      hash_ckey_terminal<KeyCons, Value, HashCons>,
                      hash_ckey_normal<KeyCons, Value, HashCons>>::type {};

template <typename ValCons, typename HashCons> struct hash_cval; /* fwd decl. */

template <typename ValCons, typename HashCons> struct hash_cval_terminal {
  static std::size_t hash(const ValCons &, const HashCons &,
                          std::size_t carry) {
    return carry;
  }
};

template <typename ValCons, typename HashCons> struct hash_cval_normal {
  static std::size_t hash(const ValCons &vc, const HashCons &h,
                          std::size_t carry = 0) {
    carry ^=
        h.get_head()(vc.get_head()) + 0x9e3779b9 + (carry << 6) + (carry >> 2);
    return hash_cval<
        BOOST_DEDUCED_TYPENAME ValCons::tail_type,
        BOOST_DEDUCED_TYPENAME HashCons::tail_type>::hash(vc.get_tail(),
                                                          h.get_tail(), carry);
  }
};
...
struct hash_cval
    : boost::mpl::if_<boost::is_same<ValCons, boost::tuples::null_type>,
...
struct key_from_value_normal {
  static Key extract(const KeyExtractor &e, const Value &v) {
    return Key(
        e.get_head()(v),
        key_from_value<typename KeyExtractor::tail_type,
                       typename Key::tail_type, Value>::extract(e.get_tail(),
                                                                v));
  }
};
...
struct key_from_value
    : boost::mpl::if_<boost::is_same<KeyExtractor, boost::tuples::null_type>,
                      key_from_value_terminal<KeyExtractor, Key, Value>,
                      key_from_value_normal<KeyExtractor, Key, Value>>::type {};

template <typename Key, typename Tuple> struct convert_compatible_key;

template <typename Key, typename Tuple> struct convert_compatible_key_terminal {
  static Key convert(const Tuple &t) {
    return Key(t.get_head(), typename Key::tail_type());
  }
};

template <typename Key, typename Tuple> struct convert_compatible_key_normal {
  static Key convert(const Tuple &t) {
    return Key(t.get_head(),
               convert_compatible_key<
                   typename Key::tail_type,
                   typename Tuple::tail_type>::convert(t.get_tail()));
  }
};

template <typename Key, typename Tuple>
struct convert_compatible_key
    : boost::mpl::if_<boost::mpl::or_<boost::is_same<typename Key::tail_type,
                                                     boost::tuples::null_type>,
                                      boost::is_same<typename Tuple::tail_type,
                                                     boost::tuples::null_type>>,
                      convert_compatible_key_terminal<Key, Tuple>,
                      convert_compatible_key_normal<Key, Tuple>>::type {};
...
struct convert_compatible_key< Key, boost::tuples::cons< CompKeyHT,
boost::tuples::null_type > >
{
   static Key convert( const boost::tuples::cons< CompKeyHT,
boost::tuples::null_type >& k )
   {
...
struct convert_compatible_key< Key, boost::tuples::tuple< Args... > >
{
   ...
   {
      return convert_to_cons< Key, boost::tuples::tuple< Args ... > >::convert(
k );
   }
};
...
template <typename HT, typename TT> struct result_type_extractor {
  typedef
      typename boost::tuples::cons<typename HT::result_type,
...
struct result_type_extractor<HT, boost::tuples::null_type> {
  typedef typename boost::tuples::cons<typename HT::result_type,
                                       boost::tuples::null_type>
      type;
};

template <typename KeyCons> struct composite_key_conversion {
  typedef
      typename result_type_extractor<typename KeyCons::head_type,
                                     typename KeyCons::tail_type>::type type;
};

template <typename CompositeKey> struct composite_key_result {
  typedef CompositeKey composite_key_type;
  typedef typename composite_key_conversion<
      typename composite_key_type::key_extractor_tuple>::type key_type;
  typedef typename composite_key_type::value_type value_type;

  explicit composite_key_result(const composite_key_type &composite_key,
                                const value_type &value)
      : key(key_from_value<typename composite_key_type::key_extractor_tuple,
                           key_type, value_type>::extract(composite_key
                                                              .key_extractors(),
                                                          value)) {}
  ...
  explicit composite_key_result(const boost::tuples::tuple<Args...> compat_key)
      : key(convert_compatible_key<
            key_type, boost::tuples::tuple<Args...>>::convert(compat_key)) {}

  template <typename CompatibleKey>
  explicit composite_key_result(const CompatibleKey &k)
      : composite_key_result(boost::make_tuple(k)) {}
...
template <typename Value, BOOST_MULTI_INDEX_CK_ENUM(
                              BOOST_MULTI_INDEX_CK_TEMPLATE_PARM, KeyFromValue)>
...
public:
  ...
  composite_key(BOOST_MULTI_INDEX_CK_ENUM(BOOST_MULTI_INDEX_CK_CTOR_ARG,
                                          KeyFromValue))
...
#if !defined(BOOST_NO_SFINAE)
  typename disable_if<is_convertible<const ChainedPtr &, const value_type &>,
                      result_type>::type
#else
  result_type
#endif
...
inline bool operator==(const composite_key_result<CompositeKey1> &x,
  ...
  BOOST_STATIC_ASSERT(boost::tuples::length<key_extractor_tuple1>::value ==
                      boost::tuples::length<key_extractor_tuple2>::value);

  return detail::equal_key_key<typename CompositeKey1::result_type::key_type,
                               typename CompositeKey2::result_type::key_type,
                               detail::generic_operator_equal_tuple>::
      compare(x.key, y.key, detail::generic_operator_equal_tuple());
}

template <typename CompositeKey,
          BOOST_MULTI_INDEX_CK_ENUM_PARAMS(typename Value)>
inline bool
operator==(const composite_key_result<CompositeKey> &x,
           const tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> &y) {
  typedef typename CompositeKey::key_extractor_tuple key_extractor_tuple;
  typedef boost::tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> key_tuple;

  BOOST_STATIC_ASSERT(boost::tuples::length<key_extractor_tuple>::value ==
                      boost::tuples::length<key_tuple>::value);

  return detail::equal_key_key<typename CompositeKey::result_type::key_type,
                               key_tuple,
                               detail::generic_operator_equal_tuple>::
      compare(x.key, y, detail::generic_operator_equal_tuple());
}

template <BOOST_MULTI_INDEX_CK_ENUM_PARAMS(typename Value),
          typename CompositeKey>
inline bool operator==(const tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> &x,
  ...
  return detail::equal_key_key<key_tuple,
                               typename CompositeKey::result_type::key_type,
                               detail::generic_operator_equal_tuple>::
      compare(x, y.key, detail::generic_operator_equal_tuple());
}
...
inline bool operator==(const composite_key_result<CompositeKey> &x,
  ...
  BOOST_STATIC_ASSERT(static_cast<std::size_t>(
  ...
  return detail::equal_key_key<typename CompositeKey::result_type::key_type,
                               cons_key_tuple,
                               detail::generic_operator_equal_tuple>::
      compare(x.key, detail::make_cons_stdtuple(y),
              detail::generic_operator_equal_tuple());
}
...
inline bool operator==(const std::tuple<Values...> &x,
  ...
  return detail::equal_key_key<cons_key_tuple,
                               typename CompositeKey::result_type::key_type,
                               detail::generic_operator_equal_tuple>::
      compare(detail::make_cons_stdtuple(x), y.key,
              detail::generic_operator_equal_tuple());
}
#endif
...
inline bool operator<(const composite_key_result<CompositeKey1> &x,
                      const composite_key_result<CompositeKey2> &y) {
  return detail::compare_key_key<typename CompositeKey1::result_type::key_type,
                                 typename CompositeKey2::result_type::key_type,
                                 detail::generic_operator_less_tuple>::
      compare(x.key, y.key, detail::generic_operator_less_tuple());
}
...
inline bool operator<(const composite_key_result<CompositeKey> &x,
  ...
  return detail::compare_key_key<typename CompositeKey::result_type::key_type,
                                 key_tuple,
                                 detail::generic_operator_less_tuple>::
      compare(x.key, y, detail::generic_operator_less_tuple());
}
...
inline bool operator<(const tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> &x,
  ...
  return detail::compare_key_key<key_tuple,
                                 typename CompositeKey::result_type::key_type,
                                 detail::generic_operator_less_tuple>::
      compare(x, y.key, detail::generic_operator_less_tuple());
}
...
inline bool operator<(const std::tuple<Values...> &x,
  ...
  return detail::compare_key_key<cons_key_tuple,
                                 typename CompositeKey::result_type::key_type,
                                 detail::generic_operator_less_tuple>::
      compare(detail::make_cons_stdtuple(x), y.key,
              detail::generic_operator_less_tuple());
...
BOOST_MULTI_INDEX_CK_COMPLETE_COMP_OPS(typename CompositeKey1,
...
BOOST_MULTI_INDEX_CK_COMPLETE_COMP_OPS(
    typename CompositeKey, BOOST_MULTI_INDEX_CK_ENUM_PARAMS(typename Value),
    composite_key_result<CompositeKey>,
    tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)>)

BOOST_MULTI_INDEX_CK_COMPLETE_COMP_OPS(
    BOOST_MULTI_INDEX_CK_ENUM_PARAMS(typename Value), typename CompositeKey,
    tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)>,
    composite_key_result<CompositeKey>)

#if !defined(BOOST_NO_CXX11_HDR_TUPLE) &&                                      \
    !defined(BOOST_NO_CXX11_VARIADIC_TEMPLATES)
BOOST_MULTI_INDEX_CK_COMPLETE_COMP_OPS(typename CompositeKey,
                                       typename... Values,
                                       composite_key_result<CompositeKey>,
                                       std::tuple<Values...>)
...
template <BOOST_MULTI_INDEX_CK_ENUM(BOOST_MULTI_INDEX_CK_TEMPLATE_PARM, Pred)>
...
public:
  ...
  bool operator()(const composite_key_result<CompositeKey1> &x,
    ...
    return detail::equal_key_key<
        ...
        typename CompositeKey2::result_type::key_type,
        detail::generic_operator_less_tuple>::compare(x.key, y.key, key_eqs());
  }
  ...
  operator()(const composite_key_result<CompositeKey> &x,
    ...
    return detail::equal_key_key<typename CompositeKey::result_type::key_type,
                                 key_tuple, key_eq_tuple>::compare(x.key, y,
                                                                   key_eqs());
  }

  template <BOOST_MULTI_INDEX_CK_ENUM_PARAMS(typename Value),
            typename CompositeKey>
  bool operator()(const tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> &x,
                  const composite_key_result<CompositeKey> &y) const {
    typedef typename CompositeKey::key_extractor_tuple key_extractor_tuple;
    typedef boost::tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> key_tuple;

    BOOST_STATIC_ASSERT(boost::tuples::length<key_tuple>::value <=
                            boost::tuples::length<key_eq_tuple>::value &&
                        boost::tuples::length<key_tuple>::value ==
                            boost::tuples::length<key_extractor_tuple>::value);
    ...
    return detail::equal_key_key<key_tuple,
                                 typename CompositeKey::result_type::key_type,
                                 key_eq_tuple>::compare(x, y.key, key_eqs());
  }
  ...
  bool operator()(const composite_key_result<CompositeKey> &x,
    ...
    return detail::equal_key_key<
        typename CompositeKey::result_type::key_type, cons_key_tuple,
        key_eq_tuple>::compare(x.key, detail::make_cons_stdtuple(y), key_eqs());
  }
  ...
  bool operator()(const std::tuple<Values...> &x,
    ...
    return detail::equal_key_key<
        cons_key_tuple, typename CompositeKey::result_type::key_type,
        key_eq_tuple>::compare(detail::make_cons_stdtuple(x), y.key, key_eqs());
  }
#endif
};
...
template <BOOST_MULTI_INDEX_CK_ENUM(BOOST_MULTI_INDEX_CK_TEMPLATE_PARM,
                                    Compare)>
...
public:
  ...
  composite_key_compare(BOOST_MULTI_INDEX_CK_ENUM(BOOST_MULTI_INDEX_CK_CTOR_ARG,
                                                  Compare))
      : super(BOOST_MULTI_INDEX_CK_ENUM_PARAMS(k)) {}
  ...
  bool operator()(const composite_key_result<CompositeKey1> &x,
    ...
    return detail::compare_key_key<
        typename CompositeKey1::result_type::key_type,
        typename CompositeKey2::result_type::key_type,
        key_comp_tuple>::compare(x.key, y.key, key_comps());
  }
  ...
  operator()(const composite_key_result<CompositeKey> &x,
    ...
    return detail::compare_key_key<typename CompositeKey::result_type::key_type,
                                   key_tuple,
                                   key_comp_tuple>::compare(x.key, y,
                                                            key_comps());
  }
  ...
  bool operator()(const tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> &x,
    ...
    return detail::compare_key_key<key_tuple,
                                   typename CompositeKey::result_type::key_type,
                                   key_comp_tuple>::compare(x, y.key,
                                                            key_comps());
  }
  ...
  bool operator()(const composite_key_result<CompositeKey> &x,
    ...
    return detail::compare_key_key<
        typename CompositeKey::result_type::key_type, cons_key_tuple,
        key_comp_tuple>::compare(x.key, detail::make_cons_stdtuple(y),
                                 key_comps());
  }
  ...
  bool operator()(const std::tuple<Values...> &x,
    ...
    BOOST_STATIC_ASSERT(std::tuple_size<key_tuple>::value <=
                            static_cast<std::size_t>(
                                boost::tuples::length<key_comp_tuple>::value) ||
                        boost::tuples::length<key_extractor_tuple>::value <=
                            boost::tuples::length<key_comp_tuple>::value);
    ...
    return detail::compare_key_key<
        cons_key_tuple, typename CompositeKey::result_type::key_type,
        key_comp_tuple>::compare(detail::make_cons_stdtuple(x), y.key, y.value,
                                 key_comps());
  }
#endif
};
...
template <BOOST_MULTI_INDEX_CK_ENUM(BOOST_MULTI_INDEX_CK_TEMPLATE_PARM, Hash)>
...
public:
  ...
  composite_key_hash(BOOST_MULTI_INDEX_CK_ENUM(BOOST_MULTI_INDEX_CK_CTOR_ARG,
                                               Hash))
      : super(BOOST_MULTI_INDEX_CK_ENUM_PARAMS(k)) {}
  ...
  std::size_t operator()(const composite_key_result<CompositeKey> &x) const {
    ...
    return detail::hash_ckey<key_extractor_tuple, value_type,
                             key_hasher_tuple>::hash(x.composite_key
                                                         .key_extractors(),
                                                     x.value,
                                                     key_hash_functions());
  }
  ...
  template <BOOST_MULTI_INDEX_CK_ENUM_PARAMS(typename Value)>
  std::size_t
  operator()(const tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> &x) const {
    typedef boost::tuple<BOOST_MULTI_INDEX_CK_ENUM_PARAMS(Value)> key_tuple;

    BOOST_STATIC_ASSERT(boost::tuples::length<key_tuple>::value ==
                        boost::tuples::length<key_hasher_tuple>::value);

    return detail::hash_cval<key_tuple, key_hasher_tuple>::hash(
        x, key_hash_functions());
  }
  ...
  std::size_t operator()(const std::tuple<Values...> &x) const {
    ...
    return detail::hash_cval<cons_key_tuple, key_hasher_tuple>::hash(
        detail::make_cons_stdtuple(x), key_hash_functions());
  }
#endif
};
...
#define BOOST_MULTI_INDEX_CK_RESULT_EQUAL_TO_SUPER                             \
  composite_key_equal_to<BOOST_MULTI_INDEX_CK_ENUM(                            \
...
#define BOOST_MULTI_INDEX_CK_RESULT_LESS_SUPER                                 \
  composite_key_compare<BOOST_MULTI_INDEX_CK_ENUM(                             \
      BOOST_MULTI_INDEX_CK_APPLY_METAFUNCTION_N, /* the argument is a PP list  \
                                                  */                           \
...
template <typename T>
struct is_static_length<multi_index::composite_key_result<T>>
    : public is_static_length<
          typename multi_index::composite_key_result<T>::key_type> {};
/*
template< typename T >
struct slice_packer< multi_index::composite_key_result< T > >
{
   ...
   {
      if( is_static_length< typename multi_index::composite_key_result< T
>::key_type >::value )
      {
         static_packer< multi_index::composite_key_result< T >::key_type
>::pack( s, t.key );
      }
      else
      {
         dynamic_packer< multi_index::composite_key_result< T >::key_type
>::pack( s, t.key );
      }
   }
...
/*
template< typename T >
inline void pack_to_slice< multi_index::composite_key_result< T > >(
PinnableSlice& s, const multi_index::composite_key_result< T >& t )
{

}

template< typename T >
inline void unpack_from_slice< multi_index::composite_key_result< T > >( const
Slice& s, multi_index::composite_key_result< T >& t )
{
   if( is_static_length< typename multi_index::composite_key_result< T
>::key_type >::value )
   {
   ...
   {
      fc::raw::unpack_from_char_array< multi_index::composite_key_result< T > >(
s.data(), s.size(), t );
   }
}
//*/
...
template <typename T>
void to_variant(const mira::multi_index::composite_key_result<T> &var,
                variant &vo) {
  to_variant(var.key, vo);
}

template <typename T>
void from_variant(const variant &vo,
                  mira::multi_index::composite_key_result<T> &var) {
  from_variant(vo, var.key);
}

template <typename Cons> struct cons_to_variant;

template <typename Cons> struct cons_to_variant_terminal {
...
template <typename Cons> struct cons_to_variant_normal {
  static void push_variant(const Cons &c, fc::variants &vars) {
    fc::variant v;
    to_variant(c.get_head(), v);
    vars.push_back(v);
...
struct cons_to_variant
    : boost::mpl::if_<
          ...
          cons_to_variant_terminal<Cons>, cons_to_variant_normal<Cons>>::type {
};

template <typename HT, typename TT>
void to_variant(const boost::tuples::cons<HT, TT> &var, variant &vo) {
  fc::variants v;
...
struct get_typename<mira::multi_index::composite_key_result<T>> {
  static const char *name() {
    static std::string n =
        std::string("mira::multi_index::composite_key_result<") +
...
void inline pack(Stream &s,
                 const mira::multi_index::composite_key_result<T> &var) {
  pack(s, var.key);
}
...
void pack(Stream &, const boost::tuples::null_type) {}
template <typename Stream>
void unpack(Stream &s, boost::tuples::null_type, uint32_t depth) {}

template <typename Stream, typename H, typename T>
void pack(Stream &s, const boost::tuples::cons<H, T> &var) {
  pack(s, var.get_head());
  pack(s, var.get_tail());
}

template <typename Stream, typename H, typename T>
void unpack(Stream &s, boost::tuples::cons<H, T> &var, uint32_t depth) {
  depth++;
