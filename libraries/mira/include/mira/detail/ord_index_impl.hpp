...
#define ROCKSDB_ITERATOR_PARAM_PACK                                            \
  const_cast<column_handles *>(&_handles), COLUMN_INDEX, super::_db, *_cache
...
public:
  ...
  typedef slice_comparator<key_type, Compare> key_compare;
  ...
  //*
  typedef typename boost::mpl::if_<typename super::is_terminal_node, key_type,
  ...
  //*
  typedef
      typename boost::mpl::if_<typename super::is_terminal_node, KeyFromValue,
                               typename super::id_from_value>::type
          id_from_value;
  //*/
  ...
  */
  typedef rocksdb_iterator<value_type, key_type, KeyFromValue, key_compare,
                           id_type, id_from_value>
      iterator;
...
protected:
  typedef boost::tuples::cons<ctor_args, typename super::ctor_args_list>
      ctor_args_list;
  typedef typename boost::mpl::push_front<
      typename super::index_type_list,
      ordered_index<KeyFromValue, Compare, SuperMeta, TagList, Category,
                    AugmentPolicy>>::type index_type_list;
  typedef typename boost::mpl::push_front<typename super::iterator_type_list,
                                          iterator>::type iterator_type_list;
  typedef
      typename boost::mpl::push_front<typename super::const_iterator_type_list,
                                      const_iterator>::type
...
protected:
  ...
  typedef std::pair<typename primary_index_type::iterator, bool>
      emplace_return_type;
...
public:
  ...
  iterator end() BOOST_NOEXCEPT {
    return iterator::end(ROCKSDB_ITERATOR_PARAM_PACK);
  }
  const_iterator end() const BOOST_NOEXCEPT {
    return const_iterator::end(ROCKSDB_ITERATOR_PARAM_PACK);
  }
  ...
  iterator iterator_to(const value_type &x) { return make_iterator(key(x)); }
  ...
  BOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS
      ...
      : super(args_list.get_tail()),
        ...
        comp_(std::make_shared<key_compare>(
            boost::tuples::get<1>(args_list.get_head()))) {
    empty_initialize();
    _cache->set_index_cache(
        COLUMN_INDEX,
        std::make_unique<index_cache<value_type, key_type, key_from_value>>());
  }

  ordered_index_impl()
      : super(), _cache(object_cache_factory_type::get_shared_cache()),
        _handles(super::_handles) {
    empty_initialize();
    _cache->set_index_cache(
        COLUMN_INDEX,
  ...
  ordered_index_impl(const ordered_index_impl &other)
      : super(other), _cache(other._cache), _handles(super::_handles),
        _key_modification_count(other._key_modification_count),
        _flush_opts(other._flush_opts), _first_key(other._first_key),
        _first_key_update(other._first_key_update),
        _delete_first_key(other._delete_first_key), key(other.key),
        comp_(other.comp_), id(other.id) {}

  ordered_index_impl(ordered_index_impl &&other)
      : super(std::move(other)), _cache(std::move(other._cache)),
        _handles(super::_handles),
        _key_modification_count(other._key_modification_count),
        ...
        key(std::move(other.key)), comp_(std::move(other.comp_)),
  ...
  bool insert_rocksdb_(const value_type &v) {
    if (super::insert_rocksdb_(v)) {
      ...
      s = super::_db->Get(::rocksdb::ReadOptions(),
                          &*super::_handles[COLUMN_INDEX], key_slice,
                          &read_buffer);
      ...
      s = super::_write_buffer.Put(&*super::_handles[COLUMN_INDEX], key_slice,
                                   value_slice);
  ...
  void erase_(value_type &v) {
    ...
    super::_write_buffer.Delete(&*super::_handles[COLUMN_INDEX], old_key_slice);

    if (_first_key.valid() && (key_comp()(old_key, *_first_key) ==
                               key_comp()(*_first_key, old_key))) {
  ...
  bool modify_(Modifier mod, value_type &v,
    ...
    if (super::modify_(mod, v, modified_indices)) {
      ...
      if (COLUMN_INDEX == 1) {
        ...
        s = super::_db->Get(::rocksdb::ReadOptions(),
                            &*super::_handles[COLUMN_INDEX], new_key_slice,
                            &read_buffer);
        ...
        s = super::_write_buffer.Delete(&*super::_handles[COLUMN_INDEX],
                                        old_key_slice);
      ...
      s = super::_write_buffer.Put(&*super::_handles[COLUMN_INDEX],
                                   new_key_slice, value_slice);

      // If the new key is less than the current first, it will be the new first
      // key
      if (_first_key.valid() && key_comp()(new_key, *_first_key)) {
        _first_key_update = new_key;
      }
      // Else if we are updating the current first key AND the new key is
      // different...
      else if (_first_key.valid() && key_comp()(*_first_key, new_key) &&
               (key_comp()(old_key, *_first_key) ==
                key_comp()(*_first_key, old_key))) {
        ...
        if (first_key_update_itr != end()) {
          ...
          // If the second key is less than the new key, then the second key is
          // the next first key
          if (key_comp()(first_key_update, new_key)) {
            _first_key_update = first_key_update;
          }
          // Otherise the new key is between the first key and the second key
          // and will be the next fist key
          else {
            _first_key_update = new_key;
          }
        }
  ...
  void populate_column_definitions_(column_definitions &defs) const {
    ...
    // TODO: Clean this up so it outputs the tag name instead of a tempalte
    // type. But it is unique, so it works.
    std::string tags = boost::core::demangle(typeid(tag_list).name());
    // std::cout <<  COLUMN_INDEX << ':' << tags << std::endl;
    defs.emplace_back(tags, ::rocksdb::ColumnFamilyOptions());
  ...
  void dump_lb_call_counts() {
    ...
    wdump((iterator::lb_call_count())(iterator::lb_prev_call_count())(
...
private:
  ...
  emplace_impl(BOOST_MULTI_INDEX_FUNCTION_PARAM_PACK) {
    ...
    return std::pair<typename primary_index_type::iterator, bool>(
        res ? primary_index_type::iterator_to(v) : primary_index_type::end(),
        res);
  }
...
protected:
  ordered_index(const ctor_args_list &args_list) : super(args_list) {}
