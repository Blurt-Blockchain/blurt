...
struct rocksdb_iterator
    : public boost::bidirectional_iterator_helper<
          rocksdb_iterator<Value, Key, KeyFromValue, KeyCompare, ID,
                           IDFromValue>,
          Value, std::size_t, const Value *, const Value &> {
...
private:
  ...
  // The following declarations exist solely for the iterator to have a default
  // constructor
...
public:
  ...
  rocksdb_iterator(rocksdb_iterator &other)
      : _handles(other._handles), _index(other._index),
        _snapshot(other._snapshot), _db(other._db), _cache(other._cache),
  ...
  rocksdb_iterator(rocksdb_iterator &&other)
      : _handles(other._handles), _index(other._index),
        _iter(std::move(other._iter)), _snapshot(other._snapshot),
        _db(other._db), _cache(other._cache), _cache_value(other._cache_value) {
    //_opts.snapshot = _snapshot->snapshot();
    other._snapshot.reset();
    other._db.reset();
  }

  rocksdb_iterator(column_handles *handles, size_t index, db_ptr db,
                   cache_type &cache)
      : _handles(handles), _index(index), _db(db), _cache(&cache) {
    // Not sure the implicit move constuctor for ManageSnapshot isn't going to
    // release the snapshot...
    //_snapshot = std::make_shared< ::rocksdb::ManagedSnapshot >( &(*_db) );
    //_opts.snapshot = _snapshot->snapshot();
    //_iter.reset( _db->NewIterator( _opts, &*(*_handles)[ _index ] ) );
  }
  rocksdb_iterator(std::shared_ptr<Value> &cache_value, column_handles *handles,
                   size_t index, db_ptr db, cache_type &cache)
      : _handles(handles), _index(index), _db(db), _cache(&cache),
  ...
  rocksdb_iterator(std::shared_ptr<Value> &cache_value, column_handles *handles,
      ...
      : _handles(handles), _index(index), _iter(std::move(iter)), _db(db),
        _cache(&cache), _cache_value(cache_value) {}

  rocksdb_iterator(column_handles *handles, size_t index, db_ptr db,
                   cache_type &cache, const Key &k)
      : _handles(handles), _index(index), _db(db), _cache(&cache) {
    key_type *id = (key_type *)&k;
    std::lock_guard<std::mutex> lock(
        _cache->get_index_cache(_index)->get_lock());
  ...
  rocksdb_iterator(column_handles *handles, size_t index, db_ptr db,
                   cache_type &cache, const ::rocksdb::Slice &s)
      : _handles(handles), _index(index), _db(db), _cache(&cache) {
    Key k;
    ...
    if (_cache_value != nullptr) {
      ...
      if (!ptr) {
        if (_index == ID_INDEX) {
          ...
          auto s = _db->Get(_opts, &*(*_handles)[ID_INDEX], _iter->value(),
                            &value_slice);
          assert(s.ok());
      ...
      if (_cache_value != nullptr) {
        ...
        if (_iter == nullptr) {
          ...
          if (_iter->Valid()) {
            ...
            if (memcmp(slice.data(), found_key.data(),
                       std::min(slice.size(), found_key.size())) != 0) {
              _iter.reset(_db->NewIterator(_opts, &*(*_handles)[_index]));
              return *this;
            }
          } else {
  ...
  static rocksdb_iterator begin(column_handles *handles, size_t index,
                                db_ptr db, cache_type &cache) {
  ...
  static rocksdb_iterator end(column_handles *handles, size_t index, db_ptr db,
                              cache_type &cache) {
    return rocksdb_iterator(handles, index, db, cache);
  }

  template <typename CompatibleKey>
  static rocksdb_iterator find(column_handles *handles, size_t index, db_ptr db,
                               cache_type &cache, const CompatibleKey &k) {
    static KeyCompare compare = KeyCompare();
    ...
    if (itr.valid()) {
      ...
      if (compare(k, found_key) != compare(found_key, k)) {
        itr._iter.reset(
            itr._db->NewIterator(itr._opts, &*(*itr._handles)[itr._index]));
      }
    } else {
      itr._iter.reset(
          itr._db->NewIterator(itr._opts, &*(*itr._handles)[itr._index]));
  ...
  static rocksdb_iterator lower_bound(column_handles *handles, size_t index,
                                      db_ptr db, cache_type &cache,
                                      const Key &k) {
    key_type *id = (key_type *)&k;
    std::lock_guard<std::mutex> lock(cache.get_index_cache(index)->get_lock());
  ...
  */
  template <typename CompatibleKey>
  static rocksdb_iterator upper_bound(column_handles *handles, size_t index,
                                      db_ptr db, cache_type &cache,
                                      const CompatibleKey &k) {
    static KeyCompare compare = KeyCompare();
    rocksdb_iterator itr(handles, index, db, cache);
  ...
  static std::pair<rocksdb_iterator, rocksdb_iterator>
  range(column_handles *handles, size_t index, db_ptr db, cache_type &cache,
        const LowerBoundType &lower, const UpperBoundType &upper) {
  ...
  template <typename CompatibleKey>
  static std::pair<rocksdb_iterator, rocksdb_iterator>
  equal_range(column_handles *handles, size_t index, db_ptr db,
              cache_type &cache, const CompatibleKey &k) {
    return std::make_pair<rocksdb_iterator, rocksdb_iterator>(
...
template <typename Value, typename Key, typename KeyFromValue,
          typename KeyCompare, typename ID, typename IDFromValue>
bool operator==(const rocksdb_iterator<Value, Key, KeyFromValue, KeyCompare, ID,
                                       IDFromValue> &x,
                const rocksdb_iterator<Value, Key, KeyFromValue, KeyCompare, ID,
                                       IDFromValue> &y) {
  return x.equals(y);
}
