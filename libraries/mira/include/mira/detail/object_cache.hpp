...
public:
  typedef std::list<std::pair<
      boost::any, std::shared_ptr<abstract_multi_index_cache_manager>>>
      list_type;
...
public:
  iterator_type
  insert(boost::any v,
         std::shared_ptr<abstract_multi_index_cache_manager> &&m) {
  ...
  void set_object_threshold(size_t capacity) { _obj_threshold = capacity; }

  void adjust_capacity() { adjust_capacity(_obj_threshold); }
...
struct cache_manager {
  ...
  static void reset() { get(true); }
};

typedef const void *cache_key_type;

template <typename Value> struct cache_factory;

template <typename Value> class multi_index_cache_manager;

template <typename Value> class abstract_index_cache {
public:
  ...
  typedef std::pair<ptr_type, lru_cache_manager::iterator_type>
      cache_bundle_type;

  virtual ptr_type get(cache_key_type key) = 0;
  virtual void update(cache_key_type key, Value &&v) = 0;
  virtual void update(cache_key_type key, Value &&v,
                      const std::vector<size_t> &modified_indices) = 0;
...
private:
  virtual void set_multi_index_cache_manager(
      std::shared_ptr<multi_index_cache_manager<Value>> &&m) {
...
public:
  ...
  void update(cache_bundle_type bundle, Value &&new_value,
              const std::vector<size_t> &modified_indices) {
  ...
  size_t size() const { return _index_caches.find(1)->second->size(); }

  std::mutex &get_lock() { return _lock; }
};
...
public:
  typedef typename std::shared_ptr<Value> ptr_type;
  typedef typename std::pair<ptr_type, lru_cache_manager::iterator_type>
      cache_bundle_type;
...
private:
  ...
  const Key key(cache_key_type k) { return *((Key *)k); }
  ...
  virtual void invalidate(const Value &v,
                          bool update_cache_manager) override final {
    auto k = _get_key(v);
  ...
  virtual void clear() override final { _cache.clear(); }
  ...
  virtual size_t size() const override final { return _cache.size(); }
...
public:
  ...
  virtual void update(cache_key_type k, Value &&v) override final {
    ...
    abstract_index_cache<Value>::_multi_index_cache_manager->update(
        _cache[key(k)], std::move(v));
  }

  virtual void
  update(cache_key_type k, Value &&v,
         const std::vector<size_t> &modified_indices) override final {
    assert(abstract_index_cache<Value>::_multi_index_cache_manager != nullptr);
    abstract_index_cache<Value>::_multi_index_cache_manager->update(
        _cache[key(k)], std::move(v), modified_indices);
  }
...
template <typename Value> struct cache_factory {
  static std::shared_ptr<multi_index_cache_manager<Value>>
  get_shared_cache(bool reset = false) {
  ...
  static void reset() { get_shared_cache(true); }
};
