...
struct abstract_slice_comparator : ::rocksdb::Comparator, CompareType {
  ...
  FindShortestSeparator(std::string *start,
                        const ::rocksdb::Slice &limit) const override final {
    /// Nothing to do.
  }
...
struct slice_comparator final : abstract_slice_comparator<Key, CompareType> {
  ...
  virtual int Compare(const ::rocksdb::Slice &x,
                      const ::rocksdb::Slice &y) const override {
    Key x_key;
    unpack_from_slice(x, x_key);
    Key y_key;
    unpack_from_slice(y, y_key);
  ...
  virtual bool Equal(const ::rocksdb::Slice &x,
                     const ::rocksdb::Slice &y) const override {
    if (x.size() != y.size())
      return false;
    Key x_key;
...
// TODO: Primitive Types can implement custom comparators that understand the
// independent format
