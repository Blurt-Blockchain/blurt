...
static std::map<std::string,
                std::function<void(::rocksdb::Options &, fc::variant)>>
    global_database_option_map{
        {ALLOW_MMAP_READS,
        ...
        {BLOCK_BASED_TABLE_OPTIONS, [](::rocksdb::Options &o, auto v) {
           ...
           if (obj.contains(BLOCK_SIZE)) {
             FC_ASSERT(obj[BLOCK_SIZE].is_uint64(),
                       "Expected '${key}' to be an unsigned integer",
                       ("key", BLOCK_SIZE));
           ...
           if (obj.contains(BLOOM_FILTER_POLICY)) {
             ...
             FC_ASSERT(filter_policy.contains(BITS_PER_KEY),
                       "Expected '${parent}' to contain '${key}'",
                       ("parent", BLOOM_FILTER_POLICY)("key", BITS_PER_KEY));

             FC_ASSERT(filter_policy[BITS_PER_KEY].is_uint64(),
                       "Expected '${key}' to be an unsigned integer",
                       ("key", BITS_PER_KEY));
             ...
             if (filter_policy.contains(USE_BLOCK_BASED_BUILDER)) {
               FC_ASSERT(filter_policy[USE_BLOCK_BASED_BUILDER].is_bool(),
                         "Expected '${key}' to be a boolean",
                         ("key", USE_BLOCK_BASED_BUILDER));

               table_options.filter_policy.reset(rocksdb::NewBloomFilterPolicy(
                   bits_per_key,
                   filter_policy[USE_BLOCK_BASED_BUILDER].template as<bool>()));
             } else {
...
fc::variant_object
configuration::apply_configuration_overlay(const fc::variant &base,
  ...
  FC_ASSERT(overlay.is_object(),
            "Expected database overlay configuration to be an object");
...
fc::variant_object
configuration::retrieve_global_configuration(const fc::variant_object &obj) {
  fc::mutable_variant_object global_config;

  FC_ASSERT(obj[GLOBAL].is_object(),
            "Expected '${key}' configuration to be an object", ("key", GLOBAL));
...
fc::variant_object
configuration::retrieve_active_configuration(const fc::variant_object &obj,
  ...
  FC_ASSERT(obj[BASE].is_object(),
            "Expected '${key}' configuration to be an object", ("key", BASE));
...
size_t configuration::get_object_count(const boost::any &cfg) {
  ...
  FC_ASSERT(global_config.contains(OBJECT_COUNT),
            "Expected '${parent}' configuration to contain '${key}'",
            ("parent", GLOBAL)("key", OBJECT_COUNT));
...
bool configuration::gather_statistics(const boost::any &cfg) {
  ...
  FC_ASSERT(global_config.contains(STATISTICS),
            "Expected '${parent}' configuration to contain '${key}'",
            ("parent", GLOBAL)("key", STATISTICS));

  FC_ASSERT(global_config[STATISTICS].is_bool(),
            "Expected '${key}' to be a boolean value", ("key", STATISTICS));
...
::rocksdb::Options configuration::get_options(const boost::any &cfg,
                                              std::string type_name) {
  ...
  if (global_shared_cache == nullptr) {
    ...
    FC_ASSERT(global_config.contains(SHARED_CACHE),
              "Expected '${parent}' configuration to contain '${key}'",
              ("parent", GLOBAL)("key", SHARED_CACHE));

    FC_ASSERT(global_config[SHARED_CACHE].is_object(),
              "Expected '${key}' to be an object", ("key", SHARED_CACHE));

    auto &shared_cache_obj = global_config[SHARED_CACHE].get_object();

    FC_ASSERT(shared_cache_obj.contains(CAPACITY),
              "Expected '${parent}' configuration to contain '${key}'",
              ("parent", SHARED_CACHE)("key", CAPACITY));

    FC_ASSERT(shared_cache_obj[CAPACITY].is_string(),
              "Expected '${key}' to be a string representation of an unsigned "
              "integer",
              ("key", CAPACITY));
    ...
    if (shared_cache_obj.contains(NUM_SHARD_BITS)) {
      FC_ASSERT(shared_cache_obj[NUM_SHARD_BITS].is_uint64(),
                "Expected '${key}' to be an unsigned integer",
                ("key", NUM_SHARD_BITS));
  ...
  if (global_write_buffer_manager == nullptr) {
    ...
    FC_ASSERT(global_config.contains(WRITE_BUFFER_MANAGER),
              "Expected '${parent}' configuration to contain '${key}'",
              ("parent", GLOBAL)("key", WRITE_BUFFER_MANAGER));

    FC_ASSERT(global_config[WRITE_BUFFER_MANAGER].is_object(),
              "Expected '${key}' to be an object",
              ("key", WRITE_BUFFER_MANAGER));

    auto &write_buffer_mgr_obj =
        global_config[WRITE_BUFFER_MANAGER].get_object();

    FC_ASSERT(write_buffer_mgr_obj.contains(WRITE_BUFFER_SIZE),
              "Expected '${parent}' configuration to contain '${key}'",
              ("parent", WRITE_BUFFER_MANAGER)("key", WRITE_BUFFER_SIZE));

    FC_ASSERT(write_buffer_mgr_obj[WRITE_BUFFER_SIZE].is_string(),
              "Expected '${key}' to be a string representation of an unsigned "
              "integer",
    ...
    global_write_buffer_manager =
        std::make_shared<::rocksdb::WriteBufferManager>(write_buf_size,
                                                        global_shared_cache);
  }
  ...
  for (auto it = config.begin(); it != config.end(); ++it) {
    try {
      if (global_database_option_map.find(it->key()) !=
          global_database_option_map.end())
        global_database_option_map[it->key()](opts, it->value());
      else
        wlog("Encountered an unknown database configuration option: ${key}",
             ("key", it->key()));
    } catch (...) {
