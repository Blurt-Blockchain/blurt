...
uint64_t endian_reverse(uint64_t x) {
  ...
  return (x0 << 0x38) | (x1 << 0x30) | (x2 << 0x28) | (x3 << 0x20) |
         (x4 << 0x18) | (x5 << 0x10) | (x6 << 0x08) | (x7);
}
...
int main(int argc, char **argv, char **envp) {
  ...
  while (true) {
    ...
    if (h != fc::sha256()) {
      ...
      if (!ok) {
        ...
        std::cerr << "ref_log:" << std::hex << std::setw(8) << ref_log
                  << std::endl;
        std::cerr << "log(h) :" << std::hex << std::setw(8) << h.approx_log_32()
                  << std::endl;
        std::cerr << std::endl;
        ++errors;
      }
  ...
  std::cerr << "sha256_log_test checked " << cases << " cases, got " << errors
            << " errors" << std::endl;
  if (errors)
    return 1;
  return 0;
