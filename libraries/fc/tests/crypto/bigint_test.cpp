...
BOOST_AUTO_TEST_CASE(bigint_test_2)
{
    ...
    fc::bigint bi_accu2 = bi_accu;

    BOOST_CHECK( bi_accu2 && !bi_accu2.is_negative() && bi_accu2 != bi_1 );

    BOOST_CHECK( bi_3.exp( bi_accu2.log2() ) > bi_accu );

    fc::bigint big(1);
    big <<= 30; big += bi_17; big <<= 30; big++;
    big <<= 30; big -= bi_17; big >>= 5; big--;
    fc::bigint rest = bi_accu2 % big;
    BOOST_CHECK( (bi_accu2 - rest) / big == bi_accu2 / big );
    ...
    BOOST_CHECK_EQUAL( (std::string) bi_accu2, "12864938683278671740537145998360961546653259485195806" );
