...
static void interop_do(const fc::sha512 &data) { interop_do(data.data(), 64); }
...
int main(int argc, char **argv) {
  ...
  for (uint32_t i = 0; i < 3000; ++i) {
    try {
      ...
      fc::ecc::private_key priv1 =
          fc::ecc::private_key::generate_from_seed(h, h2);
