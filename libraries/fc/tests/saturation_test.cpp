...
template <typename Signed, typename BiggerSigned> int test_all() {
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (z0 != z1) {
        std::cout << "Addition does not work for " << x << ", " << y
                  << std::endl;
        return 1;
      }
      ...
      if (z0 != z1) {
        std::cout << "Subtraction does not work for " << x << ", " << y
                  << std::endl;
        return 1;
      }
...
int64_t sat_add_i64(int64_t a, int64_t b) { return fc::signed_sat_add(a, b); }

int64_t sat_sub_i64(int64_t a, int64_t b) { return fc::signed_sat_sub(a, b); }
