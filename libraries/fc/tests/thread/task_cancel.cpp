    ...
    fc::future<void> test_task;
    {
      ...
      test_task = fc::async(
          [&test_mutex]() {
            ...
            BOOST_TEST_MESSAGE(
                "--- In test_task, mutex locked, commencing sleep");
            for (int i = 0; i < 10; ++i)
              fc::usleep(fc::seconds(1));
      ...
      BOOST_TEST_MESSAGE(
          "Unlocking mutex locked from the main task so the test task will "
          "have the opportunity to lock it and be canceled");
    }
    fc::usleep(fc::seconds(3));
...
BOOST_AUTO_TEST_CASE(test_non_preemptable_assertion) {
  return; // this isn't a real test, because the thing it tries to test works by
          // asserting, not by throwing
...
BOOST_AUTO_TEST_CASE(cancel_an_sleeping_task) {
  ...
  try {
    ...
    BOOST_CHECK_MESSAGE(result != sleep_completed,
                        "sleep should have been canceled");
  } catch (fc::exception &e) {
    BOOST_TEST_MESSAGE("Caught exception from canceled task: " << e.what());
    BOOST_CHECK_MESSAGE(fc::time_point::now() - start_time < fc::seconds(4),
                        "Task was not canceled quickly");
  }
}
...
BOOST_AUTO_TEST_CASE(cancel_a_task_waiting_on_promise) {
  ...
  fc::future<task_result> task = fc::async(
      [promise_to_wait_on]() {
        BOOST_TEST_MESSAGE("Starting async task");
        try {
          promise_to_wait_on->wait_until(fc::time_point::now() +
                                         fc::seconds(5));
          return task_completed;
        } catch (const fc::canceled_exception &) {
          BOOST_TEST_MESSAGE(
              "Caught canceled_exception inside task-to-be-canceled");
          throw;
        } catch (const fc::timeout_exception &) {
          BOOST_TEST_MESSAGE(
              "Caught timeout_exception inside task-to-be-canceled");
          throw;
        } catch (const fc::exception &e) {
          BOOST_TEST_MESSAGE(
              "Caught unexpected exception inside task-to-be-canceled: "
              << e.to_detail_string());
          return task_aborted;
        }
      },
  ...
  try {
    task_result result = task.wait();
    BOOST_CHECK_MESSAGE(result != task_completed,
...
BOOST_AUTO_TEST_CASE(cleanup_cancelled_task) {
  std::shared_ptr<std::string> some_string(
      std::make_shared<std::string>("some string"));
  fc::future<void> task = fc::async(
      [some_string]() {
        BOOST_TEST_MESSAGE("Starting async task, bound string is "
                           << *some_string);
        try {
          fc::usleep(fc::seconds(5));
          BOOST_TEST_MESSAGE(
              "Finsihed usleep in async task, leaving the task's functor");
        } catch (...) {
          BOOST_TEST_MESSAGE(
              "Caught exception in async task, leaving the task's functor");
        }
      },
      "test_task");
  std::weak_ptr<std::string> weak_string_ptr(some_string);
  some_string.reset();
  BOOST_CHECK_MESSAGE(!weak_string_ptr.expired(),
                      "Weak pointer should still be valid because async task "
                      "should be holding the strong pointer");
  fc::usleep(fc::milliseconds(100));
  BOOST_TEST_MESSAGE("Canceling task");
  task.cancel("canceling to test if cancel works");
  try {
  ...
  //  BOOST_CHECK_MESSAGE(weak_string_ptr.expired(), "Weak pointer should now be
  //  invalid because async task should be done with it");
  task = fc::future<void>();
  BOOST_CHECK_MESSAGE(weak_string_ptr.expired(),
                      "Weak pointer should now be invalid because async task "
...
void simple_task() {
  task_execute_count++;
  simple_task_done =
      fc::schedule([]() { simple_task(); },
                   fc::time_point::now() + fc::seconds(3), "simple_task");
}
