...
public:
  ...
  T &operator*() const { return *get(); }
  operator bool() const { return get() != static_cast<T *>(0); }
  static void cleanup_function(void *obj) { delete static_cast<T *>(obj); }
  void reset(T *new_value = 0) {
    detail::set_thread_specific_data(slot, new_value, cleanup_function);
  }
};

template <typename T> class task_specific_ptr {
private:
  unsigned slot;
