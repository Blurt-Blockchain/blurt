...
void set_thread_specific_data(unsigned slot, void *new_value,
                              void (*cleanup)(void *));
unsigned get_next_unused_task_storage_slot();
void *get_task_specific_data(unsigned slot);
void set_task_specific_data(unsigned slot, void *new_value,
                            void (*cleanup)(void *));
} // namespace detail
...
public:
   ...
   *  Returns the current thread.
      Note: Creates fc::thread object (but not a boost thread) if no current
   thread assigned yet (this can happend if current() is called from the main
   thread of application or from an existing "unknown" boost thread). In such
   cases, thread_d doesn't have access boost::thread object.
   */
   ...
   *
   *  This method is helpful to figure out where your program is 'hung' by
   * listing every async operation (context) and what it is blocked on (future).
   *
  ...
  /**
   *  Calls function <code>f</code> in this thread and returns a future<T> that
   * can be used to wait on the result.
   *
  ...
  /**
   *  This method will cancel all pending tasks causing them to throw
   * cmt::error::thread_quit.
   *
   *  If the <i>current</i> thread is not <code>this</code> thread, then the
   * <i>current</i> thread will wait for <code>this</code> thread to exit.
   *
   ...
   *
   *  @todo make quit non-blocking of the calling thread by eliminating the call
   * to <code>boost::thread::join</code>
   */
...
private:
  ...
  friend void detail::set_thread_specific_data(unsigned slot, void *new_value,
                                               void (*cleanup)(void *));
  friend unsigned detail::get_next_unused_task_storage_slot();
  friend void *detail::get_task_specific_data(unsigned slot);
  friend void detail::set_task_specific_data(unsigned slot, void *new_value,
                                             void (*cleanup)(void *));
#ifndef NDEBUG
  ...
  friend int wait_any_until(std::vector<promise_base::ptr> &&v,
                            const time_point &tp);
...
template <typename Functor>
auto schedule(Functor &&f, const fc::time_point &t,
              const char *desc FC_TASK_NAME_DEFAULT_ARG,
              priority prio = priority()) -> fc::future<decltype(f())> {
 ...
 */
template <typename Functor>
auto sync_call(thread *t, Functor &&f,
               const char *desc FC_TASK_NAME_DEFAULT_ARG,
