...
template <typename T = void> class promise : virtual public promise_base {
public:
  ...
  void on_complete(CompletionHandler &&c) {
    _on_complete(new detail::completion_handler_impl<CompletionHandler, T>(
        fc::forward<CompletionHandler>(c)));
  }
...
template <> class promise<void> : virtual public promise_base {
public:
  ...
  void wait_until(const time_point &tp) { this->_wait_until(tp); }
  ...
  template <typename CompletionHandler>
  void on_complete(CompletionHandler &&c) {
    _on_complete(new detail::completion_handler_impl<CompletionHandler, void>(
        fc::forward<CompletionHandler>(c)));
  }
 ...
 *
 *  If you would like to use an asynchronous interface instead of the
 * synchronous 'wait' method you could specify a CompletionHandler which is a
 * method that takes two parameters, a const reference to the value and an
 * exception_ptr.  If the exception_ptr is set, the value reference is invalid
...
public:
  ...
  /// @post ready()
  /// @throws timeout
  void wait_until(const time_point &tp) { m_prom->wait_until(tp); }
