 ...
 *  threads.
 */
template <typename T = void_t> class wait_condition {
public:
  ...
  void wait(const microseconds &timeout = microseconds::maximum()) {
    ...
    { synchronized(_prom_lock) _promises.push_back(p); }
    p->wait(timeout);
  }
  ...
  void notify_one(const T &t = T()) {
    ...
    {
      synchronized(_prom_lock) if (_promises.size()) {
  ...
  void notify_all(const T &t = T()) {
    ...
    { synchronized(_prom_lock) std::swap(all, _promises); }
