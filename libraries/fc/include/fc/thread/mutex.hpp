 ...
 *  the socket members are thread safe, but the <code>write_message()</code>
 * operation is not rentrant because the context could yield while waiting for a
 * partial write to complete.
 *
 ...
 *
 *  As a result we now have to extend the normal discussion on thread-safe vs
 * reentrant.
 *
 *  - prempt-thread-safe : the code may be called by multiple os threads at the
 * same time.
 *  - coop-thread-safe   : the code may be called by multiple contexts within
 * the same thread.
 *  - thread-unsafe      : the code may only be called by one context at a time
 * for a set of data.
 *
 *  In the example above, before we added the mutex the code was thread-unsafe
 *  After we added the mutex the code became coop-thread-safe, and potentially
 * prempt-thread-safe
 *
 *  To be preempt-thread-safe any operations must be atomic or protected by a
 * lock because the OS could switch you out between any two instructions.
 *
 *  To be coop-thread-safe all operations are 'atomic' unless they span a
 * 'yield'.  If they span a yield (such as writing parts of a message), then a
 * mutex is required.
 *
 */
class mutex {
