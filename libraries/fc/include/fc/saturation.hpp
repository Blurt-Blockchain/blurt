...
template <typename IntType> struct corresponding_unsigned;

template <> struct corresponding_unsigned<int8_t> { typedef uint8_t type; };

template <> struct corresponding_unsigned<int16_t> { typedef uint16_t type; };

template <> struct corresponding_unsigned<int32_t> { typedef uint32_t type; };

template <> struct corresponding_unsigned<int64_t> { typedef uint64_t type; };

template <typename Signed> Signed signed_sat_add(Signed x, Signed y) {
...
template <typename Signed> Signed signed_sat_sub(Signed x, Signed y) {
  typedef typename corresponding_unsigned<Signed>::type u;
  u ux(x), uy(y);
  u res(ux - uy);
