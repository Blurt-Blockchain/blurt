...
enum canonical_signature_type { non_canonical, bip_0062, fc_canonical };
...
public:
  ...
  public_key(const compact_signature &c, const fc::sha256 &digest,
             canonical_signature_type canon_type = fc_canonical);
...
public:
   ...
   * deterministic manner relative to an initial seed.   A public_key created
   * from the seed can be multiplied by the offset to calculate the new public
   * key without having to know the private key.
   */
  static private_key
  ...
  compact_signature
  sign_compact(const fc::sha256 &digest,
               canonical_signature_type canon_type = fc_canonical) const;
