 ...
 * exceptions are wrapped in an fc::error_report and the additional helper
 * method fc::path::windows_string(), can be used to calculate paths intended
 * for systems different than the host.
 *
...
public:
   ...
   * displaying to users.  On other platforms, it does the same as
   * generic_string()
   */
 ...
 * on OS X, and APPDATA on windows
 */
const path &app_path();
...
// mac, or it may need slight tweaks or extra includes.  It's disabled now to
