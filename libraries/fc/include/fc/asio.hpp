...
template <typename C> struct non_blocking {
...
template <> struct non_blocking<boost::asio::windows::stream_handle> {
 ...
 *
 * This IO service is automatically running in its own thread to service
 * asynchronous requests without blocking any other threads.
 */
...
future<size_t> read_some(AsyncReadStream &s, const MutableBufferSequence &buf) {
  promise<size_t>::ptr completion_promise(
      new promise<size_t>("fc::asio::async_read_some"));
...
template <typename AsyncReadStream>
future<size_t> read_some(AsyncReadStream &s,
                         const std::shared_ptr<char> &buffer, size_t length,
                         size_t offset) {
  promise<size_t>::ptr completion_promise(
      new promise<size_t>("fc::asio::async_read_some"));
  s.async_read_some(
      boost::asio::buffer(buffer.get() + offset, length),
...
template <typename AsyncReadStream>
void async_read_some(AsyncReadStream &s, char *buffer, size_t length,
                     promise<size_t>::ptr completion_promise) {
...
future<size_t> write_some(AsyncWriteStream &s, const char *buffer,
  ...
  s.async_write_some(boost::asio::buffer(buffer + offset, length),
                     detail::read_write_handler(p));
  return p; //->wait();
}

template <typename AsyncWriteStream>
future<size_t> write_some(AsyncWriteStream &s,
                          const std::shared_ptr<const char> &buffer,
                          size_t length, size_t offset) {
  promise<size_t>::ptr p(new promise<size_t>("fc::asio::write_some"));
...
template <typename AsyncWriteStream>
void async_write_some(AsyncWriteStream &s,
                      const std::shared_ptr<const char> &buffer, size_t length,
...
std::vector<endpoint> resolve(const std::string &hostname,
...
void accept(AcceptorType &acc, SocketType &sock) {
  // promise<boost::system::error_code>::ptr p( new
  // promise<boost::system::error_code>("fc::asio::tcp::accept") );
...
template <typename AsyncReadStream> class istream : public virtual fc::istream {
public:
