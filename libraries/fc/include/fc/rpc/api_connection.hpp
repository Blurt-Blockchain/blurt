 ...
 * api<T> of the correct type in api_visitor::from_variant().  This binding [1]
 * needs a reference to the api_connection, which is made available to
 * from_variant() as a parameter.
 *
 * However, in the case of a remote method which returns api_base which can
 * subsequently be cast by the caller with as<T>, we need to keep track of the
 * connection because the binding is done later (when the client code actually
 * calls as<T>).
 *
...
public:
  ...
  virtual variant send_call(string api_name, string method_name,
                            variants args = variants()) = 0;
...
private:
  ...
  struct api_visitor {
    ...
    api_visitor(uint32_t api_id, string &api_name,
                std::shared_ptr<fc::api_connection> con)
    ...
    void operator()(const char *name,
      ...
      memb = [con, api_id, name, api_name](Args... args) {
        variant var_result;
        if (api_name)
          var_result = con->send_call(*api_name, name,
                                      {convert_callbacks(con, args)...});
        else
          var_result =
 ...
 * being able to call register_api() on an api<T> through its base class
 * api_base.  But register_api() must know the template parameters!
 *
...
template <typename Signature>
template <typename... Args>
typename callback_functor<Signature>::result_type
callback_functor<Signature>::operator()(Args... args) const {
