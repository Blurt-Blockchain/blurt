...
public:
  ...
  template <typename Functor> connection_id_type connect(Functor &&f) {
...
#ifdef WIN32
  template <typename Arg> void emit(Arg &&arg) {
  ...
  template <typename Arg, typename Arg2> void emit(Arg &&arg, Arg2 &&arg2) {
    list_type handlers = getHandlers();
    for (size_t i = 0; i < handlers.size(); ++i) {
