...
public:
   ...
   *  (via agent or explicitly-set key), empty password, then
   * keyboard-interactive
   */
  void connect(const fc::string &user, const fc::string &host,
               uint16_t port = 22);

  /**
   *  Connect, specifying a password to be used for password authentication
   */
  void connect(const fc::string &user, const fc::string &pass,
               const fc::string &host, uint16_t port = 22);
   ...
   *  @param pty_type - whether or not to request a PTY when executing this
  process, this is necessary
   *          for interactive (non-buffered) IO with the remote process, if left
  empty no pty will be
   *          requested
   *
   *  @note Processes launched in this manner will fully buffer stdin and stdout
  regardless of whether
   *     the process calls flush().  If you need unbuffered (streaming,
  realtime) access to standard
   ...
   *         The callback takes two parameters, bytes sent and file size.  To
   * continue the transfer, the callback should return true.  To cancel the
   * callback should return false.
   */
  void scp_send(
      const fc::path &local_path, const fc::path &remote_path,
   ...
   *
   *  If remote_path ends in '/' then a new directory at
   * <code>remote_path/local_dir.filename()</code> will be created, otherwise
   * <code>local_dir / *</code> will be copied to <code>remote_path / *</code>
   *
   *  Progress will be reported as total bytes transferred for all files.
   */
  void scp_send_dir(
      const fc::path &local_dir, const fc::path &remote_path,
      std::function<bool(uint64_t, uint64_t)> progress =
          [](uint64_t, uint64_t) { return true; });
   ...
   */
  void mkdir(const fc::path &remote_dir,
             int mode = owner_read | owner_write | owner_exec);
   ...
   *  @post remote_dir exists.
   */
  void create_directories(const fc::path &remote_dir,
                          int mode = owner_read | owner_write | owner_exec);

  /**
   * Sets whether the remote system is believed to be a Windows box (by default,
   * it's assumed to be running UNIX.  This alters how command-line arguments
   * are quoted and possibly how filenames are altered when copying files
   */
