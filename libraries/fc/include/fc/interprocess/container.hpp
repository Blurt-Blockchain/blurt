...
// so it uses the reflection/serialization for flat_map and does not have any
// here BUT we add typename for flat_map so it works with allocators
...
struct get_typename<bip::deque<E, Allocator>> {
  static const char *name() {
    static std::string n =
        std::string("bip::deque<") + get_typename<E>::name() + ">";
    return n.c_str();
  }
};
...
struct get_typename<bip::map<E, Allocator>> {
  static const char *name() {
    static std::string n =
        std::string("bip::map<") + get_typename<E>::name() + ">";
    return n.c_str();
  }
};
...
struct get_typename<bip::set<E, Allocator>> {
  static const char *name() {
    static std::string n =
        std::string("bip::set<") + get_typename<E>::name() + ">";
    return n.c_str();
  }
};
...
struct get_typename<bip::vector<E, Allocator>> {
  static const char *name() {
    static std::string n =
        std::string("bip::vector<") + get_typename<E>::name() + ">";
    return n.c_str();
  }
};
