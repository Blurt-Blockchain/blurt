...
template <typename Stream, typename... T>
inline void unpack(Stream &s, std::multiset<T...> &value, uint32_t depth = 0);
template <typename Stream, typename T>
...
template <typename Stream, typename T, size_t N>
inline void unpack(Stream &s, fc::int_array<T, N> &v, uint32_t depth = 0);
...
template <typename T>
inline T unpack_from_vector(const std::vector<char> &s, uint32_t depth = 0);
template <typename T>
inline T unpack_from_char_array(const char *d, uint32_t s, uint32_t depth = 0);
template <typename T>
