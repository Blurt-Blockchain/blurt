...
inline void unpack(Stream &s, fc::int_array<T, N> &v, uint32_t) {
  ...
  FC_RETHROW_EXCEPTIONS(warn, "fc::int_array<type,length>",
                        ("type", fc::get_typename<T>::name())("length", N))
}
...
template <typename Stream>
inline void unpack(Stream &s, fc::string &v, uint32_t depth) {
  depth++;
  std::vector<char> tmp;
  fc::raw::unpack(s, tmp, depth);
  if (tmp.size())
...
inline void unpack(Stream &s, T &v, uint32_t depth) {
  try {
    ...
    fc::raw::detail::if_reflected<
        typename fc::reflector<T>::is_defined>::unpack(s, v, depth);
  }
...
template <typename Stream, int Index, typename Tuple> struct tuple_packer {
  static void tuple_pack(Stream &s, const Tuple &t) {
...
void pack(Stream &s, const boost::tuples::tuple<Args...> &var) {
  tuple_packer<Stream,
               boost::tuples::length<boost::tuples::tuple<Args...>>::value - 1,
               boost::tuples::tuple<Args...>>::tuple_pack(s, var);
}

template <typename Stream, typename... Args>
void unpack(Stream &s, boost::tuples::tuple<Args...> &var) {
  tuple_packer<Stream,
               boost::tuples::length<boost::tuples::tuple<Args...>>::value - 1,
               boost::tuples::tuple<Args...>>::tuple_unpack(s, var);
}
