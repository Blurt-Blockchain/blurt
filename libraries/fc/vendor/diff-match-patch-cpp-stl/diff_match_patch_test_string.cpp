...
struct wastring : wstring // The same as wstring, but can be constructed from
                          // char* (to use ASCII strings in the test functions)
{
  wastring() {}
  ...
  wastring operator+=(value_type c) {
    append(1, c);
    return *this;
  }
  wastring operator+=(const wastring &s) {
    append(s);
    return *this;
  }
  wastring operator+=(const char *s) {
    append(s, s + strlen(s));
    return *this;
  }
...
inline wastring operator+(const wastring &s, const char *p) {
  return s + wastring(p);
}
inline wastring operator+(const char *p, const wastring &s) {
  return wastring(p) + s;
}

ostream &operator<<(ostream &o, const wastring &s) {
  o << setfill('0');
  for (wastring::const_pointer p = s.c_str(), end = p + s.length(); p != end;
       ++p) {
    if (*p >= 0x80)
...
public:
  ...
  void run_all_tests() {
    ...
    cout << "Total time: " << int((clock() - t) * 1000 / CLOCKS_PER_SEC)
         << " ms" << endl;
  }
  ...
  void testDiffCommonOverlap() {
    ...
    assertEquals("diff_commonOverlap: Unicode.", 0,
                 dmp.diff_commonOverlap("fi", "\xEF\xAC\x81i"));
  }
  ...
  void testDiffHalfmatch() {
    ...
    assertEquals("diff_halfMatch: Non-optimal halfmatch.",
                 split("qHillo,w,x,Hulloy,HelloHe", ','),
  ...
  void testDiffLinesToChars() {
    ...
    assertEquals("diff_linesToChars:", "\1\2\1", "\2\1\2", tmpVector, text1,
                 text2, resVector);
    ...
    tmpVector[1] = LinePtr(tmpVector.text1.c_str(), tmpVector.text1.length());
    tmpVector[2] =
        LinePtr(tmpVector.text2.c_str(), tmpVector.text2.length() - 2);
    tmpVector[3] = LinePtr(tmpVector.text2.c_str() + 6, 2);
    text1.clear(), text2 = "alpha\r\nbeta\r\n\r\n\r\n";
    dmp.diff_linesToChars(text1, text2, resVector);
    assertEquals("diff_linesToChars:", string_t(), "\1\2\3\3", tmpVector, text1,
                 text2, resVector);
    ...
    resVector.clear();
    dmp.diff_linesToChars(text1, text2, resVector);
    assertEquals("diff_linesToChars:", "\1", "\2", tmpVector, text1, text2,
                 resVector);
    ...
    assertEquals("diff_linesToChars: More than 256 (setup).", n + 1,
                 tmpVector.size());
    ...
    resVector.clear();
    dmp.diff_linesToChars(text1, text2, resVector);
    assertEquals("diff_linesToChars:", chars, "", tmpVector, text1, text2,
                 resVector);
  }
  ...
  void testDiffCharsToLines() {
    ...
    assertEquals("diff_charsToLines: More than 256.",
                 diffList(Diff(DELETE, tmpVector.text1)), diffs);
  }
  ...
  void testDiffCleanupMerge() {
    ...
    diffs =
        diffList(Diff(DELETE, "a"), Diff(INSERT, "abc"), Diff(DELETE, "dc"));
    dmp.diff_cleanupMerge(diffs);
    assertEquals("diff_cleanupMerge: Prefix and suffix detection.",
                 diffList(Diff(EQUAL, "a"), Diff(DELETE, "d"),
  ...
  void testDiffCleanupSemantic() {
    ...
    assertEquals("diff_cleanupSemantic: Two overlap eliminations.",
                 diffList(Diff(DELETE, "abcd"), Diff(EQUAL, "1212"),
                          Diff(INSERT, "efghi"), Diff(EQUAL, "----"),
                          Diff(DELETE, "A"), Diff(EQUAL, "3"),
                          Diff(INSERT, "BC")),
                 diffs);
  }
  ...
  void testDiffCleanupEfficiency() {
    ...
    assertEquals("diff_cleanupEfficiency: No elimination.",
                 diffList(Diff(DELETE, "ab"), Diff(INSERT, "12"),
                          Diff(EQUAL, "wxyz"), Diff(DELETE, "cd"),
                          Diff(INSERT, "34")),
                 diffs);
  ...
  void testDiffDelta() {
    ...
    //    diffs = diffList(Diff(EQUAL, string_t("\u0680 \000 \t %", 7)),
    //    Diff(DELETE, string_t("\u0681 \001 \n ^", 7)), Diff(INSERT,
    //    string_t("\u0682 \002 \\ |", 7))); text1 = dmp.diff_text1(diffs);
    //    assertEquals("diff_text1: Unicode text.", string_t(L"\u0680 \000 \t
    //    %\u0681 \001 \n ^", 14), text1);
  ...
  void testDiffXIndex() {
    ...
    diffs =
        diffList(Diff(EQUAL, "a"), Diff(DELETE, "1234"), Diff(EQUAL, "xyz"));
  ...
  void testDiffBisect() {
    ...
    assertEquals("diff_bisect: Normal.", diffs,
                 dmp.diff_bisect(a, b, numeric_limits<clock_t>::max()));
  ...
  void testDiffMain() {
    ...
    //    diffs = diffList(Diff(DELETE, "a"), Diff(INSERT, L"\u0680"),
    //    Diff(EQUAL, "x"), Diff(DELETE, "\t"), Diff(INSERT, string_t("\000",
    //    1))); assertEquals("diff_main: Simple case #3.", diffs,
    //    dmp.diff_main("ax\t", string_t(L"\u0680x\000", 3), false));
    ...
    string_t b = "I am the very model of a modern major general,\nI've "
                 ...
                 "kings of England, and I quote the fights historical,\nFrom "
    ...
    b = "abcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghijabcdefghij"
    ...
    assertEquals("diff_main: Overlap line-mode.", texts_textmode,
                 texts_linemode);

    // Test null inputs -- not needed because nulls can't be passed in
    // string_t&.
  }
  ...
  void testPatchFromText() {
    ...
    assertEquals("patch_fromText: #1.", strp,
                 dmp.patch_fromText(strp).front().toString());

    assertEquals(
        "patch_fromText: #2.", "@@ -1 +1 @@\n-a\n+b\n",
  ...
  void testPatchSplitMax() {
    ...
    patches = dmp.patch_make("abcdef1234567890123456789012345678901234567890123"
                             "456789012345678901234567890uvwxyz",
                             "abcdefuvwxyz");
    string_t oldToText = dmp.patch_toText(patches);
    dmp.patch_splitMax(patches);
    assertEquals("patch_splitMax: #2.", oldToText, dmp.patch_toText(patches));

    patches = dmp.patch_make("1234567890123456789012345678901234567890123456789"
    ...
    patches = dmp.patch_make("abcdefghij , h : 0 , t : 1 abcdefghij , h : 0 , "
                             "t : 1 abcdefghij , h : 0 , t : 1",
                             "abcdefghij , h : 1 , t : 1 abcdefghij , h : 1 , "
                             "t : 1 abcdefghij , h : 0 , t : 1");
    dmp.patch_splitMax(patches);
    assertEquals(
        "patch_splitMax: #4.",
  ...
  void testPatchApply() {
    ...
    Patches patches;
    patches = dmp.patch_make("", "");
    pair<string_t, vector<bool>> results =
    ...
    patches = dmp.patch_make("x123456789012345678901234567890123456789012345678"
                             "9012345678901234567890y",
                             "xabcy");
    results =
        dmp.patch_apply(patches, "x123456789012345678901234567890-----+++++++++"
    ...
    resultStr = results.first + "\t" + (boolArray[0] ? "true" : "false") +
                "\t" + (boolArray[1] ? "true" : "false");
    assertEquals("patch_apply: Big delete, large change 2.",
...
private:
  ...
  void assertEquals(const char *strCase, const string_t &s1,
                    const string_t &s2) {
    if (s1 != s2) {
      cerr << strCase << " FAIL\nExpected: " << s1 << "\nActual: " << s2
           << endl;
  ...
  void assertEquals(const char *strCase, const Diffs &list1,
                    const Diffs &list2) {
    bool fail = false;
    if (list1.size() == list2.size()) {
      for (Diffs::const_iterator d1 = list1.begin(), d2 = list2.begin();
           d1 != list1.end(); ++d1, ++d2) {
        if (*d1 != *d2) {
          fail = true;
          break;
  ...
  void assertEquals(const char *strCase, const string_t &text1_1,
                    const string_t &text2_1, const Lines &lines1,
                    const string_t &text1_2, const string_t &text2_2,
                    const Lines &lines2) {
    bool fail = false;
    if (text1_1 != text1_2 || text2_1 != text2_2 ||
        lines1.size() != lines2.size())
      fail = true;
    else
      for (Lines::const_iterator i = lines1.begin(), j = lines2.begin();
           i != lines1.end(); ++i, ++j)
        if (string_t((*i).first, (*i).second) !=
            string_t((*j).first, (*j).second)) {
          fail = true;
          break;
        }
    ...
    if (fail) {
      ...
      bool first = true;
      for (Lines::const_iterator i = lines1.begin() + 1; i != lines1.end();
           ++i) {
        if (!first)
          listString1 += "\", \"";
      ...
      first = true;
      for (Lines::const_iterator j = lines2.begin() + 1; j != lines2.end();
           ++j) {
        if (!first)
          listString2 += "\", \"";
  ...
  void assertEquals(const char *strCase, const map<char_t, int> &m1,
                    const map<char_t, int> &m2) {
    map<char_t, int>::const_iterator i1 = m1.begin(), i2 = m2.begin();

    while (i1 != m1.end() && i2 != m2.end()) {
      if ((*i1).first != (*i2).first || (*i1).second != (*i2).second) {
        cerr << strCase << " FAIL\nExpected: (" << char((*i1).first) << ", "
             << (*i1).second << ")\nActual: (" << char((*i2).first) << ", "
             << (*i2).second << ')' << endl;
        throw strCase;
      }
      ++i1, ++i2;
  ...
  void assertEquals(const char *strCase, const Strings &list1,
                    const Strings &list2) {
    if (list1 != list2) {
      cerr << strCase << " FAIL\nExpected: " << join(list1, ',')
  ...
  static Strings diff_rebuildtexts(Diffs diffs) {
    ...
    for (Diffs::const_iterator myDiff = diffs.begin(); myDiff != diffs.end();
         ++myDiff) {
  ...
  static Strings split(const string_t &str, char delim) {
    ...
    for (string_t::const_pointer token = str.c_str(),
                                 end = token + str.length(), p;
         token < end; token += token_len + 1) {
      for (p = token; p != end; ++p)
        if (*p == delim)
          break;
