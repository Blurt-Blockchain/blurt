...
public:
  ...
  // 5 min), and if _request_interval_sec (currently 1 hour) has passed since
  // the last successful update, it sends a new request
  void request_time_task() {
    assert(_ntp_thread.is_current());
    if (_last_valid_ntp_reply_received_time <=
        fc::time_point::now() - fc::seconds(_request_interval_sec - 5))
      request_now();
    if (!_request_time_task_done.valid() || !_request_time_task_done.canceled())
      _request_time_task_done =
          schedule([=]() { request_time_task(); },
                   fc::time_point::now() +
                       fc::seconds(_retry_failed_request_interval_sec),
                   "request_time_task");
  } // request_loop

  void start_read_loop() {
    _read_loop_done =
        _ntp_thread.async([this]() { read_loop(); }, "ntp_read_loop");
  }
  ...
  void read_loop() {
    ...
    // outer while to restart read-loop if exception is thrown while waiting to
    // receive on socket.
    while (!_read_loop_done.canceled()) {
      // if you start the read while loop here, the recieve_from call will throw
      // "invalid argument" on win32, so instead we start the loop after making
      // our first request
      try {
        _sock.open();
        ...
        while (!_read_loop_done.canceled()) {
          ...
          fc::time_point origin_time =
              ntp_timestamp_to_fc_time_point(recv_buf[3]);
          ...
          // if the reply we just received has occurred more than a second after
          // our last time request (it was more than a second ago since our last
          // request)
          if (round_trip_delay > fc::microseconds(300000)) {
            wlog("received stale ntp reply requested at ${request_time}, send "
                 "a new time request",
          ...
          [=]() {
            try {
              my->_request_time_task_done.cancel_and_wait(
                  "ntp object is destructing");
            } catch (const fc::exception &e) {
              wlog("Exception thrown while shutting down NTP's "
                   "request_time_task, ignoring: ${e}",
                   ("e", e));
            } catch (...) {
              wlog("Exception thrown while shutting down NTP's "
                   "request_time_task, ignoring");
            }

            try {
              my->_read_loop_done.cancel_and_wait("ntp object is destructing");
            } catch (const fc::exception &e) {
              wlog("Exception thrown while shutting down NTP's read_loop, "
                   "ignoring: ${e}",
                   ("e", e));
            } catch (...) {
              wlog("Exception thrown while shutting down NTP's read_loop, "
