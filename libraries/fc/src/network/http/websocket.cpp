...
public:
  ...
  websocket_tls_client_impl(const std::string &ca_filename)
    ...
    // object might be destroyed by the caller by the time tls_init_handler() is
    // called.  According to [1], capture-by-value results in the desired
    // behavior (i.e. creation of a copy which is stored in the closure) on
    // standards compliant compilers, but some compilers on some optimization
    // levels are buggy and are not standards compliant in this situation. Also,
    // keep in mind this is the opinion of a single forum poster and might be
    // wrong.
    //
    // To be safe, the following line explicitly creates a non-reference string
    // which is captured by value, which should have the correct behavior on all
    // compilers.
    //
...
websocket_connection_ptr websocket_client::connect(const std::string &uri) {
  try {
    ...
    my->_uri = uri;
    my->_connected =
        fc::promise<void>::ptr(new fc::promise<void>("websocket::connect"));
    ...
    my->_client.set_open_handler([=](websocketpp::connection_hdl hdl) {
      ...
      my->_closed =
          fc::promise<void>::ptr(new fc::promise<void>("websocket::closed"));
