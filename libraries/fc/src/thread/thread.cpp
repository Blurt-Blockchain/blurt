...
namespace fc {
const char *thread_name() { return thread::current().name().c_str(); }
void *thread_ptr() { return &thread::current(); }
...
thread::thread(const std::string &name) {
  ...
  boost::thread *t = new boost::thread([this, p, name]() {
    try {
      set_thread_name(
          name.c_str()); // set thread's name for the debugger to display
      ...
      wlog("unhandled exception");
      p->set_exception(std::make_shared<unhandled_exception>(FC_LOG_MESSAGE(
          warn, "unhandled exception: ${diagnostic}",
          ("diagnostic", boost::current_exception_diagnostic_information()))));
      // assert( !"unhandled exception" );
      // elog( "Caught unhandled exception %s",
  ...
  my->boost_thread = t;
  my->name = name;
  // wlog("name:${n} tid:${tid}", ("n", name)("tid",
  // (uintptr_t)my->boost_thread->native_handle()) );
}
thread::thread(thread_d *) { my = new thread_d(*this); }
...
void thread::set_name(const fc::string &n) {
  ...
  my->name = n;
  set_thread_name(
      my->name.c_str()); // set thread's name for the debugger to display
}
...
void thread::quit(fc::promise<void> *quitDone /*= nullptr*/) {
  ...
  // If we have and know our attached boost thread, wait for it to finish, then
  // return.
  if (&current() != this) {
    async([=]() { quit(quitDone); }, "thread::quit"); //.wait();
    if (my->boost_thread) {
      // wlog("destroying boost thread
      // ${tid}",("tid",(uintptr_t)my->boost_thread->native_handle()));
  ...
  while (my->blocked) {
    ...
    while (cur) {
      ...
      cur->set_exception_on_blocking_promises(
          std::make_shared<canceled_exception>(
...
bool thread::is_running() const { return !my->done; }
...
int thread::wait_any_until(std::vector<promise_base::ptr> &&p,
  ...
  if (timeout != time_point::maximum()) {
    ...
    std::push_heap(my->sleep_pqueue.begin(), my->sleep_pqueue.end(),
                   sleep_priority_less());
  }
...
void thread::async_task(task_base *t, const priority &p, const time_point &tp) {
  ...
  do {
    t->_next = stale_head;
  } while (!my->task_in_queue.compare_exchange_weak(
      stale_head, t, boost::memory_order_release));

  // Because only one thread can post the 'first task', only that thread will
  // attempt to aquire the lock and therefore there should be no contention on
  // this lock except when *this thread is about to block on a wait condition.
...
void sleep_until(const time_point &tp) { thread::current().sleep_until(tp); }

void exec() { return thread::current().exec(); }

int wait_any(std::vector<promise_base::ptr> &&v,
             const microseconds &timeout_us) {
  return thread::current().wait_any_until(fc::move(v),
                                          time_point::now() + timeout_us);
}
...
void thread::wait_until(promise_base::ptr &&p, const time_point &timeout) {
  ...
  // slog( "                                 %1% blocking on %2%", my->current,
  // p.get() );
    ...
    if (cur_blocked->try_unblock(p.get())) {
      ...
      for (uint32_t i = 0; i < my->sleep_pqueue.size(); ++i) {
        if (my->sleep_pqueue[i] == cur_blocked) {
          ...
          std::make_heap(my->sleep_pqueue.begin(), my->sleep_pqueue.end(),
                         sleep_priority_less());
          break;
        }
      }
...
bool thread::is_current() const { return this == &current(); }

void thread::notify_task_has_been_canceled() {
  async([=]() { my->notify_task_has_been_canceled(); },
        "notify_task_has_been_canceled", priority::max());
}

void thread::unblock(fc::context *c) { my->unblock(c); }
...
void set_unhandled_structured_exception_filter(
    unhandled_exception_filter_type new_filter) {
