...
promise_base::promise_base(const char *desc)
    : _ready(false), _blocked_thread(nullptr), _blocked_fiber_count(0),
      _timeout(time_point::maximum()), _canceled(false),
#ifndef NDEBUG
      _cancellation_reason(nullptr),
#endif
      _desc(desc), _compl(nullptr) {
}

const char *promise_base::get_desc() const { return _desc; }
...
bool promise_base::ready() const { return _ready; }
bool promise_base::error() const {
  { synchronized(_spin_yield) return _exceptp != nullptr; }
}
...
void promise_base::_wait_until(const time_point &timeout_us) {
  {
    synchronized(_spin_yield) if (_ready) {
      if (_exceptp)
...
void promise_base::_enqueue_thread() {
  ...
  assert(!_blocked_thread || _blocked_thread == &thread::current());
  _blocked_thread = &thread::current();
}
void promise_base::_dequeue_thread() {
  synchronized(_spin_yield) if (!--_blocked_fiber_count) _blocked_thread =
      nullptr;
}
void promise_base::_notify() {
  ...
  { synchronized(_spin_yield) blocked_thread = _blocked_thread; }
  if (blocked_thread)
...
void promise_base::_set_value(const void *s) {
  ...
  {
    synchronized(
        _spin_yield) if (_ready) // don't allow promise to be set more than once
        return;
    _ready = true;
  }
  _notify();
...
void promise_base::_on_complete(detail::completion_handler *c) {
  {
    synchronized(_spin_yield) delete _compl;
    _compl = c;
  }
}
} // namespace fc
