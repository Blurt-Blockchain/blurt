...
struct context {
  ...
  context(context_fn sf, stack_allocator &alloc, fc::thread *t)
      : caller_context(0), stack_alloc(&alloc), next_blocked(0),
        next_blocked_mutex(0), next(0), ctx_thread(t), canceled(false),
#ifndef NDEBUG
        cancellation_reason(nullptr),
#endif
        complete(false), cur_task(0), context_posted_num(0) {
...
#endif
        caller_context(0), stack_alloc(0), next_blocked(0),
        next_blocked_mutex(0), next(0), ctx_thread(t), canceled(false),
#ifndef NDEBUG
        cancellation_reason(nullptr),
#endif
  ...
  void clear_blocking_promises() { blocking_prom.clear(); }
  ...
  // time_point                   ready_time; // time that this context was put
  // on ready queue
  ...
  bool complete;
  task_base *cur_task;
  uint64_t context_posted_num; // serial number set each tiem the context is
