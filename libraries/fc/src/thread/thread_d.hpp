...
public:
  ...
  thread_d(fc::thread &s)
      : self(s), boost_thread(0), task_in_queue(0), next_posted_num(1),
        done(false), current(0), pt_head(0), blocked(0),
  ...
  std::vector<task_base *>
      task_pqueue; // heap of tasks that have never started, ordered by proirity
                   // & scheduling time
  uint64_t next_posted_num; // each task or context gets assigned a number in
                            // the order it is ready to execute, tracked here
  std::vector<task_base *>
      task_sch_queue; // heap of tasks that have never started but are scheduled
                      // for a time in the future, ordered by the time they
                      // should be run
  std::vector<fc::context *>
      sleep_pqueue; // heap of running tasks that have sleeped, ordered by the
                    // time they should resume
  std::vector<fc::context *>
      free_list; // list of unused contexts that are ready for deletion
  ...
  std::vector<fc::context *>
      ready_heap; // priority heap of contexts that are ready to run

  fc::context *blocked; // linked list of contexts (using 'next_blocked')
                        // blocked on promises via wait()
  ...
  struct task_priority_less {
    bool operator()(const task_base *a, const task_base *b) const {
      return a->_prio.value < b->_prio.value
                 ? true
                 : (a->_prio.value > b->_prio.value
                        ? false
                        : a->_posted_num > b->_posted_num);
    }
    bool operator()(const task_base *a, const context *b) const {
      return a->_prio.value < b->prio.value
                 ? true
                 : (a->_prio.value > b->prio.value
                        ? false
                        : a->_posted_num > b->context_posted_num);
    }
    bool operator()(const context *a, const task_base *b) const {
      return a->prio.value < b->_prio.value
                 ? true
                 : (a->prio.value > b->_prio.value
                        ? false
  ...
  struct task_when_less {
    bool operator()(task_base *a, task_base *b) { return a->_when > b->_when; }
  };
  ...
  void enqueue(task_base *t) {
    ...
    while (cur) {
      if (cur->_when > now) {
        ...
        std::push_heap(task_sch_queue.begin(), task_sch_queue.end(),
                       task_when_less());
      } else {
        cur->_posted_num = next_posted_num - (++tasks_posted);
        task_pqueue.push_back(cur);
        std::push_heap(task_pqueue.begin(), task_pqueue.end(),
                       task_priority_less());
  ...
  void move_newly_scheduled_tasks_to_task_pqueue() {
    ...
    task_base *pending_list =
        task_in_queue.exchange(0, boost::memory_order_seq_cst);
    if (pending_list)
      enqueue(pending_list);

    // second, walk through task_sch_queue and move any scheduled tasks that are
    // now able to run (because their scheduled time has arrived) to task_pqueue

    while (!task_sch_queue.empty() &&
           task_sch_queue.front()->_when <= time_point::now()) {
      task_base *ready_task = task_sch_queue.front();
      std::pop_heap(task_sch_queue.begin(), task_sch_queue.end(),
                    task_when_less());
...
#else
      FC_THROW_EXCEPTION(canceled_exception, "cancellation reason: ${reason}",
  ...
  bool start_next_fiber(bool reschedule = false) {
    /* If this assert fires, it means you are executing an operation that is
     * causing the current task to yield, but there is a
     * ASSERT_TASK_NOT_PREEMPTED() in effect (somewhere up the stack) */
    assert(non_preemptable_scope_count == 0);

    /* If this assert fires, it means you are causing the current task to yield
     * while in the middle of handling an exception.  The boost::context
     * library's behavior is not well-defined in this case, and this has the
     * potential to corrupt the
    ...
    if (!ready_heap.empty()) {
      ...
      // fc_dlog( logger::get("fc_context"), "from ${from} to ${to}", ( "from",
      // int64_t(prev) )( "to", int64_t(next) ) );
  ...
  static void start_process_tasks(intptr_t my) {
    ...
    try {
      ...
      elog("fiber ${name} exited with uncaught exception: ${e}",
           ("e", fc::except_str())("name", self->name));
      // assert( !"fiber exited with uncaught exception" );
      // TODO replace errror           fc::cerr<<"fiber exited with uncaught
      // exception:\n "<<
  ...
  void process_tasks() {
    while (!done || blocked) {
      ...
      if (!task_pqueue.empty()) {
        ...
        // if we made it here, either there's no ready context, or the ready
        // context is scheduled after the ready task, so we should run the task
        // first
        run_next_task();
        continue;
      }
      ...
      { // lock scope
        ...
        if (timeout_time == time_point::maximum())
          ...
          // there may be tasks that have been canceled we should filter them
          // out now rather than waiting...

          /* This bit is kind of sloppy -- this wait was originally implemented
           * as a wait with respect to boost::chrono::system_clock.  This
           * behaved rather comically if you were to do a:
           *   fc::usleep(fc::seconds(60));
           * and then set your system's clock back a month, it would sleep for a
           * month plus a minute before waking back up (this happened on Linux,
           * it seems Windows' behavior in this case was less unexpected).
           *
           * Boost Chrono's steady_clock will always increase monotonically so
           * it will avoid this behavior.
           *
           * Right now we don't really have a way to distinguish when a
           * timeout_time is coming from a function that takes a relative time
           * like fc::usleep() vs something that takes an absolute time like
           * fc::promise::wait_until(), so we can't always do the right thing
           * here.
           */
          task_ready.wait_until(
              lock, boost::chrono::steady_clock::now() +
                        boost::chrono::microseconds(
                            timeout_time.time_since_epoch().count() -
                            time_point::now().time_since_epoch().count()));
        }
      }
    }
  }
   ...
   *    Return the time the next task needs to be run if there is anything
   * scheduled.
   */
  time_point check_for_timeouts() {
    ...
    while (sleep_pqueue.size() && sleep_pqueue.front()->resume_time < now) {
      ...
      std::pop_heap(sleep_pqueue.begin(), sleep_pqueue.end(),
                    sleep_priority_less());
      // ilog( "sleep pop back..." );
  ...
  void yield_until(const time_point &tp, bool reschedule) {
    ...
    std::push_heap(sleep_pqueue.begin(), sleep_pqueue.end(),
                   sleep_priority_less());
    ...
    for (uint32_t i = 0; i < sleep_pqueue.size(); ++i) {
      if (sleep_pqueue[i] == current) {
        ...
        sleep_pqueue.pop_back();
        std::make_heap(sleep_pqueue.begin(), sleep_pqueue.end(),
                       sleep_priority_less());
        break;
      }
    }
  ...
  void wait(const promise_base::ptr &p, const time_point &timeout) {
    ...
    // slog( "                                 %1% unblocking blocking on %2%",
    // current, p.get() );
  ...
  void cleanup_thread_specific_data() {
    for (auto iter = non_task_specific_data.begin();
         iter != non_task_specific_data.end(); ++iter)
      if (iter->cleanup)
        iter->cleanup(iter->value);

    for (auto iter = thread_specific_data.begin();
         iter != thread_specific_data.end(); ++iter)
      if (iter->cleanup)
        iter->cleanup(iter->value);
  }
  ...
  void notify_task_has_been_canceled() {
    ...
    for (auto sleep_iter = sleep_pqueue.begin();
         sleep_iter != sleep_pqueue.end();) {
      if ((*sleep_iter)->canceled) {
        bool already_on_ready_list =
            std::find(ready_heap.begin(), ready_heap.end(), *sleep_iter) !=
            ready_heap.end();
