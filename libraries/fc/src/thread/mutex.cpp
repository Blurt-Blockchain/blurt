...
mutex::mutex() : m_blist(0), recursive_lock_count(0) {}
...
/**
 *  @param  last_context - is set to the next context to get the lock (the
 * next-to-last element of the list)
...
static void cleanup(fc::mutex &m, fc::spin_yield_lock &syl, fc::context *&bl,
                    fc::context *cc) {
  {
...
void mutex::lock() {
  ...
  if (!current_context)
    current_context = fc::thread::current().my->current =
        new fc::context(&fc::thread::current());

  {
    fc::unique_lock<fc::spin_yield_lock> lock(m_blist_lock);
    if (!m_blist) {
      // nobody else owns the mutex, so we get it; add our context as the last
      // and only element on the mutex's list
  ...
  std::exception_ptr e; // cleanup calls yield so we need to move the exception
                        // outside of the catch block
  try {
    fc::thread::current().yield(false);
    // if yield() returned normally, we should now own the lock (we should be at
