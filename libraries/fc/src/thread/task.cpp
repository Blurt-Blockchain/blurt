...
task_base::task_base(void *func)
    : promise_base("task_base"), _posted_num(0), _active_context(nullptr),
      _next(nullptr), _task_specific_data(nullptr), _promise_impl(nullptr),
      _functor(func) {}
...
void task_base::run() {
  ...
  __try {
    ...
    run_impl();
#ifdef _MSC_VER
  } __except (get_unhandled_structured_exception_filter()
                  ? get_unhandled_structured_exception_filter()(
  ...
  try {
    if (!canceled())
      ...
      FC_THROW_EXCEPTION(canceled_exception,
                         "task ${description} canceled before starting",
                         ("description", get_desc()));
#else
      FC_THROW_EXCEPTION(
          canceled_exception,
          "task ${description} canceled before starting, reason ${reason}",
          ("description", get_desc())("reason", _cancellation_reason
    ...
    set_exception(std::make_shared<unhandled_exception>(FC_LOG_MESSAGE(
...
void task_base::cancel(const char *reason /* = nullptr */) {
  ...
  if (_active_context) {
    if (_active_context->next_blocked_mutex) {
      // this task is blocked on a mutex, we probably don't handle this
      // correctly
...
void task_base::_set_active_context(context *c) {
  { synchronized(*_spinlock) _active_context = c; }
}

void task_base::cleanup_task_specific_data() {
  if (_task_specific_data) {
    for (auto iter = _task_specific_data->begin();
         iter != _task_specific_data->end(); ++iter)
      if (iter->cleanup)
