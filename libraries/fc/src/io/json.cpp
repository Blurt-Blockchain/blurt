...
variant variant_from_stream(T &in, uint32_t depth = 0);
...
namespace fc {
template <typename T> char parseEscape(T &in, uint32_t) {
  if (in.peek() == '\\') {
    try {
      in.get();
      switch (in.peek()) {
...
template <typename T> bool skip_white_space(T &in, uint32_t) {
  bool skipped = false;
  while (true) {
    switch (in.peek()) {
    case ' ':
...
template <typename T> fc::string stringFromStream(T &in, uint32_t depth) {
...
template <typename T> fc::string stringFromToken(T &in, uint32_t depth) {
  fc::stringstream token;
  try {
    char c = in.peek();
...
template <typename T> variant token_from_stream(T &in, uint32_t depth) {
  depth++;
  std::stringstream ss;
...
variant json::from_string(const std::string &utf8_str, parse_type ptype,
  ...
  try {
    ...
    case legacy_parser_with_string_doubles:
      return variant_from_stream<fc::stringstream,
                                 legacy_parser_with_string_doubles>(in, depth);
    case strict_parser:
...
variants json::variants_from_string(const std::string &utf8_str,
                                    parse_type ptype, uint32_t depth) {
  try {
    depth++;
    FC_ASSERT(depth <= JSON_MAX_RECURSION_DEPTH);
    check_string_depth(utf8_str);
...
fc::string json::to_pretty_string(
    const variant &v,
    output_formatting format /* = stringify_large_ints_and_doubles */) {
...
variant json::from_file(const fc::path &p, parse_type ptype, uint32_t depth) {
  ...
  case legacy_parser_with_string_doubles:
    return variant_from_stream<boost::filesystem::ifstream,
                               legacy_parser_with_string_doubles>(bi, depth);
  case strict_parser:
...
bool json::is_valid(const std::string &utf8_str, parse_type ptype,
  ...
  case legacy_parser:
    ...
    break;
  case legacy_parser_with_string_doubles:
    variant_from_stream<fc::stringstream, legacy_parser_with_string_doubles>(
        in, depth);
    break;
  case strict_parser:
