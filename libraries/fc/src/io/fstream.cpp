...
ofstream::ofstream() : my(new impl()) {}

ofstream::ofstream(const fc::path &file, int m) : my(new impl()) {
  this->open(file, m);
}
ofstream::~ofstream() {}
...
size_t ofstream::writesome(const std::shared_ptr<const char> &buffer,
                           size_t len, size_t offset) {
  return writesome(buffer.get() + offset, len);
}

void ofstream::put(char c) { my->ofs.put(c); }
void ofstream::close() { my->ofs.close(); }
void ofstream::flush() { my->ofs.flush(); }

ifstream::ifstream() : my(new impl()) {}
ifstream::ifstream(const fc::path &file, int m) : my(new impl()) {
  this->open(file, m);
}
ifstream::~ifstream() {}
...
size_t ifstream::readsome(const std::shared_ptr<char> &buffer, size_t max,
                          size_t offset) {
...
ifstream &ifstream::seekg(size_t p, seekdir d) {
  switch (d) {
  case beg:
    my->ifs.seekg(p, std::ios_base::beg);
    return *this;
  case cur:
    my->ifs.seekg(p, std::ios_base::cur);
    return *this;
  case end:
    my->ifs.seekg(p, std::ios_base::end);
    return *this;
  }
  return *this;
}
