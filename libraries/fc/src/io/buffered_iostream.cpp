...
size_t buffered_istream::readsome(const std::shared_ptr<char> &buf, size_t len,
  ...
  // call to readsome at a time so it reuses _shared_read_buffer.  If you really
  // need to make concurrent calls to readsome(), you'll need to prevent reusing
  // _shared_read_buffer here
...
#ifndef NDEBUG
  // This code was written with the assumption that you'd only be making one
  // call to flush at a time so it reuses _shared_write_buffer.  If you really
  // need to make concurrent calls to flush(), you'll need to prevent reusing
  // _shared_write_buffer here
  struct check_buffer_in_use {
    bool &_buffer_in_use;
    check_buffer_in_use(bool &buffer_in_use) : _buffer_in_use(buffer_in_use) {
      assert(!_buffer_in_use);
