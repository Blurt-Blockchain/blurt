  ...
  void read() {
    ...
    while (!std::cin.eof()) {
      while (write_pos - read_pos > 0xfffff) {
        fc::promise<void>::ptr wr(
            new fc::promise<void>("cin_buffer::write_ready"));
        write_ready = wr;
...
size_t cout_t::writesome(const std::shared_ptr<const char> &buf, size_t len,
                         size_t offset) {
...
size_t cerr_t::writesome(const std::shared_ptr<const char> &buf, size_t len,
                         size_t offset) {
  return writesome(buf.get() + offset, len);
}
void cerr_t::close(){};
...
size_t cin_t::readsome(const std::shared_ptr<char> &buf, size_t len,
                       size_t offset) {
...
istream &cin_t::read(char *buf, size_t len) {
  ...
  do {
    while (!b.eof && (b.write_pos - b.read_pos) == 0) {
      // wait for more...
      fc::promise<void>::ptr rr(
          new fc::promise<void>("cin_buffer::read_ready"));
      { // copy read_ready because it is accessed from multiple threads
    ...
    if (b.eof)
      FC_THROW_EXCEPTION(eof_exception, "cin");
    size_t r = readsome(buf, len);
    buf += r;
    len -= r;

    auto tmp = b.write_ready; // copy write_writey because it is accessed from
                              // multiple thwrites
    if (tmp && !tmp->ready()) {
      tmp->set_value();
    }
