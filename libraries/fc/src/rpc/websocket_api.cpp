...
variant websocket_api_connection::send_call(string api_name, string method_name,
                                            variants args) {
  auto request = _rpc_state.start_remote_call(
      "call", {std::move(api_name), std::move(method_name), std::move(args)});
  _connection.send_message(fc::json::to_string(request));
  return _rpc_state.wait_for_response(*request.id);
}
...
void websocket_api_connection::send_notice(uint64_t callback_id,
                                           variants args /* = variants() */) {
  fc::rpc::request req{
      "2.0", optional<uint64_t>(), "notice", {callback_id, std::move(args)}};
      ...
      try {
        try {
          ...
          if (end - start > fc::milliseconds(LOG_LONG_API_MAX_MS))
            elog("API call execution time limit exceeded. method: ${m} params: "
                 "${p} time: ${t}",
                 ("m", call.method)("p", call.params)("t", end - start));
          else if (end - start > fc::milliseconds(LOG_LONG_API_WARN_MS))
            wlog("API call execution time nearing limit. method: ${m} params: "
                 "${p} time: ${t}",
                 ("m", call.method)("p", call.params)("t", end - start));
#endif
