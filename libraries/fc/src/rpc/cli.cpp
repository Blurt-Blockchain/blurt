...
// version that ships on some macs is missing some functions we require.  We're
// developing against 6.3, but probably anything in the 6.x series is fine
...
variant cli::send_call(string api_name, string method_name,
                       variants args /* = variants() */) {
  FC_ASSERT(false);
}
