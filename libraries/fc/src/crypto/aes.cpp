 ...
 * the integrety of the file / key prior to decryption.
 */
...
#else
  return (unsigned long)(&fc::thread::current()); // TODO: should expose boost
                                                  // thread id
#endif
}
...
openssl_thread_config::~openssl_thread_config() {
  if (CRYPTO_get_id_callback() != NULL && &get_thread_id != NULL &&
