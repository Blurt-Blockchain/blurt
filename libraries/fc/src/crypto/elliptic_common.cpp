...
static private_key_secret _get_curve_order() {
  ...
  FC_ASSERT(static_cast<size_t>(BN_bn2bin(
                order, (unsigned char *)bin.data())) == bin.data_size());
  return bin;
}
...
bool is_bip_0062_canonical(const compact_signature &c) {
  ...
  // BIP-0062 states that sig must be in [1,n/2], however because a sig of value
  // 0 is an invalid signature under all circumstances, the lower bound does not
  // need checking
