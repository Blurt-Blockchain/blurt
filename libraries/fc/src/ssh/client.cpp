...
client::client() : my(new detail::client_impl()) {
  (void)detail::ssh_init; /* fix unused warning...*/
}
...
void client::connect(const fc::string &user, const fc::string &host,
                     uint16_t port) {
  my->hostname = host;
  my->uname = user;
  my->port = port;
 ...
 */
void client::scp_send_dir(const fc::path &local_dir,
                          const fc::path &remote_path,
                          std::function<bool(uint64_t, uint64_t)> progress) {
  fc::path remote_dir = remote_path;
  if (remote_dir.filename() == fc::path("."))
    remote_dir /= local_dir.filename();

  fc_dlog(my->logr, "scp -r ${local} ${remote}",
          ("local", local_dir)("remote", remote_dir));
  ...
  while (ditr != dend) {
    if ((*ditr).filename() == "." || (*ditr).filename() == "..") {
...
void client::scp_send(const fc::path &local_path, const fc::path &remote_path,
  ...
  try {
    // libssh2_scp_send64 stores state data in the session object, and it calls
    // channel_open which stores its own state data, so lock both.
    fc::scoped_lock<fc::mutex> channel_open_lock(my->channel_open_mutex);
    fc::scoped_lock<fc::mutex> scp_send_lock(my->scp_send_mutex);
    chan = my->call_ssh2_ptr_function_throw<LIBSSH2_CHANNEL *>(boost::bind(
        libssh2_scp_send64, my->session, remote_path.generic_string().c_str(),
        0700, fsize, now, now));
  } catch (fc::exception &er) {
    FC_RETHROW_EXCEPTION(
        er, error, "scp ${local_file} to ${remote_file} failed",
        ("local_file", local_path)("remote_file", remote_path));
  }
  uint64_t total_bytes_written = 0;
  try {
    const size_t max_mapping_size = 1024 * 1024 * 1024; // 1GB
    for (uint64_t current_offset = 0; current_offset < fsize;
         current_offset += max_mapping_size) {
      uint64_t total_bytes_left_to_send = fsize - current_offset;
      size_t bytes_to_send_this_iteration = (size_t)std::min<uint64_t>(
          total_bytes_left_to_send, max_mapping_size);
      mapped_region mr(fmap, fc::read_only, current_offset,
                       bytes_to_send_this_iteration);
      size_t bytes_written_this_iteration = 0;
      char *pos = reinterpret_cast<char *>(mr.get_address());
      while (progress(total_bytes_written, fsize) &&
             bytes_written_this_iteration < bytes_to_send_this_iteration) {
        int r = my->call_ssh2_function_throw(
            boost::bind(libssh2_channel_write_ex, chan, 0, pos,
                        bytes_to_send_this_iteration -
                            bytes_written_this_iteration),
...
void client::rm(const fc::path &remote_path) {
  try {
...

    {
      ...
      my->call_ssh2_function_throw(
          boost::bind(libssh2_sftp_unlink_ex, my->sftp,
                      remote_path.generic_string().c_str(),
                      remote_path.generic_string().size()),
          "sftp rm failed ${code}");
    }
  } catch (fc::exception &er) {
    FC_RETHROW_EXCEPTION(er, error, "sftp remove '${remote_path}' failed",
...
void client::rmdir(const fc::path &remote_path) {
  try {
    auto s = stat(remote_path);
    if (!s.is_directory())
      FC_THROW("sftp cannot rmdir non-directory ${path}",
               ("path", remote_path));
    else if (!s.exists())
      return; // nothing to do

    {
      ...
      my->call_ssh2_function_throw(
          boost::bind(libssh2_sftp_rmdir_ex, my->sftp,
                      remote_path.generic_string().c_str(),
                      remote_path.generic_string().size()),
          "sftp rmdir failed ${code}");
    }
  } catch (fc::exception &er) {
    FC_RETHROW_EXCEPTION(er, error, "sftp rmdir '${remote_path}' failed",
                         ("remote_path", remote_path));
  }
}
...
void client::rmdir_recursive(const fc::path &remote_path) {
  try {
    ...
    LIBSSH2_SFTP_HANDLE *dir_handle;
    {
      ...
      dir_handle = my->call_ssh2_ptr_function_throw<LIBSSH2_SFTP_HANDLE *>(
          boost::bind(libssh2_sftp_open_ex, my->sftp,
                      remote_path.generic_string().c_str(),
                      remote_path.generic_string().size(), 0, 0,
    ...
    do {
      ...
      int rc;
      {
        ...
        rc = my->call_ssh2_function_throw(
            boost::bind(libssh2_sftp_readdir_ex, dir_handle, mem, sizeof(mem),
                        (char *)NULL, 0, &attrs),
      ...
      if (rc > 0) {
        ...
        if (LIBSSH2_SFTP_S_ISDIR(attrs.permissions))
          ...
          my->call_ssh2_function_throw(
              boost::bind(libssh2_sftp_unlink_ex, my->sftp,
                          full_remote_path.generic_string().c_str(),
                          full_remote_path.generic_string().size()),
              "sftp rm failed ${code}");
        }
      } else
        break;
...

    {
      ...
      my->call_ssh2_function_throw(
          boost::bind(libssh2_sftp_close_handle, dir_handle),
          "sftp libssh2_sftp_closedir failed ${code}");
    }
    {
      fc::scoped_lock<fc::mutex> scp_rmdir_lock(my->scp_rmdir_mutex);
      my->call_ssh2_function_throw(
    ...
    FC_RETHROW_EXCEPTION(er, error,
                         "sftp rmdir recursive '${remote_path}' failed",
                         ("remote_path", remote_path));
  }
}
...
file_attrib client::stat(const fc::path &remote_path) {
  ...
  int ec;
  {
    ...
    ec = my->call_ssh2_function(boost::bind(
        libssh2_sftp_stat_ex, my->sftp, remote_path.generic_string().c_str(),
        remote_path.generic_string().size(), LIBSSH2_SFTP_STAT, &att));
  }
  if (ec)
    return file_attrib();
  file_attrib ft;
  ...
  try {
...

    {
      ...
      my->call_ssh2_function_throw(boost::bind(libssh2_sftp_mkdir_ex, my->sftp,
                                               rdir.generic_string().c_str(),
                                               rdir.generic_string().size(),
                                               mode),
                                   "sftp mkdir error");
    }
  } catch (fc::exception &er) {
    FC_RETHROW_EXCEPTION(er, error,
                         "sftp failed to create directory '${directory}'",
                         ("directory", rdir));
  }
}
...
bool file_attrib::is_directory() { return LIBSSH2_SFTP_S_ISDIR(permissions); }
bool file_attrib::is_file() { return LIBSSH2_SFTP_S_ISREG(permissions); }
bool file_attrib::exists() { return 0 != permissions; }

detail::client_impl::client_impl()
    : session(nullptr), knownhosts(nullptr), sftp(nullptr), agent(nullptr),
...
detail::client_impl::~client_impl() { close(); }

/* static */
void detail::client_impl::kbd_callback(
...
void detail::client_impl::connect() {
  try {
    ...
    auto eps = fc::asio::tcp::resolve(hostname,
                                      boost::lexical_cast<std::string>(port));
    if (eps.size() == 0)
      FC_THROW("Unable to resolve host '${host}'", ("host", hostname));

    sock.reset(
        new boost::asio::ip::tcp::socket(fc::asio::default_io_service()));
    ...
    for (uint32_t i = 0; i < eps.size(); ++i) {
      ...
      try {
        ...
        fc_ilog(logr, "Attempting to connect to ${endpoint}",
        ...
        fc_ilog(logr, "Failed to connect to ${endpoint}\n${error_reprot}",
                ("endpoint", ss.str().c_str())("error_report",
                                               er.to_detail_string()));
        sock->close();
      }
    }
    ...
    *libssh2_session_abstract(session) = this;
    ...
    try {
      call_ssh2_function_throw(
          boost::bind(libssh2_session_handshake, session, sock->native()),
    ...
    // LIBSSH2_HOSTKEY_HASH_SHA1); slog( "fingerprint: %s", fingerprint );
    ...
    elog("Unable to connect to ssh server: ${detail}",
         ("detail", er.to_detail_string().c_str()));
    close();
    FC_RETHROW_EXCEPTION(er, error, "Unable to connect to ssh server");
    ;
  } catch (...) {
    close();
    FC_THROW("Unable to connect to ssh server",
             ("exception", fc::except_str()));
  }
}

/* static */
void detail::client_impl::handle_trace(LIBSSH2_SESSION *session, void *context,
...
void detail::client_impl::close() {
  if (session) {
    ...
    try {
      call_ssh2_function(boost::bind(libssh2_session_disconnect_ex, session,
...
void detail::client_impl::authenticate() {
  try {
    ...
    // call_blocking_libssh2_function-type functions to wait and retry, so we
    // must explicitly lock around the libssh2_userauth_list calls. hence, this
    // anonymous scope:
    {
    ...
    std::for_each(
        split_alist.begin(), split_alist.end(), [&](const std::string &s) {
          if (s == "publickey")
            pubkey = true;
          else if (s == "password")
            pass = true;
            ...
            fc_dlog(logr,
                    "Unknown/unsupported authentication type '${auth_type}'",
                    ("auth_type", s.c_str()));
        });
...
bool detail::client_impl::try_pass() {
  return !call_ssh2_function(boost::bind(
      libssh2_userauth_password_ex, session, uname.c_str(), uname.size(),
      upass.c_str(), upass.size(), (LIBSSH2_PASSWD_CHANGEREQ_FUNC((*)))NULL));
}
bool detail::client_impl::try_keyboard() {
  return !call_ssh2_function(
      boost::bind(libssh2_userauth_keyboard_interactive_ex, session,
                  uname.c_str(), uname.size(), &client_impl::kbd_callback));
}
bool detail::client_impl::try_pub_key() {
  if (privkey.size()) {
    if (!call_ssh2_function(boost::bind(
            libssh2_userauth_publickey_fromfile_ex, session, uname.c_str(),
            uname.size(), pubkey.c_str(), privkey.c_str(), passphrase.c_str())))
      return true; // successful authentication from file
    fc_ilog(logr,
            "failed to authenticate with private key from file "
            "'${privkey_filename}'",
            ("privkey_filename", privkey));
  } else
    fc_ilog(logr,
            "no private key file set, skiping pubkey authorization from file");
  ...
  while (1) {
    ...
    int ec = call_ssh2_function(boost::bind(libssh2_agent_get_identity, agent,
    ...
    if (call_ssh2_function(boost::bind(libssh2_agent_userauth, agent,
                                       uname.c_str(), identity)))
      fc_ilog(logr, "unable to authenticate with public key '${key_comment}'",
              ("key_comment", identity->comment));
    else {
      fc_ilog(logr, "authenticated with public key '${key_comment}'",
              ("key_comment", identity->comment));
      return true;
    }
...
void detail::client_impl::wait_on_socket(int additionalDirections /* = 0 */) {
    ...
    if (!read_prom) {
      ...
      sock->async_read_some(boost::asio::null_buffers(),
                            [=](const boost::system::error_code &e, size_t) {
                              fc::scoped_lock<fc::spin_lock> lock(
                                  this->_spin_lock);
  ...
  if (dir & LIBSSH2_SESSION_BLOCK_OUTBOUND) {
    ...
    if (!write_prom) {
      write_prom.reset(
          new fc::promise<boost::system::error_code>("write_prom"));
...
void detail::client_impl::init_sftp() {
  if (!sftp)
    sftp = call_ssh2_ptr_function_throw<LIBSSH2_SFTP *>(
        boost::bind(libssh2_sftp_init, session),
...
LIBSSH2_CHANNEL *detail::client_impl::open_channel(const fc::string &pty_type) {
  ...
  /* anonymous scope */ {
    ...
    chan = call_ssh2_ptr_function_throw<LIBSSH2_CHANNEL *>(
        boost::bind(libssh2_channel_open_ex, session, "session",
                    sizeof("session") - 1, LIBSSH2_CHANNEL_WINDOW_DEFAULT,
                    LIBSSH2_CHANNEL_PACKET_DEFAULT, (const char *)NULL, 0),
        "libssh2_channel_open_session failed: ${message}");
  }

  if (pty_type.size())
    call_ssh2_function_throw(
        boost::bind(libssh2_channel_request_pty_ex, chan, pty_type.c_str(),
                    pty_type.size(), (char *)NULL, 0, LIBSSH2_TERM_WIDTH,
                    LIBSSH2_TERM_HEIGHT, LIBSSH2_TERM_WIDTH_PX,
