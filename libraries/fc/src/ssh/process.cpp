...
public:
  process_istream(process_impl &p, int c) : proc(p), chan(c) {}
...
public:
  process_ostream(process_impl &p) : proc(p) {}
...
public:
  ...
  ~process_impl();
  // process_impl( const client& c, const fc::string& cmd, const fc::string&
  // pty_type );
...
private:
  ...
  static fc::string windows_shell_escape_command(const fc::path &exe,
                                                 const vector<string> &args);
  static fc::string unix_shell_escape_command(const fc::path &exe,
                                              const vector<string> &args);
};

} // end namespace detail

process::process(client_ptr c) : my(new detail::process_impl(c)) {}
...
int process::result() {
  if (!my->return_code && !my->return_signal) {
    ...
    my->sshc->my->call_ssh2_function(
        boost::bind(libssh2_channel_wait_eof, my->chan));
    my->sshc->my->call_ssh2_function_throw(
        boost::bind(libssh2_channel_wait_closed, my->chan),
        "Error waiting on socket to close: ${message}");

    char *exit_signal;
    char *error_message;
    libssh2_channel_get_exit_signal(my->chan, &exit_signal, NULL,
  ...
  if (my->return_signal)
    FC_THROW("process terminated with signal ${signal}: ${signal_message}",
             ("signal", *my->return_signal)(
                 "signal_message",
                 my->return_signal_message ? *my->return_signal_message : ""));
  else
    return *my->return_code;
}

void process::kill() { elog("error: fc::ssh::process::kill() not supported"); }

/**
 *  @brief returns a stream that writes to the procss' stdin
 */
fc::buffered_ostream_ptr process::in_stream() { return my->buffered_std_in; }

/**
 *  @brief returns a stream that reads from the process' stdout
 */
fc::buffered_istream_ptr process::out_stream() { return my->buffered_std_out; }

/**
 *  @brief returns a stream that reads from the process' stderr
 */
fc::buffered_istream_ptr process::err_stream() { return my->buffered_std_err; }
...
void detail::process_impl::flush() {
   ...
   *
   int ec = libssh2_channel_flush_ex( chan,
   LIBSSH2_CHANNEL_FLUSH_EXTENDED_DATA); while( ec == LIBSSH2_ERROR_EAGAIN ) {
...
int detail::process_impl::read_some(char *data, size_t len, int stream_id) {
  if (!sshc->my->session) {
    FC_THROW("Session closed");
  }
  int rc;
  char *buf = data;
  size_t buflen = len;
  do {
    rc = sshc->my->call_ssh2_function_throw(
        boost::bind(libssh2_channel_read_ex, chan, stream_id, buf, buflen),
        "read failed: ${message}");
    if (rc > 0) {
      buf += rc;
      buflen -= rc;
...
int detail::process_impl::write_some(const char *data, size_t len,
                                     int stream_id) {
  if (!sshc->my->session) {
    FC_THROW("Session closed");
  }
  ...
  do {
    rc = sshc->my->call_ssh2_function_throw(
        boost::bind(libssh2_channel_write_ex, chan, stream_id, buf, buflen),
...
void detail::process_impl::send_eof() {
  if (sshc->my->session)
    sshc->my->call_ssh2_function_throw(
        boost::bind(libssh2_channel_send_eof, chan),
...
void detail::process_ostream::close() { proc.send_eof(); }

void detail::process_ostream::flush() { proc.flush(); }

detail::process_impl::process_impl(client_ptr c)
    : chan(nullptr), sshc(c), buffered_std_in(new buffered_ostream(
                                  ostream_ptr(new process_ostream(*this)))),
      buffered_std_out(
          new buffered_istream(istream_ptr(new process_istream(*this, 0)))),
      buffered_std_err(new buffered_istream(
          istream_ptr(new process_istream(*this, SSH_EXTENDED_DATA_STDERR)))) {}
...
fc::string detail::process_impl::unix_shell_escape(const fc::string &str) {
  ...
  for (size_t start = escaped_quotes.find("'"); start != fc::string::npos;
...
fc::string
detail::process_impl::unix_shell_escape_command(const fc::path &exe,
                                                const vector<string> &args) {
...
// windows command-line escaping rules are a disaster, partly because how the
// command-line is parsed depends on what program you're running.  In windows,
// the command line is passed in as a single string, and the process is left to
// interpret it as it sees fit.  The standard C runtime uses one set of rules,
// the function CommandLineToArgvW usually used by GUI-mode programs uses a
// different set. Here we try to find a common denominator that works well for
// simple cases it's only minimally tested right now due to time constraints.
...
fc::string
detail::process_impl::windows_shell_escape_command(const fc::path &exe,
                                                   const vector<string> &args) {
  fc::stringstream command_line;
...
void detail::process_impl::exec(const fc::path &exe, vector<string> args,
  ...
  sshc->my->call_ssh2_function(
      boost::bind(libssh2_channel_handle_extended_data2, chan,
                  LIBSSH2_CHANNEL_EXTENDED_DATA_NORMAL));

  try {
    fc::scoped_lock<fc::mutex> process_startup_lock(
        sshc->my->process_startup_mutex);
    fc::string command_line = sshc->my->remote_system_is_windows
                                  ? windows_shell_escape_command(exe, args)
                                  : unix_shell_escape_command(exe, args);
    sshc->my->call_ssh2_function_throw(
        boost::bind(libssh2_channel_process_startup, chan, "exec",
                    sizeof("exec") - 1, command_line.c_str(),
                    command_line.size()),
        "exec failed: ${message}"); // equiv to libssh2_channel_exec(chan, cmd)
                                    // macro
