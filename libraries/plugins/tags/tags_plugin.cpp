...
inline double calculate_hot(const share_type &score,
                            const time_point_sec &created) {
  return calculate_score<10000000, 10000>(score, created);
}

inline double calculate_trending(const share_type &score,
                                 const time_point_sec &created) {
...
public:
  ...
  comment_metadata filter_tags(const comment_object &c,
                               const comment_content_object &con) const;
  void update_tag(const tag_object &current, const comment_object &comment,
                  double hot, double trending) const;
  void create_tag(const string &tag, const comment_object &comment, double hot,
...
void tags_plugin_impl::remove_stats(const tag_object &tag,
                                    const tag_stats_object &stats) const {
...
void tags_plugin_impl::add_stats(const tag_object &tag,
                                 const tag_stats_object &stats) const {
  _db.modify(stats, [&](tag_stats_object &s) {
    if (tag.parent == comment_id_type()) {
      s.top_posts++;
...
void tags_plugin_impl::remove_tag(const tag_object &tag) const {
  const auto &idx = _db.get_index<author_tag_stats_index>()
                        .indices()
                        .get<by_author_tag_posts>();
  auto itr = idx.lower_bound(boost::make_tuple(tag.author, tag.tag));
  if (itr != idx.end() && itr->author == tag.author && itr->tag == tag.tag) {
    _db.modify(*itr,
               [&](author_tag_stats_object &stats) { stats.total_posts--; });
  }
...
const tag_stats_object &tags_plugin_impl::get_stats(const string &tag) const {
  const auto &stats_idx =
      _db.get_index<tag_stats_index>().indices().get<by_tag>();
  auto itr = stats_idx.find(tag);
  if (itr != stats_idx.end())
    return *itr;

  return _db.create<tag_stats_object>(
      [&](tag_stats_object &stats) { stats.tag = tag; });
}

comment_metadata
tags_plugin_impl::filter_tags(const comment_object &c,
                              const comment_content_object &con) const {
  comment_metadata meta;

  if (con.json_metadata.size()) {
    try {
      meta = fc::json::from_string(to_string(con.json_metadata))
                 .as<comment_metadata>();
  ...
  /// the universal tag applies to everything safe for work or nsfw with a
  /// non-negative payout
  if (c.net_rshares >= 0) {
...
void tags_plugin_impl::update_tag(const tag_object &current,
                                  const comment_object &comment, double hot,
                                  double trending) const {
...
void tags_plugin_impl::create_tag(const string &tag,
                                  const comment_object &comment, double hot,
                                  double trending) const {
...
void tags_plugin_impl::update_tags(const comment_object &c,
                                   bool parse_tags) const {
  try {
    ...
    const auto &comment_idx =
        _db.get_index<tag_index>().indices().get<by_comment>();

#ifndef IS_LOW_MEM
    if (parse_tags) {
      auto meta = filter_tags(
          c, _db.get<comment_content_object, chain::by_comment>(c.id));
...
struct pre_apply_operation_visitor {
  ...
  void operator()(const delete_comment_operation &op) const {
    const auto &comment = _db.find<comment_object, chain::by_permlink>(
        boost::make_tuple(op.author, op.permlink));
    ...
    while (tag_itr != idx.end() && tag_itr->author == auth.id &&
           tag_itr->comment == comment->id) {
...
struct operation_visitor {
  operation_visitor(tags_plugin_impl &my) : _my(my){};
  ...
  void operator()(const comment_operation &op) const {
    if (_my._started) {
      ...
      _my.update_tags(_my._db.get_comment(op.author, op.permlink),
                      op.json_metadata.size());
    }
  }

  void operator()(const transfer_operation &op) const {
    if (_my._db.head_block_time() >= _my._promoted_start_time &&
        op.to == BLURT_NULL_ACCOUNT && op.amount.symbol == BLURT_SYMBOL) {
      vector<string> part;
      part.reserve(4);
      auto path = op.memo;
      ...
      if (part.size() > 1 && part[0].size() && part[0][0] == '@') {
        ...
        if (c && c->parent_author.size() == 0) {
          const auto &comment_idx =
              _my._db.get_index<tag_index>().indices().get<by_comment>();
          auto citr = comment_idx.lower_bound(c->id);
          while (citr != comment_idx.end() && citr->comment == c->id) {
...
#ifndef IS_LOW_MEM
    comment_metadata meta = _my.filter_tags(
        c, _my._db.get<comment_content_object, chain::by_comment>(c.id));
...
void tags_plugin_impl::on_pre_apply_operation(
...
void tags_plugin_impl::on_post_apply_operation(
    const operation_notification &note) {
  try {
    /// plugins shouldn't ever throw
...
void tags_plugin::set_program_options(
  ...
  cfg.add_options()(
      ...
      "Block time (in epoch seconds) when to start calculating promoted "
      "content. Should be 1 week prior to current time.")(
      "tags-skip-startup-update", bpo::value<bool>()->default_value(false),
      "Skip updating tags on startup. Can safely be skipped when starting a "
      "previously running node. Should not be skipped when reindexing.");
  cli.add_options()(
      "tags-skip-startup-update", bpo::bool_switch()->default_value(false),
      "Skip updating tags on startup. Can safely be skipped when starting a "
      "previously running node. Should not be skipped when reindexing.");
}

void tags_plugin::plugin_initialize(
  ...
  my->_pre_apply_operation_conn = my->_db.add_pre_apply_operation_handler(
      ...
      *this, 0);
  my->_post_apply_operation_conn = my->_db.add_post_apply_operation_handler(
      [&](const operation_notification &note) {
  ...
  if (!options.at("tags-skip-startup-update").as<bool>()) {
    my->on_sync_connection =
        appbase::app().get_plugin<chain::chain_plugin>().on_sync.connect(
            0, [this]() {
              ilog("Updating all tag stats on startup");
              my->_db.with_write_lock([this]() {
                // for each comment that has not been paid, update tags
                const auto &comment_idx =
                    my->_db.get_index<comment_index, by_cashout_time>();
  ...
  if (options.count("tags-start-promoted")) {
    my->_promoted_start_time =
        fc::time_point_sec(options["tags-start-promoted"].as<uint32_t>());
...
void tags_plugin::plugin_startup() { my->_started = true; }
