...
struct count_operation_visitor {
  ...
  count_operation_visitor(const state_object_size_info &w,
                          const operation_exec_info &e)
      : _w(w), _e(e) {}
  ...
  void operator()(const account_create_operation &op) const {
    state_bytes_count += _w.account_object_base_size +
  ...
  void operator()(const comment_payout_beneficiaries &bens) const {
    state_bytes_count +=
        _w.comment_object_beneficiaries_member_size * bens.beneficiaries.size();
  }
  ...
  void operator()(const delegate_vesting_shares_operation &op) const {
    state_bytes_count +=
        std::max(_w.vesting_delegation_object_base_size,
  ...
  void operator()(const witness_update_operation &op) const {
    state_bytes_count += _w.witness_object_base_size +
  ...
  void operator()(const update_proposal_votes_operation &op) const {
    ...
    state_bytes_count +=
        _w.proposal_vote_object_member_size * op.proposal_ids.size();
...
void count_resources(const signed_transaction &tx,
  ...
  result.resource_count[resource_state_bytes] +=
      ...
      size_info.transaction_object_byte_size * tx_size + vtor.state_bytes_count;
