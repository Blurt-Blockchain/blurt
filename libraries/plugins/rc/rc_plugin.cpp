...
public:
  ...
  bool before_first_block() { return (_db.count<rc_account_object>() == 0); }
...
inline int64_t get_next_vesting_withdrawal(const account_object &account) {
  ...
  int64_t next_withdrawal =
      (withdraw_per_period <= total_left) ? withdraw_per_period : total_left;
  bool is_done =
      (account.next_vesting_withdrawal == fc::time_point_sec::maximum());
  return is_done ? 0 : next_withdrawal;
}

template <bool account_may_exist = false>
void create_rc_account(database &db, uint32_t now,
                       const account_object &account,
                       asset max_rc_creation_adjustment) {
  // ilog( "create_rc_account( ${a} )", ("a", account.name) );
  if (account_may_exist) {
    const rc_account_object *rc_account =
        db.find<rc_account_object, by_name>(account.name);
  ...
  if (max_rc_creation_adjustment.symbol == BLURT_SYMBOL) {
    ...
    max_rc_creation_adjustment =
        max_rc_creation_adjustment * gpo.get_vesting_share_price();
    ...
    // wlog( "Encountered max_rc_creation_adjustment.symbol == VESTS_SYMBOL
    // creating account ${acct}", ("acct", account.name) );
  } else {
    elog("Encountered unknown max_rc_creation_adjustment creating account "
         "${acct}",
         ("acct", account.name));
...
void create_rc_account(database &db, uint32_t now,
  ...
  create_rc_account<account_may_exist>(db, now, account,
                                       max_rc_creation_adjustment);
}

std::vector<std::pair<int64_t, account_name_type>>
dump_all_accounts(const database &db) {
...
std::vector<std::pair<int64_t, account_name_type>>
dump_all_rc_accounts(const database &db) {
  std::vector<std::pair<int64_t, account_name_type>> result;
...
struct get_resource_user_visitor {
  ...
  account_name_type
  operator()(const witness_set_properties_operation &op) const {
    return op.owner;
  }
  ...
  template <typename Op> account_name_type operator()(const Op &op) const {
...
void use_account_rcs(database &db, const dynamic_global_property_object &gpo,
                     const account_name_type &account_name, int64_t rc,
...
#ifdef IS_TEST_NET
  if (whitelist.count(account_name))
    return;
#endif
  ...
  const account_object &account = db.get<account_object, by_name>(account_name);
  const rc_account_object &rc_account =
      db.get<rc_account_object, by_name>(account_name);
  ...
  try {
    ...
    db.modify(rc_account, [&](rc_account_object &rca) {
      rca.rc_manabar.regenerate_mana<true>(mbparams,
                                           gpo.time.sec_since_epoch());
      ...
      if ((!skip.skip_reject_not_enough_rc)) {
        if (db.is_producing()) {
          BLURT_ASSERT(
              has_mana, plugin_exception,
              "Account: ${account} has ${rc_current} RC, needs ${rc_needed} "
              "RC. Please wait to transact, or power up BLURT.",
              ("account", account_name)("rc_needed", rc)(
                  "rc_current", rca.rc_manabar.current_mana));
        } else {
          if (!has_mana) {
            const dynamic_global_property_object &gpo =
                db.get_dynamic_global_properties();
            ilog("Accepting transaction by ${account}, has ${rc_current} RC, "
                 "needs ${rc_needed} RC, block ${b}, witness ${w}.",
                 ("account", account_name)("rc_needed", rc)(
                     "rc_current", rca.rc_manabar.current_mana)(
                     "b", gpo.head_block_number)("w", gpo.current_witness));
          }
        }
      }

      if ((!has_mana) && (skip.skip_negative_rc_balance ||
                          (gpo.time.sec_since_epoch() <= 1538211600)))
        return;
      ...
      int64_t min_mana = -1 *
                         (BLURT_RC_MAX_NEGATIVE_PERCENT * mbparams.max_mana) /
                         BLURT_100_PERCENT;
...
void rc_plugin_impl::on_post_apply_transaction(
  ...
  try {
    ...
    int64_t rc_regen = (gpo.total_vesting_shares.amount.value /
                        (BLURT_RC_REGEN_TIME / BLURT_BLOCK_INTERVAL));
    ...
    const rc_resource_param_object &params_obj =
        _db.get<rc_resource_param_object, by_id>(
            rc_resource_param_object::id_type());
    const rc_pool_object &pool_obj =
        _db.get<rc_pool_object, by_id>(rc_pool_object::id_type());
    ...
    if (rc_regen > 0) {
      for (size_t i = 0; i < BLURT_NUM_RESOURCE_TYPES; i++) {
        ...
        tx_info.usage.resource_count[i] *=
            int64_t(params.resource_dynamics_params.resource_unit);
        tx_info.cost[i] = compute_rc_cost_of_resource(
            params.price_curve_params, pool, tx_info.usage.resource_count[i],
            rc_regen);
    ...
    std::shared_ptr<exp_rc_data> export_data =
        blurt::plugins::block_data_export::find_export_data<exp_rc_data>(
            BLURT_RC_PLUGIN_NAME);
...
void rc_plugin_impl::on_post_apply_block(const block_notification &note) {
  try {
    ...
    ilog(
    "\n\n************************************************************************"
    ); ilog( "Block ${b}", ("b", note.block_num) ); for( const account_object&
    acct : idx )
    {
       auto it = _account_to_max_rc.find( acct.name );
       const rc_account_object& rc_account = _db.get< rc_account_object, by_name
    >( acct.name ); int64_t max_rc = get_maximum_rc( acct, rc_account ); if( it
    == _account_to_max_rc.end() )
       {
    ...
    */
    ...
    if (params_obj.resource_param_array[resource_new_accounts]
            .resource_dynamics_params != wso.account_subsidy_rd) {
      dlog("Copying changed subsidy params from consensus in block ${b}",
      ...
      _db.modify(params_obj, [&](rc_resource_param_object &p) {
        ...
        // Hardcoded to use default values of rc_curve_gen_params() fields for
        // now
        generate_rc_curve_params(
            p.resource_param_array[resource_new_accounts].price_curve_params,
            p.resource_param_array[resource_new_accounts]
    ...
    _db.modify(
        ...
        [&](rc_pool_object &pool_obj) {
          ...
          for (size_t i = 0; i < BLURT_NUM_RESOURCE_TYPES; i++) {
            const rd_dynamics_params &params =
                params_obj.resource_param_array[i].resource_dynamics_params;
            int64_t &pool = pool_obj.pool_array[i];
            uint32_t dt = 1;
            ...
            block_info.budget[i] =
                int64_t(params.budget_per_time_unit) * int64_t(dt);
            block_info.usage[i] =
                count.resource_count[i] * int64_t(params.resource_unit);
            block_info.decay[i] = rd_compute_pool_decay(
                params.decay_params, pool - block_info.usage[i], dt);

            int64_t new_pool = pool - block_info.decay[i] +
                               block_info.budget[i] - block_info.usage[i];

            if (i == resource_new_accounts) {
              int64_t new_consensus_pool = _db.get_dynamic_global_properties()
              ...
              if (block_info.adjustment[i] != 0) {
                ilog("resource_new_accounts adjustment on block ${b}: ${a}",
                     ("a", block_info.adjustment[i])("b",
                                                     gpo.head_block_number));
              }
            } else {
            ...
            if (debug_print) {
              ...
              a /= k * double(pool);
              dlog("a=${a}   aR=${aR}",
                   ("a", a)("aR", a * gpo.total_vesting_shares.amount.value /
                                      BLURT_RC_REGEN_TIME));
            }
          }
          if (debug_print) {
...
void rc_plugin_impl::on_first_block() {
  // Initial values are located at
  // `libraries/jsonball/data/resource_parameters.json`
  std::string resource_params_json = blurt::jsonball::get_resource_parameters();
  fc::variant resource_params_var =
      fc::json::from_string(resource_params_json, fc::json::strict_parser);
  std::vector<
      std::pair<fc::variant, std::pair<fc::variant_object, fc::variant_object>>>
      resource_params_pairs;
  ...
  _db.create<rc_resource_param_object>(
      [&](rc_resource_param_object &params_obj) {
        for (auto &kv : resource_params_pairs) {
          ...
          fc::from_variant(fc::variant(mvo),
                           params_obj.resource_param_array[k]);
        }
  ...
  _db.create<rc_pool_object>([&](rc_pool_object &pool_obj) {
    for (size_t i = 0; i < BLURT_NUM_RESOURCE_TYPES; i++) {
      const rc_resource_params &params = params_obj.resource_param_array[i];
  ...
  for (auto it = idx.begin(); it != idx.end(); ++it) {
    create_rc_account(_db, now.sec_since_epoch(), *it,
                      asset(1000, BLURT_SYMBOL));
  }

  //   if( op.hardfork_id == BLURT_HARDFORK_0_1 )
  //   {
  //      const auto& idx = _db.get_index< account_index >().indices().get<
  //      by_id >(); for( auto it=idx.begin(); it!=idx.end(); ++it )
  //      {
  ...
  {
    const auto &params = _db.get<rc_resource_param_object, by_id>(
        rc_resource_param_object::id_type());
    ...
    _db.modify(_db.get<rc_pool_object, by_id>(rc_pool_object::id_type()),
               [&](rc_pool_object &p) {
                 for (size_t i = 0; i < BLURT_NUM_RESOURCE_TYPES; i++) {
                   p.pool_array[i] =
                       int64_t(params.resource_param_array[i]
                                   .resource_dynamics_params.max_pool_size);
                 }
...
// - Save regenerated account names in a local array for further
...
struct pre_apply_operation_visitor {
  ...
  void regenerate(const account_object &account,
                  const rc_account_object &rc_account) const {
    //
    ...
    // TODO:  Issue number
    //
    static_assert(BLURT_RC_REGEN_TIME <= BLURT_VOTING_MANA_REGENERATION_SECONDS,
      ...
      if (mbparams.max_mana != rc_account.last_max_rc) {
        if (!_skip.skip_reject_unknown_delta_vests) {
          BLURT_ASSERT(
              ...
              "Account ${a} max RC changed from ${old} to ${new} without "
              "triggering an op, noticed on block ${b}",
              ("a", account.name)("old", rc_account.last_max_rc)(
                  "new", mbparams.max_mana)("b", _db.head_block_num()));
        } else {
          wlog("NOTIFYALERT! Account ${a} max RC changed from ${old} to ${new} "
               "without triggering an op, noticed on block ${b}",
               ("a", account.name)("old", rc_account.last_max_rc)(
                   "new", mbparams.max_mana)("b", _db.head_block_num()));
        }
      }
  ...
  void regenerate(const account_name_type &name) const {
    ...
    if (account_may_not_exist) {
      ...
      FC_ASSERT(account != nullptr, "Unexpectedly, account ${a} does not exist",
                ("a", name));
    }

    const rc_account_object *rc_account =
        _db.find<rc_account_object, by_name>(name);
    FC_ASSERT(rc_account != nullptr,
              "Unexpectedly, rc_account ${a} does not exist", ("a", name));
    try {
...
struct post_apply_operation_visitor {
  ...
  post_apply_operation_visitor(vector<account_regen_info> &ma, database &db,
                               uint32_t t, uint32_t b, account_name_type w)
      : _mod_accounts(ma), _db(db), _current_time(t), _current_block_number(b),
        _current_witness(w) {}
  ...
  void operator()(const create_claimed_account_operation &op) const {
    create_rc_account(
        _db, _current_time, op.new_account_name,
        _db.get_witness_schedule_object().median_props.account_creation_fee);
  }
...
void rc_plugin_impl::on_pre_apply_operation(
...
void update_modified_accounts(
    database &db, const std::vector<account_regen_info> &modified_accounts) {
  for (const account_regen_info &regen_info : modified_accounts) {
    const account_object &account =
        db.get<account_object, by_name>(regen_info.account_name);
    const rc_account_object &rc_account =
        db.get<rc_account_object, by_name>(regen_info.account_name);
...
void rc_plugin_impl::on_post_apply_operation(
    const operation_notification &note) {
  try {
    if (before_first_block())
      return;
    ...
    post_apply_operation_visitor vtor(modified_accounts, _db, now,
                                      gpo.head_block_number,
                                      gpo.current_witness);
    note.op.visit(vtor);
...
void rc_plugin_impl::validate_database() {
  const auto &rc_idx =
      _db.get_index<rc_account_index>().indices().get<by_name>();

  for (const rc_account_object &rc_account : rc_idx) {
    const account_object &account =
        _db.get<account_object, by_name>(rc_account.account);
    ...
    FC_ASSERT(max_rc == rc_account.last_max_rc,
              "Account ${a} max RC changed from ${old} to ${new} without "
              "triggering an op, noticed on block ${b} in validate_database()",
              ("a", account.name)("old", rc_account.last_max_rc)("new", max_rc)(
                  "b", _db.head_block_num()));
  }
}
...
void rc_plugin::set_program_options(options_description &cli,
                                    options_description &cfg) {
  cfg.add_options()("rc-skip-reject-not-enough-rc",
                    bpo::value<bool>()->default_value(false),
                    "Skip rejecting transactions when account has insufficient "
                    "RCs. This is not recommended.")(
      "rc-compute-historical-rc", bpo::value<bool>()->default_value(false),
      "Generate historical resource credits")
#ifdef IS_TEST_NET
      ("rc-start-at-block", bpo::value<uint32_t>()->default_value(0),
       "Start calculating RCs at a specific block")(
          "rc-account-whitelist", bpo::value<vector<string>>()->composing(),
          "Ignore RC calculations for the whitelist")
#endif
      ;
  cli.add_options()("rc-skip-reject-not-enough-rc",
                    bpo::bool_switch()->default_value(false),
                    "Skip rejecting transactions when account has insufficient "
                    "RCs. This is not recommended.")(
      "rc-compute-historical-rc", bpo::bool_switch()->default_value(false),
      "Generate historical resource credits")
#ifdef IS_TEST_NET
      ("rc-start-at-block", bpo::value<uint32_t>()->default_value(0),
       "Start calculating RCs at a specific block")(
...
void rc_plugin::plugin_initialize(
  ...
  try {
    block_data_export_plugin *export_plugin =
        appbase::app().find_plugin<block_data_export_plugin>();
    if (export_plugin != nullptr) {
      ilog("Registering RC export data factory");
      export_plugin->register_export_data_factory(
          BLURT_RC_PLUGIN_NAME, []() -> std::shared_ptr<exportable_block_data> {
            return std::make_shared<exp_rc_data>();
          });
    }

    chain::database &db =
        appbase::app().get_plugin<blurt::plugins::chain::chain_plugin>().db();
    ...
    my->_post_apply_block_conn = db.add_post_apply_block_handler(
        ...
        *this, 0);
    // my->_pre_apply_transaction_conn = db.add_pre_apply_transaction_handler(
    // [&]( const transaction_notification& note )
    //   { try { my->on_pre_apply_transaction( note ); } FC_LOG_AND_RETHROW() },
    //   *this, 0 );
    my->_post_apply_transaction_conn = db.add_post_apply_transaction_handler(
        [&](const transaction_notification &note) {
          try {
        ...
        *this, 0);
    my->_pre_apply_operation_conn = db.add_pre_apply_operation_handler(
        [&](const operation_notification &note) {
          try {
            my->on_pre_apply_operation(note);
          }
          FC_LOG_AND_RETHROW()
        },
        *this, 0);
    ...
    my->_skip.skip_reject_not_enough_rc =
        options.at("rc-skip-reject-not-enough-rc").as<bool>();
    state_opts["rc-compute-historical-rc"] =
        options.at("rc-compute-historical-rc").as<bool>();
#ifndef IS_TEST_NET
    ...
    ilog("RC's will be computed starting at block ${b}",
         ("b", my->_enable_at_block));
  }
...
void rc_plugin::validate_database() { my->validate_database(); }

exp_rc_data::exp_rc_data() {}
exp_rc_data::~exp_rc_data() {}

void exp_rc_data::to_variant(fc::variant &v) const { fc::to_variant(*this, v); }

int64_t get_maximum_rc(const account_object &account,
                       const rc_account_object &rc_account) {
  int64_t result = account.vesting_shares.amount.value;
  result =
      fc::signed_sat_sub(result, account.delegated_vesting_shares.amount.value);
  result =
      fc::signed_sat_add(result, account.received_vesting_shares.amount.value);
  result = fc::signed_sat_add(
      result, rc_account.max_rc_creation_adjustment.amount.value);
  result =
      fc::signed_sat_sub(result, detail::get_next_vesting_withdrawal(account));
  return result;
}
