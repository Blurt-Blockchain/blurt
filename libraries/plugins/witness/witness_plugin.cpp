...
public:
  ...
  std::map<blurt::protocol::public_key_type, fc::ecc::private_key>
      _private_keys;
...
struct comment_options_extension_visitor {
  ...
  void operator()(const comment_payout_beneficiaries &cpb) const {
    BLURT_ASSERT(cpb.beneficiaries.size() <= 8, plugin_exception,
                 "Cannot specify more than 8 beneficiaries.");
  }
};

void check_memo(const string &memo, const chain::account_object &account,
...
struct operation_visitor {
  ...
  void operator()(const comment_operation &o) const {
    if (o.parent_author != BLURT_ROOT_POST_PARENT) {
      ...
      if (parent != nullptr)
        BLURT_ASSERT(
            parent->depth < BLURT_SOFT_MAX_COMMENT_DEPTH, plugin_exception,
            "Comment is nested ${x} posts deep, maximum depth is ${y}.",
            ("x", parent->depth)("y", BLURT_SOFT_MAX_COMMENT_DEPTH));
    }
  }

  void operator()(const transfer_operation &o) const {
    if (o.memo.length() > 0)
      check_memo(o.memo, _db.get<chain::account_object, chain::by_name>(o.from),
...
void witness_plugin_impl::on_pre_apply_operation(
...
void witness_plugin_impl::on_post_apply_operation(
    const chain::operation_notification &note) {
  ...
  case operation::tag<custom_binary_operation>::value:
    if (_db.is_producing()) {
      ...
      for (const account_name_type &account : impacted) {
        ...
        // the exception thrown by db.create() when violating uniqueness
        // (std::logic_error).
        //
        // This alternative implementation isn't "idiomatic" (i.e. AFAICT no
        // existing code uses this approach).  However, it may improve
        // performance.

        const witness_custom_op_object *coo =
            _db.find<witness_custom_op_object, by_account>(account);

        if (!coo) {
          _db.create<witness_custom_op_object>(
              [&](witness_custom_op_object &o) {
                o.account = account;
                o.count = 1;
              });
        } else {
          BLURT_ASSERT(coo->count < WITNESS_CUSTOM_OP_BLOCK_LIMIT,
                       plugin_exception,
                       "Account ${a} already submitted ${n} custom json "
                       "operation(s) this block.",
                       ("a", account)("n", WITNESS_CUSTOM_OP_BLOCK_LIMIT));

          _db.modify(*coo, [&](witness_custom_op_object &o) { o.count++; });
        }
      }
    }
...
void witness_plugin_impl::on_post_apply_block(const block_notification &note) {
  const auto &idx =
      _db.get_index<witness_custom_op_index>().indices().get<by_id>();
  while (true) {
    auto it = idx.begin();
    if (it == idx.end())
      break;
...
void witness_plugin_impl::schedule_production_loop() {
  ...
  int64_t time_to_sleep =
      BLOCK_PRODUCTION_LOOP_SLEEP_TIME -
      (now.time_since_epoch().count() % BLOCK_PRODUCTION_LOOP_SLEEP_TIME);
  ...
  _timer.async_wait(
      boost::bind(&witness_plugin_impl::block_production_loop, this));
}

block_production_condition::block_production_condition_enum
witness_plugin_impl::block_production_loop() {
  if (fc::time_point::now() < fc::time_point(BLURT_GENESIS_TIME)) {
    wlog("waiting until genesis time to produce block: ${t}",
         ("t", BLURT_GENESIS_TIME));
  ...
  case block_production_condition::not_synced:
    //         ilog("Not producing block because production is disabled until we
    //         receive a recent block (see: --enable-stale-production)");
    break;
  ...
  case block_production_condition::lag:
    elog("Not producing block because node didn't wake up within ${t}ms of the "
         "slot time.",
...
block_production_condition::block_production_condition_enum
witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object &capture) {
  ...
  chain::public_key_type scheduled_key =
      _db.get<chain::witness_object, chain::by_name>(scheduled_witness)
          .signing_key;
  ...
  if (llabs((scheduled_time - now).count()) >
      fc::milliseconds(BLOCK_PRODUCING_LAG_TIME).count()) {
    capture("scheduled_time", scheduled_time)("now", now);
    return block_production_condition::lag;
  }

  auto block = _chain_plugin.generate_block(scheduled_time, scheduled_witness,
                                            private_key_itr->second,
                                            _production_skip_flags);
  capture("n", block.block_num())("t", block.timestamp)("c", now);

  appbase::app().get_plugin<blurt::plugins::p2p::p2p_plugin>().broadcast_block(
      block);
...
void witness_plugin::set_program_options(
  ...
  cfg.add_options()("enable-stale-production",
      ...
      "witness-skip-enforce-bandwidth", bpo::value<bool>()->default_value(true),
      "Skip enforcing bandwidth restrictions. Default is true in favor of "
      "rc_plugin.");
  cli.add_options()("enable-stale-production",
                    bpo::bool_switch()->default_value(false),
                    "Enable block production, even if the chain is stale.")(
      "witness-skip-enforce-bandwidth", bpo::bool_switch()->default_value(true),
      "Skip enforcing bandwidth restrictions. Default is true in favor of "
      "rc_plugin.");
}

void witness_plugin::plugin_initialize(
    const boost::program_options::variables_map &options) {
  try {
    ilog("Initializing witness plugin");
    my = std::make_unique<detail::witness_plugin_impl>(
        appbase::app().get_io_service());

    my->_chain_plugin.register_block_generator(get_name(), my->_block_producer);

    BLURT_LOAD_VALUE_SET(options, "witness", my->_witnesses,
                         blurt::protocol::account_name_type)
    ...
    if (options.count("private-key")) {
      ...
      for (const std::string &wif_key : keys) {
        fc::optional<fc::ecc::private_key> private_key =
            blurt::utilities::wif_to_key(wif_key);
    ...
    if (my->_witnesses.size() > 0) {
      ...
      FC_ASSERT(
          !appbase::app()
               .get_plugin<rc::rc_plugin>()
               .get_rc_plugin_skip_flags()
               .skip_reject_not_enough_rc,
          "rc-skip-reject-not-enough-rc=false is required to produce blocks");
    }

    if (options.count("required-participation")) {
      my->_required_witness_participation =
          BLURT_1_PERCENT * options.at("required-participation").as<uint32_t>();
    }
...
void witness_plugin::plugin_startup() {
  try {
    ...
    if (!my->_witnesses.empty()) {
      ilog("Launching block production for ${n} witnesses.",
           ("n", my->_witnesses.size()));
      appbase::app()
          .get_plugin<blurt::plugins::p2p::p2p_plugin>()
      ...
      elog("No witnesses configured! Please add witness IDs and private keys "
           "to configuration.");
