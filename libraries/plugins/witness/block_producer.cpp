...
chain::signed_block block_producer::generate_block(
  ...
  blurt::chain::detail::with_skip_flags(_db, skip, [&]() {
    try {
...
void block_producer::adjust_hardfork_version_vote(
    const chain::witness_object &witness, chain::signed_block &pending_block) {
  using namespace blurt::protocol;

  if (witness.running_version != BLURT_BLOCKCHAIN_VERSION)
    pending_block.extensions.insert(
        block_header_extensions(BLURT_BLOCKCHAIN_VERSION));
  ...
  if (hfp.current_hardfork_version <
          BLURT_BLOCKCHAIN_VERSION // Binary is newer hardfork than has been
                                   // applied
      && (witness.hardfork_version_vote !=
              hf_versions.versions[hfp.last_hardfork + 1] ||
          witness.hardfork_time_vote !=
              hf_versions
                  .times[hfp.last_hardfork + 1])) // Witness vote does not match
                                                  // binary configuration
  {
    // Make vote match binary configuration
    pending_block.extensions.insert(block_header_extensions(
        hardfork_version_vote(hf_versions.versions[hfp.last_hardfork + 1],
                              hf_versions.times[hfp.last_hardfork + 1])));
  } else if (hfp.current_hardfork_version ==
                 BLURT_BLOCKCHAIN_VERSION // Binary does not know of a new
                                          // hardfork
             &&
             witness.hardfork_version_vote >
                 BLURT_BLOCKCHAIN_VERSION) // Voting for hardfork in the future,
                                           // that we do not know of...
  {
    // Make vote match binary configuration. This is vote to not apply the new
    // hardfork.
    pending_block.extensions.insert(block_header_extensions(
        hardfork_version_vote(hf_versions.versions[hfp.last_hardfork],
                              hf_versions.times[hfp.last_hardfork])));
  }
}

void block_producer::apply_pending_transactions(
    const chain::account_name_type &witness_owner, fc::time_point_sec when,
    chain::signed_block &pending_block) {
  ...
  for (const chain::signed_transaction &tx : _db._pending_tx) {
    // Only include transactions that have not expired yet for currently
    // generating block, this should clear problem transactions and allow block
    ...
    try {
      ...
      // wlog( "Transaction was not processed while generating block due to
      // ${e}", ("e", e) ); wlog( "The transaction was ${t}", ("t", tx) );
    }
  }
  if (postponed_tx_count > 0) {
    wlog("Postponed ${n} transactions due to block size limit",
         ("n", _db._pending_tx.size() - pending_block.transactions.size()));
  }
