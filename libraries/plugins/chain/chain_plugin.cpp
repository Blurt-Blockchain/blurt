...
struct generate_block_request {
  generate_block_request(const fc::time_point_sec w,
                         const account_name_type &wo,
                         const fc::ecc::private_key &priv_key, uint32_t s)
      : when(w), witness_owner(wo), block_signing_private_key(priv_key),
        skip(s) {}
...
typedef fc::static_variant<const signed_block *, const signed_transaction *,
                           generate_block_request *>
    write_request_ptr;
typedef fc::static_variant<boost::promise<void> *, fc::future<void> *>
    promise_ptr;
...
struct write_request_visitor {
  ...
  bool operator()(generate_block_request *req) {
    ...
    try {
      if (!block_generator)
        FC_THROW_EXCEPTION(chain_exception,
                           "Received a generate block request, but no block "
      ...
      req->block = block_generator->generate_block(
          req->when, req->witness_owner, req->block_signing_private_key,
          req->skip);
...
struct request_promise_visitor {
  ...
  template <typename T> void operator()(T *t) { t->set_value(); }
};
...
void chain_plugin_impl::start_write_processing() {
  write_processor_thread = std::make_shared<std::thread>([&]() {
    ...
    /* This loop monitors the write request queue and performs writes to the
     * database. These can be blocks or pending transactions. Because the caller
     * needs to know the success of the write and any exceptions that are
     * thrown, a write context is passed in the queue to the processing thread
     * which it will use to store the results of the write. It is the caller's
     * responsibility to ensure the pointer to the write context remains valid
     * until the contained promise is complete.
     *
     * The loop has two modes, sync mode and live mode. In sync mode we want to
     * process writes as quickly as possible with minimal overhead. The outer
     * loop busy waits on the queue and the inner loop drains the queue as
     * quickly as possible. We exit sync mode when the head block is within 1
     * minute of system time.
     *
     * Live mode needs to balance between processing pending writes and allowing
     * readers access to the database. It will batch writes together as much as
     * possible to minimize lock overhead but will willingly give up the write
     * lock after 500ms. The thread then sleeps for 10ms. This allows time for
     * readers to access the database as well as more writes to come in. When
     * the node is live the rate at which writes come in is slower and busy
     * waiting is not an optimal use of system resources when we could give CPU
     * time to read threads.
     */
    while (running) {
      if (!is_syncing)
          ...
          while (true) {
            ...
            if (!is_syncing && write_lock_hold_time >= 0 &&
                fc::time_point::now() - start >
                    fc::milliseconds(write_lock_hold_time)) {
              break;
            }
...
void chain_plugin::set_program_options(options_description &cli,
  ...
  cfg.add_options()("sps-remove-threshold",
                    bpo::value<uint16_t>()->default_value(200),
                    "Maximum numbers of proposals/votes which can be removed "
                    "in the same cycle")(
      ...
      "shared-file-size", bpo::value<string>()->default_value("8G"),
      "Size of the shared memory file. Default: 8G. If running a full node, "
      "increase this value to 16G.")(
      "shared-file-full-threshold", bpo::value<uint16_t>()->default_value(0),
      "A 2 precision percentage (0-10000) that defines the threshold for when "
      "to autoscale the shared memory file. Setting this to 0 disables "
      "autoscaling. Recommended value for consensus node is 9500 (95%). Full "
      "node is 9900 (99%)")(
      "shared-file-scale-rate", bpo::value<uint16_t>()->default_value(0),
      "A 2 precision percentage (0-10000) that defines how quickly to scale "
      "the shared memory file. When autoscaling occurs the file's size will be "
      "increased by this percent. Setting this to 0 disables autoscaling. "
      "Recommended value is between 1000-2000 (10-20%)")(
...
#endif
          ("spam-accounts", bpo::value<vector<string>>()->composing(),
           "Defines a list of accounts which will be explicitly ignored in "
           "account history storage and post content. Eg., spam-accounts = "
           "account1 account2");
  cli.add_options()(
      "sps-remove-threshold", bpo::value<uint16_t>()->default_value(200),
      "Maximum numbers of proposals/votes which can be removed in the same "
      "cycle")("replay-blockchain", bpo::bool_switch()->default_value(false),
               "clear chain database and replay all blocks")(
      "force-open", bpo::bool_switch()->default_value(false),
      ...
      "set-benchmark-interval", bpo::value<uint32_t>(),
      "Print time and memory usage every given number of blocks")(
      "dump-memory-details", bpo::bool_switch()->default_value(false),
      "Dump database objects memory usage info. Use set-benchmark-interval to "
      "set dump interval.")("check-locks",
...
#ifdef ENABLE_MIRA
      ("database-cfg", bpo::value<bfs::path>()->default_value("database.cfg"),
       "The database configuration file location")(
          "memory-replay,m", bpo::bool_switch()->default_value(false),
          "Replay with state in memory instead of on disk")
#endif
#ifdef IS_TEST_NET
          ("chain-id", bpo::value<std::string>()->default_value(BLURT_CHAIN_ID),
...
void chain_plugin::plugin_initialize(const variables_map &options) {
  ...
  my->shared_memory_size =
      fc::parse_size(options.at("shared-file-size").as<string>());

  if (options.count("shared-file-full-threshold"))
    my->shared_file_full_threshold =
        options.at("shared-file-full-threshold").as<uint16_t>();

  if (options.count("shared-file-scale-rate"))
    my->shared_file_scale_rate =
        options.at("shared-file-scale-rate").as<uint16_t>();

  my->sps_remove_threshold = options.at("sps-remove-threshold").as<uint16_t>();

  my->chainbase_flags |= options.at("force-open").as<bool>()
                             ? chainbase::skip_env_check
                             : chainbase::skip_nothing;

  my->replay = options.at("replay-blockchain").as<bool>();
  my->resync = options.at("resync-blockchain").as<bool>();
  my->stop_replay_at = options.count("stop-replay-at-block")
                           ? options.at("stop-replay-at-block").as<uint32_t>()
                           : 0;
  my->benchmark_interval =
      options.count("set-benchmark-interval")
          ? options.at("set-benchmark-interval").as<uint32_t>()
          : 0;
  my->check_locks = options.at("check-locks").as<bool>();
  my->validate_invariants =
      options.at("validate-database-invariants").as<bool>();
  ...
  if (options.count("memory-replay-indices")) {
    std::vector<std::string> indices =
        options.at("memory-replay-indices").as<vector<string>>();
    for (auto &element : indices) {
      std::vector<std::string> tmp;
      boost::split(tmp, element, boost::is_any_of("\t "));
      my->replay_memory_indices.insert(my->replay_memory_indices.end(),
                                       tmp.begin(), tmp.end());
    }
  }
#endif
  ...
  if (options.count("chain-id")) {
    ...
    try {
      ...
      FC_ASSERT(false,
                "Could not parse chain_id as hex string. Chain ID String: ${s}",
                ("s", chain_id_str));
    }
  }
#endif
...
void chain_plugin::plugin_startup() {
  if (my->statsd_on_replay) {
    auto statsd =
        appbase::app().find_plugin<blurt::plugins::statsd::statsd_plugin>();
    if (statsd != nullptr) {
      statsd->start_logging();
    }
  }

  ilog("Starting chain with shared_file_size: ${n} bytes",
       ("n", my->shared_memory_size));
  ...
  typedef blurt::utilities::benchmark_dumper::index_memory_details_cntr_t
      index_memory_details_cntr_t;
  auto get_indexes_memory_details =
      [dump_memory_details, &abstract_index_cntr](
        ...
        for (auto idx : abstract_index_cntr) {
          ...
          index_memory_details_cntr.emplace_back(
              std::move(info._value_type_name), info._item_count,
              info._item_sizeof, info._item_additional_allocation,
              info._additional_container_allocation);
        }
      };
  ...
  try {
    database_config =
        fc::json::from_file(my->database_cfg, fc::json::strict_parser);
  ...
  auto benchmark_lambda = [&dumper, &get_indexes_memory_details,
                           dump_memory_details](
                              uint32_t current_block_number,
                              const chainbase::database::abstract_index_cntr_t
                                  &abstract_index_cntr) {
    if (current_block_number == 0) // initial call
    {
      typedef blurt::utilities::benchmark_dumper::database_object_sizeof_cntr_t
          database_object_sizeof_cntr_t;
      auto get_database_objects_sizeofs =
          [dump_memory_details, &abstract_index_cntr](
            ...
            for (auto idx : abstract_index_cntr) {
              ...
              database_object_sizeof_cntr.emplace_back(
                  std::move(info._value_type_name), info._item_sizeof);
            }
          };
    ...
    ilog("Performance report at block ${n}. Elapsed time: ${rt} ms (real), "
         "${ct} ms (cpu). Memory usage: ${cm} (current), ${pm} (peak) "
         "kilobytes.",
         ("n", current_block_number)("rt", measure.real_ms)("ct",
                                                            measure.cpu_ms)(
  ...
  if (my->replay) {
    ...
    db_open_args.benchmark = blurt::chain::database::TBenchmark(
        my->benchmark_interval, benchmark_lambda);
    last_block_number = my->db.reindex(db_open_args);

    if (my->benchmark_interval > 0) {
      const blurt::utilities::benchmark_dumper::measurement &total_data =
          dumper.dump(true, get_indexes_memory_details);
      ilog("Performance report (total). Blocks: ${b}. Elapsed time: ${rt} ms "
           "(real), ${ct} ms (cpu). Memory usage: ${cm} (current), ${pm} "
           "(peak) kilobytes.",
           ("b", total_data.block_number)("rt", total_data.real_ms)(
               "ct", total_data.cpu_ms)("cm", total_data.current_mem)(
               "pm", total_data.peak_mem));
    }

    if (my->stop_replay_at > 0 && my->stop_replay_at == last_block_number) {
      ilog("Stopped blockchain replaying on user request. Last applied block "
           "number: ${n}.",
           ("n", last_block_number));
      exit(EXIT_SUCCESS);
    }
    ...
    db_open_args.benchmark = blurt::chain::database::TBenchmark(
        dump_memory_details, benchmark_lambda);

    try {
      ilog("Opening shared memory from ${path}",
           ("path", my->shared_memory_dir.generic_string()));
      ...
      wlog(
          "Error opening database. If the binary or configuration has changed, "
          "replay the blockchain explicitly using `--replay-blockchain`.");
      wlog("If you know what you are doing you can skip this check and force "
           "open the database using `--force-open`.");
      wlog("WARNING: THIS MAY CORRUPT YOUR DATABASE. FORCE OPEN AT YOUR OWN "
           "RISK.");
      wlog(" Error: ${e}", ("e", e));
      exit(EXIT_FAILURE);
    }
  ...
  ilog("Started on blockchain with ${n} blocks",
...
void chain_plugin::report_state_options(const string &plugin_name,
                                        const fc::variant_object &opts) {
  my->loaded_plugins.push_back(plugin_name);
  my->plugin_state_opts(opts);
}

bool chain_plugin::accept_block(const blurt::chain::signed_block &block,
                                bool currently_syncing, uint32_t skip) {
...
void chain_plugin::accept_transaction(
    const blurt::chain::signed_transaction &trx) {
  boost::promise<void> prom;
  write_context cxt;
  cxt.req_ptr = &trx;
  cxt.prom_ptr = &prom;
...
blurt::chain::signed_block chain_plugin::generate_block(
    const fc::time_point_sec when, const account_name_type &witness_owner,
    const fc::ecc::private_key &block_signing_private_key, uint32_t skip) {
  generate_block_request req(when, witness_owner, block_signing_private_key,
                             skip);
  boost::promise<void> prom;
  write_context cxt;
  cxt.req_ptr = &req;
...
int16_t chain_plugin::set_write_lock_hold_time(int16_t new_time) {
  FC_ASSERT(get_state() == appbase::abstract_plugin::state::initialized,
            "Can only change write_lock_hold_time while chain_plugin is "
            "initialized.");
...
bool chain_plugin::block_is_on_preferred_chain(
    const blurt::chain::block_id_type &block_id) {
  ...
  // from the database. If the database's block ID matches block_id, then
  // block_id is on the preferred chain. Otherwise, it's on a fork.
  return db().get_block_id_for_num(
             blurt::chain::block_header::num_from_id(block_id)) == block_id;
}

void chain_plugin::check_time_in_block(
    const blurt::chain::signed_block &block) {
...
void chain_plugin::register_block_generator(
    const std::string &plugin_name,
    std::shared_ptr<abstract_block_producer> block_producer) {
  FC_ASSERT(get_state() == appbase::abstract_plugin::state::initialized,
            "Can only register a block generator when the chain_plugin is "
            "initialized.");

  if (my->block_generator)
    wlog("Overriding a previously registered block generator by: ${registrant}",
         ("registrant", my->block_generator_registrant));
