...
public:
  ...
  static const std::string &name() {
    static std::string name = BLURT_CHAIN_PLUGIN_NAME;
    return name;
  }
  ...
  void report_state_options(const string &plugin_name,
                            const fc::variant_object &opts);

  bool accept_block(const blurt::chain::signed_block &block,
                    bool currently_syncing, uint32_t skip);
  void accept_transaction(const blurt::chain::signed_transaction &trx);
  blurt::chain::signed_block
  generate_block(const fc::time_point_sec when,
  ...
  const ObjectType *
  find(chainbase::oid<ObjectType> key = chainbase::oid<ObjectType>()) {
  ...
  template <typename ObjectType>
  const ObjectType &
  get(const chainbase::oid<ObjectType> &key = chainbase::oid<ObjectType>()) {
    return db().get<ObjectType>(key);
  }

  // Exposed for backwards compatibility. In the future, plugins should manage
  // their own internal database
  database &db();
  const database &db() const;

  // Emitted when the blockchain is syncing/live.
  // This is to synchronize plugins that have the chain plugin as an optional
  // dependency.
