...
public:
  ...
  static const std::string &name() {
    static std::string name = BLURT_DEBUG_NODE_PLUGIN_NAME;
    return name;
  }
  ...
  void debug_update(Lambda &&callback,
                    uint32_t skip = blurt::chain::database::skip_nothing) {
    ...
    if (it == _debug_updates.end())
      it = _debug_updates
               .emplace(head_id,
                        std::vector<std::function<void(chain::database &)>>())
               .first;
    it->second.emplace_back(callback);

    fc::optional<chain::signed_block> head_block =
        db.fetch_block_by_id(head_id);
    FC_ASSERT(head_block.valid());

    // What the last block does has been changed by adding to
    // node_property_object, so we have to re-apply it
    db.pop_block();
    db.push_block(*head_block, skip);
  }

  void debug_generate_blocks(debug_generate_blocks_return &ret,
                             const debug_generate_blocks_args &args);

  uint32_t
  debug_generate_blocks(const std::string &debug_key, uint32_t count,
                        uint32_t skip = blurt::chain::database::skip_nothing,
                        uint32_t miss_blocks = 0);
...
private:
  ...
  // std::map< protocol::block_id_type, std::vector< fc::variant_object > >
  // _debug_updates;
  std::map<protocol::block_id_type,
           std::vector<std::function<void(chain::database &)>>>
      _debug_updates;
};
...
FC_REFLECT(blurt::plugins::debug_node::debug_generate_blocks_args,
           (debug_key)(count)(skip)(miss_blocks)(edit_if_needed))
FC_REFLECT(blurt::plugins::debug_node::debug_generate_blocks_return, (blocks))
