...
void debug_node_plugin::set_program_options(options_description &cli,
  ...
  cfg.add_options()(
      ...
      boost::program_options::value<std::vector<std::string>>()->composing(),
      "Database edits to apply on startup (may specify multiple times). "
      "Deprecated in favor of debug-node-edit-script.");
}
...
void debug_node_plugin::plugin_initialize(const variables_map &options) {
  ...
  if (options.count("debug-node-edit-script") > 0) {
    _edit_scripts =
        options.at("debug-node-edit-script").as<std::vector<std::string>>();
  }
  ...
  my->_post_apply_block_conn = my->_db.add_post_apply_block_handler(
      [this](const chain::block_notification &note) {
...
void debug_node_plugin::plugin_startup() {
  ...
  {
     std::shared_ptr< fc::ifstream > stream = std::make_shared< fc::ifstream >(
  fc::path(fn) ); fc::buffered_istream bstream(stream); fc::variant v =
  fc::json::from_stream( bstream, fc::json::strict_parser ); load_debug_updates(
  v.get_object() );
  }*/
}
...
/*
void debug_apply_update( chain::database& db, const fc::variant_object& vo, bool
logging )
{
   ...
   // "_action" : "create"   object must not exist, unspecified fields take
defaults
   // "_action" : "write"    object may exist, is replaced entirely, unspecified
...
uint32_t debug_node_plugin::debug_generate_blocks(const std::string &debug_key,
                                                  uint32_t count, uint32_t skip,
...
void debug_node_plugin::debug_generate_blocks(
    debug_generate_blocks_return &ret, const debug_generate_blocks_args &args) {
  if (args.count == 0) {
    ret.blocks = 0;
    return;
  }
  ...
  while (produced < args.count) {
    ...
    const chain::witness_object &scheduled_witness =
        db.get_witness(scheduled_witness_name);
    chain::public_key_type scheduled_key = scheduled_witness.signing_key;
    if (logging) {
      wlog("slot: ${sl}   time: ${t}   scheduled key is: ${sk}   dbg key is: "
           "${dk}",
           ("sk", scheduled_key)("dk", debug_public_key)("sl", slot)(
               "t", scheduled_time));
    }
    ...
    bp.generate_block(scheduled_time, scheduled_witness_name,
                      *debug_private_key, args.skip);
    ++produced;
    slot = new_slot;
  }
...
uint32_t debug_node_plugin::debug_generate_blocks_until(
    const std::string &debug_key, const fc::time_point_sec &head_block_time,
...
void debug_node_plugin::on_post_apply_block(
...
{
   for( const std::pair< chain::block_id_type, std::vector< fc::variant_object >
>& update : _debug_updates )
   {
      fc::variant v;
