  ...
  webserver_plugin_impl(thread_pool_size_t thread_pool_size)
    ...
    for (uint32_t i = 0; i < thread_pool_size; ++i)
      thread_pool.create_thread(
          boost::bind(&asio::io_service::run, &thread_pool_ios));
  }

  void start_webserver();
  void stop_webserver();

  void handle_ws_message(websocket_server_type *, connection_hdl,
                         detail::websocket_server_type::message_ptr);
...
void webserver_plugin_impl::start_webserver() {
    ...
    ws_thread = std::make_shared<std::thread>([&]() {
      ...
      try {
        ...
        ws_server.set_message_handler(
            boost::bind(&webserver_plugin_impl::handle_ws_message, this,
                        &ws_server, _1, _2));

        if (http_endpoint && http_endpoint == ws_endpoint) {
          ws_server.set_http_handler(
              boost::bind(&webserver_plugin_impl::handle_http_message, this,
                          &ws_server, _1));
  ...
  if (http_endpoint &&
      ((ws_endpoint && ws_endpoint != http_endpoint) || !ws_endpoint)) {
...
void webserver_plugin_impl::handle_ws_message(
    ...
    detail::websocket_server_type::message_ptr msg) {
...
void webserver_plugin_impl::handle_http_message(websocket_server_type *server,
                                                connection_hdl hdl) {
  auto con = server->get_con_from_hdl(hdl);
...
void webserver_plugin::set_program_options(options_description &,
                                           options_description &cfg) {
  cfg.add_options()("webserver-http-endpoint",
                    bpo::value<string>()->default_value("127.0.0.1:8091"),
                    "Local http endpoint for webserver requests.")(
      "webserver-ws-endpoint",
      bpo::value<string>()->default_value("127.0.0.1:8090"),
      "Local websocket endpoint for webserver requests.")(
      "rpc-endpoint", bpo::value<string>(),
      "Local http and websocket endpoint for webserver requests. Deprecated in "
      "favor of webserver-http-endpoint and webserver-ws-endpoint")(
      "webserver-thread-pool-size",
      bpo::value<thread_pool_size_t>()->default_value(32),
...
void webserver_plugin::plugin_initialize(const variables_map &options) {
  ...
  auto thread_pool_size =
      options.at("webserver-thread-pool-size").as<thread_pool_size_t>();
  FC_ASSERT(thread_pool_size > 0,
            "webserver-thread-pool-size must be greater than 0");
  ...
  if (options.count("webserver-http-endpoint")) {
    ...
    FC_ASSERT(endpoints.size(),
              "webserver-http-endpoint ${hostname} did not resolve",
              ("hostname", http_endpoint));
    my->http_endpoint = tcp::endpoint(boost::asio::ip::address_v4::from_string(
                                          (string)endpoints[0].get_address()),
                                      endpoints[0].port());
  ...
  if (options.count("webserver-ws-endpoint")) {
    ...
    FC_ASSERT(endpoints.size(),
              "ws-server-endpoint ${hostname} did not resolve",
              ("hostname", ws_endpoint));
    my->ws_endpoint = tcp::endpoint(boost::asio::ip::address_v4::from_string(
                                        (string)endpoints[0].get_address()),
                                    endpoints[0].port());
  ...
  if (options.count("rpc-endpoint")) {
    ...
    FC_ASSERT(endpoints.size(), "rpc-endpoint ${hostname} did not resolve",
              ("hostname", endpoint));

    auto tcp_endpoint = tcp::endpoint(boost::asio::ip::address_v4::from_string(
                                          (string)endpoints[0].get_address()),
                                      endpoints[0].port());
...
void webserver_plugin::plugin_startup() {
  ...
  plugins::chain::chain_plugin *chain =
      appbase::app().find_plugin<plugins::chain::chain_plugin>();
  if (chain != nullptr &&
      chain->get_state() != appbase::abstract_plugin::started) {
    ilog("Waiting for chain plugin to start");
    my->chain_sync_con =
        chain->on_sync.connect(0, [this]() { my->start_webserver(); });
  } else {
    my->start_webserver();
  }
}

void webserver_plugin::plugin_shutdown() { my->stop_webserver(); }
