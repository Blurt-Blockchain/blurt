...
class block_data_export_plugin
    : public appbase::plugin<block_data_export_plugin> {
public:
  ...
  static const std::string &name() {
    static std::string name = BLURT_BLOCK_DATA_EXPORT_PLUGIN_NAME;
    return name;
  }
  ...
  void add_abstract_export_data(const std::string &name,
                                std::shared_ptr<exportable_block_data> data);
  ...
  std::shared_ptr<T> find_export_data(const std::string &name) {
    std::shared_ptr<exportable_block_data> adata =
        find_abstract_export_data(name);
    if (!adata)
  ...
  std::shared_ptr<T> get_or_create_export_data(const std::string &name,
                                               const CtorArg &arg) {
  ...
  void register_export_data_type(const std::string &name) {
    register_export_data_factory(
        name, []() -> std::shared_ptr<exportable_block_data> {
