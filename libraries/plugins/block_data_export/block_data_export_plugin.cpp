...
struct api_export_data_object {
  ...
  flat_map<string, std::shared_ptr<exportable_block_data>> export_data;
...
FC_REFLECT(blurt::plugins::block_data_export::detail::api_export_data_object,
           (block_id)(previous)(export_data))
...
struct work_item {
  ...
  boost::future<std::shared_ptr<std::string>> edo_json_future =
      edo_json_promise.get_future();
};
...
public:
  ...
  std::shared_ptr<api_export_data_object> _edo;
  std::vector<std::pair<
      string, std::function<std::shared_ptr<exportable_block_data>()>>>
      _factory_list;
  ...
  boost::concurrent::sync_bounded_queue<std::shared_ptr<work_item>>
      _output_queue;
...
void block_data_export_plugin_impl::start_threads() {
  ...
  for (size_t i = 0; i < num_threads; i++) {
    _json_conversion_threads.emplace_back(
        attrs, [this]() { convert_to_json_thread_main(); });
  }

  _output_thread = std::make_shared<boost::thread>(
      attrs, [this]() { output_thread_main(); });
}

void block_data_export_plugin_impl::stop_threads() {
  //
  // We must close the output queue first:  The output queue may be waiting on a
  // future. If the conversion threads are still alive, the future will
  // complete, the output thread will then wait on _output_queue and see close()
  // has been called.
  //
  // (If we closed the conversion threads first, the future would never
  // complete, and the output thread would wait forever.)
  //
  _output_queue.close();
  _output_thread->join();
...
void block_data_export_plugin_impl::convert_to_json_thread_main() {
  while (true) {
    ...
    std::shared_ptr<std::string> edo_json =
        std::make_shared<std::string>(fc::json::to_string(work->edo));
...
void block_data_export_plugin_impl::create_export_data(
    const block_id_type &previous, const block_id_type &block_id) {
  _edo.reset();
  if (!_enabled)
    return;
...
void block_data_export_plugin_impl::send_export_data() {
  ...
  try {
    ...
    // We should never see this exception because we should be done handling
    // blocks by the time we're closing queues
    elog("Caught unexpected sync_queue_is_closed in "
         "block_data_export_plugin_impl::push_work()");
  }
  return;
}
...
void block_data_export_plugin_impl::on_pre_apply_block(
    const block_notification &note) {
  create_export_data(note.block.previous, note.block_id);
}

void block_data_export_plugin_impl::on_post_apply_block(
    const block_notification &note) {
  send_export_data();
}
...
std::shared_ptr<exportable_block_data>
block_data_export_plugin::find_abstract_export_data(const std::string &name) {
  return my->find_abstract_export_data(name);
}

void block_data_export_plugin::set_program_options(options_description &cli,
                                                   options_description &cfg) {
  cfg.add_options()(
      "block-data-export-file",
      boost::program_options::value<string>()->default_value("NONE"),
      "Where to export data (NONE to discard)");
}

void block_data_export_plugin::plugin_initialize(
  ...
  try {
    ...
    my->_pre_apply_block_conn = my->_db.add_pre_apply_block_handler(
        [&](const block_notification &note) { my->on_pre_apply_block(note); },
        *this, -9300);
    my->_post_apply_block_conn = my->_db.add_post_apply_block_handler(
        [&](const block_notification &note) { my->on_post_apply_block(note); },
        *this, 9300);
