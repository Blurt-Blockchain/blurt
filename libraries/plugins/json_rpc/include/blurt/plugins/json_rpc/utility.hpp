...
#define DECLARE_API_METHOD_HELPER(r, data, method)                             \
  BOOST_PP_CAT(method, _return)                                                \
  method(const BOOST_PP_CAT(method, _args) & args, bool lock = false);
...
#define FOR_EACH_API_HELPER(r, callback, method)                               \
  {                                                                            \
    typedef std::remove_pointer<decltype(this)>::type this_type;               \
                                                                               \
    callback((*this),                                                          \
             BOOST_PP_STRINGIZE( method ), &this_type::method,                                  \
...
#define DEFINE_API_IMPL(class, method)                                         \
  BOOST_PP_CAT(method, _return)                                                \
  class ::method(const BOOST_PP_CAT(method, _args) & args)

#define DEFINE_READ_API_HELPER(r, class, method)                               \
  BOOST_PP_CAT(method, _return)                                                \
  class ::method(const BOOST_PP_CAT(method, _args) & args, bool lock) {        \
    if (lock) {                                                                \
      return my->_db.with_read_lock(                                           \
          [&args, this]() { return my->method(args); });                       \
    } else {                                                                   \
