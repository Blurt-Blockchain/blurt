...
#define JSON_RPC_REGISTER_API(API_NAME)                                        \
  {                                                                            \
    blurt::plugins::json_rpc::detail::register_api_method_visitor vtor(        \
        API_NAME);                                                             \
    for_each_api(vtor);                                                        \
  }
...
public:
  ...
  static const std::string &name() {
    static std::string name = BLURT_JSON_RPC_PLUGIN_NAME;
    return name;
  }
...
public:
  register_api_method_visitor(const std::string &api_name)
      : _api_name(api_name),
        _json_rpc_plugin(
            appbase::app()
                .get_plugin<blurt::plugins::json_rpc::json_rpc_plugin>()) {}

  template <typename Plugin, typename Method, typename Args, typename Ret>
  void operator()(Plugin &plugin, const std::string &method_name, Method method,
                  Args *args, Ret *ret) {
    _json_rpc_plugin.add_api_method(
        _api_name, method_name,
