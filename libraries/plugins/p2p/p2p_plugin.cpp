...
resolve_string_to_ip_endpoints(const std::string &endpoint_string) {
  try {
    string::size_type colon_pos = endpoint_string.find(':');
    if (colon_pos == std::string::npos)
      FC_THROW("Missing required port number in endpoint string "
               "\"${endpoint_string}\"",
...
public:
  p2p_plugin_impl(plugins::chain::chain_plugin &c)
      : running(true), activeHandleBlock(false), activeHandleTx(false),
        chain(c) {
    handleBlockFinished.second =
        std::shared_future<void>(handleBlockFinished.first.get_future());
    handleTxFinished.second =
        std::shared_future<void>(handleTxFinished.first.get_future());
  }
  ...
  virtual bool handle_block(const graphene::net::block_message &, bool,
                            std::vector<fc::uint160_t> &) override;
  ...
  get_block_ids(const std::vector<graphene::net::item_hash_t> &, uint32_t &,
                uint32_t) override;
  virtual graphene::net::message
  get_item(const graphene::net::item_id &) override;
  virtual std::vector<graphene::net::item_hash_t>
  get_blockchain_synopsis(const graphene::net::item_hash_t &,
                          uint32_t) override;
  virtual void sync_status(uint32_t, uint32_t) override;
  virtual void connection_count_changed(uint32_t) override;
  virtual uint32_t
  get_block_number(const graphene::net::item_hash_t &) override;
  virtual fc::time_point_sec
  get_block_time(const graphene::net::item_hash_t &) override;
  ...
  virtual void error_encountered(const std::string &message,
...
bool p2p_plugin_impl::handle_block(const graphene::net::block_message &blk_msg,
                                   bool sync_mode,
    ...
    if (running.load()) {
      ...
      if (sync_mode)
        fc_ilog(fc::logger::get("sync"),
                "chain pushing sync block #${block_num} ${block_hash}, head is "
                "${head}",
                ("block_num", blk_msg.block.block_num())(
                    "block_hash", blk_msg.block_id)("head", head_block_num));
      else
        fc_ilog(
            fc::logger::get("sync"),
      ...
      try {
        // TODO: in the case where this block is valid but on a fork that's too
        // old for us to switch to, you can help the network code out by
        // throwing a block_older_than_undo_history exception. when the net code
        // sees that, it will stop trying to push blocks from that chain, but
        // leave that peer connected so that they can get sync blocks from us
        bool result = chain.accept_block(
            blk_msg.block, sync_mode,
            (block_producer | force_validate)
                ? chain::database::skip_nothing
                : chain::database::skip_transaction_signatures);

        if (!sync_mode) {
          fc::microseconds offset =
              fc::time_point::now() - blk_msg.block.timestamp;
          STATSD_TIMER("p2p", "offset", "block_arrival", offset, 1.0f)
          ilog("Got ${t} transactions on block ${b} by ${w} -- Block Time "
               "Offset: ${l} ms",
               ("t", blk_msg.block.transactions.size())(
                   "b", blk_msg.block.block_num())("w", blk_msg.block.witness)(
                   "l", offset.count() / 1000));
        }
...
void p2p_plugin_impl::handle_transaction(
    const graphene::net::trx_message &trx_msg) {
  if (running.load()) {
    try {
...
void p2p_plugin_impl::handle_message(
    const graphene::net::message &message_to_process) {
  // not a transaction, not a block
  FC_THROW("Invalid Message Type");
}

std::vector<graphene::net::item_hash_t> p2p_plugin_impl::get_block_ids(
  ...
  try {
    return chain.db().with_read_lock([&]() {
      ...
      if (blockchain_synopsis.empty() ||
        ...
        // A bug in old versions would cause them to send a synopsis containing
        // block 000000000 when they had an empty blockchain, so pretend they
        // sent the right thing here. do nothing, leave last_known_block_id set
        // to zero
      } else {
        bool found_a_block_in_synopsis = false;

        for (const item_hash_t &block_id_in_synopsis :
             boost::adaptors::reverse(blockchain_synopsis)) {
        ...
        if (!found_a_block_in_synopsis)
          FC_THROW_EXCEPTION(graphene::net::peer_is_on_an_unreachable_fork,
                             "Unable to provide a list of blocks starting at "
                             "any of the blocks in peer's synopsis");
      }
      ...
      if (!result.empty() && block_header::num_from_id(result.back()) <
                                 chain.db().head_block_num())
        remaining_item_count = chain.db().head_block_num() -
...
p2p_plugin_impl::get_item(const graphene::net::item_id &id) {
  try {
    if (id.item_type == graphene::net::block_message_type) {
      return chain.db().with_read_lock([&]() {
        auto opt_block = chain.db().fetch_block_by_id(id.item_hash);
        if (!opt_block)
          elog("Couldn't find block ${id} -- corresponding ID in our chain is "
               "${id2}",
               ("id", id.item_hash)(
                   "id2", chain.db().get_block_id_for_num(
...
std::vector<graphene::net::item_hash_t>
p2p_plugin_impl::get_blockchain_synopsis(
    ...
    chain.db().with_read_lock([&]() {
      ...
      if (reference_point != item_hash_t()) {
        ...
        if (is_included_block(reference_point)) {
          ...
          uint32_t reference_point_block_num =
              block_header::num_from_id(reference_point);
          ...
          if (reference_point_block_num < low_block_num) {
            // we're on the same fork (at least as far as reference_point) but
            // we've passed reference point and could no longer undo that far if
            // we diverged after that block.  This should probably only happen
            // due to a race condition where the network thread calls this
            // function, and then immediately pushes a bunch of blocks, then the
            // main thread finally processes this function. with the current
            // framework, there's not much we can do to tell the network thread
            // what our current head block is, so we'll just pretend that our
            // head is actually the reference point. this *may* enable us to
            // fetch blocks that we're unable to push, but that should be a rare
            // case (and correctly handled)
          ...
          try {
            ...
            // returns a vector where the last element is the common ancestor
            // with the preferred chain, and the first element is the reference
            // point you passed in
            ...
            if (last_non_fork_block ==
                block_id_type()) // if the fork goes all the way back to genesis
                                 // (does graphene's fork db allow this?)
              non_fork_high_block_num = 0;
            else
              non_fork_high_block_num =
                  block_header::num_from_id(last_non_fork_block);
            ...
            elog("Unable to construct a blockchain synopsis for reference hash "
                 "${hash}: ${exception}",
                 ("hash", reference_point)("exception", e));
            throw;
          }
          if (non_fork_high_block_num < low_block_num) {
            wlog("Unable to generate a usable synopsis because the peer we're "
                 "generating it for forked too long ago "
                 "(our chains diverge after block #${non_fork_high_block_num} "
                 "but only undoable to block #${low_block_num})",
      ...
      // non_fork_high_block_num is the block before the fork (if the peer is on
      // a fork, or otherwise it is the same as high_block_num) high_block_num
      // is the block number of the reference block, or the end of the chain if
      // no reference provided

      // true_high_block_num is the ending block number after the network code
      // appends any item ids it knows about that we don't
      uint32_t true_high_block_num =
          high_block_num + number_of_blocks_after_reference_point;
      do {
        // for each block in the synopsis, figure out where to pull the block id
        // from. if it's <= non_fork_high_block_num, we grab it from the main
        // blockchain; if it's not, we pull it from the fork history
        if (low_block_num <= non_fork_high_block_num)
          synopsis.push_back(chain.db().get_block_id_for_num(low_block_num));
        else
          synopsis.push_back(
              fork_history[low_block_num - non_fork_high_block_num - 1]);
...
uint32_t
p2p_plugin_impl::get_block_number(const graphene::net::item_hash_t &block_id) {
  try {
...
fc::time_point_sec
p2p_plugin_impl::get_block_time(const graphene::net::item_hash_t &block_id) {
  try {
    return chain.db().with_read_lock([&]() {
...
uint32_t p2p_plugin_impl::estimate_last_known_fork_from_git_revision_timestamp(
    uint32_t) const {
  return 0; // there are no forks in graphene
}

void p2p_plugin_impl::error_encountered(const string &message,
  ...
  try {
    return chain.db().with_read_lock([&]() {
      ...
      block_id_type block_id_in_preferred_chain =
          chain.db().get_block_id_for_num(block_num);
...
void p2p_plugin::set_program_options(bpo::options_description &cli,
  ...
  cfg.add_options()(
      ...
      "The IP address and port of a remote peer to sync with. Deprecated in "
      "favor of p2p-seed-node.")(
      "p2p-seed-node",
      bpo::value<vector<string>>()->composing()->default_value(default_seeds,
                                                               seed_ss.str()),
      "The IP address and port of a remote peer to sync with.")(
      "p2p-parameters", bpo::value<string>(),
      ("P2P network parameters. (Default: " +
       fc::json::to_string(graphene::net::node_configuration()) + " )")
          .c_str());
  cli.add_options()("force-validate", bpo::bool_switch()->default_value(false),
                    "Force validation of all transactions. Deprecated in favor "
                    "of p2p-force-validate")(
      "p2p-force-validate", bpo::bool_switch()->default_value(false),
      "Force validation of all transactions.");
}

void p2p_plugin::plugin_initialize(
    const boost::program_options::variables_map &options) {
  ilog("Initializing p2p_plugin");
  my = std::make_unique<detail::p2p_plugin_impl>(
      appbase::app().get_plugin<plugins::chain::chain_plugin>());

  if (options.count("p2p-endpoint"))
    my->endpoint =
        fc::ip::endpoint::from_string(options.at("p2p-endpoint").as<string>());
    ...
    for (const string &endpoint_string : seeds) {
      try {
        std::vector<fc::ip::endpoint> endpoints =
            detail::resolve_string_to_ip_endpoints(endpoint_string);
  ...
  if (options.count("p2p-parameters")) {
    fc::variant var = fc::json::from_string(
        options.at("p2p-parameters").as<string>(), fc::json::strict_parser);
      ...
      .async([this] {
        ...
        for (const auto &seed : my->seeds) {
          try {
            ...
            wlog("Already connected to seed node ${s}. Is it specified twice "
                 "in config?",
                 ("s", seed));
          }
        }
        ...
        if (my->max_connections) {
          if (my->config.find("maximum_number_of_connections") !=
              my->config.end())
            ilog("Overriding advanded_node_parameters[ "
                 "\"maximum_number_of_connections\" ] with ${cons}",
                 ("cons", my->max_connections));

          my->config.set("maximum_number_of_connections",
        ...
        my->node->sync_from(
            graphene::net::item_id(graphene::net::block_message_type, block_id),
            std::vector<uint32_t>());
        ilog("P2P node listening at ${ep}",
             ("ep", my->node->get_actual_listening_endpoint()));
      })
      .wait();
...
void p2p_plugin::plugin_shutdown() {
  ...
  do {
    if (my->activeHandleBlock.load()) {
      bfState = my->handleBlockFinished.second.wait_for(
    ...
    if (my->activeHandleTx.load()) {
      tfState =
          my->handleTxFinished.second.wait_for(std::chrono::milliseconds(100));
      if (tfState != std::future_status::ready) {
        ilog(
            "waiting for handle_transaction finish: ${s}, future status: ${fs}",
            ("s", fStatus(tfState))(
  ...
  } while (bfState != std::future_status::ready &&
           tfState != std::future_status::ready);
...
void p2p_plugin::broadcast_transaction(
