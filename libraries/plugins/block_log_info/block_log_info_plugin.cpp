...
void block_log_info_plugin_impl::on_post_apply_block(
  ...
  if (is_genesis) {
    _db.create<block_log_hash_state_object>(
        [](block_log_hash_state_object &bso) {});
  }

  const block_log_hash_state_object &state =
      _db.get<block_log_hash_state_object, by_id>(
          block_log_hash_state_id_type(0));
  ...
  if ((print_interval_seconds > 0) && !is_genesis) {
    ...
    if (current_interval != state.last_interval) {
      ...
      if (!print_irreversible) {
        ...
        _db.create<block_log_pending_message_object>(
            [&data](block_log_pending_message_object &msg) {
              msg.data = data;
            });
      }
    }
...
void block_log_info_plugin_impl::print_message(
    const block_log_message_data &data) {
  std::stringstream ss;
  ss << "block_num=" << data.block_num << "   size=" << data.total_size
     << "   hash=" << data.rsha256.hexdigest();
...
void block_log_info_plugin::set_program_options(options_description &cli,
                                                options_description &cfg) {
  cfg.add_options()(
      "block-log-info-print-interval-seconds",
      boost::program_options::value<int32_t>()->default_value(60 * 60 * 24),
      ...
      boost::program_options::value<string>()->default_value("ILOG"),
      "Where to print (filename or special sink ILOG, STDOUT, STDERR)");
}

void block_log_info_plugin::plugin_initialize(
  ...
  try {
    ...
    my->print_interval_seconds =
        options.at("block-log-info-print-interval-seconds").as<int32_t>();
    my->print_irreversible =
        options.at("block-log-info-print-irreversible").as<bool>();
    my->output_name = options.at("block-log-info-print-file").as<string>();

    if (my->print_interval_seconds <= 0) {
      wlog("print_interval_seconds set to value <= 0, if you don't need "
           "printing, consider disabling block_log_info_plugin entirely to "
           "improve performance");
    }

    appbase::app().get_plugin<chain::chain_plugin>().report_state_options(
        name(), fc::variant_object());
  }
