...
class api_stats_export_data_object : public exportable_block_data {
public:
...
FC_REFLECT(
    blurt::plugins::stats_export::detail::api_stats_transaction_data_object,
    (user)(size))
FC_REFLECT(blurt::plugins::stats_export::detail::api_stats_export_data_object,
...
public:
  stats_export_plugin_impl(stats_export_plugin &_plugin)
      : _db(appbase::app().get_plugin<chain_plugin>().db()), _self(_plugin),
...
void stats_export_plugin_impl::on_post_apply_block(
    const block_notification &note) {
  std::shared_ptr<api_stats_export_data_object> stats =
      _export_plugin.find_export_data<api_stats_export_data_object>(
          BLURT_STATS_EXPORT_PLUGIN_NAME);
  if (!stats)
    return;
  ...
  for (const signed_transaction &tx : note.block.transactions) {
    ...
    api_stats_transaction_data_object &tx_stats =
        stats->transaction_stats.back();
...
void stats_export_plugin::set_program_options(options_description &cli,
                                              options_description &cfg) {
  /*
  cfg.add_options()
        ("block-log-info-print-interval-seconds", boost::program_options::value<
  int32_t >()->default_value(60*60*24), "How often to print out stats_export
  (default 1 day)")
        ("block-log-info-print-irreversible", boost::program_options::value<
  bool >()->default_value(true), "Whether to defer printing until block is
  irreversible")
        ("block-log-info-print-file", boost::program_options::value< string
  >()->default_value("ILOG"), "Where to print (filename or special sink ILOG,
  STDOUT, STDERR)")
        ;
  */
}

void stats_export_plugin::plugin_initialize(
  ...
  try {
    ...
    my->_post_apply_block_conn = my->_db.add_post_apply_block_handler(
        ...
        *this);
    my->_export_plugin.register_export_data_factory(
        BLURT_STATS_EXPORT_PLUGIN_NAME,
