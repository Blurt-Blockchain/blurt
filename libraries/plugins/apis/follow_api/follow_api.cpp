...
public:
  ...
  DECLARE_API_IMPL((get_followers)(get_following)(get_follow_count)(
      get_feed_entries)(get_feed)(get_blog_entries)(get_blog)(get_reblogged_by)(
      get_blog_authors))
...
DEFINE_API_IMPL(follow_api_impl, get_followers) {
  ...
  const auto &idx = _db.get_index<follow::follow_index>()
                        .indices()
                        .get<follow::by_following_follower>();
  auto itr = idx.lower_bound(boost::make_tuple(args.account, args.start));
  while (itr != idx.end() && result.followers.size() < args.limit &&
         itr->following == args.account) {
...
DEFINE_API_IMPL(follow_api_impl, get_following) {
  ...
  const auto &idx = _db.get_index<follow::follow_index>()
                        .indices()
                        .get<follow::by_follower_following>();
  auto itr = idx.lower_bound(boost::make_tuple(args.account, args.start));
  while (itr != idx.end() && result.following.size() < args.limit &&
         itr->follower == args.account) {
    if (args.type == follow::undefined || itr->what & (1 << args.type)) {
      api_follow_object entry;
      entry.follower = itr->follower;
      entry.following = itr->following;
...
DEFINE_API_IMPL(follow_api_impl, get_follow_count) {
  ...
  auto itr =
      _db.find<follow::follow_count_object, follow::by_account>(args.account);

  if (itr != nullptr)
    result = get_follow_count_return{itr->account, itr->follower_count,
                                     itr->following_count};
  else
...
DEFINE_API_IMPL(follow_api_impl, get_feed_entries) {
  FC_ASSERT(args.limit <= 500,
            "Cannot retrieve more than 500 feed entries at a time.");
  ...
  const auto &feed_idx =
      _db.get_index<follow::feed_index>().indices().get<follow::by_feed>();
  auto itr = feed_idx.lower_bound(boost::make_tuple(args.account, entry_id));

  while (itr != feed_idx.end() && itr->account == args.account &&
         result.feed.size() < args.limit) {
...
DEFINE_API_IMPL(follow_api_impl, get_blog_entries) {
  FC_ASSERT(args.limit <= 500,
            "Cannot retrieve more than 500 blog entries at a time.");
  ...
  const auto &blog_idx =
      _db.get_index<follow::blog_index>().indices().get<follow::by_blog>();
  auto itr = blog_idx.lower_bound(boost::make_tuple(args.account, entry_id));

  while (itr != blog_idx.end() && itr->account == args.account &&
         result.blog.size() < args.limit) {
    const auto &comment = _db.get(itr->comment);
    blog_entry entry;
    entry.author = comment.author;
    entry.permlink = chain::to_string(comment.permlink);
...
DEFINE_API_IMPL(follow_api_impl, get_blog_authors) {
  ...
  const auto &stats_idx = _db.get_index<follow::blog_author_stats_index,
                                        follow::by_blogger_guest_count>();
  auto itr = stats_idx.lower_bound(args.blog_account);

  while (itr != stats_idx.end() && itr->blogger == args.blog_account &&
         result.blog_authors.size() < 2000) {
...
DEFINE_READ_APIS(follow_api, (get_followers)(get_following)(get_follow_count)(
                                 get_feed_entries)(get_feed)(get_blog_entries)(
                                 get_blog)(get_reblogged_by)(get_blog_authors))
