...
public:
  abstract_account_history_api_impl()
      : _db(appbase::app()
                .get_plugin<blurt::plugins::chain::chain_plugin>()
                .db()) {}
  virtual ~abstract_account_history_api_impl() {}

  virtual get_ops_in_block_return
  get_ops_in_block(const get_ops_in_block_args &) = 0;
  virtual get_transaction_return
  get_transaction(const get_transaction_args &) = 0;
  virtual get_account_history_return
  get_account_history(const get_account_history_args &) = 0;
  virtual enum_virtual_ops_return
  enum_virtual_ops(const enum_virtual_ops_args &) = 0;

  chain::database &_db;
};

class account_history_api_chainbase_impl
    : public abstract_account_history_api_impl {
public:
  ...
  get_ops_in_block(const get_ops_in_block_args &) override;
  get_transaction_return get_transaction(const get_transaction_args &) override;
  get_account_history_return
  get_account_history(const get_account_history_args &) override;
  enum_virtual_ops_return
  enum_virtual_ops(const enum_virtual_ops_args &) override;
};

DEFINE_API_IMPL(account_history_api_chainbase_impl, get_ops_in_block) {
  return _db.with_read_lock([&]() {
    const auto &idx =
        _db.get_index<chain::operation_index, chain::by_location>();
...
DEFINE_API_IMPL(account_history_api_chainbase_impl, get_transaction) {
  ...
  return _db.with_read_lock([&]() {
    ...
    const auto &idx =
        _db.get_index<chain::operation_index, chain::by_transaction_id>();
...
DEFINE_API_IMPL(account_history_api_chainbase_impl, get_account_history) {
  FC_ASSERT(args.limit <= 10000,
            "limit of ${l} is greater than maxmimum allowed",
            ("l", args.limit));
...
DEFINE_API_IMPL(account_history_api_chainbase_impl, enum_virtual_ops) {
  FC_ASSERT(
      false,
      "This API is not supported for account history backed by Chainbase");
}

class account_history_api_rocksdb_impl
    : public abstract_account_history_api_impl {
public:
  account_history_api_rocksdb_impl()
      : abstract_account_history_api_impl(),
        _dataSource(appbase::app()
                        .get_plugin<blurt::plugins::account_history_rocksdb::
...
DEFINE_API_IMPL(account_history_api_rocksdb_impl, get_ops_in_block) {
  get_ops_in_block_return result;
  _dataSource.find_operations_by_block(
      args.block_num,
      [&result,
       &args](const account_history_rocksdb::rocksdb_operation_object &op) {
...
DEFINE_API_IMPL(account_history_api_rocksdb_impl, get_account_history) {
  ...
  _dataSource.find_account_history_data(
      args.account, args.start, args.limit,
      [&result](unsigned int sequence,
                const account_history_rocksdb::rocksdb_operation_object &op) {
...
DEFINE_API_IMPL(account_history_api_rocksdb_impl, enum_virtual_ops) {
  ...
  result.next_block_range_begin = _dataSource.enum_operations_from_block_range(
...
account_history_api::account_history_api() {
  ...
  auto ah_rocks = appbase::app()
                      .find_plugin<blurt::plugins::account_history_rocksdb::
                                       account_history_rocksdb_plugin>();

  if (ah_rocks != nullptr) {
    if (ah_cb != nullptr)
      wlog("account_history and account_history_rocksdb plugins are both "
           "enabled. account_history_api will query from "
    ...
    FC_ASSERT(false, "Account History API only works if account_history or "
                     "account_history_rocksdb plugins are enabled");
  }
...
DEFINE_LOCKLESS_APIS(
    account_history_api,
    (get_ops_in_block)(get_transaction)(get_account_history)(enum_virtual_ops))
