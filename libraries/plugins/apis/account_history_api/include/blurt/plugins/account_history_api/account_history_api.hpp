...
struct api_operation_object {
  ...
  api_operation_object(const T &op_obj)
      : trx_id(op_obj.trx_id), block(op_obj.block),
        trx_in_block(op_obj.trx_in_block), virtual_op(op_obj.virtual_op),
        timestamp(op_obj.timestamp) {
    op = fc::raw::unpack_from_buffer<blurt::protocol::operation>(
        op_obj.serialized_op);
  }
  ...
  bool operator<(const api_operation_object &obj) const {
    return std::tie(block, trx_in_block, op_in_trx, virtual_op) <
           std::tie(obj.block, obj.trx_in_block, obj.op_in_trx, obj.virtual_op);
  }
};
 ...
 *  \param block_range_begin - starting block number (inclusive) to search for
 * virtual operations \param block_range_end   - last block number (exclusive)
 * to search for virtual operations
 */
...
public:
  ...
  DECLARE_API((get_ops_in_block)(get_transaction)(get_account_history)(
      enum_virtual_ops))
...
FC_REFLECT(blurt::plugins::account_history::get_ops_in_block_return, (ops))

FC_REFLECT(blurt::plugins::account_history::get_transaction_args, (id))
