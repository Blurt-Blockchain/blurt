...
public:
  ...
  DECLARE_API_IMPL((get_resource_params)(get_resource_pool)(find_rc_accounts))
...
DEFINE_API_IMPL(rc_api_impl, get_resource_params) {
  ...
  // No need to expose other STATE_ constants, since they only affect
  // state_size_info fields which are already returned

  operation_exec_info exec_info;

  fc::mutable_variant_object size_info_mvo;
  size_info_mvo("resource_state_bytes",
                state_bytes_mvo)("resource_execution_time", exec_info);
...
DEFINE_API_IMPL(rc_api_impl, find_rc_accounts) {
  ...
  for (const account_name_type &a : args.accounts) {
    const rc_account_object *rc_account =
        _db.find<rc_account_object, by_name>(a);
    ...
    api_rc_account.max_rc_creation_adjustment =
        rc_account->max_rc_creation_adjustment;
...
DEFINE_READ_APIS(rc_api,
                 (get_resource_params)(get_resource_pool)(find_rc_accounts))
