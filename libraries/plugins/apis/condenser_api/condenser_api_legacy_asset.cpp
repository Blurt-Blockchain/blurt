...
legacy_asset legacy_asset::from_string(const string &from) {
  try {
    ...
    if (dot_pos != std::string::npos) {
      ...
      result.symbol.asset_num =
          string_to_asset_num(str_symbol.c_str(), decimals);

      int64_t prec = precision(result.symbol);

      // Max amount = 9223372036854775.807 BLURT/SBD
      //`inpart` * `prec` can cause overflow, better is to emulate
      //multiplication using additional zeros
      ...
      // adding `_new_value` can cause overflow, better is to check sum before
      // addition
