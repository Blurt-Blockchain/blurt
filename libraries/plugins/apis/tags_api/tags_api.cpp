...
public:
  ...
  DECLARE_API_IMPL((get_trending_tags)(get_tags_used_by_author)(get_discussion)(
      get_content_replies)(get_post_discussions_by_payout)(
      get_comment_discussions_by_payout)(get_discussions_by_trending)(
      get_discussions_by_created)(get_discussions_by_active)(
      get_discussions_by_cashout)(get_discussions_by_votes)(
      get_discussions_by_children)(get_discussions_by_hot)(
      get_discussions_by_feed)(get_discussions_by_blog)(
  ...
  discussion lookup_discussion(chain::comment_id_type,
                               uint32_t truncate_body = 0);

  static bool filter_default(const database_api::api_comment_object &c) {
    return false;
  }
  static bool exit_default(const database_api::api_comment_object &c) {
    return false;
  }
  static bool tag_exit_default(const tags::tag_object &c) { return false; }

  template <typename Index, typename StartItr>
  discussion_query_result get_discussions(
      const discussion_query &q, const string &tag,
      chain::comment_id_type parent, const Index &idx, StartItr itr,
      uint32_t truncate_body = 0,
      const std::function<bool(const database_api::api_comment_object &)>
          &filter = &tags_api_impl::filter_default,
      const std::function<bool(const database_api::api_comment_object &)>
          &exit = &tags_api_impl::exit_default,
      const std::function<bool(const tags::tag_object &)> &tag_exit =
          &tags_api_impl::tag_exit_default,
...
DEFINE_API_IMPL(tags_api_impl, get_trending_tags) {
  FC_ASSERT(args.limit <= 1000,
            "Cannot retrieve more than 1000 tags at a time.");
...
DEFINE_API_IMPL(tags_api_impl, get_tags_used_by_author) {
  ...
  const auto &tidx =
      _db.get_index<tags::author_tag_stats_index, tags::by_author_posts_tag>();
...
DEFINE_API_IMPL(tags_api_impl, get_discussion) {
  ...
  auto itr =
      by_permlink_idx.find(boost::make_tuple(args.author, args.permlink));

  if (itr != by_permlink_idx.end()) {
    get_discussion_return result(*itr, _db);
    set_pending_payout(result);
    result.active_votes =
        get_active_votes(get_active_votes_args({args.author, args.permlink}))
            .votes;
    return result;
  }
...
DEFINE_API_IMPL(tags_api_impl, get_content_replies) {
  ...
  while (itr != by_permlink_idx.end() && itr->parent_author == args.author &&
         chain::to_string(itr->parent_permlink) == args.permlink) {
...
DEFINE_API_IMPL(tags_api_impl, get_post_discussions_by_payout) {
  ...
  const auto &tidx =
      _db.get_index<tags::tag_index, tags::by_reward_fund_net_rshares>();
  auto tidx_itr = tidx.lower_bound(boost::make_tuple(tag, true));

  return get_discussions(
      args, tag, parent, tidx, tidx_itr, args.truncate_body,
      [](const database_api::api_comment_object &c) {
...
DEFINE_API_IMPL(tags_api_impl, get_discussions_by_created) {
  ...
  auto tidx_itr = tidx.lower_bound(
      boost::make_tuple(tag, parent, fc::time_point_sec::maximum()));
...
DEFINE_API_IMPL(tags_api_impl, get_discussions_by_votes) {
  ...
  const auto &tidx =
      _db.get_index<tags::tag_index, tags::by_parent_net_votes>();
  auto tidx_itr = tidx.lower_bound(
...
DEFINE_API_IMPL(tags_api_impl, get_discussions_by_feed) {
  args.validate();
  FC_ASSERT(_db.has_index<follow::feed_index>(),
  ...
  if (start_author.size() || start_permlink.size()) {
    auto start_c = c_idx.find(boost::make_tuple(
        _db.get_comment(start_author, start_permlink).id, account.name));
  ...
  while (result.discussions.size() < args.limit && feed_itr != f_idx.end()) {
    ...
    try {
      ...
      if (feed_itr->first_reblogged_by != account_name_type()) {
        result.discussions.back().reblogged_by = vector<account_name_type>(
            feed_itr->reblogged_by.begin(), feed_itr->reblogged_by.end());
        result.discussions.back().first_reblogged_by =
            feed_itr->first_reblogged_by;
        result.discussions.back().first_reblogged_on =
            feed_itr->first_reblogged_on;
      }
...
DEFINE_API_IMPL(tags_api_impl, get_discussions_by_blog) {
  args.validate();
  FC_ASSERT(_db.has_index<follow::blog_index>(),
            "Node is not running the follow plugin");
  auto start_author = args.start_author ? *(args.start_author) : "";
  auto start_permlink = args.start_permlink ? *(args.start_permlink) : "";
  ...
  const auto &tag_idx =
      _db.get_index<tags::tag_index>().indices().get<tags::by_comment>();
  ...
  while (result.discussions.size() < args.limit && blog_itr != b_idx.end()) {
    ...
    try {
      if (args.select_authors.size() &&
          args.select_authors.find(blog_itr->account) ==
              args.select_authors.end()) {
        ++blog_itr;
        continue;
      }
      ...
      result.discussions.push_back(
          lookup_discussion(blog_itr->comment, args.truncate_body));
...
DEFINE_API_IMPL(tags_api_impl, get_discussions_by_promoted) {
  ...
  auto tidx_itr = tidx.lower_bound(
      boost::make_tuple(tag, parent, share_type(BLURT_MAX_SHARE_SUPPLY)));

  return get_discussions(
      args, tag, parent, tidx, tidx_itr, args.truncate_body, filter_default,
      exit_default,
...
DEFINE_API_IMPL(tags_api_impl, get_replies_by_last_update) {
  ...
  if (args.start_permlink.size()) {
    const auto &comment =
        _db.get_comment(args.start_parent_author, args.start_permlink);
  ...
  while (itr != last_update_idx.end() &&
         result.discussions.size() < args.limit &&
         itr->parent_author == parent_author) {
    result.discussions.push_back(discussion(*itr, _db));
    set_pending_payout(result.discussions.back());
    result.discussions.back().active_votes =
        get_active_votes(get_active_votes_args(
                             {itr->author, chain::to_string(itr->permlink)}))
            .votes;
    ++itr;
  }
...
DEFINE_API_IMPL(tags_api_impl, get_discussions_by_author_before_date) {
  try {
    ...
    auto itr = didx.lower_bound(
        boost::make_tuple(args.author, time_point_sec::maximum()));
    if (args.start_permlink.size()) {
    ...
    while (itr != didx.end() && itr->author == args.author &&
           count < args.limit) {
...
void tags_api_impl::set_pending_payout(discussion &d) {
  ...
  u256 total_r2 = chain::util::to256(
...
discussion tags_api_impl::lookup_discussion(chain::comment_id_type id,
                                            uint32_t truncate_body) {
  discussion d(_db.get(id), _db);
  set_url(d);
  set_pending_payout(d);
  d.active_votes =
      get_active_votes(get_active_votes_args({d.author, d.permlink})).votes;
...
discussion_query_result tags_api_impl::get_discussions(
    const discussion_query &query, const string &tag,
    chain::comment_id_type parent, const Index &tidx, StartItr tidx_itr,
    uint32_t truncate_body,
  ...
  while (count > 0 && tidx_itr != tidx.end()) {
    ...
    if (itr_count > max_itr_count) {
      wlog("Maximum iteration count exceeded serving query: ${q}",
           ("q", query));
      wlog("count=${count}   itr_count=${itr_count}   "
           "filter_count=${filter_count}   exc_count=${exc_count}",
           ("count", count)("itr_count", itr_count)(
               "filter_count", filter_count)("exc_count", exc_count));
      break;
    }
    ...
    try {
      result.discussions.push_back(
          lookup_discussion(tidx_itr->comment, truncate_body));
      result.discussions.back().promoted =
          asset(tidx_itr->promoted_balance, BLURT_SYMBOL);
...
chain::comment_id_type
tags_api_impl::get_parent(const discussion_query &query) {
...
DEFINE_READ_APIS(tags_api,
                 (get_trending_tags)(get_tags_used_by_author)(get_discussion)(
                     get_content_replies)(get_post_discussions_by_payout)(
                     ...
                     get_discussions_by_author_before_date)(get_active_votes))

void tags_api::set_pending_payout(discussion &d) { my->set_pending_payout(d); }

void tags_api::api_startup() {
  auto follow_api_plugin =
      appbase::app().find_plugin<blurt::plugins::follow::follow_api_plugin>();
