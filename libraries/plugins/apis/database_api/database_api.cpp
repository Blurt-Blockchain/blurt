...
public:
  ...
  DECLARE_API_IMPL((get_config)(get_version)(get_dynamic_global_properties)(
      get_witness_schedule)(get_hardfork_properties)(get_reward_funds)(
      list_witnesses)(find_witnesses)(list_witness_votes)(get_active_witnesses)(
      list_accounts)(find_accounts)(list_owner_histories)(find_owner_histories)(
      ...
      list_vesting_delegation_expirations)(find_vesting_delegation_expirations)(
      list_decline_voting_rights_requests)(find_decline_voting_rights_requests)(
      list_comments)(find_comments)(list_votes)(find_votes)(list_proposals)(
      find_proposals)(list_proposal_votes)(get_transaction_hex)(
      get_required_signatures)(get_potential_signatures)(verify_authority)(
      verify_account_authority)(verify_signatures))

  template <typename ResultType>
  static ResultType on_push_default(const ResultType &r) {
    return r;
  }

  template <typename ValueType> static bool filter_default(const ValueType &r) {
    return true;
  }

  template <typename IndexType, typename OrderType, typename StartType,
            typename ResultType, typename OnPushType, typename FilterType>
  void iterate_results(StartType start, std::vector<ResultType> &result,
                       uint32_t limit, OnPushType &&on_push,
                       FilterType &&filter,
...
DEFINE_API_IMPL(database_api_impl, list_witnesses) {
  ...
  case (by_name): {
    iterate_results<chain::witness_index, chain::by_name>(
        args.start.as<protocol::account_name_type>(), result.witnesses,
        args.limit,
  ...
  case (by_vote_name): {
    ...
    iterate_results<chain::witness_index, chain::by_vote_name>(
        boost::make_tuple(key.first, key.second), result.witnesses, args.limit,
        [&](const witness_object &w) { return api_witness_object(w); },
        &database_api_impl::filter_default<witness_object>);
    break;
  }
  case (by_schedule_time): {
    ...
    iterate_results<chain::witness_index, chain::by_schedule_time>(
        boost::make_tuple(key.first, wit_id), result.witnesses, args.limit,
        [&](const witness_object &w) { return api_witness_object(w); },
        &database_api_impl::filter_default<witness_object>);
    break;
  }
...
DEFINE_API_IMPL(database_api_impl, list_witness_votes) {
  ...
  case (by_account_witness): {
    ...
    iterate_results<chain::witness_vote_index, chain::by_account_witness>(
        boost::make_tuple(key.first, key.second), result.votes, args.limit,
        [&](const witness_vote_object &v) {
          return api_witness_vote_object(v);
        },
...
DEFINE_API_IMPL(database_api_impl, list_accounts) {
  ...
  case (by_name): {
    iterate_results<chain::account_index, chain::by_name>(
        args.start.as<protocol::account_name_type>(), result.accounts,
        args.limit,
  ...
  case (by_proxy): {
    ...
    iterate_results<chain::account_index, chain::by_proxy>(
        boost::make_tuple(key.first, key.second), result.accounts, args.limit,
        [&](const account_object &a) { return api_account_object(a, _db); },
        &database_api_impl::filter_default<account_object>);
    break;
  }
  case (by_next_vesting_withdrawal): {
    auto key =
        args.start.as<std::pair<fc::time_point_sec, account_name_type>>();
...
DEFINE_API_IMPL(database_api_impl, list_owner_histories) {
  ...
  iterate_results<chain::owner_authority_history_index, chain::by_account>(
      boost::make_tuple(key.first, key.second), result.owner_auths, args.limit,
      [&](const owner_authority_history_object &o) {
        return api_owner_authority_history_object(o);
      },
...
DEFINE_API_IMPL(database_api_impl, find_owner_histories) {
  ...
  while (itr != hist_idx.end() && itr->account == args.owner &&
         result.owner_auths.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
...
DEFINE_API_IMPL(database_api_impl, list_account_recovery_requests) {
  ...
  case (by_account): {
    iterate_results<chain::account_recovery_request_index, chain::by_account>(
        args.start.as<account_name_type>(), result.requests, args.limit,
        [&](const account_recovery_request_object &a) {
          return api_account_recovery_request_object(a);
        },
        &database_api_impl::filter_default<
  ...
  case (by_expiration): {
    auto key =
        args.start.as<std::pair<fc::time_point_sec, account_name_type>>();
    iterate_results<chain::account_recovery_request_index,
                    chain::by_expiration>(
        boost::make_tuple(key.first, key.second), result.requests, args.limit,
        [&](const account_recovery_request_object &a) {
          return api_account_recovery_request_object(a);
        },
        &database_api_impl::filter_default<
...
DEFINE_API_IMPL(database_api_impl, find_account_recovery_requests) {
  ...
  for (auto &a : args.accounts) {
    auto request =
        _db.find<chain::account_recovery_request_object, chain::by_account>(a);
...
DEFINE_API_IMPL(database_api_impl, list_change_recovery_account_requests) {
  ...
  switch (args.order) {
  case (by_account): {
    iterate_results<chain::change_recovery_account_request_index,
                    chain::by_account>(
        args.start.as<account_name_type>(), result.requests, args.limit,
        &database_api_impl::on_push_default<
...
DEFINE_API_IMPL(database_api_impl, find_change_recovery_account_requests) {
  ...
  for (auto &a : args.accounts) {
    auto request = _db.find<chain::change_recovery_account_request_object,
                            chain::by_account>(a);
...
DEFINE_API_IMPL(database_api_impl, list_escrows) {
  ...
  case (by_from_id): {
    ...
    iterate_results<chain::escrow_index, chain::by_from_id>(
        boost::make_tuple(key.first, key.second), result.escrows, args.limit,
  ...
  case (by_ratification_deadline): {
    ...
    FC_ASSERT(key.size() == 3,
              "by_ratification_deadline start requires 3 values. (bool, "
              "time_point_sec, escrow_id_type)");
    iterate_results<chain::escrow_index, chain::by_ratification_deadline>(
        boost::make_tuple(key[0].as<bool>(), key[1].as<fc::time_point_sec>(),
                          key[2].as<escrow_id_type>()),
        result.escrows, args.limit,
        &database_api_impl::on_push_default<escrow_object>,
        &database_api_impl::filter_default<escrow_object>);
    break;
  }
...
DEFINE_API_IMPL(database_api_impl, find_escrows) {
  ...
  while (itr != escrow_idx.end() && itr->from == args.from &&
         result.escrows.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
...
DEFINE_API_IMPL(database_api_impl, list_withdraw_vesting_routes) {
  ...
  case (by_withdraw_route): {
    auto key = args.start.as<std::pair<account_name_type, account_name_type>>();
    iterate_results<chain::withdraw_vesting_route_index,
                    chain::by_withdraw_route>(
        boost::make_tuple(key.first, key.second), result.routes, args.limit,
  ...
  case (by_destination): {
    auto key =
        args.start
            .as<std::pair<account_name_type, withdraw_vesting_route_id_type>>();
...
DEFINE_API_IMPL(database_api_impl, find_withdraw_vesting_routes) {
  ...
  switch (args.order) {
  case (by_withdraw_route): {
    const auto &route_idx = _db.get_index<chain::withdraw_vesting_route_index,
                                          chain::by_withdraw_route>();
    auto itr = route_idx.lower_bound(args.account);

    while (itr != route_idx.end() && itr->from_account == args.account &&
           result.routes.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
  ...
  case (by_destination): {
    const auto &route_idx = _db.get_index<chain::withdraw_vesting_route_index,
                                          chain::by_destination>();
    auto itr = route_idx.lower_bound(args.account);

    while (itr != route_idx.end() && itr->to_account == args.account &&
           result.routes.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
      result.routes.push_back(*itr);
      ++itr;
    }
...
DEFINE_API_IMPL(database_api_impl, list_savings_withdrawals) {
  ...
  case (by_from_id): {
    ...
    iterate_results<chain::savings_withdraw_index, chain::by_from_rid>(
        boost::make_tuple(key.first, key.second), result.withdrawals,
        args.limit,
        [&](const savings_withdraw_object &w) {
          return api_savings_withdraw_object(w);
        },
  ...
  case (by_complete_from_id): {
    auto key = args.start.as<std::vector<fc::variant>>();
    FC_ASSERT(key.size() == 3, "by_complete_from_id start requires 3 values. "
                               "(time_point_sec, account_name_type, uint32_t)");
    iterate_results<chain::savings_withdraw_index, chain::by_complete_from_rid>(
        boost::make_tuple(key[0].as<fc::time_point_sec>(),
                          key[1].as<account_name_type>(),
                          key[2].as<uint32_t>()),
        result.withdrawals, args.limit,
        [&](const savings_withdraw_object &w) {
          return api_savings_withdraw_object(w);
        },
        &database_api_impl::filter_default<savings_withdraw_object>);
  ...
  case (by_to_complete): {
    auto key = args.start.as<std::vector<fc::variant>>();
    FC_ASSERT(key.size() == 3,
              "by_to_complete start requires 3 values. (account_name_type, "
              "time_point_sec, savings_withdraw_id_type");
    iterate_results<chain::savings_withdraw_index, chain::by_to_complete>(
        boost::make_tuple(key[0].as<account_name_type>(),
                          key[1].as<fc::time_point_sec>(),
                          key[2].as<savings_withdraw_id_type>()),
        result.withdrawals, args.limit,
        [&](const savings_withdraw_object &w) {
          return api_savings_withdraw_object(w);
        },
...
DEFINE_API_IMPL(database_api_impl, find_savings_withdrawals) {
  ...
  const auto &withdraw_idx =
      _db.get_index<chain::savings_withdraw_index, chain::by_from_rid>();
  auto itr = withdraw_idx.lower_bound(args.account);

  while (itr != withdraw_idx.end() && itr->from == args.account &&
         result.withdrawals.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
...
DEFINE_API_IMPL(database_api_impl, list_vesting_delegations) {
  ...
  case (by_delegation): {
    ...
    iterate_results<chain::vesting_delegation_index, chain::by_delegation>(
        boost::make_tuple(key.first, key.second), result.delegations,
        args.limit,
...
DEFINE_API_IMPL(database_api_impl, find_vesting_delegations) {
  ...
  while (itr != delegation_idx.end() && itr->delegator == args.account &&
         result.delegations.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
...
DEFINE_API_IMPL(database_api_impl, list_vesting_delegation_expirations) {
  ...
  switch (args.order) {
  case (by_expiration): {
    auto key = args.start.as<
        std::pair<time_point_sec, vesting_delegation_expiration_id_type>>();
    iterate_results<chain::vesting_delegation_expiration_index,
                    chain::by_expiration>(
        boost::make_tuple(key.first, key.second), result.delegations,
        args.limit,
        &database_api_impl::on_push_default<
  ...
  case (by_account_expiration): {
    ...
    FC_ASSERT(
        key.size() == 3,
        "by_account_expiration start requires 3 values. (account_name_type, "
        "time_point_sec, vesting_delegation_expiration_id_type");
    iterate_results<chain::vesting_delegation_expiration_index,
                    chain::by_account_expiration>(
        boost::make_tuple(key[0].as<account_name_type>(),
                          key[1].as<time_point_sec>(),
                          key[2].as<vesting_delegation_expiration_id_type>()),
        result.delegations, args.limit,
        &database_api_impl::on_push_default<
            api_vesting_delegation_expiration_object>,
        &database_api_impl::filter_default<
            vesting_delegation_expiration_object>);
...
DEFINE_API_IMPL(database_api_impl, find_vesting_delegation_expirations) {
  ...
  const auto &del_exp_idx =
      _db.get_index<chain::vesting_delegation_expiration_index,
                    chain::by_account_expiration>();
  auto itr = del_exp_idx.lower_bound(args.account);

  while (itr != del_exp_idx.end() && itr->delegator == args.account &&
         result.delegations.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
...
DEFINE_API_IMPL(database_api_impl, list_decline_voting_rights_requests) {
  ...
  switch (args.order) {
  case (by_account): {
    iterate_results<chain::decline_voting_rights_request_index,
                    chain::by_account>(
        args.start.as<account_name_type>(), result.requests, args.limit,
        &database_api_impl::on_push_default<
...
DEFINE_API_IMPL(database_api_impl, find_decline_voting_rights_requests) {
  ...
  for (auto &a : args.accounts) {
    auto request = _db.find<chain::decline_voting_rights_request_object,
                            chain::by_account>(a);
...
DEFINE_API_IMPL(database_api_impl, list_comments) {
  ...
  case (by_cashout_time): {
    ...
    FC_ASSERT(key.size() == 3, "by_cashout_time start requires 3 values. "
                               "(time_point_sec, account_name_type, string)");
    ...
    if (author != account_name_type() || permlink.size()) {
      auto comment = _db.find<chain::comment_object, chain::by_permlink>(
          boost::make_tuple(author, permlink));
      FC_ASSERT(comment != nullptr, "Could not find comment ${a}/${p}.",
                ("a", author)("p", permlink));
  ...
  case (by_permlink): {
    ...
    iterate_results<chain::comment_index, chain::by_permlink>(
        boost::make_tuple(key.first, key.second), result.comments, args.limit,
        [&](const comment_object &c) { return api_comment_object(c, _db); },
        &database_api_impl::filter_default<comment_object>);
    break;
  }
  case (by_root): {
    auto key = args.start.as<vector<fc::variant>>();
    FC_ASSERT(key.size() == 4,
              "by_root start requires 4 values. (account_name_type, string, "
    ...
    if (root_author != account_name_type() || root_permlink.size()) {
      auto root = _db.find<chain::comment_object, chain::by_permlink>(
          boost::make_tuple(root_author, root_permlink));
      FC_ASSERT(root != nullptr, "Could not find comment ${a}/${p}.",
                ("a", root_author)("p", root_permlink));
      root_id = root->id;
    }
    ...
    if (child_author != account_name_type() || child_permlink.size()) {
      auto child = _db.find<chain::comment_object, chain::by_permlink>(
          boost::make_tuple(child_author, child_permlink));
      FC_ASSERT(child != nullptr, "Could not find comment ${a}/${p}.",
                ("a", child_author)("p", child_permlink));
      child_id = child->id;
    }

    iterate_results<chain::comment_index, chain::by_root>(
        boost::make_tuple(root_id, child_id), result.comments, args.limit,
        [&](const comment_object &c) { return api_comment_object(c, _db); },
        &database_api_impl::filter_default<comment_object>);
    break;
  }
  case (by_parent): {
    auto key = args.start.as<vector<fc::variant>>();
    FC_ASSERT(key.size() == 4,
              "by_parent start requires 4 values. (account_name_type, string, "
              "account_name_type, string)");
    ...
    iterate_results<chain::comment_index, chain::by_parent>(
        boost::make_tuple(key[0].as<account_name_type>(), key[1].as<string>(),
                          child_id),
        result.comments, args.limit,
        [&](const comment_object &c) { return api_comment_object(c, _db); },
        &database_api_impl::filter_default<comment_object>);
  ...
  case (by_last_update): {
    auto key = args.start.as<vector<fc::variant>>();
    FC_ASSERT(key.size() == 4,
              "by_last_update start requires 4 values. (account_name_type, "
              "time_point_sec, account_name_type, string)");
    ...
    iterate_results<chain::comment_index, chain::by_last_update>(
        boost::make_tuple(key[0].as<account_name_type>(),
                          key[1].as<fc::time_point_sec>(), child_id),
        result.comments, args.limit,
        [&](const comment_object &c) { return api_comment_object(c, _db); },
        &database_api_impl::filter_default<comment_object>);
    break;
  ...
  case (by_author_last_update): {
    auto key = args.start.as<vector<fc::variant>>();
    FC_ASSERT(key.size() == 4,
              "by_author_last_update start requires 4 values. "
              "(account_name_type, time_point_sec, account_name_type, string)");
    ...
    iterate_results<chain::comment_index, chain::by_last_update>(
        boost::make_tuple(key[0].as<account_name_type>(),
                          key[1].as<fc::time_point_sec>(), comment_id),
        result.comments, args.limit,
        [&](const comment_object &c) { return api_comment_object(c, _db); },
        &database_api_impl::filter_default<comment_object>);
    break;
...
void votes_impl(database_api_impl &_impl, vector<api_comment_vote_object> &c,
                size_t nr_args, uint32_t limit, vector<fc::variant> &key,
                fc::time_point_sec &timestamp, uint64_t weight) {
  if (SORTORDERTYPE == by_comment_voter)
    FC_ASSERT(key.size() == nr_args,
              "by_comment_voter start requires ${nr_args} values. "
              "(account_name_type, string, account_name_type)",
              ("nr_args", nr_args));
  else
    FC_ASSERT(
        key.size() == nr_args,
        "by_comment_voter start requires ${nr_args} values. "
        "(account_name_type, ${desc}account_name_type, string)",
        ("nr_args", nr_args)("desc", (nr_args == 4) ? "time_point_sec, " : ""));
  ...
  if (author != account_name_type() || permlink.size()) {
    auto comment = _impl._db.find<chain::comment_object, chain::by_permlink>(
        boost::make_tuple(author, permlink));
    FC_ASSERT(comment != nullptr, "Could not find comment ${a}/${p}.",
              ("a", author)("p", permlink));
    comment_id = comment->id;
  ...
  if (SORTORDERTYPE == by_comment_voter) {
    _impl.iterate_results<chain::comment_vote_index, chain::by_comment_voter>(
        boost::make_tuple(comment_id, voter_id), c, limit,
        [&](const comment_vote_object &cv) {
          return api_comment_vote_object(cv, _impl._db);
        },
    ...
    _impl.iterate_results<chain::comment_vote_index, chain::by_voter_comment>(
        boost::make_tuple(voter_id, comment_id), c, limit,
        [&](const comment_vote_object &cv) {
          return api_comment_vote_object(cv, _impl._db);
        },
        &database_api_impl::filter_default<comment_vote_object>);
...
DEFINE_API_IMPL(database_api_impl, list_votes) {
  ...
  case (by_comment_voter): {
    ...
    last_votes_misc::votes_impl<by_comment_voter>(
        *this, result.votes, 3 /*nr_args*/, args.limit, key, t, 0);
    break;
  }
  case (by_voter_comment): {
    static fc::time_point_sec t(-1);
    last_votes_misc::votes_impl<by_voter_comment>(
        *this, result.votes, 3 /*nr_args*/, args.limit, key, t, 0);
    break;
  }
  default:
...
DEFINE_API_IMPL(database_api_impl, find_votes) {
  ...
  auto comment = _db.find<chain::comment_object, chain::by_permlink>(
      boost::make_tuple(args.author, args.permlink));
  FC_ASSERT(comment != nullptr, "Could not find comment ${a}/${p}",
            ("a", args.author)("p", args.permlink));

  const auto &vote_idx =
      _db.get_index<chain::comment_vote_index, chain::by_comment_voter>();
  auto itr = vote_idx.lower_bound(comment->id);

  while (itr != vote_idx.end() && itr->comment == comment->id &&
         result.votes.size() <= DATABASE_API_SINGLE_QUERY_LIMIT) {
...
proposal_status get_proposal_status(const proposal_object &po,
                                    const time_point_sec current_time) {
...
bool filter_proposal_status(const proposal_object &po, proposal_status filter,
                            time_point_sec current_time) {
  if (po.removed)
    return false;
  ...
  return filter == prop_status ||
         (filter == votable &&
          (prop_status == active || prop_status == inactive));
}
...
DEFINE_API_IMPL(database_api_impl, list_proposals) {
  ...
  case by_creator: {
    ...
    iterate_results<blurt::chain::proposal_index, blurt::chain::by_creator>(
        boost::make_tuple(key.first, key.second), result.proposals, args.limit,
        [&](const proposal_object &po) {
          return api_proposal_object(po, current_time);
        },
        [&](const proposal_object &po) {
          return filter_proposal_status(po, args.status, current_time);
        },
...
DEFINE_API_IMPL(database_api_impl, find_proposals) {
  ...
  std::for_each(
      args.proposal_ids.begin(), args.proposal_ids.end(), [&](auto &id) {
        auto po = _db.find<blurt::chain::proposal_object,
                           blurt::chain::by_proposal_id>(id);
...
DEFINE_API_IMPL(database_api_impl, list_proposal_votes) {
  ...
  case by_voter_proposal: {
    auto key = args.start.as<std::pair<account_name_type, api_id_type>>();
    iterate_results<blurt::chain::proposal_vote_index,
                    blurt::chain::by_voter_proposal>(
        boost::make_tuple(key.first, key.second), result.proposal_votes,
        args.limit,
        [&](const proposal_vote_object &po) {
          return api_proposal_vote_object(po, _db);
        },
        [&](const proposal_vote_object &po) {
          auto itr =
              _db.find<blurt::chain::proposal_object, blurt::chain::by_id>(
                  po.proposal_id);
...
DEFINE_API_IMPL(database_api_impl, get_transaction_hex) {
  return get_transaction_hex_return(
      {fc::to_hex(fc::raw::pack_to_vector(args.trx))});
}
...
DEFINE_API_IMPL(database_api_impl, get_required_signatures) {
  ...
  result.keys = args.trx.get_required_signatures(
      _db.get_chain_id(), args.available_keys,
      [&](string account_name) {
        return authority(
            _db.get<chain::account_authority_object, chain::by_account>(
                   account_name)
                .active);
      },
      [&](string account_name) {
      ...
      BLURT_MAX_SIG_CHECK_DEPTH, fc::ecc::canonical_signature_type::bip_0062);
...
DEFINE_API_IMPL(database_api_impl, get_potential_signatures) {
  ...
  args.trx.get_required_signatures(
      _db.get_chain_id(), flat_set<public_key_type>(),
...
// TODO: This is broken. By the look of is, it has been since BitShares.
// verify_authority always returns false because the TX is not signed.
...
DEFINE_API_IMPL(database_api_impl, verify_signatures) {
  ...
  for (const auto &sig : args.signatures) {
    BLURT_ASSERT(sig_keys.insert(fc::ecc::public_key(sig, args.hash)).second,
                 protocol::tx_duplicate_sig, "Duplicate Signature detected");
  }
  ...
  try {
    blurt::protocol::verify_authority<verify_signatures_args>(
        ...
        [this](const string &name) {
          return authority(
              _db.get<chain::account_authority_object, chain::by_account>(name)
                  .owner);
        },
        [this](const string &name) {
          return authority(
...
DEFINE_READ_APIS(
    database_api,
    (get_dynamic_global_properties)(get_witness_schedule)(
        get_hardfork_properties)(get_reward_funds)(list_witnesses)(
        find_witnesses)(list_witness_votes)(get_active_witnesses)(
        list_accounts)(find_accounts)(list_owner_histories)(
        find_owner_histories)(list_account_recovery_requests)(
        find_account_recovery_requests)(list_change_recovery_account_requests)(
        find_change_recovery_account_requests)(list_escrows)(find_escrows)(
        list_withdraw_vesting_routes)(find_withdraw_vesting_routes)(
        list_savings_withdrawals)(find_savings_withdrawals)(
        list_vesting_delegations)(find_vesting_delegations)(
        ...
        list_votes)(find_votes)(list_proposals)(find_proposals)(
        list_proposal_votes)(get_transaction_hex)(get_required_signatures)(
