...
typedef change_recovery_account_request_object
...
typedef decline_voting_rights_request_object
...
typedef vesting_delegation_expiration_object
...
struct api_comment_object {
  api_comment_object(const comment_object &o, const database &db)
      : id(o.id), category(to_string(o.category)),
        parent_author(o.parent_author),
        parent_permlink(to_string(o.parent_permlink)), author(o.author),
        permlink(to_string(o.permlink)), last_update(o.last_update),
        created(o.created), active(o.active), last_payout(o.last_payout),
        depth(o.depth), children(o.children), net_rshares(o.net_rshares),
        abs_rshares(o.abs_rshares), vote_rshares(o.vote_rshares),
        ...
        percent_blurt(o.percent_blurt), allow_replies(o.allow_replies),
...
#ifndef IS_LOW_MEM
    const auto &con =
        db.get<chain::comment_content_object, chain::by_comment>(o.id);
...
struct api_comment_vote_object {
  api_comment_vote_object(const comment_vote_object &cv, const database &db)
      : id(cv.id), weight(cv.weight), rshares(cv.rshares),
        vote_percent(cv.vote_percent), last_update(cv.last_update),
...
struct api_account_object {
  api_account_object(const account_object &a, const database &db)
      : id(a.id), name(a.name), memo_key(a.memo_key), proxy(a.proxy),
        ...
        last_root_post(a.last_root_post), last_post_edit(a.last_post_edit),
        last_vote_time(a.last_vote_time), post_bandwidth(a.post_bandwidth),
...
struct api_account_recovery_request_object {
  api_account_recovery_request_object(const account_recovery_request_object &o)
      : id(o.id), account_to_recover(o.account_to_recover),
...
struct api_savings_withdraw_object {
  api_savings_withdraw_object(const savings_withdraw_object &o)
      : id(o.id), from(o.from), to(o.to), memo(to_string(o.memo)),
        request_id(o.request_id), amount(o.amount), complete(o.complete) {}
...
struct api_witness_object {
  api_witness_object(const witness_object &w)
      : id(w.id), owner(w.owner), created(w.created), url(to_string(w.url)),
...
struct api_witness_schedule_object {
  ...
  api_witness_schedule_object(const witness_schedule_object &wso)
      : id(wso.id), current_virtual_time(wso.current_virtual_time),
        ...
        median_props(wso.median_props), majority_version(wso.majority_version),
        ...
        min_witness_account_subsidy_decay(
            wso.min_witness_account_subsidy_decay) {
    size_t n = wso.current_shuffled_witnesses.size();
    current_shuffled_witnesses.reserve(n);
    std::transform(wso.current_shuffled_witnesses.begin(),
                   wso.current_shuffled_witnesses.end(),
  ...
  vector<string> current_shuffled_witnesses; // fc::array<account_name_type,...>
                                             // -> vector<string>
...
struct api_hardfork_property_object {
  api_hardfork_property_object(const hardfork_property_object &h)
      : id(h.id), last_hardfork(h.last_hardfork),
...
enum proposal_status { all, inactive, active, expired, votable };
...
struct api_proposal_object {
  ...
  api_proposal_object(const proposal_object &po,
                      const time_point_sec &current_time)
      : id(po.id), proposal_id(po.proposal_id), creator(po.creator),
        receiver(po.receiver), start_date(po.start_date), end_date(po.end_date),
        daily_pay(po.daily_pay), subject(to_string(po.subject)),
        permlink(to_string(po.permlink)), total_votes(po.total_votes),
...
struct api_proposal_vote_object {
  ...
  api_proposal_vote_object(const proposal_vote_object &pvo, const database &db)
      : id(pvo.id), voter(pvo.voter),
        proposal(db.get<proposal_object, by_id>(pvo.proposal_id),
                 db.head_block_time()) {}
...
FC_REFLECT(
    blurt::plugins::database_api::api_comment_object,
...
FC_REFLECT(blurt::plugins::database_api::api_comment_vote_object,
           (id)(voter)(author)(permlink)(weight)(rshares)(vote_percent)(
               last_update)(num_changes))

FC_REFLECT(
    blurt::plugins::database_api::api_account_object,
    (id)(name)(owner)(active)(posting)(memo_key)(json_metadata)(
        ...
        voting_manabar)(balance)(savings_balance)(savings_withdraw_requests)(
        ...
        witnesses_voted_for)(last_post)(last_root_post)(last_post_edit)(
        last_vote_time)(post_bandwidth)(pending_claimed_accounts)(is_smt))
...
FC_REFLECT(
    blurt::plugins::database_api::api_witness_schedule_object,
...
FC_REFLECT_DERIVED(blurt::plugins::database_api::api_signed_block_object,
                   (blurt::protocol::signed_block),
