...
struct pre_operation_visitor {
  ...
  void operator()(const account_update_operation &op) const {
    _plugin.clear_cache();
    auto acct_itr =
        _plugin._db.find<account_authority_object, by_account>(op.account);
    if (acct_itr)
  ...
  void operator()(const recover_account_operation &op) const {
    _plugin.clear_cache();
    auto acct_itr = _plugin._db.find<account_authority_object, by_account>(
...
struct post_operation_visitor {
  ...
  post_operation_visitor(account_by_key_plugin_impl &plugin)
      : _plugin(plugin) {}
...
void account_by_key_plugin_impl::cache_auths(
...
void account_by_key_plugin_impl::update_key_lookup(
    const account_authority_object &a) {
  ...
  for (const auto &key : new_keys) {
    ...
    if (cached_keys.find(key) == cached_keys.end()) {
      auto lookup_itr = _db.find<key_lookup_object, by_key>(
          boost::make_tuple(key, a.account));
      ...
      // If the key was already in the auths, remove it from the set so we don't
      // delete it
      cached_keys.erase(key);
    }
  }

  // Loop over the keys that were in authority but are no longer and remove them
  // from the lookup
  for (const auto &key : cached_keys) {
    auto lookup_itr =
        _db.find<key_lookup_object, by_key>(boost::make_tuple(key, a.account));
...
void account_by_key_plugin_impl::on_pre_apply_operation(
    const operation_notification &note) {
  note.op.visit(pre_operation_visitor(*this));
}

void account_by_key_plugin_impl::on_post_apply_operation(
    const operation_notification &note) {
...
void account_by_key_plugin::set_program_options(options_description &cli,
                                                options_description &cfg) {}

void account_by_key_plugin::plugin_initialize(
