...
struct performance_data {
  ...
  performance_data() { memset(&s, 0, sizeof(s)); }

  performance_data(const comment_id_type &_comment, bool _is_empty) {
    init(_comment, _is_empty);
  }

  void init(const account_name_type &_account, const time_point_sec &_time,
            const comment_id_type &_comment, bool _is_empty, uint32_t _old_id) {
    account = &_account;
    time = &_time;
    comment = &_comment;
    s.creation = true;
