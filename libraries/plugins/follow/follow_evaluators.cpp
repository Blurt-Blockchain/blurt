...
void follow_evaluator::do_apply(const follow_operation &o) {
  try {
    ...
    const auto &idx =
        _db.get_index<follow_index>().indices().get<by_follower_following>();
    ...
    const auto &follower =
        _db.find<follow_count_object, by_account>(o.follower);
    ...
    const auto &following =
        _db.find<follow_count_object, by_account>(o.following);
...
void reblog_evaluator::do_apply(const reblog_operation &o) {
  try {
    ...
    FC_ASSERT(c.parent_author.size() == 0,
    ...
    const auto &blog_comment_idx =
        _db.get_index<blog_index>().indices().get<by_comment>();
    ...
    const auto &stats_idx =
        _db.get_index<blog_author_stats_index, by_blogger_guest_count>();
    auto stats_itr =
        stats_idx.lower_bound(boost::make_tuple(o.account, c.author));
    ...
    const auto &comment_idx =
        _db.get_index<feed_index>().indices().get<by_comment>();
    const auto &idx =
        _db.get_index<follow_index>().indices().get<by_following_follower>();
    auto itr = idx.find(o.account);

#ifndef ENABLE_MIRA
    const auto &old_feed_idx =
        _db.get_index<feed_index>().indices().get<by_feed>();
#endif
    ...
    if (_db.head_block_time() >= _plugin->start_feeds) {
      while (itr != idx.end() && itr->following == o.account) {
        if (itr->what & (1 << blog)) {
          ...
          pd.init(o.account, _db.head_block_time(), c.id, is_empty,
                  is_empty ? 0 : feed_itr->account_feed_id);
          uint32_t next_id = 0;
#ifndef ENABLE_MIRA
          perf.delete_old_objects<performance_data::t_creation_type::full_feed>(
              old_feed_idx, itr->follower, _plugin->max_feed_size, pd);
#endif
