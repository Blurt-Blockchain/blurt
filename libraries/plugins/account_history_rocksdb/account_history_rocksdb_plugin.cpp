...
/** Because localtion_id_pair stores block_number paired with
 * (VIRTUAL_OP_FLAG|operation_id), max allowed operation-id is max_int (instead
 * of max_uint).
 */
...
namespace blurt {
namespace plugins {
namespace account_history_rocksdb {
 ...
 *  All operations specific to given account, are next mapped to ID of given
 * object.
 */
...
public:
  ...
  /// Timestamp of oldest operation, just to quickly decide if start detail
  /// prune checking at all.
...
namespace {
template <class T> serialize_buffer_t dump(const T &obj) {
...
template <class T> void load(T &obj, const char *data, size_t size) {
  fc::datastream<const char *> ds(data, size);
  fc::raw::unpack(ds, obj);
}

template <class T> void load(T &obj, const serialize_buffer_t &source) {
  load(obj, source.data(), source.size());
}

/** Helper class to simplify construction of Slice objects holding primitive
 * type values.
 *
 */
template <typename T> class PrimitiveTypeSlice final : public Slice {
public:
...
/** Helper base class to cover all common functionality across defined
 * comparators.
 *
 */
...
public:
  ...
  virtual void FindShortestSeparator(std::string *start,
                                     const Slice &limit) const override final {
    /// Nothing to do.
  }
...
/// Pairs account_name storage type and the ID to make possible nonunique index
/// definition over names.
typedef std::pair<account_name_type::Storage, size_t>
    account_name_storage_id_pair;
...
typedef PrimitiveTypeComparatorImpl<account_name_type::Storage>
...
/** Location index is nonunique. Since RocksDB supports only unique indexes, it
 * must be extended by some unique part (ie ID).
 *
 */
typedef std::pair<uint32_t, uint64_t> block_op_id_pair;
typedef PrimitiveTypeComparatorImpl<block_op_id_pair>
    op_by_block_num_ComparatorImpl;
...
#define checkStatus(s)                                                         \
  FC_ASSERT((s).ok(), "Data access failed: ${m}", ("m", (s).ToString()))
...
public:
  CachableWriteBatch(const std::unique_ptr<DB> &storage,
                     const std::vector<ColumnFamilyHandle *> &columnHandles)
      : _storage(storage), _columnHandles(columnHandles) {}

  bool getAHInfo(const account_name_type &name,
                 account_history_info *ahInfo) const {
    ...
    auto s = _storage->Get(ReadOptions(), _columnHandles[AH_INFO_BY_NAME], key,
                           &buffer);
    if (s.ok()) {
  ...
  void putAHInfo(const account_name_type &name,
                 const account_history_info &ahInfo) {
    _ahInfoCache[name] = ahInfo;
    auto serializeBuf = dump(ahInfo);
    ah_info_by_name_slice_t nameSlice(name.data);
    auto s = Put(_columnHandles[AH_INFO_BY_NAME], nameSlice,
                 Slice(serializeBuf.data(), serializeBuf.size()));
    checkStatus(s);
  }
...
public:
  impl(account_history_rocksdb_plugin &self, const bpo::variables_map &options,
       const bfs::path &storagePath)
      : _self(self),
        _mainDb(appbase::app()
                    .get_plugin<blurt::plugins::chain::chain_plugin>()
                    .db()),
        _storagePath(storagePath), _writeBuffer(_storage, _columnHandles) {
    collectOptions(options);

    _mainDb.add_pre_reindex_handler(
        [&](const blurt::chain::reindex_notification &note) -> void {
          on_pre_reindex(note);
        },
        _self, 0);
  ...
  void openDb() {
    ...
    auto status =
        DB::Open(dbOptions, strPath, columnDefs, &_columnHandles, &storageDb);

    if (status.ok()) {
      ilog("RocksDB opened successfully storage at location: `${p}'.",
           ("p", strPath));
      verifyStoreVersion(storageDb);
      loadSeqIdentifiers(storageDb);
      _storage.reset(storageDb);

      const auto &rocksdb_plugin =
          appbase::app().get_plugin<account_history_rocksdb_plugin>();

      // I do not like using exceptions for control paths, but column
      // definitions are set multiple times opening the db, so that is not a
      // good place to write the initial lib.
      try {
        get_lib();
      ...
      _on_irreversible_block_conn = _mainDb.add_irreversible_block_handler(
          [&](uint32_t block_num) { on_irreversible_block(block_num); },
          rocksdb_plugin);
    } else {
  ...
  void find_account_history_data(
      const account_name_type &name, uint64_t start, uint32_t limit,
      std::function<void(unsigned int, const rocksdb_operation_object &)>
          processor) const;
  bool find_operation_object(size_t opId, rocksdb_operation_object *op) const;
  /// Allows to look for all operations present in given block and call
  /// `processor` for them.
  void find_operations_by_block(
      size_t blockNum,
  ...
  uint32_t enumVirtualOperationsFromBlockRange(
      uint32_t blockRangeBegin, uint32_t blockRangeEnd,
      std::function<void(const rocksdb_operation_object &)> processor) const;
...
private:
  ...
  void importOperation(rocksdb_operation_object &obj, const T &impacted) {
    ...
    auto s =
        _writeBuffer.Put(_columnHandles[OPERATION_BY_ID], idSlice,
                         Slice(serializedObj.data(), serializedObj.size()));
    checkStatus(s);

    // uint64_t location = ( (uint64_t) obj.trx_in_block << 32 ) | ( (uint64_t)
    // obj.op_in_trx << 16 ) | ( obj.virtual_op );
    ...
    op_by_block_num_slice_t blockLocSlice(
        block_op_id_pair(obj.block, encoded_id));
    s = _writeBuffer.Put(_columnHandles[OPERATION_BY_BLOCK], blockLocSlice,
                         idSlice);
    checkStatus(s);
  ...
  void buildAccountHistoryRecord(const account_name_type &name,
                                 const rocksdb_operation_object &obj);
  void prunePotentiallyTooOldItems(account_history_info *ahInfo,
  ...
  void loadSeqIdentifiers(DB *storageDb) {
    ...
    ilog("Loaded OperationObject seqId: ${o}, AccountHistoryObject seqId: "
         "${ah}.",
   ...
   *  Depends on `"account-history-whitelist-ops"`,
   * `account-history-blacklist-ops` option usage. Only some accounts can be
   * chosen for tracking operation history.
   */
   ...
   *  Depends on `account-history-blacklist-ops`,
...
private:
  ...
  /// Helper member to be able to detect another incomming tx and increment
  /// tx-counter.
  transaction_id_type _lastTx;
  size_t _txNo = 0;
  /// Total processed ops in this session (counts every operation, even excluded
  /// by filtering).
  size_t _totalOps = 0;
  /// Total number of ops being skipped by filtering options.
  size_t _excludedOps = 0;
  /// Total number of accounts (impacted by ops) excluded from processing
  ...
  /// Number of data-chunks for ops being stored inside _writeBuffer. To decide
  /// when to flush.
  unsigned int _collectedOps = 0;
  /** Limit which value depends on block data source:
   *    - if blocks come from network, there is no need for delaying write,
   becasue they appear quite rare (limit == 1)
   *    - if reindex process or direct import has been spawned, this massive
   operation can need reduction of direct writes (limit ==
   WRITE_BUFFER_FLUSH_LIMIT).
   */
...
void account_history_rocksdb_plugin::impl::collectOptions(
    const boost::program_options::variables_map &options) {
  fc::mutable_variant_object state_opts;

  typedef std::pair<account_name_type, account_name_type> pairstring;
  BLURT_LOAD_VALUE_SET(options, "account-history-rocksdb-track-account-range",
                       _tracked_accounts, pairstring);
  ...
  if (options.count("account-history-rocksdb-whitelist-ops")) {
    const auto &args = options.at("account-history-rocksdb-whitelist-ops")
  ...
  if (options.count("account-history-rocksdb-blacklist-ops")) {
    const auto &args = options.at("account-history-rocksdb-blacklist-ops")
                           .as<std::vector<std::string>>();
    storeOpFilteringParameters(args, &_blacklisted_op_list);

    if (_blacklisted_op_list.size()) {
      state_opts["account-history-rocksdb-blacklist-ops"] =
          _blacklisted_op_list;
    }
  }
  ...
  appbase::app().get_plugin<chain::chain_plugin>().report_state_options(
      _self.name(), state_opts);
}

inline bool account_history_rocksdb_plugin::impl::isTrackedAccount(
    const account_name_type &name) const {
  if (_tracked_accounts.empty())
    return true;

  /// Code below is based on original contents of
  /// account_history_plugin_impl::on_post_apply_operation
  auto itr = _tracked_accounts.lower_bound(name);

  /*
   * The map containing the ranges uses the key as the lower bound and the value
   * as the upper bound. Because of this, if a value exists with the range (key,
   * value], then calling lower_bound on the map will return the key of the next
   * pair. Under normal circumstances of those ranges not intersecting, the
   * value we are looking for will not be present in range that is returned via
   * lower_bound.
   *
   * Consider the following example using ranges ["a","c"], ["g","i"]
   * If we are looking for "bob", it should be tracked because it is in the
   * lower bound. However, lower_bound( "bob" ) returns an iterator to
   * ["g","i"]. So we need to decrement the iterator to get the correct range.
   *
   * If we are looking for "g", lower_bound( "g" ) will return ["g","i"], so we
   * need to make sure we don't decrement.
   *
   * If the iterator points to the end, we should check the previous (equivalent
   * to rbegin)
   *
   * And finally if the iterator is at the beginning, we should not decrement it
   * for obvious reasons
   */
  if (itr != _tracked_accounts.begin() &&
      ((itr != _tracked_accounts.end() && itr->first != name) ||
       itr == _tracked_accounts.end())) {
    --itr;
  }

  bool inRange = itr != _tracked_accounts.end() && itr->first <= name &&
                 name <= itr->second;

  _excludedAccountCount += inRange;

  DIAGNOSTIC(if (inRange) ilog(
                 "Account: ${a} matched to defined account range: [${b},${e}]",
                 ("a", name)("b", itr->first)("e", itr->second));
             else ilog("Account: ${a} ignored due to defined tracking filters.",
                       ("a", name)););
...
account_history_rocksdb_plugin::impl::getImpactedAccounts(
...
inline bool account_history_rocksdb_plugin::impl::isTrackedOperation(
    const operation &op) const {
  ...
  bool isTracked =
      (_op_list.empty() || (_op_list.find(opName) != _op_list.end()));
  return isTracked;
}

void account_history_rocksdb_plugin::impl::storeOpFilteringParameters(
...
void account_history_rocksdb_plugin::impl::find_account_history_data(
    const account_name_type &name, uint64_t start, uint32_t limit,
    std::function<void(unsigned int, const rocksdb_operation_object &)>
        processor) const {
  ReadOptions rOptions;
  ...
  ah_info_by_name_slice_t nameSlice(name.data);
  PinnableSlice buffer;
  auto s = _storage->Get(rOptions, _columnHandles[AH_INFO_BY_NAME], nameSlice,
                         &buffer);
  ...
  ah_op_by_id_slice_t lowerBoundSlice(
      std::make_pair(ahInfo.id, ahInfo.oldestEntryId));
  ah_op_by_id_slice_t upperBoundSlice(
      std::make_pair(ahInfo.id, ahInfo.newestEntryId + 1));
  ...
  std::unique_ptr<::rocksdb::Iterator> it(
      _storage->NewIterator(rOptions, _columnHandles[AH_OPERATION_BY_ID]));
...
bool account_history_rocksdb_plugin::impl::find_operation_object(
    size_t opId, rocksdb_operation_object *op) const {
  std::string data;
  id_slice_t idSlice(opId);
  ::rocksdb::Status s = _storage->Get(
      ReadOptions(), _columnHandles[OPERATION_BY_ID], idSlice, &data);
...
void account_history_rocksdb_plugin::impl::find_operations_by_block(
    size_t blockNum,
    std::function<void(const rocksdb_operation_object &)> processor) const {
  std::unique_ptr<::rocksdb::Iterator> it(
      _storage->NewIterator(ReadOptions(), _columnHandles[OPERATION_BY_BLOCK]));
  by_block_slice_t blockNumSlice(blockNum);
  op_by_block_num_slice_t key(block_op_id_pair(blockNum, 0));

  for (it->Seek(key); it->Valid() && it->key().starts_with(blockNumSlice);
       it->Next()) {
    auto valueSlice = it->value();
    const auto &opId = id_slice_t::unpackSlice(valueSlice);
...
uint32_t
account_history_rocksdb_plugin::impl::enumVirtualOperationsFromBlockRange(
    uint32_t blockRangeBegin, uint32_t blockRangeEnd,
    std::function<void(const rocksdb_operation_object &)> processor) const {
  ...
  std::unique_ptr<::rocksdb::Iterator> it(
      _storage->NewIterator(rOptions, _columnHandles[OPERATION_BY_BLOCK]));
  ...
  op_by_block_num_slice_t nextRangeBeginSlice(
      block_op_id_pair(lastFoundBlock + 1, 0));
...
uint32_t account_history_rocksdb_plugin::impl::get_lib() {
  std::string data;
  auto s =
      _storage->Get(ReadOptions(), _columnHandles[CURRENT_LIB], LIB_ID, &data);
...
void account_history_rocksdb_plugin::impl::update_lib(uint32_t lib) {
  auto s =
      _writeBuffer.Put(_columnHandles[CURRENT_LIB], LIB_ID, lib_slice_t(lib));
  checkStatus(s);
}

account_history_rocksdb_plugin::impl::ColumnDefinitions
account_history_rocksdb_plugin::impl::prepareColumnDefinitions(
    bool addDefaultColumn) {
  ColumnDefinitions columnDefs;
  if (addDefaultColumn)
    columnDefs.emplace_back(::rocksdb::kDefaultColumnFamilyName,
                            ColumnFamilyOptions());
  ...
  columnDefs.emplace_back("account_history_info_by_name",
                          ColumnFamilyOptions());
...
bool account_history_rocksdb_plugin::impl::createDbSchema(
    const bfs::path &path) {
  DB *db = nullptr;
  ...
  auto s =
      DB::OpenForReadOnly(options, strPath, columnDefs, &_columnHandles, &db);
  ...
  if (s.ok()) {
    ...
    if (s.ok()) {
      ...
      elog("RocksDB can not create column definitions at location: "
           "`${p}'.\nReturned error: ${e}",
...
void account_history_rocksdb_plugin::impl::buildAccountHistoryRecord(
    const account_name_type &name, const rocksdb_operation_object &obj) {
  ...
  if (found) {
    ...
    if (_prune && count > ACCOUNT_HISTORY_LENGTH_LIMIT &&
        ((obj.timestamp - ahInfo.oldestEntryTimestamp) >
         fc::days(ACCOUNT_HISTORY_TIME_LIMIT))) {
    ...
    auto s = _writeBuffer.Put(_columnHandles[AH_OPERATION_BY_ID], ahInfoOpSlice,
                              valueSlice);
    checkStatus(s);
  } else {
...
void account_history_rocksdb_plugin::impl::prunePotentiallyTooOldItems(
    account_history_info *ahInfo, const account_name_type &name,
    const fc::time_point_sec &now) {
  std::string strName = name;
  ...
  ah_op_by_id_slice_t oldestEntrySlice(
      std::make_pair(ahInfo->id, ahInfo->oldestEntryId));
  auto lookupUpperBound = std::make_pair(
      ahInfo->id + 1, ahInfo->newestEntryId - ACCOUNT_HISTORY_LENGTH_LIMIT + 1);
  ...
  auto s = _writeBuffer.SingleDelete(_columnHandles[AH_OPERATION_BY_ID],
                                     oldestEntrySlice);
  checkStatus(s);

  std::unique_ptr<::rocksdb::Iterator> dataItr(
      _storage->NewIterator(rOptions, _columnHandles[AH_OPERATION_BY_ID]));

  /** To clean outdated records we have to iterate over all AH records having
   * subsequent number greater than limit and additionally verify date of
   * operation, to clean up only these exceeding a date limit. So just
   * operations having a list position > ACCOUNT_HISTORY_LENGTH_LIMIT and older
   * that ACCOUNT_HISTORY_TIME_LIMIT shall be removed.
   */
  ...
  for (; dataItr->Valid(); dataItr->Next()) {
    ...
    if (foundEntry.first != ahInfo->id ||
        foundEntry.second >= lookupUpperBound.second)
      break;
...
void account_history_rocksdb_plugin::impl::on_pre_reindex(
    const blurt::chain::reindex_notification &note) {
  ilog(
      "Received onReindexStart request, attempting to clean database storage.");
...
void account_history_rocksdb_plugin::impl::on_post_reindex(
    const blurt::chain::reindex_notification &note) {
  ilog("Reindex completed up to block: ${b}. Setting back write limit to "
       "non-massive level.",
...
void account_history_rocksdb_plugin::impl::printReport(
    uint32_t blockNo, const char *detailText) const {
  ilog("${t}Processed blocks: ${n}, containing: ${tx} transactions and ${op} "
       "operations.\n"
       ...
       ("t", detailText)("n", blockNo)("tx", _txNo)("op", _totalOps)(
           "ep", _excludedOps)("ea", _excludedAccountCount));
}
...
void account_history_rocksdb_plugin::impl::importData(unsigned int blockLimit) {
  ...
  dumper.initialize([](benchmark_dumper::database_object_sizeof_cntr_t &) {},
                    "rocksdb_data_import.json");

  _mainDb.foreach_operation(
      [blockLimit, &blockNo, &lastBlock,
       this](const signed_block_header &prevBlockHeader,
             const signed_block &block, const signed_transaction &tx,
  ...
  const auto &measure = dumper.measure(
      blockNo, [](benchmark_dumper::index_memory_details_cntr_t &, bool) {});
  ilog("RocksDb data import - Performance report at block ${n}. Elapsed time: "
       "${rt} ms (real), ${ct} ms (cpu). Memory usage: ${cm} (current), ${pm} "
       "(peak) kilobytes.",
       ("n", blockNo)("rt", measure.real_ms)("ct", measure.cpu_ms)(
           "cm", measure.current_mem)("pm", measure.peak_mem));

  printReport(blockNo, "RocksDB data import finished. ");
}

void account_history_rocksdb_plugin::impl::on_post_apply_operation(
    const operation_notification &n) {
  if (n.block % 10000 == 0 && n.trx_in_block == 0 && n.op_in_trx == 0 &&
      n.virtual_op == 0) {
    ilog("RocksDb data import processed blocks: ${n}, containing: ${tx} "
         "transactions and ${op} operations.\n"
         " ${ep} operations have been filtered out due to configured options.\n"
         " ${ea} accounts have been filtered out due to configured options.",
         ("n", n.block)("tx", _txNo)("op", _totalOps)("ep", _excludedOps)(
  ...
  if (impacted.empty())
    return; // Ignore operations not impacting any account (according to
  ...
  if (_reindexing) {
    ...
    _mainDb.create<volatile_operation_object>(
        [&](volatile_operation_object &o) {
          o.trx_id = n.trx_id;
          o.block = n.block;
...
void account_history_rocksdb_plugin::impl::on_irreversible_block(
    uint32_t block_num) {
  if (_reindexing)
    return;
  ...
  const auto &volatile_idx =
      _mainDb.get_index<volatile_operation_index, by_block>();
...
void account_history_rocksdb_plugin::set_program_options(
  ...
  cfg.add_options()(
      ...
      "The location of the rocksdb database for account history. By default it "
      "is $DATA_DIR/blockchain/account-history-rocksdb-storage")(
      "account-history-rocksdb-track-account-range",
      boost::program_options::value<std::vector<std::string>>()
          ->composing()
          ->multitoken(),
      "Defines a range of accounts to track as a json pair [\"from\",\"to\"] "
      "[from,to] Can be specified multiple times.")(
  ...
  command_line_options.add_options()(
      ...
      "Allows to force immediate data import at plugin startup. By default "
      "storage is supplied during reindex process.")(
      "account-history-rocksdb-stop-import-at-block",
      bpo::value<uint32_t>()->default_value(0),
      "Allows to specify block number, the data import process should stop "
      "at.");
}

void account_history_rocksdb_plugin::plugin_initialize(
    const boost::program_options::variables_map &options) {
  ilog("Initializing account_history_rocksdb_plugin");

  if (options.count("account-history-rocksdb-stop-import-at-block"))
    _blockLimit = options.at("account-history-rocksdb-stop-import-at-block")
                      .as<uint32_t>();

  _doImmediateImport =
      options.at("account-history-rocksdb-immediate-import").as<bool>();
...
void account_history_rocksdb_plugin::find_account_history_data(
    const account_name_type &name, uint64_t start, uint32_t limit,
    std::function<void(unsigned int, const rocksdb_operation_object &)>
        processor) const {
...
bool account_history_rocksdb_plugin::find_operation_object(
    size_t opId, rocksdb_operation_object *op) const {
  return _my->find_operation_object(opId, op);
}

void account_history_rocksdb_plugin::find_operations_by_block(
    size_t blockNum,
    std::function<void(const rocksdb_operation_object &)> processor) const {
  _my->find_operations_by_block(blockNum, processor);
}

uint32_t account_history_rocksdb_plugin::enum_operations_from_block_range(
    uint32_t blockRangeBegin, uint32_t blockRangeEnd,
    std::function<void(const rocksdb_operation_object &)> processor) const {
  return _my->enumVirtualOperationsFromBlockRange(blockRangeBegin,
                                                  blockRangeEnd, processor);
}
