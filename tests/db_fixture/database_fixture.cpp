...
clean_database_fixture::clean_database_fixture(
  ...
  try {
    ...
    db_plugin =
        &appbase::app()
             .register_plugin<blurt::plugins::debug_node::debug_node_plugin>();
    ...
    appbase::app()
        .initialize<blurt::plugins::account_history::account_history_plugin,
                    blurt::plugins::debug_node::debug_node_plugin,
                    blurt::plugins::rc::rc_plugin,
                    blurt::plugins::witness::witness_plugin>(argc, argv);
    ...
    appbase::app()
        .get_plugin<blurt::plugins::rc::rc_plugin>()
        .set_rc_plugin_skip_flags(rc_skip);
...
void database_fixture::open_database(uint16_t shared_file_size_in_mb) {
  if (!data_dir) {
    ...
    args.shared_mem_dir = args.data_dir;
    args.initial_supply = BLURT_INIT_SUPPLY;
    args.shared_file_size =
        1024 * 1024 *
        shared_file_size_in_mb; // 8MB(default) or more:  file for testing
    args.database_cfg = blurt::utilities::default_database_configuration();
...
void database_fixture::generate_block(uint32_t skip,
                                      ...
                                      int miss_blocks) {
  skip |= default_skip;
  db_plugin->debug_generate_blocks(blurt::utilities::key_to_wif(key), 1, skip,
                                   miss_blocks);
}

void database_fixture::generate_blocks(uint32_t block_count) {
  auto produced =
      db_plugin->debug_generate_blocks(debug_key, block_count, default_skip, 0);
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp,
  ...
  db_plugin->debug_generate_blocks_until(
      debug_key, timestamp, miss_intermediate_blocks, default_skip);
...
const account_object &database_fixture::account_create(
    const string &name, const string &creator,
    const private_key_type &creator_key, const share_type &fee,
    const public_key_type &key, const public_key_type &post_key,
    const string &json_metadata) {
  try {
    auto actual_fee =
        std::min(fee, db->get_witness_schedule_object()
                          .median_props.account_creation_fee.amount);
...
database_fixture::account_create(const string &name, const public_key_type &key,
                                 const public_key_type &post_key) {
  try {
    return account_create(
        name, BLURT_INIT_MINER_NAME, init_account_priv_key,
        std::max(db->get_witness_schedule_object()
                     .median_props.account_creation_fee.amount,
                 share_type(100)),
        key, post_key, "");
  }
...
const account_object &
database_fixture::account_create(const string &name,
                                 const public_key_type &key) {
  return account_create(name, key, key);
}

const witness_object &database_fixture::witness_create(
    const string &owner, const private_key_type &owner_key, const string &url,
...
void database_fixture::fund(const string &account_name,
...
void database_fixture::vest(const string &from, const string &to,
                            const asset &amount) {
  try {
    ...
    // This sign() call fixes some tests, like withdraw_vesting_apply, that use
    // this method
    //   with debug_plugin such that trx may be re-applied with less generous
    //   skip flags.
    if (from == BLURT_INIT_MINER_NAME) {
      sign(trx, init_account_priv_key);
    }
...
void database_fixture::set_witness_props(
  ...
  FC_ASSERT(false, "Couldn't apply properties in ${n} blocks",
            ("n", 2 * BLURT_MAX_WITNESSES + 1));
}
...
void database_fixture::sign(signed_transaction &trx,
                            const fc::ecc::private_key &key) {
...
vector<operation> database_fixture::get_last_operations(uint32_t num_ops) {
  ...
  while (itr != acc_hist_idx.begin() && ops.size() < num_ops) {
    ...
    std::copy(_serialized_op.begin(), _serialized_op.end(),
              std::back_inserter(serialized_op));
    ops.push_back(
        fc::raw::unpack_from_vector<blurt::chain::operation>(serialized_op));
  }
...
void sps_proposal_database_fixture::plugin_prepare() {
  ...
  db_plugin->logging = false;
  appbase::app().initialize<blurt::plugins::debug_node::debug_node_plugin>(
      argc, argv);
...
int64_t sps_proposal_database_fixture::create_proposal(
    std::string creator, std::string receiver, time_point_sec start_date,
    time_point_sec end_date, asset daily_pay, const fc::ecc::private_key &key) {
  ...
  const auto &proposal_idx = db->template get_index<proposal_index>()
                                 .indices()
                                 .template get<by_proposal_id>();
...
void sps_proposal_database_fixture::vote_proposal(
    std::string voter, const std::vector<int64_t> &id_proposals, bool approve,
    const fc::ecc::private_key &key) {
...
bool sps_proposal_database_fixture::exist_proposal(int64_t id) {
  const auto &proposal_idx =
      db->get_index<proposal_index>().indices().template get<by_proposal_id>();
  return proposal_idx.find(id) != proposal_idx.end();
}

const proposal_object *
sps_proposal_database_fixture::find_proposal(int64_t id) {
  const auto &proposal_idx =
      db->get_index<proposal_index>().indices().template get<by_proposal_id>();
...
void sps_proposal_database_fixture::remove_proposal(
    account_name_type _deleter, flat_set<int64_t> _proposal_id,
    const fc::ecc::private_key &_key) {
...
bool sps_proposal_database_fixture::find_vote_for_proposal(
    const std::string &_user, int64_t _proposal_id) {
  const auto &proposal_vote_idx = db->get_index<proposal_vote_index>()
                                      .indices()
                                      .template get<by_voter_proposal>();
  auto found_vote =
      proposal_vote_idx.find(boost::make_tuple(_user, _proposal_id));
  return found_vote != proposal_vote_idx.end();
}

uint64_t
sps_proposal_database_fixture::get_nr_blocks_until_maintenance_block() {
  auto block_time = db->head_block_time();

  auto next_maintenance_time =
      db->get_dynamic_global_properties().next_maintenance_time;
  auto ret =
      (next_maintenance_time - block_time).to_seconds() / BLURT_BLOCK_INTERVAL;
...
void sps_proposal_database_fixture::post_comment(
    std::string _authro, std::string _permlink, std::string _title,
    std::string _body, std::string _parent_permlink,
    const fc::ecc::private_key &_key) {
...
json_rpc_database_fixture::json_rpc_database_fixture() {
  try {
    ...
    rpc_plugin =
        &appbase::app()
             .register_plugin<blurt::plugins::json_rpc::json_rpc_plugin>();
    appbase::app()
        .register_plugin<blurt::plugins::block_api::block_api_plugin>();
    appbase::app()
        .register_plugin<blurt::plugins::database_api::database_api_plugin>();
    appbase::app()
        .register_plugin<blurt::plugins::condenser_api::condenser_api_plugin>();
    ...
    appbase::app()
        .initialize<blurt::plugins::account_history::account_history_plugin,
                    ...
                    blurt::plugins::condenser_api::condenser_api_plugin>(argc,
                                                                         argv);
...
void json_rpc_database_fixture::review_answer(fc::variant &answer, int64_t code,
                                              bool is_warning, bool is_fail,
...
void json_rpc_database_fixture::make_array_request(std::string &request,
                                                   int64_t code,
                                                   bool is_warning,
                                                   bool is_fail) {
...
fc::variant json_rpc_database_fixture::make_request(std::string &request,
                                                    int64_t code,
                                                    bool is_warning,
                                                    bool is_fail) {
  fc::variant answer = get_answer(request);
...
bool _push_block(database &db, const signed_block &b,
