...
#define PUSH_TX blurt::chain::test::_push_transaction

#define PUSH_BLOCK blurt::chain::test::_push_block
...
#define BLURT_REQUIRE_THROW(expr, exc_type) BOOST_REQUIRE_THROW(expr, exc_type);
...
#define BLURT_CHECK_THROW(expr, exc_type)                                      \
  {                                                                            \
    ...
    if (fc::enable_record_assert_trip)                                         \
      std::cout << "BLURT_CHECK_THROW begin " << req_throw_info << std::endl;  \
    BOOST_CHECK_THROW(expr, exc_type);                                         \
    if (fc::enable_record_assert_trip)                                         \
      std::cout << "BLURT_CHECK_THROW end " << req_throw_info << std::endl;    \
  }
...
/// This simply resets v back to its default-constructed value. Requires v to
/// have a working assingment operator and
/// default constructor.
#define RESET(v) v = decltype(v)()
/// This allows me to build consecutive test cases. It's pretty ugly, but it
/// works well enough for unit tests.
/// i.e. This allows a test on update_account to begin with the database at the
/// end state of create_account.
#define INVOKE(test)                                                           \
  ((struct test *)this)->test_method();                                        \
  trx.clear()

#define PREP_ACTOR(name)                                                       \
  fc::ecc::private_key name##_private_key =                                    \
      generate_private_key(BOOST_PP_STRINGIZE(name));                          \
  fc::ecc::private_key name##_post_key =                                       \
      generate_private_key(std::string(BOOST_PP_STRINGIZE(name)) + "_post");   \
  public_key_type name##_public_key = name##_private_key.get_public_key();

#define ACTOR(name)                                                            \
  PREP_ACTOR(name)                                                             \
  const auto &name =                                                           \
      account_create(BOOST_PP_STRINGIZE(name), name##_public_key,              \
                                        name##_post_key.get_public_key());     \
...
struct database_fixture {
  ...
  uint32_t default_skip =
      0 | database::skip_undo_history_check | database::skip_authority_check;
  ...
  /**
   * @brief Generates blocks until the head block time matches or exceeds
   * timestamp
  ...
  account_create(const string &name, const string &creator,
...
struct sps_proposal_database_fixture : public clean_database_fixture {
  ...
  void vote_proposal(std::string voter,
                     const std::vector<int64_t> &id_proposals, bool approve,
  ...
  void post_comment(std::string _authro, std::string _permlink,
                    std::string _title, std::string _body,
                    std::string _parent_permlink,
                    const fc::ecc::private_key &_key);
};

struct sps_proposal_database_fixture_performance
    : public sps_proposal_database_fixture {
  sps_proposal_database_fixture_performance(
      uint16_t shared_file_size_in_mb = 512)
...
private:
  ...
  void review_answer(fc::variant &answer, int64_t code, bool is_warning,
                     bool is_fail, fc::optional<fc::variant> id);

public:
  json_rpc_database_fixture();
  virtual ~json_rpc_database_fixture();

  void make_array_request(std::string &request, int64_t code = 0,
                          bool is_warning = false, bool is_fail = true);
  fc::variant make_request(std::string &request, int64_t code = 0,
                           bool is_warning = false, bool is_fail = true);
