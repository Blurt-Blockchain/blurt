...
#define TRANSCATION_STATUS_TRACK_AFTER_BLOCK_STR                               \
  BOOST_PP_STRINGIZE( TRANSCATION_STATUS_TRACK_AFTER_BLOCK )
#define TRANSACTION_STATUS_TEST_BLOCK_DEPTH 30
#define TRANSACTION_STATUS_TEST_BLOCK_DEPTH_STR                                \
  BOOST_PP_STRINGIZE( TRANSACTION_STATUS_TEST_BLOCK_DEPTH )
...
BOOST_AUTO_TEST_CASE(transaction_status_test) {
  ...
  try {
    ...
    appbase::app()
        .register_plugin<blurt::plugins::transaction_status_api::
    ...
    // We create an argc/argv so that the transaction_status plugin can be
    // initialized with a reasonable block depth
    int test_argc = 5;
    ...
    appbase::app()
        .initialize<blurt::plugins::transaction_status_api::
                        transaction_status_api_plugin,
    ...
    auto tx_status_api =
        &appbase::app()
             .get_plugin<blurt::plugins::transaction_status_api::
                             transaction_status_api_plugin>();
    BOOST_REQUIRE(tx_status_api);

    auto tx_status = &appbase::app()
                          .get_plugin<blurt::plugins::transaction_status::
                                          transaction_status_plugin>();
    BOOST_REQUIRE(tx_status);

    open_database();

    BOOST_REQUIRE(db->get_index<transaction_status_index>()
                      .indices()
                      .get<by_id>()
                      .empty());
    BOOST_REQUIRE(db->get_index<transaction_status_index>()
    ...
    for (int i = 1; i < BLURT_MAX_WITNESSES; i++) {
      ...
      fund(BLURT_INIT_MINER_NAME + fc::to_string(i),
           BLURT_MIN_PRODUCER_REWARD.amount.value);
    ...
    auto tx0_expiration =
        db->head_block_time() + BLURT_MAX_TIME_UNTIL_EXPIRATION;
    ...
    // TRANSCATION_STATUS_TRACK_AFTER_BLOCK - ( BLURT_MAX_TIME_UNTIL_EXPIRATION
    // / BLURT_BLOCK_INTERVAL ) blocks
    BOOST_REQUIRE(db->get_index<transaction_status_index>()
                      .indices()
                      .get<by_id>()
                      .empty());
    ...
    auto api_return =
        tx_status_api->api->find_transaction({.transaction_id = tx0.id()});
    BOOST_REQUIRE(api_return.status == unknown);
    BOOST_REQUIRE(api_return.block_num.valid() == false);

    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = tx0.id(), tx0_expiration});
    BOOST_REQUIRE(api_return.status == unknown);
    BOOST_REQUIRE(api_return.block_num.valid() == false);
    ...
    api_return =
        tx_status_api->api->find_transaction({.transaction_id = tx1.id()});
    BOOST_REQUIRE(api_return.status == within_mempool);
    BOOST_REQUIRE(api_return.block_num.valid() == false);

    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = tx1.id(), tx1_expiration});
    BOOST_REQUIRE(api_return.status == within_mempool);
    BOOST_REQUIRE(api_return.block_num.valid() == false);
    ...
    api_return =
        tx_status_api->api->find_transaction({.transaction_id = tx2.id()});
    BOOST_REQUIRE(api_return.status == within_mempool);
    BOOST_REQUIRE(api_return.block_num.valid() == false);

    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = tx2.id(), tx2_expiration});
    BOOST_REQUIRE(api_return.status == within_mempool);
    BOOST_REQUIRE(api_return.block_num.valid() == false);
    ...
    api_return =
        tx_status_api->api->find_transaction({.transaction_id = tx3.id()});
    BOOST_REQUIRE(api_return.status == within_mempool);
    BOOST_REQUIRE(api_return.block_num.valid() == false);

    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = tx3.id(), tx3_expiration});
    BOOST_REQUIRE(api_return.status == within_mempool);
    BOOST_REQUIRE(api_return.block_num.valid() == false);
    ...
    auto lib_time =
        db->fetch_block_by_number(
            ...
            ->timestamp;
    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = transaction_id_type(), .expiration = lib_time});
    ...
    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = transaction_id_type(), after_lib_time});
    BOOST_REQUIRE(api_return.status == expired_reversible);
    BOOST_REQUIRE(api_return.block_num.valid() == false);

    // One second before our block depth
    auto old_time =
        db->fetch_block_by_number(db->head_block_num() -
                                  TRANSACTION_STATUS_TEST_BLOCK_DEPTH + 1)
            ->timestamp -
        fc::seconds(1);
    api_return = tx_status_api->api->find_transaction(
        {.transaction_id = transaction_id_type(), old_time});
    BOOST_REQUIRE(api_return.status == too_old);
    BOOST_REQUIRE(api_return.block_num.valid() == false);
    ...
    const auto &tx_status_obj =
        db->get<transaction_status_object, by_trx_id>(tx4.id());
    ...
    const auto &tx_status_obj2 =
        db->get<transaction_status_object, by_trx_id>(tx5.id());
