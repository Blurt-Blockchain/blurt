...
/*
   ...
   a) There are implemented wrappers for 3 methods: database::create,
   database::modify, database::remove. b) Methods 'remember_old_values' and
   'check' help to compare state before and after running 'undo' mechanism.
*/
template <typename Object> class undo_scenario {
private:
  chain::database &db;
  ...
  const Object *run_impl(const Object *old_obj, u_types::op_type op,
                         CALL call) {
    switch (op) {
    case u_types::create:
      assert(!old_obj);
...
public:
  ...
  template <typename CALL> const Object &create(CALL call) {
  ...
  template <typename Index> void remember_old_values() {
    old_values.clear();
  ...
  template <typename Index> uint32_t size() {
    const auto &idx = db.get_index<Index, by_id>();
    return idx.size();
  }

  // Reads current objects( for given index ) and compares with objects which
  // have been saved before. The comparision is according to 'id' field.
  template <typename Index> bool check() {
    try {
      const auto &idx = db.get_index<Index, by_id>();
