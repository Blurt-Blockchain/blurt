...
BOOST_AUTO_TEST_CASE(undo_basic) {
  try {
    ...
    const account_object &obj1 =
        ao.create([&](account_object &obj) { obj.name = "name00"; });
    BOOST_REQUIRE(std::string(obj1.name) == "name00");
    const account_object &obj2 =
        ao.modify(obj1, [&](account_object &obj) { obj.name = "name01"; });
    ...
    const account_object &obj_c =
        ao.create([&](account_object &obj) { obj.name = "name00"; });
    BOOST_REQUIRE(std::string(obj_c.name) == "name00");

    const account_object &obj_cm =
    ...
    const account_object &obj_cr =
        ao.create([&](account_object &obj) { obj.name = "name03"; });
...
BOOST_AUTO_TEST_CASE(undo_object_disapear) {
  try {
    BOOST_TEST_MESSAGE("--- 2 objects. Modifying 1 object - uniqueness of "
                       "complex index is violated");
    ...
    ao.create([&](account_object &obj) {
      obj.name = "name00";
      obj.proxy = "proxy00";
    });
    BOOST_REQUIRE(old_size + 1 == ao.size<account_index>());

    const account_object &obj1 = ao.create([&](account_object &obj) {
      obj.name = "name01";
      obj.proxy = "proxy01";
    });
    ...
    /*
       Important!!!
          Method 'generic_index::modify' works incorrectly - after 'contraint
       violation', element is removed. Solution: It's necessary to write fix,
       according to issue #2154.
    */
    // Temporary. After fix, this line should be enabled.
    // BLURT_REQUIRE_THROW( ao.modify( obj1, [&]( account_object& obj ){
    // obj.name = "name00"; obj.proxy = "proxy00"; } ), boost::exception );

    // Temporary. After fix, this line should be removed.
    ao.modify(obj1, [&](account_object &obj) {
      obj.name = "nameXYZ";
      obj.proxy = "proxyXYZ";
    });
...
BOOST_AUTO_TEST_CASE(undo_key_collision) {
  try {
    ...
    BOOST_TEST_MESSAGE("--- 2 objects. Object 'obj0' is created before 'undo' "
                       "and has modified key in next step.");
    BOOST_TEST_MESSAGE(
        "--- Object 'obj1' retrieves old key from object 'obj0'.");
    ...
    /*
       Important!!!
          The mechanism 'undo' works incorrectly, when a unique key is changed
       in object, but such object is saved in 'undo' earlier. In next step
       another object gets identical key what first object had before. In such
       situation, attempt of modification by 'undo', raises 'uniqueness
       constraint' violation. Currently, simple solution used in whole project
       is adding unique key 'id' to save uniqueness. Solution: Is necessary to
       write another version of 'undo'?
    */
    // Temporary. After fix, this line should be enabled.
    // ao.create( [&]( account_object& obj ){ obj.name = "name00"; } );
    ...
    BOOST_TEST_MESSAGE("Version B - work without fix, because key "
                       "'cashout_time' is combined with 'id' key.");
    BOOST_TEST_MESSAGE("--- 2 objects. Object 'obj0' is created before 'undo' "
                       "and has modified key in next step.");
    BOOST_TEST_MESSAGE(
        "--- Object 'obj1' retrieves old key from object 'obj0'.");
    ...
    const comment_object &objc0 = co.create([&](comment_object &obj) {
      obj.cashout_time = time_point_sec(20);
      obj.author = "a1";
    });
    ...
    co.modify(objc0, [&](comment_object &obj) {
      obj.cashout_time = time_point_sec(21);
    });
    co.create([&](comment_object &obj) {
      obj.cashout_time = time_point_sec(20);
      obj.author = "a2";
    });
...
BOOST_AUTO_TEST_CASE(undo_different_indexes) {
  try {
    ...
    const account_object &obja0 =
        ao.create([&](account_object &obj) { obj.name = "name00"; });
    BOOST_REQUIRE(std::string(obja0.name) == "name00");
    BOOST_REQUIRE(old_size_ao + 1 == ao.size<account_index>());

    const comment_object &objc0 = co.create(
        [&](comment_object &obj) { obj.cashout_time = time_point_sec(11); });
    ...
    BOOST_TEST_MESSAGE("--- 'account_objects' - ( obj A )create, ( obj B "
                       ")create/modify, ( obj C )create/remove.");
    BOOST_TEST_MESSAGE(
        "--- 'comment_objects' - ( obj A )create, ( obj B )create/remove.");
    ao.remember_old_values<account_index>();
    co.remember_old_values<comment_index>();
    old_size_ao = ao.size<account_index>();
    old_size_co = co.size<comment_index>();
    ...
    ao.create([&](account_object &obj) { obj.name = "name00"; });
    const account_object &obja1 =
        ao.create([&](account_object &obj) { obj.name = "name01"; });
    const account_object &obja2 =
        ao.create([&](account_object &obj) { obj.name = "name02"; });
    ...
    co.create([&](comment_object &obj) {
      obj.cashout_time = time_point_sec(11);
      obj.author = "a0";
    });
    const comment_object &objc1 = co.create([&](comment_object &obj) {
      obj.cashout_time = time_point_sec(12);
      obj.author = "a1";
    });
    co.modify(objc1, [&](comment_object &obj) {
      obj.cashout_time = time_point_sec(13);
    });
    ...
    BOOST_TEST_MESSAGE("--- 3 objects: 'account_object' + 'comment_object' + "
    ...
    BOOST_TEST_MESSAGE(
        "--- 'comment_content_object' - ( obj A )create/remove.");
    ao.remember_old_values<account_index>();
    co.remember_old_values<comment_index>();
    ...
    ao.create([&](account_object &obj) { obj.name = "name01"; });
    const comment_object &objc2 = co.create([&](comment_object &obj) {
      obj.cashout_time = time_point_sec(12);
      obj.author = "a1";
    });
    const comment_content_object &objcc1 =
        cc.create([&](comment_content_object &obj) { obj.comment = 13; });
    BOOST_REQUIRE(old_size_ao + 1 == ao.size<account_index>());
    BOOST_REQUIRE(old_size_co + 1 == co.size<comment_index>());
    ...
    const comment_content_object &cc1 =
        cc.create([&](comment_content_object &obj) { obj.comment = 0; });
    const comment_object &co1 = co.create([&](comment_object &obj) {
      obj.cashout_time = time_point_sec(12);
      obj.author = std::to_string(0);
    });
    const account_object &ao1 =
        ao.create([&](account_object &obj) { obj.name = std::to_string(0); });
    ...
    for (int32_t i = 1; i <= 5; ++i) {
      co.modify(co1, [&](comment_object &obj) {
        obj.cashout_time = time_point_sec(12);
        obj.author = std::to_string(0);
      });
      ao.modify(ao1,
                [&](account_object &obj) { obj.name = std::to_string(0); });
...
BOOST_AUTO_TEST_CASE(undo_generate_blocks) {
  try {
    ...
    struct _data {
      ...
      _data(std::string a, std::string pl, std::string pa, std::string ppl,
            std::string t, std::string b)
          : author(a), permlink(pl), parent_author(pa), parent_permlink(ppl),
            title(t), body(b) {}
    ...
    _data data[4] = {_data("bob", "post4", std::string(BLURT_ROOT_POST_PARENT),
                           "pl4", "t4", "b4"),
                     _data("alice", "post", std::string(BLURT_ROOT_POST_PARENT),
                           "pl", "t", "b"),
