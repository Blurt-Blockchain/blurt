...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: BLURT cannot be an SMT" );
      op.symbol = STEEM_SYMBOL;
      STEEM_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: BBD cannot be an SMT" );
...
BOOST_AUTO_TEST_CASE( smt_creation_fee_test )
{
   try
   {
      ...
      for ( int i = 0; i < 2; i++ )
      {
         ...
         BOOST_TEST_MESSAGE( " -- Valid creation fee, using BBD" );
...
BOOST_AUTO_TEST_CASE( smt_contribute_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( " -- Failure on BBD contribution" );
      ...
      BOOST_TEST_MESSAGE( " -- Failure on contribution to BLURT" );
      op.symbol = STEEM_SYMBOL;
      STEEM_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.symbol = new_symbol;
      ...
      BOOST_TEST_MESSAGE( " -- Failure on contribution to BBD" );
      op.symbol = SBD_SYMBOL;
      STEEM_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.symbol = new_symbol;
