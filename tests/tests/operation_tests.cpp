...
inline uint16_t get_voting_power(const account_object &a) {
  return (uint16_t)(a.voting_manabar.current_mana /
                    chain::util::get_effective_vesting_shares(a));
}
  ...
  try {
...

    {
      ...
      props["account_creation_fee"] =
          fc::raw::pack_to_vector(ASSET("0.100 TESTS"));
    ...
    const account_authority_object &acct_auth =
        db->get<account_authority_object, by_account>("alice");
    ...
    BOOST_REQUIRE((init_starting_balance - ASSET("0.100 TESTS")).amount.value ==
    ...
    tx.signatures.clear();
    tx.operations.clear();
    op.fee = asset(db->get_account(BLURT_INIT_MINER_NAME).balance.amount + 1,
                   BLURT_SYMBOL);
    op.new_account_name = "bob";
    tx.operations.push_back(op);
    sign(tx, init_account_priv_key);
    ...
    BOOST_TEST_MESSAGE("--- Test account creation with temp account does not "
    ...
    BOOST_REQUIRE(db->get_account("bob").recovery_account ==
                  account_name_type());
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(comment_authorities) {
  try {
    ...
    op.body = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do "
    ...
    BOOST_TEST_MESSAGE("--- Test failure when signed by an additional "
                       "signature not in the creator's authority");
    sign(tx, bob_private_key);
    BLURT_REQUIRE_THROW(
        db->push_transaction(tx, database::skip_transaction_dupe_check),
        tx_irrelevant_sig);

    BOOST_TEST_MESSAGE("--- Test failure when signed by a signature not in the "
                       "creator's authority");
    tx.signatures.clear();
    sign(tx, bob_private_key);
    BLURT_REQUIRE_THROW(
...
BOOST_AUTO_TEST_CASE(comment_apply) {
  try {
    ...
    const comment_object &alice_comment =
        db->get_comment("alice", string("lorem"));
    ...
    BOOST_REQUIRE(
        alice_comment.cashout_time ==
        fc::time_point_sec(db->head_block_time() +
                           fc::seconds(BLURT_CASHOUT_WINDOW_SECONDS)));

#ifndef IS_LOW_MEM
    const auto &alice_comment_content =
        db->get<comment_content_object, by_comment>(alice_comment.id);
    BOOST_REQUIRE(to_string(alice_comment_content.title) == op.title);
    BOOST_REQUIRE(to_string(alice_comment_content.body) == op.body);
    BOOST_REQUIRE(to_string(alice_comment_content.json_metadata) ==
                  op.json_metadata);
#else
    const auto *alice_comment_content =
        db->find<comment_content_object, by_comment>(alice_comment.id);
    ...
    BOOST_TEST_MESSAGE(
        "--- Test Bob posting a comment on a non-existent comment");
    op.author = "bob";
    op.permlink = "ipsum";
    ...
    BOOST_REQUIRE(bob_comment.cashout_time ==
                  bob_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
    ...
    BOOST_REQUIRE(sam_comment.cashout_time ==
                  sam_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
    ...
    db->modify(db->get_dynamic_global_properties(),
               [&](dynamic_global_property_object &o) {
                 o.total_reward_shares2 =
                     blurt::chain::util::evaluate_reward_curve(10);
               });
    ...
    BOOST_REQUIRE(mod_sam_comment.cashout_time ==
                  mod_sam_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
    validate_database();
...
BOOST_AUTO_TEST_CASE(comment_delete_apply) {
  try {
    ...
    tx.set_expiration(db->head_block_time() +
                      BLURT_MIN_TRANSACTION_EXPIRATION_LIMIT);
    sign(tx, alice_private_key);
    db->push_transaction(tx, 0);

    BOOST_TEST_MESSAGE(
        "--- Test failure deleting a comment with positive rshares");
    ...
    //      BOOST_TEST_MESSAGE( "--- Test success deleting a comment with
    //      negative rshares" );
    //
    //      generate_block();
    ...
    //      db->push_transaction( tx, 0 );
    //
    //      auto test_comment = db->find< comment_object, by_permlink >(
    //      boost::make_tuple( "alice", string( "test1" ) ) ); BOOST_REQUIRE(
    //      test_comment == nullptr );

    BOOST_TEST_MESSAGE("--- Test failure deleting a comment past cashout");
    generate_blocks(BLURT_MIN_ROOT_COMMENT_INTERVAL.to_seconds() /
                    BLURT_BLOCK_INTERVAL);
    ...
    BOOST_REQUIRE(db->get_comment("alice", string("test1")).cashout_time ==
...
BOOST_AUTO_TEST_CASE(vote_apply) {
  try {
    ...
    const auto &vote_idx =
        db->get_index<comment_vote_index>().indices().get<by_comment_voter>();

    {
      ...
      int64_t max_vote_denom =
          (db->get_dynamic_global_properties().vote_power_reserve_rate *
      ...
      BOOST_REQUIRE(alice_comment.net_rshares.value ==
                    (old_mana - alice.voting_manabar.current_mana) -
                        BLURT_VOTE_DUST_THRESHOLD);
      BOOST_REQUIRE(alice_comment.cashout_time ==
                    alice_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
      ...
      util::manabar_params params(
          util::get_effective_vesting_shares(db->get_account("alice")),
          BLURT_VOTING_MANA_REGENERATION_SECONDS);
      ...
      BOOST_REQUIRE(bob_comment.net_rshares.value ==
                    (old_manabar.current_mana -
                     db->get_account("alice").voting_manabar.current_mana) -
                        BLURT_VOTE_DUST_THRESHOLD);
      BOOST_REQUIRE(bob_comment.cashout_time ==
                    bob_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
      BOOST_REQUIRE(itr != vote_idx.end());
      ...
      auto old_abs_rshares =
          db->get_comment("alice", string("foo")).abs_rshares.value;

      generate_blocks(db->head_block_time() +
                          fc::seconds((BLURT_CASHOUT_WINDOW_SECONDS / 2)),
                      true);
      ...
      BOOST_REQUIRE(new_alice_comment.net_rshares.value ==
                    old_abs_rshares +
                        (old_mana - new_bob.voting_manabar.current_mana) -
                        BLURT_VOTE_DUST_THRESHOLD);
      BOOST_REQUIRE(new_alice_comment.cashout_time ==
                    new_alice_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
      BOOST_REQUIRE(itr != vote_idx.end());
      validate_database();
      ...
      params.max_mana =
          util::get_effective_vesting_shares(db->get_account("sam"));
      old_manabar.regenerate_mana(params, db->head_block_time());
      ...
      //         itr = vote_idx.find( boost::make_tuple( new_bob_comment.id,
      //         new_sam.id ) );
      //
      //         util::manabar old_downvote_manabar;
      //         util::manabar_params downvote_params(
      //         util::get_effective_vesting_shares( db->get_account( "alice" )
      //         ) / 4, BLURT_VOTING_MANA_REGENERATION_SECONDS );
      //         old_downvote_manabar.regenerate_mana( downvote_params,
      //         db->head_block_time() ); int64_t sam_weight =
      //         old_downvote_manabar.current_mana - db->get_account( "sam"
      //         ).downvote_manabar.current_mana - BLURT_VOTE_DUST_THRESHOLD;
      //
      //         BOOST_REQUIRE( new_bob_comment.net_rshares.value ==
      //         old_abs_rshares - sam_weight ); BOOST_REQUIRE(
      //         new_bob_comment.abs_rshares.value == old_abs_rshares +
      //         sam_weight ); BOOST_REQUIRE( new_bob_comment.cashout_time ==
      //         new_bob_comment.created + BLURT_CASHOUT_WINDOW_SECONDS );
      //         BOOST_REQUIRE( itr != vote_idx.end() );
      validate_database();
      ...
      int64_t regenerated_power =
          (BLURT_100_PERCENT *
           (db->head_block_time() - db->get_account("alice").last_vote_time)
               .to_seconds()) /
          BLURT_VOTING_MANA_REGENERATION_SECONDS;
      int64_t used_power = (get_voting_power(db->get_account("alice")) +
      ...
      auto alice_bob_vote =
          vote_idx.find(boost::make_tuple(new_bob_comment.id, new_alice.id));
      auto old_vote_rshares = alice_bob_vote->rshares;
      auto old_net_rshares = new_bob_comment.net_rshares.value;
      old_abs_rshares = new_bob_comment.abs_rshares.value;
      used_power = ((BLURT_1_PERCENT * 25 * (get_voting_power(new_alice)) /
                     BLURT_100_PERCENT) +
                    max_vote_denom - 1) /
                   max_vote_denom;
      ...
      new_rshares = old_manabar.current_mana -
                    db->get_account("alice").voting_manabar.current_mana -
                    BLURT_VOTE_DUST_THRESHOLD;
      ...
      old_abs_rshares = new_bob_comment.abs_rshares.value;
      used_power =
          (uint64_t(BLURT_1_PERCENT) * 75 * uint64_t(alice_voting_power)) /
          BLURT_100_PERCENT;
      ...
      //         old_downvote_manabar = db->get_account( "alice"
      //         ).downvote_manabar; downvote_params.max_mana =
      //         util::get_effective_vesting_shares( db->get_account( "alice" )
      //         ) / 4; old_downvote_manabar.regenerate_mana( downvote_params,
      //         db->head_block_time() );
      ...
      BOOST_REQUIRE(new_bob_comment.net_rshares ==
                    -1 * (old_net_rshares - old_vote_rshares - new_rshares));
      BOOST_REQUIRE(new_bob_comment.abs_rshares ==
                    old_abs_rshares + new_rshares);
      BOOST_REQUIRE(new_bob_comment.cashout_time ==
                    new_bob_comment.created + BLURT_CASHOUT_WINDOW_SECONDS);
      ...
      BOOST_TEST_MESSAGE(
          "--- Test changing a vote to 0 weight (aka: removing a vote)");
      ...
      //         BOOST_TEST_MESSAGE( "--- Test downvote overlap when downvote
      //         mana is low" );
      //
      //         generate_block();
      //         db_plugin->debug_update( [=]( database& db )
      ...
      //               a.downvote_manabar.last_update_time =
      ...
      //         {
      //            const auto& alice = db->get_account( "alice" );
      //            old_downvote_manabar.current_mana =
      //            alice.downvote_manabar.current_mana;
      //            old_downvote_manabar.last_update_time =
      //            alice.downvote_manabar.last_update_time;
      //            old_manabar.current_mana =
      //            alice.voting_manabar.current_mana;
      //            old_manabar.last_update_time =
      //            alice.voting_manabar.last_update_time;
      //            old_manabar.regenerate_mana( params, db->head_block_time()
      //            ); old_downvote_manabar.regenerate_mana( params,
      //            db->head_block_time() ); alice_weight =
      //            old_manabar.current_mana * 60 * 60 * 24; auto max_vote_denom
      //            =
      //            db->get_dynamic_global_properties().vote_power_reserve_rate
      //            * BLURT_VOTING_MANA_REGENERATION_SECONDS; alice_weight = (
      //            alice_weight + max_vote_denom - 1 ) / max_vote_denom;
      //
      //            const auto& bob_comment = db->get_comment( "bob", string(
      //            "foo" ) ); old_net_rshares = bob_comment.net_rshares.value;
      ...
      //         sign( tx, alice_private_key );
      //         db->push_transaction( tx, 0 );
      //         alice_bob_vote = vote_idx.find( boost::make_tuple(
      //         new_bob_comment.id, new_alice.id ) );
      //
      //         {
      //            const auto& bob_comment = db->get_comment( "bob", string(
      //            "foo" ) ); const auto& alice = db->get_account( "alice" );
      //            BOOST_REQUIRE( bob_comment.net_rshares == old_net_rshares -
      //            alice_weight + BLURT_VOTE_DUST_THRESHOLD ); BOOST_REQUIRE(
      //            bob_comment.abs_rshares == old_abs_rshares + alice_weight -
      //            BLURT_VOTE_DUST_THRESHOLD ); BOOST_REQUIRE(
      //            alice_bob_vote->rshares == -1 * ( alice_weight -
      //            BLURT_VOTE_DUST_THRESHOLD ) ); BOOST_REQUIRE(
      //            alice_bob_vote->last_update == db->head_block_time() );
      //            BOOST_REQUIRE( alice_bob_vote->vote_percent == op.weight );
      //            BOOST_REQUIRE( alice.downvote_manabar.current_mana == 0 );
      //            BOOST_REQUIRE( alice.voting_manabar.current_mana ==
      //            old_manabar.current_mana - alice_weight +
      //            old_downvote_manabar.current_mana ); validate_database();
      //         }

      BOOST_TEST_MESSAGE("--- Test reduced effectiveness when increasing "
                         "rshares within lockout period");

      generate_blocks(fc::time_point_sec((new_bob_comment.cashout_time -
                                          BLURT_UPVOTE_LOCKOUT_HF17)
                                             .sec_since_epoch() +
                                         BLURT_BLOCK_INTERVAL),
                      true);
      ...
      params.max_mana =
          util::get_effective_vesting_shares(db->get_account("dave"));
      old_manabar.regenerate_mana(params, db->head_block_time());
      ...
      new_rshares = old_manabar.current_mana -
                    db->get_account("dave").voting_manabar.current_mana -
                    BLURT_VOTE_DUST_THRESHOLD;
      new_rshares = (new_rshares *
                     (BLURT_UPVOTE_LOCKOUT_SECONDS - BLURT_BLOCK_INTERVAL)) /
      ...
      {
        auto dave_bob_vote = db->get<comment_vote_object, by_comment_voter>(
            boost::make_tuple(bob_comment_id, dave_id));
        BOOST_REQUIRE(dave_bob_vote.rshares = new_rshares);
      }
      validate_database();

      BOOST_TEST_MESSAGE("--- Test reduced effectiveness when reducing rshares "
      ...
      new_rshares = old_manabar.current_mana -
                    ...
                    BLURT_VOTE_DUST_THRESHOLD;
      new_rshares =
          (new_rshares * (BLURT_UPVOTE_LOCKOUT_SECONDS - BLURT_BLOCK_INTERVAL -
                          BLURT_BLOCK_INTERVAL)) /
          BLURT_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE(signature_stripping) {
  try {
    ...
    // Sam shouldn't be able to add or remove signatures to get the transaction
...
BOOST_AUTO_TEST_CASE(transfer_apply) {
  try {
    ...
    BOOST_REQUIRE(db->get_account(BLURT_TREASURY_ACCOUNT).balance ==
                  treasury_sbd_balance + ASSET("1.000 TESTS"));
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(withdraw_vesting_apply) {
  try {
...

    {
      ...
      BOOST_REQUIRE(
          alice.vesting_withdraw_rate.amount.value ==
          (old_vesting_shares.amount / (BLURT_VESTING_WITHDRAW_INTERVALS * 2))
                  .value +
              1);
      ...
      BOOST_REQUIRE(
          alice.vesting_withdraw_rate.amount.value ==
          (old_vesting_shares.amount / (BLURT_VESTING_WITHDRAW_INTERVALS * 3))
                  .value +
              1);
      BOOST_REQUIRE(alice.to_withdraw.value == op.vesting_shares.amount.value);
      BOOST_REQUIRE(alice.next_vesting_withdrawal ==
      ...
      BOOST_TEST_MESSAGE(
          "--- Test cancelling a withdraw when below the account creation fee");
          ...
          db.modify(db.get_dynamic_global_properties(),
                    [&](dynamic_global_property_object &gpo) {
                      gpo.current_supply +=
                          ...
                          ASSET("0.001 TESTS") - gpo.total_vesting_fund_blurt;
    ...
    db->push_transaction(
        tx,
        0); // We do not need to test the result of this, simply that it works.
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(witness_update_apply) {
  try {
    ...
    op.fee = ASSET("1.000 TESTS");
    op.block_signing_key = signing_key.get_public_key();
    op.props.account_creation_fee = legacy_blurt_asset::from_asset(
        asset(BLURT_MIN_ACCOUNT_CREATION_FEE + 10, BLURT_SYMBOL));
    ...
    BOOST_REQUIRE(alice_witness.props.account_creation_fee ==
                  op.props.account_creation_fee.to_asset<true>());
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when upgrading a non-existent account");
...
BOOST_AUTO_TEST_CASE(account_witness_vote_authorities) {
  try {
    ...
    witness_create("alice", alice_private_key, "foo.bar",
                   alice_witness_key.get_public_key(), 1000);
...
BOOST_AUTO_TEST_CASE(account_witness_vote_apply) {
  try {
    ...
    witness_create("sam", sam_private_key, "foo.bar",
                   sam_witness_key.get_public_key(), 1000);
    const witness_object &sam_witness = db->get_witness("sam");

    const auto &witness_vote_idx =
        db->get_index<witness_vote_index>().indices().get<by_witness_account>();
    ...
    BOOST_REQUIRE(witness_vote_idx.find(
                      boost::make_tuple(sam_witness.owner, alice.name)) !=
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when attempting to revoke a non-existent vote");
    ...
    BOOST_REQUIRE(sam_witness.votes ==
                  (bob.proxied_vsf_votes_total() + bob.vesting_shares.amount));
    BOOST_REQUIRE(witness_vote_idx.find(boost::make_tuple(
                      sam_witness.owner, bob.name)) != witness_vote_idx.end());
    BOOST_REQUIRE(witness_vote_idx.find(
                      boost::make_tuple(sam_witness.owner, alice.name)) ==
                  witness_vote_idx.end());
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when voting for a non-existent account");
    tx.operations.clear();
    tx.signatures.clear();
    op.witness = "dave";
    op.approve = true;
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when voting for an account that is not a witness");
    tx.operations.clear();
    tx.signatures.clear();
    op.witness = "alice";
    tx.operations.push_back(op);
...
BOOST_AUTO_TEST_CASE(account_witness_proxy_apply) {
  try {
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when changing proxy to existing proxy");
    ...
    BOOST_REQUIRE(dave.proxy == BLURT_PROXY_TO_SELF_ACCOUNT);
    BOOST_REQUIRE(
        dave.proxied_vsf_votes_total() ==
        (sam.vesting_shares + bob.vesting_shares + alice.vesting_shares)
            .amount);
    validate_database();
    ...
    BOOST_REQUIRE(db->get_witness(BLURT_INIT_MINER_NAME).votes ==
...
BOOST_AUTO_TEST_CASE(custom_json_rate_limit) {
  try {
    ...
    BOOST_TEST_MESSAGE(
        "--- Testing 5 custom json ops as separate transactions");
    ...
    BOOST_TEST_MESSAGE(
        "--- Testing failure of 6 custom json ops in one transaction");
    tx.clear();
    op.required_posting_auths.clear();
...
BOOST_AUTO_TEST_CASE(custom_binary_authorities) {
  ...
  op.required_posting_auths.insert("sam");
  op.required_auths.push_back(
      db->get<account_authority_object, by_account>("alice").posting);
...
BOOST_AUTO_TEST_CASE(account_recovery) {
  try {
    ...
    acc_create.owner =
        authority(1, generate_private_key("bob_owner").get_public_key(), 1);
    acc_create.active =
        authority(1, generate_private_key("bob_active").get_public_key(), 1);
    acc_create.posting =
        authority(1, generate_private_key("bob_posting").get_public_key(), 1);
    ...
    acc_update.owner =
        authority(1, generate_private_key("bad_key").get_public_key(), 1);
    ...
    BOOST_TEST_MESSAGE(
        "Recovering bob's account with original owner auth and new secret");
    ...
    db->push_transaction(tx, 0);
    const auto &owner1 =
        db->get<account_authority_object, by_account>("bob").owner;
    ...
    recover.new_owner_authority =
        authority(1, generate_private_key("idontknow").get_public_key(), 1);
    ...
    recover.recent_owner_authority =
        authority(1, generate_private_key("idontknow").get_public_key(), 1);
    recover.new_owner_authority =
        authority(1, generate_private_key("foo bar").get_public_key(), 1);
    ...
    BOOST_TEST_MESSAGE(
        "Testing using the same old owner auth again for recovery");
    ...
    const auto &request_idx =
        db->get_index<account_recovery_request_index>().indices();
    auto req_itr = request_idx.begin();

    BOOST_REQUIRE(req_itr->account_to_recover == "bob");
    BOOST_REQUIRE(
        req_itr->new_owner_authority ==
        authority(1, generate_private_key("expire").get_public_key(), 1));
    BOOST_REQUIRE(req_itr->expires ==
                  db->head_block_time() +
                      BLURT_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD);
    ...
    generate_blocks(db->head_block_time() +
                    (BLURT_OWNER_AUTH_RECOVERY_PERIOD -
                     BLURT_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD));
    generate_block();
...
BOOST_AUTO_TEST_CASE(change_recovery_account) {
  try {
    ...
    auto recover_account = [&](const std::string &account_to_recover,
                               const fc::ecc::private_key &new_owner_key,
                               const fc::ecc::private_key &recent_owner_key) {
      recover_account_operation op;
      op.account_to_recover = account_to_recover;
      op.new_owner_authority =
          authority(1, public_key_type(new_owner_key.get_public_key()), 1);
      op.recent_owner_authority =
          authority(1, public_key_type(recent_owner_key.get_public_key()), 1);
    ...
    auto change_owner = [&](const std::string &account,
                            const fc::ecc::private_key &old_private_key,
    ...
    BLURT_REQUIRE_THROW(change_recovery_account("alice", "nobody"),
                        fc::exception);
    BLURT_REQUIRE_THROW(change_recovery_account("haxer", "sam"), fc::exception);
    BLURT_REQUIRE_THROW(change_recovery_account("haxer", "nobody"),
                        fc::exception);
    change_recovery_account("alice", "sam");

    fc::ecc::private_key alice_priv1 =
        fc::ecc::private_key::regenerate(fc::sha256::hash("alice_k1"));
    fc::ecc::private_key alice_priv2 =
        fc::ecc::private_key::regenerate(fc::sha256::hash("alice_k2"));
    public_key_type alice_pub1 = public_key_type(alice_priv1.get_public_key());

    generate_blocks(db->head_block_time() + BLURT_OWNER_AUTH_RECOVERY_PERIOD -
                        fc::seconds(BLURT_BLOCK_INTERVAL),
                    true);
    ...
    BLURT_REQUIRE_THROW(
        ...
        fc::exception);
    // unless we change it!
    change_owner("alice", alice_private_key,
                 public_key_type(alice_priv2.get_public_key()));
    recover_account("alice", alice_priv1, alice_private_key);
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(escrow_transfer_validate) {
  try {
    ...
    BOOST_TEST_MESSAGE(
        "--- failure when ratification deadline == escrow expiration");
    op.fee.amount = 100;
...
BOOST_AUTO_TEST_CASE(escrow_approve_apply) {
  try {
    ...
    BOOST_TEST_MESSAGE("--- test automatic refund when escrow is not ratified "
                       "before deadline");
    tx.operations.clear();
    tx.signatures.clear();
    tx.operations.push_back(et_op);
    ...
    BOOST_TEST_MESSAGE(
        "--- test ratification expiration when escrow is only approved by to");
    tx.operations.clear();
    tx.signatures.clear();
    et_op.ratification_deadline = db->head_block_time() + 100;
    et_op.escrow_expiration = db->head_block_time() + 200;
    ...
    BOOST_TEST_MESSAGE("--- test ratification expiration when escrow is only "
                       "approved by agent");
    tx.operations.clear();
    tx.signatures.clear();
    et_op.ratification_deadline = db->head_block_time() + 100;
    ...
    BOOST_TEST_MESSAGE(
        "--- ratification expiration does not remove an approved escrow");
...
BOOST_AUTO_TEST_CASE(escrow_release_apply) {
  try {
    ...
    BOOST_TEST_MESSAGE("--- failure when 'agent' attempts to release "
                       "non-disputed escrow to 'to'");
    op.who = et_op.agent;
    tx.clear();
    tx.operations.push_back(op);
    sign(tx, sam_private_key);
    ...
    BOOST_TEST_MESSAGE("--- failure when 'agent' attempt to release "
                       "non-disputed escrow to not 'to' or 'from'");
    op.receiver = "dave";
    ...
    BOOST_TEST_MESSAGE("--- failure when other attempts to release "
                       "non-disputed escrow to 'to'");
    op.receiver = et_op.to;
    op.who = "dave";
    ...
    BOOST_TEST_MESSAGE("--- failure when other attempt to release non-disputed "
                       "escrow to not 'to' or 'from'");
    op.receiver = "dave";
    ...
    BOOST_TEST_MESSAGE("--- failure when 'to' attemtps to release non-disputed "
                       "escrow to 'to'");
    op.receiver = et_op.to;
    op.who = et_op.to;
    ...
    BOOST_TEST_MESSAGE("--- failure when 'to' attempts to release non-dispured "
                       "escrow to 'agent' ");
    ...
    BOOST_TEST_MESSAGE("--- failure when 'to' attempts to release non-disputed "
                       "escrow to not 'from'");
    op.receiver = "dave";
    ...
    BOOST_TEST_MESSAGE(
        "--- success release non-disputed escrow to 'to' from 'from'");
    op.receiver = et_op.from;
    ...
    BOOST_REQUIRE(db->get_escrow(op.from, op.escrow_id).blurt_balance ==
                  ASSET("0.900 TESTS"));
    BOOST_REQUIRE(db->get_account("alice").balance == ASSET("9.000 TESTS"));

    BOOST_TEST_MESSAGE("--- failure when 'from' attempts to release "
                       "non-disputed escrow to 'from'");
    op.receiver = et_op.from;
    op.who = et_op.from;
    ...
    BOOST_TEST_MESSAGE("--- failure when 'from' attempts to release "
                       "non-disputed escrow to not 'from'");
    op.receiver = "dave";
    ...
    BOOST_TEST_MESSAGE(
        "--- success release non-disputed escrow to 'from' from 'to'");
    op.receiver = et_op.to;
    ...
    BOOST_TEST_MESSAGE(
        "--- failure when 'to' attempts to release disputed escrow");
    ...
    BOOST_TEST_MESSAGE(
        "--- failure when 'from' attempts to release disputed escrow");
    tx.clear();
    op.receiver = et_op.to;
    op.who = et_op.from;
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE("--- failure when releasing disputed escrow to an "
                       "account not 'to' or 'from'");
    tx.clear();
    op.who = et_op.agent;
    op.receiver = "dave";
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE(
        "--- success releasing disputed escrow with agent to 'to'");
    tx.clear();
    op.receiver = et_op.to;
    op.who = et_op.agent;
    tx.operations.push_back(op);
    ...
    BOOST_REQUIRE(db->get_escrow(et_op.from, et_op.escrow_id).blurt_balance ==
                  ASSET("0.700 TESTS"));

    BOOST_TEST_MESSAGE(
        "--- success releasing disputed escrow with agent to 'from'");
    tx.clear();
    op.receiver = et_op.from;
    op.who = et_op.agent;
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE(
        "--- failure when 'to' attempts to release disputed expired escrow");
    generate_blocks(2);
    ...
    BOOST_TEST_MESSAGE(
        "--- failure when 'from' attempts to release disputed expired escrow");
    tx.clear();
    op.receiver = et_op.to;
    op.who = et_op.from;
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE(
        "--- success releasing disputed expired escrow with agent");
    tx.clear();
    op.receiver = et_op.from;
    op.who = et_op.agent;
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE(
        "--- success deleting escrow when balances are both zero");
    tx.clear();
    ...
    BLURT_REQUIRE_THROW(db->get_escrow(et_op.from, et_op.escrow_id),
                        fc::exception);
    ...
    BOOST_TEST_MESSAGE("--- failure when 'agent' attempts to release "
                       "non-disputed expired escrow to 'to'");
    tx.clear();
    op.receiver = et_op.to;
    op.who = et_op.agent;
    op.blurt_amount = ASSET("0.100 TESTS");
    ...
    BOOST_TEST_MESSAGE("--- failure when 'agent' attempts to release "
                       "non-disputed expired escrow to 'from'");
    tx.clear();
    op.receiver = et_op.from;
    tx.operations.push_back(op);
    sign(tx, sam_private_key);
    ...
    BOOST_TEST_MESSAGE("--- failure when 'agent' attempt to release "
                       "non-disputed expired escrow to not 'to' or 'from'");
    tx.clear();
    op.receiver = "dave";
    tx.operations.push_back(op);
    sign(tx, sam_private_key);
    ...
    BOOST_TEST_MESSAGE("--- failure when 'to' attempts to release non-disputed "
                       "expired escrow to not 'from' or 'to'");
    tx.clear();
    op.receiver = "dave";
    tx.operations.push_back(op);
    sign(tx, bob_private_key);
    ...
    BOOST_TEST_MESSAGE(
        "--- success release non-disputed expired escrow to 'to' from 'to'");
    tx.clear();
    op.receiver = et_op.to;
    tx.operations.push_back(op);
    sign(tx, bob_private_key);
    ...
    BOOST_TEST_MESSAGE(
        "--- success release non-disputed expired escrow to 'from' from 'to'");
    tx.clear();
    op.receiver = et_op.from;
    tx.operations.push_back(op);
    sign(tx, bob_private_key);
    ...
    BOOST_TEST_MESSAGE("--- failure when 'from' attempts to release "
                       "non-disputed expired escrow to 'agent'");
    tx.clear();
    op.who = et_op.from;
    op.receiver = et_op.agent;
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE("--- failure when 'from' attempts to release "
                       "non-disputed expired escrow to not 'from' or 'to'");
    tx.clear();
    op.receiver = "dave";
    tx.operations.push_back(op);
    sign(tx, alice_private_key);
    ...
    BOOST_TEST_MESSAGE(
        "--- success release non-disputed expired escrow to 'to' from 'from'");
    tx.clear();
    op.receiver = et_op.to;
    tx.operations.push_back(op);
    sign(tx, alice_private_key);
    ...
    BOOST_TEST_MESSAGE("--- success release non-disputed expired escrow to "
                       "'from' from 'from'");
    tx.clear();
    op.receiver = et_op.from;
    tx.operations.push_back(op);
    ...
    BOOST_TEST_MESSAGE("--- success deleting escrow when balances are zero on "
                       "non-disputed escrow");
    tx.clear();
...
BOOST_AUTO_TEST_CASE(transfer_from_savings_apply) {
  try {
    ...
    BOOST_TEST_MESSAGE(
        "--- Success withdrawing TESTS to BLURT_TREASURY_ACCOUNT");
    ...
    BOOST_REQUIRE(db->get_account("alice").savings_withdraw_requests ==
                  op.request_id + 1);
    BOOST_REQUIRE(db->get_savings_withdraw("alice", op.request_id).from ==
                  op.from);
    ...
    BOOST_REQUIRE(
        ...
        op.memo);
    BOOST_REQUIRE(db->get_savings_withdraw("alice", op.request_id).request_id ==
                  op.request_id);
    BOOST_REQUIRE(db->get_savings_withdraw("alice", op.request_id).amount ==
                  op.amount);
    BOOST_REQUIRE(db->get_savings_withdraw("alice", op.request_id).complete ==
                  db->head_block_time() + BLURT_SAVINGS_WITHDRAW_TIME);
    validate_database();
    ...
    generate_blocks(db->head_block_time() + BLURT_SAVINGS_WITHDRAW_TIME -
                        fc::seconds(BLURT_BLOCK_INTERVAL),
                    true);
    ...
    BLURT_REQUIRE_THROW(db->push_transaction(tx, 0), fc::exception);
    BOOST_REQUIRE(db->get_account("alice").savings_withdraw_requests ==
                  BLURT_SAVINGS_WITHDRAW_REQUEST_LIMIT);
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(decline_voting_rights_apply) {
  try {
    ...
    const auto &request_idx =
        db->get_index<decline_voting_rights_request_index>()
            .indices()
            .get<by_account>();
    auto itr = request_idx.find(db->get_account("alice").name);
    BOOST_REQUIRE(itr != request_idx.end());
    BOOST_REQUIRE(itr->effective_date ==
                  db->head_block_time() + BLURT_OWNER_AUTH_RECOVERY_PERIOD);

    BOOST_TEST_MESSAGE(
        "--- failure revoking voting rights with existing request");
    generate_block();
    tx.clear();
    tx.operations.push_back(op);
    tx.set_expiration(db->head_block_time() + BLURT_MAX_TIME_UNTIL_EXPIRATION);
    ...
    witness_create("alice", alice_private_key, "foo.bar",
                   alice_private_key.get_public_key(), 0);
    ...
    BOOST_TEST_MESSAGE(
        "--- check account cannot vote after request is processed");
    generate_block();
    BOOST_REQUIRE(!db->get_account("alice").can_vote);
    validate_database();
    ...
    const auto &witness_idx =
        db->get_index<witness_vote_index>().indices().get<by_account_witness>();
    auto witness_itr = witness_idx.find(boost::make_tuple(
        db->get_account("alice").name, db->get_witness("alice").owner));
...
BOOST_AUTO_TEST_CASE(claim_reward_balance_apply) {
  try {
      ...
      db.modify(db.get_dynamic_global_properties(),
                [](dynamic_global_property_object &gpo) {
                  ...
                  gpo.pending_rewarded_vesting_shares +=
                      ASSET("10.000000 VESTS");
                  gpo.pending_rewarded_vesting_blurt += ASSET("10.000 TESTS");
                });
    });

    generate_block();
    BOOST_REQUIRE(db->get_account("alice").reward_blurt_balance ==
                  ASSET("10.000 TESTS"));
    ...
    BOOST_TEST_MESSAGE("--- Attempting to claim more BLURT than exists in the "
                       "reward balance.");
    ...
    BOOST_REQUIRE(db->get_account("alice").balance ==
                  alice_steem + op.reward_blurt);
    BOOST_REQUIRE(db->get_account("alice").reward_blurt_balance ==
                  ASSET("10.000 TESTS"));
    BOOST_REQUIRE(db->get_account("alice").vesting_shares ==
                  alice_vests + op.reward_vests);
    BOOST_REQUIRE(db->get_account("alice").reward_vesting_balance ==
                  ASSET("5.000000 VESTS"));
    BOOST_REQUIRE(db->get_account("alice").reward_vesting_blurt ==
                  ASSET("5.000 TESTS"));
    validate_database();
...
BOOST_AUTO_TEST_CASE(delegate_vesting_shares_apply) {
  try {
    ...
    //      util::manabar old_downvote_manabar = db->get_account( "alice"
    //      ).downvote_manabar; params.max_mana =
    //      util::get_effective_vesting_shares( db->get_account( "alice" ) ) /
    //      4; old_downvote_manabar.regenerate_mana( params,
    //      db->head_block_time() );

    util::manabar old_bob_manabar = db->get_account("bob").voting_manabar;
    params.max_mana =
        util::get_effective_vesting_shares(db->get_account("bob"));
    old_bob_manabar.regenerate_mana(params, db->head_block_time());

    //      util::manabar old_bob_downvote_manabar = db->get_account( "bob"
    //      ).downvote_manabar; params.max_mana =
    //      util::get_effective_vesting_shares( db->get_account( "bob" ) ) / 4;
    //      old_bob_downvote_manabar.regenerate_mana( params,
    //      db->head_block_time() );
    ...
    BOOST_REQUIRE(alice_acc.delegated_vesting_shares ==
                  ASSET("100.000000 VESTS"));
    BOOST_REQUIRE(alice_acc.voting_manabar.current_mana ==
                  old_manabar.current_mana - op.vesting_shares.amount.value);
    //      BOOST_REQUIRE( alice_acc.downvote_manabar.current_mana ==
    //      old_downvote_manabar.current_mana - op.vesting_shares.amount.value /
    //      4 );
    BOOST_REQUIRE(bob_acc.received_vesting_shares == ASSET("100.000000 VESTS"));
    BOOST_REQUIRE(bob_acc.voting_manabar.current_mana ==
                  old_bob_manabar.current_mana +
                      op.vesting_shares.amount.value);
    //      BOOST_REQUIRE( bob_acc.downvote_manabar.current_mana ==
    //      old_bob_downvote_manabar.current_mana +
    //      op.vesting_shares.amount.value / 4 );
    ...
    //      params.max_mana = util::get_effective_vesting_shares(
    //      db->get_account( "alice" ) ) / 4;
    ...
    //      old_bob_downvote_manabar = db->get_account( "bob"
    //      ).downvote_manabar; params.max_mana =
    //      util::get_effective_vesting_shares( db->get_account( "bob" ) ) / 4;
    //      old_bob_downvote_manabar.regenerate_mana( params,
    //      db->head_block_time() );
    ...
    BOOST_REQUIRE(alice_acc.delegated_vesting_shares ==
                  ASSET("200.000000 VESTS"));
    BOOST_REQUIRE(alice_acc.voting_manabar.current_mana ==
                  old_manabar.current_mana - delta);
    //      BOOST_REQUIRE( alice_acc.downvote_manabar.current_mana ==
    //      old_downvote_manabar.current_mana - delta / 4 );
    BOOST_REQUIRE(bob_acc.received_vesting_shares == ASSET("200.000000 VESTS"));
    BOOST_REQUIRE(bob_acc.voting_manabar.current_mana ==
                  old_bob_manabar.current_mana + delta);
    //      BOOST_REQUIRE( bob_acc.downvote_manabar.current_mana ==
    //      old_bob_downvote_manabar.current_mana + delta / 4 );
    ...
    BOOST_TEST_MESSAGE("--- Test that effective vesting shares is accurate and "
                       "being applied.");
    tx.operations.clear();
    tx.signatures.clear();
    ...
    BOOST_REQUIRE(alice_comment.net_rshares.value ==
                  old_manabar.current_mana -
                      db->get_account("bob").voting_manabar.current_mana -
                      BLURT_VOTE_DUST_THRESHOLD);
    BOOST_REQUIRE(itr->rshares ==
                  old_manabar.current_mana -
                      db->get_account("bob").voting_manabar.current_mana -
    ...
    BOOST_TEST_MESSAGE(
        "--- Testing failure delegating more vesting shares than account has.");
    tx.clear();
    ...
    BOOST_TEST_MESSAGE(
        "--- Testing failure delegating when there is not enough mana");

    generate_block();
    db_plugin->debug_update([=](database &db) {
      db.modify(db.get_account("sam"), [&](account_object &a) {
        a.voting_manabar.current_mana /= 100;
        a.voting_manabar.last_update_time =
            db.head_block_time().sec_since_epoch();
      });
    });
    ...
    //      BOOST_TEST_MESSAGE( "--- Testing failure delegating when there is
    //      not enough downvote mana" );
    //
    //      generate_block();
    //      db_plugin->debug_update( [=]( database& db )
    //      {
    ...
    //            util::get_effective_vesting_shares( a );
    //            a.voting_manabar.last_update_time =
    //            db.head_block_time().sec_since_epoch();
    //            a.downvote_manabar.current_mana =
    //            a.downvote_manabar.current_mana * 3 / 4;
    //            a.downvote_manabar.last_update_time =
    //            db.head_block_time().sec_since_epoch();
    //         });
    //      });
    ...
    //      tx.operations.push_back( op );
    //      sign( tx, sam_private_key );
    //      BLURT_REQUIRE_THROW( db->push_transaction( tx ),
    //      fc::assert_exception );

    //      BOOST_TEST_MESSAGE( "--- Test failure delegating vesting shares that
    //      are part of a power down" ); generate_block();
    //      db_plugin->debug_update( [=]( database& db )
    //      {
    //         db.modify( db.get_account( "sam" ), [&]( account_object& a )
    //         {
    //            a.downvote_manabar.current_mana =
    //            util::get_effective_vesting_shares( a ) / 4;
    //            a.downvote_manabar.last_update_time =
    //            db.head_block_time().sec_since_epoch();
    //         });
    //      });
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure powering down vesting shares that are delegated");
    ...
    BOOST_TEST_MESSAGE(
        "--- Remove a delegation and ensure it is returned after 1 week");

    vest(BLURT_INIT_MINER_NAME, "sam", ASSET("1000.000 TESTS"));

    op.vesting_shares =
        asset(db->get_account("sam").vesting_shares.amount / 10, VESTS_SYMBOL);
    tx.clear();
    tx.operations.push_back(op);
    sign(tx, sam_private_key);
    db->push_transaction(tx, 0);
    ...
    util::manabar_params sam_params(
        util::get_effective_vesting_shares(db->get_account("sam")),
        BLURT_VOTING_MANA_REGENERATION_SECONDS);
    util::manabar_params dave_params(
        util::get_effective_vesting_shares(db->get_account("dave")),
        BLURT_VOTING_MANA_REGENERATION_SECONDS);
    ...
    auto exp_obj =
        db->get_index<vesting_delegation_expiration_index, by_id>().begin();
    auto end =
        db->get_index<vesting_delegation_expiration_index, by_id>().end();
    auto gpo = db->get_dynamic_global_properties();

    BOOST_REQUIRE(gpo.delegation_return_period ==
                  BLURT_DELEGATION_RETURN_PERIOD_HF20);

    BOOST_REQUIRE(exp_obj != end);
    BOOST_REQUIRE(exp_obj->delegator == "sam");
    //      BOOST_REQUIRE( exp_obj->vesting_shares == sam_vest );
    BOOST_REQUIRE(exp_obj->expiration ==
                  db->head_block_time() + gpo.delegation_return_period);
    //      BOOST_REQUIRE( db->get_account( "sam" ).delegated_vesting_shares ==
    //      sam_vest );
    BOOST_REQUIRE(db->get_account("dave").received_vesting_shares ==
                  ASSET("0.000000 VESTS"));
    delegation = db->find<vesting_delegation_object, by_delegation>(
        boost::make_tuple(op.delegator, op.delegatee));
    BOOST_REQUIRE(delegation == nullptr);
    ...
    BOOST_REQUIRE(db->get_account("sam").voting_manabar.current_mana ==
                  old_sam_manabar.current_mana);
    //      BOOST_REQUIRE( db->get_account( "dave" ).voting_manabar.current_mana
    //      == old_dave_manabar.current_mana - sam_vest.amount.value );
    ...
    BOOST_REQUIRE(db->get_account("sam").delegated_vesting_shares ==
                  ASSET("0.000000 VESTS"));
    //      BOOST_REQUIRE( db->get_account( "sam" ).voting_manabar.current_mana
    //      == old_sam_manabar.current_mana + sam_vest.amount.value );
    BOOST_REQUIRE(db->get_account("dave").voting_manabar.current_mana ==
                  old_dave_manabar.current_mana);
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(comment_beneficiaries_validate) {
  try {
    ...
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("bob"), BLURT_1_PERCENT * 75));
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("sam"), BLURT_1_PERCENT * 75));
    op.extensions.clear();
    op.extensions.insert(b);
    BLURT_REQUIRE_THROW(op.validate(), fc::assert_exception);
    ...
    for (size_t i = 0; i < 127; i++) {
      b.beneficiaries.push_back(beneficiary_route_type(
          account_name_type("foo" + fc::to_string(i)), 1));
    }
    ...
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("bar"), 1));
    std::sort(b.beneficiaries.begin(), b.beneficiaries.end());
    op.extensions.clear();
    op.extensions.insert(b);
    BLURT_REQUIRE_THROW(op.validate(), fc::assert_exception);
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back(
        beneficiary_route_type("bob", BLURT_1_PERCENT * 2));
...
BOOST_AUTO_TEST_CASE(comment_beneficiaries_apply) {
  try {
    ...
    db_plugin->debug_update([=](database &db) {
      ...
      db.modify(db.get_account(BLURT_TREASURY_ACCOUNT),
                [=](account_object &a) { a.balance.amount.value = 0; });
    });
    ...
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("bob"), BLURT_1_PERCENT));

    for (size_t i = 0; i < 8; i++) {
      b.beneficiaries.push_back(beneficiary_route_type(
          account_name_type(BLURT_INIT_MINER_NAME + fc::to_string(i)),
          BLURT_1_PERCENT));
    }
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("doug"), BLURT_1_PERCENT));
    op.extensions.clear();
    op.extensions.insert(b);
    tx.clear();
    tx.operations.push_back(op);
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back(beneficiary_route_type(
        account_name_type(BLURT_TREASURY_ACCOUNT), 10 * BLURT_1_PERCENT));
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("bob"), 25 * BLURT_1_PERCENT));
    b.beneficiaries.push_back(
        beneficiary_route_type(account_name_type("sam"), 50 * BLURT_1_PERCENT));
    op.extensions.clear();
    op.extensions.insert(b);
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back(beneficiary_route_type(account_name_type("dave"),
                                                     25 * BLURT_1_PERCENT));
    op.extensions.clear();
    op.extensions.insert(b);
    sign(tx, alice_private_key);
    ...
    BOOST_REQUIRE(db->get_account("bob").reward_vesting_blurt.amount +
                      db->get_account("sam").reward_vesting_blurt.amount +
                      db->get_account(BLURT_TREASURY_ACCOUNT).balance.amount ==
                  db->get_comment("alice", string("test"))
                      .beneficiary_payout_value.amount);
    BOOST_REQUIRE((db->get_account("alice").reward_vesting_blurt.amount +
                   db->get_account(BLURT_TREASURY_ACCOUNT).balance.amount) ==
                  db->get_account("bob").reward_vesting_blurt.amount + 1);
    BOOST_REQUIRE((db->get_account("alice").reward_vesting_blurt.amount +
                   db->get_account(BLURT_TREASURY_ACCOUNT).balance.amount) ==
                  (db->get_account("sam").reward_vesting_blurt.amount) / 2 + 1);
    BOOST_REQUIRE(db->get_account("bob").reward_vesting_blurt.amount ==
                  115409207);
    BOOST_REQUIRE(db->get_account("sam").reward_vesting_blurt.amount ==
                  230818414);
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(witness_set_properties_validate) {
  try {
    ...
    prop_op.props["key"] =
        fc::raw::pack_to_vector(signing_key.get_public_key());
    prop_op.validate();

    BOOST_TEST_MESSAGE(
        "--- failure when setting account_creation_fee with incorrect symbol");
    prop_op.props["account_creation_fee"] =
        fc::raw::pack_to_vector(asset(2000, VESTS_SYMBOL));
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);

    BOOST_TEST_MESSAGE("--- failure when setting maximum_block_size below "
                       "BLURT_MIN_BLOCK_SIZE_LIMIT");
    prop_op.props.erase("account_creation_fee");
    prop_op.props["maximum_block_size"] =
        fc::raw::pack_to_vector(BLURT_MIN_BLOCK_SIZE_LIMIT - 1);
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);
    ...
    BOOST_TEST_MESSAGE(
        "--- failure when setting new url with non UTF-8 character");
    ...
    prop_op.props["key"] =
        fc::raw::pack_to_vector(signing_key.get_public_key());
    prop_op.props["account_subsidy_budget"] =
        fc::raw::pack_to_vector(int32_t(5000));
    prop_op.validate();

    BOOST_TEST_MESSAGE("--- failure when budget is zero");
    prop_op.props["account_subsidy_budget"] =
        fc::raw::pack_to_vector(int32_t(0));
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);
    ...
    prop_op.props["account_subsidy_budget"] =
        fc::raw::pack_to_vector(int32_t(268435455));
    prop_op.validate();

    BOOST_TEST_MESSAGE(
        "--- failure when account subsidy budget is just a little too big");
    prop_op.props["account_subsidy_budget"] =
        fc::raw::pack_to_vector(int32_t(268435456));
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);

    BOOST_TEST_MESSAGE("--- failure when account subsidy budget is enormous");
    prop_op.props["account_subsidy_budget"] =
        fc::raw::pack_to_vector(int32_t(0x50000000));
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);
    ...
    prop_op.props["key"] =
        fc::raw::pack_to_vector(signing_key.get_public_key());
    prop_op.props["account_subsidy_decay"] =
        fc::raw::pack_to_vector(uint32_t(300000));
    prop_op.validate();

    BOOST_TEST_MESSAGE("--- failure when account subsidy decay is zero");
    prop_op.props["account_subsidy_decay"] =
        fc::raw::pack_to_vector(uint32_t(0));
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);

    BOOST_TEST_MESSAGE("--- failure when account subsidy decay is very small");
    prop_op.props["account_subsidy_decay"] =
        fc::raw::pack_to_vector(uint32_t(40));
    BLURT_REQUIRE_THROW(prop_op.validate(), fc::assert_exception);
    ...
    prop_op.props["account_subsidy_decay"] =
        fc::raw::pack_to_vector(uint32_t(unit / BLURT_BLOCKS_PER_YEAR));
    prop_op.validate();

    BOOST_TEST_MESSAGE("--- success when account subsidy decay is one day");
    prop_op.props["account_subsidy_decay"] =
        fc::raw::pack_to_vector(uint32_t(unit / BLURT_BLOCKS_PER_DAY));
    prop_op.validate();

    BOOST_TEST_MESSAGE("--- success when account subsidy decay is one hour");
    prop_op.props["account_subsidy_decay"] = fc::raw::pack_to_vector(
        uint32_t(unit / ((60 * 60) / BLURT_BLOCK_INTERVAL)));
    prop_op.validate();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE(witness_set_properties_authorities) {
  try {
    ...
    op.owner = "alice";
    op.props["key"] =
        fc::raw::pack_to_vector(generate_private_key("key").get_public_key());
    ...
    expected_keys.push_back(
        authority(1, generate_private_key("key").get_public_key(), 1));
...
BOOST_AUTO_TEST_CASE(witness_set_properties_apply) {
  try {
    ...
    prop_op.props["key"] =
        fc::raw::pack_to_vector(signing_key.get_public_key());
    prop_op.props["account_creation_fee"] =
        fc::raw::pack_to_vector(ASSET("2.000 TESTS"));
    tx.clear();
    ...
    BOOST_TEST_MESSAGE(
        "--- Testing failure when 'key' does not match witness signing key");
    prop_op.props.erase("extranious_property");
    prop_op.props["key"].clear();
    prop_op.props["key"] =
        fc::raw::pack_to_vector(old_signing_key.get_public_key());
    tx.clear();
    tx.operations.push_back(prop_op);
    sign(tx, old_signing_key);
    BLURT_REQUIRE_THROW(db->push_transaction(tx, 0), fc::assert_exception);
    ...
    prop_op.props["key"] =
        fc::raw::pack_to_vector(signing_key.get_public_key());
    prop_op.props["account_subsidy_budget"] =
        fc::raw::pack_to_vector(BLURT_ACCOUNT_SUBSIDY_PRECISION);
    tx.clear();
    tx.operations.push_back(prop_op);
    sign(tx, signing_key);
    db->push_transaction(tx, 0);
    BOOST_REQUIRE(alice_witness.props.account_subsidy_budget ==
                  BLURT_ACCOUNT_SUBSIDY_PRECISION);

    BOOST_TEST_MESSAGE("--- Testing setting account subsidy pool cap");
    uint64_t day_decay =
        (uint64_t(1) << BLURT_RD_DECAY_DENOM_SHIFT) / BLURT_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE(claim_account_apply) {
  try {
    ...
    auto get_subsidy_pools = [&](int64_t &con_subs, int64_t &ncon_subs) {
      ...
      ncon_subs = db->get<plugins::rc::rc_pool_object>()
                      .pool_array[plugins::rc::resource_new_accounts];
    };
    ...
    // generate a half hour worth of blocks to warm up the per-witness pools,
    // etc.
    ...
    int64_t prev_c_subs = 0, prev_nc_subs = 0, c_subs = 0, nc_subs = 0;
    ...
    int64_t new_value =
        prev_c_subs -
        BLURT_ACCOUNT_SUBSIDY_PRECISION; // Usage applied before decay
    new_value =
        new_value + 5000                                           // Budget
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure claiming a partial subsidized account");
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure with no available subsidized accounts");
    generate_block();
    db_plugin->debug_update([=](database &db) {
      db.modify(db.get_dynamic_global_properties(),
                [&](dynamic_global_property_object &gpo) {
...
BOOST_AUTO_TEST_CASE(create_claimed_account_apply) {
  try {
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when creator has not claimed an account");
    op.creator = "alice";
    op.new_account_name = "bob";
    op.owner = authority(1, priv_key.get_public_key(), 1);
    op.active = authority(2, priv_key.get_public_key(), 2);
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure creating account with non-existent account auth");
    generate_block();
    db_plugin->debug_update([=](database &db) {
      db.modify(db.get_account("alice"),
                [&](account_object &a) { a.pending_claimed_accounts = 2; });
    });
    generate_block();
    ...
    BOOST_REQUIRE(bob_auth.active ==
                  authority(2, priv_key.get_public_key(), 2));
    BOOST_REQUIRE(bob_auth.posting ==
                  authority(3, priv_key.get_public_key(), 3));
    ...
    generate_block();
    db_plugin->debug_update([=](database &db) {
      db.modify(db.get_account(BLURT_TEMP_ACCOUNT),
                [&](account_object &a) { a.pending_claimed_accounts = 1; });
    });
    generate_block();
    op.creator = BLURT_TEMP_ACCOUNT;
    ...
    BOOST_REQUIRE(db->get_account("charlie").recovery_account ==
                  account_name_type());
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(account_auth_tests) {
  try {
    ...
    private_key_type bob_posting_private_key =
        generate_private_key("bob_posting");
    private_key_type charlie_active_private_key = charlie_private_key;
    private_key_type charlie_posting_private_key =
        generate_private_key("charlie_posting");
    ...
    db_plugin->debug_update([=](database &db) {
      ...
      db.modify(db.get<account_authority_object, by_account>("bob"),
                [&](account_authority_object &auth) {
                  auth.posting =
                      authority(1, bob_posting_private_key.get_public_key(), 1);
                });

      db.modify(db.get<account_authority_object, by_account>("charlie"),
                [&](account_authority_object &auth) {
                  auth.posting = authority(
                      1, charlie_posting_private_key.get_public_key(), 1);
                });
    });
...
BOOST_AUTO_TEST_CASE(account_update_validate) {
  try {
    ...
    BOOST_TEST_MESSAGE(
        " -- Testing failure when posting_json_metadata is not json");
...
BOOST_AUTO_TEST_CASE(account_update_apply) {
  try {
...
#ifndef IS_LOW_MEM
    const account_metadata_object &acct_metadata =
        db->get<account_metadata_object, by_account>(acct.id);
    ...
    BOOST_TEST_MESSAGE(
        "--- Test failure when account authority does not exist");
    tx.clear();
    op = account_update_operation();
    op.account = "alice";
    op.posting = authority();
