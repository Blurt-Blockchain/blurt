...
int64_t calc_proposals(const PROPOSAL_IDX &proposal_idx,
                       const std::vector<int64_t> &proposals_id) {
  auto cnt = 0;
...
int64_t calc_proposal_votes(const PROPOSAL_VOTE_IDX &proposal_vote_idx,
                            uint64_t proposal_id) {
  auto cnt = 0;
  auto found = proposal_vote_idx.find(proposal_id);
  while (found != proposal_vote_idx.end() &&
         static_cast<size_t>(found->proposal_id) == proposal_id) {
    ++cnt;
    ++found;
  }
  return cnt;
...
template <typename PROPOSAL_VOTE_IDX>
int64_t calc_votes(const PROPOSAL_VOTE_IDX &proposal_vote_idx,
                   const std::vector<int64_t> &proposals_id) {
  auto cnt = 0;
  for (auto id : proposals_id)
...
BOOST_AUTO_TEST_CASE(generating_payments) {
  try {
    ...
    //      auto hourly_pay = ASSET( "1.996 TBD" );// hourly_pay != ASSET(
    //      "2.000 TBD" ) because lack of rounding
    //
    ...
    //      int64_t id_proposal_00 = create_proposal( creator, receiver,
    //      start_date, end_date, daily_pay, alice_private_key );
    //      generate_blocks( 1 );
    //
    //      vote_proposal( voter_01, { id_proposal_00 }, true/*approve*/,
    //      carol_private_key ); generate_blocks( 1 );
    //
    ...
    //      generate_block( 5 );
    //      transfer( BLURT_INIT_MINER_NAME, BLURT_TREASURY_ACCOUNT, ASSET(
    //      "0.001 TBD" ) ); generate_block( 5 );
    //
    ...
    //      const account_object& _treasury = db->get_account(
    ...
    //         generate_blocks( 1 );
    //
    //         auto treasury_sbd_inflation = dgpo.current_sbd_supply -
    //         old_sbd_supply; auto after_creator_sbd_balance =
    //         _creator.sbd_balance; auto after_receiver_sbd_balance =
    //         _receiver.sbd_balance; auto after_voter_01_sbd_balance =
    //         _voter_01.sbd_balance; auto after_treasury_sbd_balance =
    //         _treasury.sbd_balance;
    //
    //         BOOST_REQUIRE( before_creator_sbd_balance ==
    //         after_creator_sbd_balance ); BOOST_REQUIRE(
    //         before_receiver_sbd_balance == after_receiver_sbd_balance -
    //         hourly_pay ); BOOST_REQUIRE( before_voter_01_sbd_balance ==
    //         after_voter_01_sbd_balance ); BOOST_REQUIRE(
    //         before_treasury_sbd_balance == after_treasury_sbd_balance -
    //         treasury_sbd_inflation + hourly_pay );
    //      }
    //
    //      validate_database();
  }
...
BOOST_AUTO_TEST_CASE(generating_payments_01) {
  try {
    ...
    auto paid = ASSET(
        "4.990 TESTS"); // paid != ASSET( "5.000 TBD" ) because lack of rounding

    FUND(BLURT_TREASURY_ACCOUNT, ASSET("5000000.000 TESTS"));
    //=====================preparing=====================
    for (int32_t i = 0; i < nr_proposals; ++i) {
      auto item = inits[i % inits.size()];
      proposals_id.push_back(create_proposal(item.account, item.account,
                                             start_date, end_date, daily_pay,
                                             item.key));
      generate_block();
    }
...
BOOST_AUTO_TEST_CASE(generating_payments_02) {
  try {
    BOOST_TEST_MESSAGE(
        "Testing: generating payments, but proposal are removed");

    ACTORS((a00)(a01)(a02)(a03)(a04)(a05)(a06)(a07)(a08)(a09)(a10)(a11)(a12)(
    ...
    std::vector<initial_data> inits = {
        {"a00", a00_private_key}, {"a01", a01_private_key},
        {"a02", a02_private_key}, {"a03", a03_private_key},
        {"a04", a04_private_key}, {"a05", a05_private_key},
        {"a06", a06_private_key}, {"a07", a07_private_key},
        {"a08", a08_private_key}, {"a09", a09_private_key},
        {"a10", a10_private_key}, {"a11", a11_private_key},
        {"a12", a12_private_key}, {"a13", a13_private_key},
        {"a14", a14_private_key}, {"a15", a15_private_key},
        {"a16", a16_private_key}, {"a17", a17_private_key},
        {"a18", a18_private_key}, {"a19", a19_private_key},
        {"a20", a20_private_key}, {"a21", a21_private_key},
        {"a22", a22_private_key}, {"a23", a23_private_key},
        {"a24", a24_private_key}, {"a25", a25_private_key},
        {"a26", a26_private_key}, {"a27", a27_private_key},
        {"a28", a28_private_key}, {"a29", a29_private_key},
        {"a30", a30_private_key}, {"a31", a31_private_key},
        {"a32", a32_private_key}, {"a33", a33_private_key},
        {"a34", a34_private_key}, {"a35", a35_private_key},
        {"a36", a36_private_key}, {"a37", a37_private_key},
        {"a38", a38_private_key}, {"a39", a39_private_key},
        {"a40", a40_private_key}, {"a41", a41_private_key},
        {"a42", a42_private_key}, {"a43", a43_private_key},
        {"a44", a44_private_key}, {"a45", a45_private_key},
        {"a46", a46_private_key}, {"a47", a47_private_key},
        {"a48", a48_private_key}, {"a49", a49_private_key}};
    ...
    const auto &proposal_idx =
        db->get_index<proposal_index>().indices().get<by_proposal_id>();
    const auto &proposal_vote_idx =
        db->get_index<proposal_vote_index>().indices().get<by_proposal_voter>();
    ...
    create_proposal(item_creator.account, item_creator.account, start_date,
                    end_date, ASSET("24.000 TESTS"), item_creator.key);
    generate_block();
...
BOOST_AUTO_TEST_CASE(generating_payments_03) {
  try {
    ...
    auto start_date = db->head_block_time();
    uint16_t interval = 0;
    std::vector<fc::microseconds> end_time_shift = {fc::hours(1), fc::hours(2),
                                                    fc::hours(3), fc::hours(4)};
    ...
    for (auto item : inits) {
      ...
      proposals_id.push_back(create_proposal(item.first, item.first, start_date,
                                             end_date, _pay, item.second));
      generate_block();
    ...
    */
    ...
    generate_blocks(start_date + end_time_shift[interval++] + fc::seconds(10),
                    false);
    /*
       `tester00` - got payout
       `tester01` - got payout
    ...
    */
    ilog("");
    payment_checker(
        {ASSET("0.998 TESTS"), ASSET("0.998 TESTS"), ASSET("0.000 TESTS")});
    // ideally: ASSET( "1.000 TBD" ), ASSET( "1.000 TBD" ), ASSET( "0.000 TBD" )
    // but there is lack of rounding

    {
      BOOST_TEST_MESSAGE("Setting proxy. The account `tester01` don't want to "
                         "vote. Every decision is made by account `tester00`");
    ...
    */
    ilog("");
    payment_checker(
        {ASSET("1.996 TESTS"), ASSET("0.998 TESTS"), ASSET("0.000 TESTS")});
    // ideally: ASSET( "2.000 TBD" ), ASSET( "1.000 TBD" ), ASSET( "0.000 TBD" )
    // but there is lack of rounding

    vote_proposal(tester02_account, {2}, true /*approve*/,
                  inits[tester02_account]);
    generate_block();
    ...
    */
    ilog("");
    payment_checker(
        {ASSET("2.994 TESTS"), ASSET("0.998 TESTS"), ASSET("2082.368 TESTS")});
    // ideally: ASSET( "3.000 TBD" ), ASSET( "1.000 TBD" ), ASSET( "2082.346
    // TBD" ) but there is lack of rounding

    {
      BOOST_TEST_MESSAGE("Proxy doesn't exist. Now proposal with id = 3 has "
                         "the most votes. This proposal grabs all payouts.");
      account_witness_proxy_operation op;
      op.account = tester01_account;
      op.proxy = "";
    ...
    generate_blocks(start_date + end_time_shift[interval++] + fc::seconds(10),
                    false);
    /*
       `tester00` - no payout, because is not enough money. `tester01` removed
       proxy and the most votes has `tester02`. Whole payout goes to `tester02`
       `tester01` - no payout, because is not enough money
       `tester02` - got payout, because voted for his proposal
    */
    ilog("");
    payment_checker(
        {ASSET("2.994 TESTS"), ASSET("0.998 TESTS"), ASSET("4164.873 TESTS")});
    // ideally: ASSET( "3.000 TBD" ), ASSET( "1.000 TBD" ), ASSET( "4164.824
    // TBD" ) but there is lack of rounding
...
BOOST_AUTO_TEST_CASE(proposals_maintenance) {
  try {
    ...
    int64_t id_proposal_00 =
        create_proposal(creator, receiver, start_date_00, end_date_00,
                        daily_pay, alice_private_key);
    generate_block();

    int64_t id_proposal_01 =
        create_proposal(creator, receiver, start_date_01, end_date_01,
                        daily_pay, alice_private_key);
    generate_block();

    int64_t id_proposal_02 =
        create_proposal(creator, receiver, start_date_02, end_date_02,
                        daily_pay, alice_private_key);
    generate_block();

    {
      ...
      generate_blocks(start_time +
                      fc::seconds(BLURT_PROPOSAL_MAINTENANCE_CLEANUP));
...
BOOST_AUTO_TEST_CASE(proposal_object_apply) {
  try {
    ...
    const account_object &before_treasury_account =
        db->get_account(BLURT_TREASURY_ACCOUNT);
    ...
    const auto &after_treasury_account =
        db->get_account(BLURT_TREASURY_ACCOUNT);
    ...
    const auto &proposal_idx =
        db->get_index<proposal_index>().indices().get<by_creator>();
...
BOOST_AUTO_TEST_CASE(proposal_vote_object_apply) {
  try {
    ...
    int64_t id_proposal_00 = create_proposal(
        creator, receiver, start_date, end_date, daily_pay, alice_private_key);

    signed_transaction tx;
    update_proposal_votes_operation op;
    const auto &proposal_vote_idx =
        db->get_index<proposal_vote_index>().indices().get<by_voter_proposal>();
...

    {
      ...
      auto found =
          proposal_vote_idx.find(boost::make_tuple(voter_01, id_proposal_00));
...
BOOST_AUTO_TEST_CASE(proposal_vote_object_01_apply) {
  try {
    ...
    int64_t id_proposal_00 =
        create_proposal(creator, receiver, start_date, end_date, daily_pay_00,
                        alice_private_key);
    int64_t id_proposal_01 =
        create_proposal(creator, receiver, start_date, end_date, daily_pay_01,
                        alice_private_key);
    ...
    {
      BOOST_TEST_MESSAGE("---Voting by `voter_01` for proposals( "
                         "`id_proposal_00`, `id_proposal_01` )---");
      op.voter = voter_01;
      op.proposal_ids.insert(id_proposal_00);
    ...
    int64_t id_proposal_02 =
        create_proposal(creator, receiver, start_date, end_date, daily_pay_02,
                        alice_private_key);
    ...
    {
      BOOST_TEST_MESSAGE(
          "---Voting by `voter_02` for proposals( `id_proposal_00`, "
          "`id_proposal_01`, `id_proposal_02` )---");
      op.voter = voter_02;
      op.proposal_ids.clear();
    ...
    {
      BOOST_TEST_MESSAGE(
          "---Unvoting by `voter_01` proposals( `id_proposal_00` )---");
      op.voter = voter_01;
      op.proposal_ids.clear();
      op.proposal_ids.insert(id_proposal_00);
      op.approve = false;
    ...
    {
      BOOST_TEST_MESSAGE(
          "---Unvoting by `voter_02` proposals( `id_proposal_00`, "
          "`id_proposal_01`, `id_proposal_02` )---");
      op.voter = voter_02;
      op.proposal_ids.clear();
      op.proposal_ids.insert(id_proposal_02);
    ...
    {
      BOOST_TEST_MESSAGE(
          "---Unvoting by `voter_01` proposals( `id_proposal_01` )---");
      op.voter = voter_01;
      op.proposal_ids.clear();
      op.proposal_ids.insert(id_proposal_01);
      op.approve = false;
...
BOOST_AUTO_TEST_CASE(create_proposal_000) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - all args are ok");
    ACTORS((alice)(bob))
    generate_block();
    ...
    {
      int64_t proposal =
          create_proposal(creator, receiver, start_date, end_date, daily_pay,
                          alice_private_key);
...
BOOST_AUTO_TEST_CASE(create_proposal_001) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - invalid creator");
    {
      create_proposal_data cpd(db->head_block_time());
      ACTORS((alice)(bob))
      generate_block();
      FUND(cpd.creator, ASSET("80.000 TESTS"));
      BLURT_REQUIRE_THROW(create_proposal("", cpd.receiver, cpd.start_date,
                                          cpd.end_date, cpd.daily_pay,
                                          alice_private_key),
                          fc::exception);
    }
    validate_database();
...
BOOST_AUTO_TEST_CASE(create_proposal_002) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - invalid receiver");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
    BLURT_REQUIRE_THROW(create_proposal(cpd.creator, "", cpd.start_date,
                                        cpd.end_date, cpd.daily_pay,
                                        alice_private_key),
                        fc::exception);
    validate_database();
...
BOOST_AUTO_TEST_CASE(create_proposal_003) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - invalid start date");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
    cpd.start_date = cpd.end_date + fc::days(2);
    BLURT_REQUIRE_THROW(create_proposal(cpd.creator, cpd.receiver,
...
BOOST_AUTO_TEST_CASE(create_proposal_005) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - invalid subject(empty)");
    ACTORS((alice)(bob))
    generate_block();
...
BOOST_AUTO_TEST_CASE(create_proposal_006) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - invalid subject(too long)");
    ACTORS((alice)(bob))
    generate_block();
    create_proposal_operation cpo;
    cpo.creator = "alice";
    ...
    cpo.end_date = cpo.start_date + fc::days(2);
    cpo.daily_pay = asset(100, BLURT_SYMBOL);
    cpo.subject = "very very very very very very long long long long long long "
...
BOOST_AUTO_TEST_CASE(create_proposal_008) {
  try {
    BOOST_TEST_MESSAGE("Testing: create proposal: opration arguments "
                       "validation - invalid daily payement (negative value)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(update_proposal_votes_000) {
  try {
    BOOST_TEST_MESSAGE("Testing: update proposal votes: opration arguments "
                       "validation - all ok (approve true)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob)(carol))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(update_proposal_votes_001) {
  try {
    BOOST_TEST_MESSAGE("Testing: update proposal votes: opration arguments "
                       "validation - all ok (approve false)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob)(carol))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(update_proposal_votes_003) {
  try {
    BOOST_TEST_MESSAGE("Testing: update proposal votes: opration arguments "
                       "validation - all ok (array with negative digits)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob)(carol))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(update_proposal_votes_004) {
  try {
    ...
    std::vector<int64_t> proposals = {proposal_1};
    BLURT_REQUIRE_THROW(
        vote_proposal("urp", proposals, false, carol_private_key),
        fc::exception);
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(update_proposal_votes_005) {
  try {
    BOOST_TEST_MESSAGE(
        "Testing: update proposal votes: opration arguments validation - "
        "invalid id array (array with greater number of digits than allowed)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob)(carol))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_000) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- proposal removal (only one).");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_001) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- proposal removal (one from many).");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_002) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- proposal removal (n from many in two steps).");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
    ...
    for (int i = 0; i < 6; i++) {
      proposal =
          create_proposal(cpd.creator, cpd.receiver, cpd.start_date,
                          cpd.end_date, cpd.daily_pay, alice_private_key);
      BOOST_REQUIRE(proposal >= 0);
    ...
    flat_set<int64_t> proposals_to_erase = {proposals[0], proposals[1],
...
BOOST_AUTO_TEST_CASE(remove_proposal_003) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- proper proposal deletion check (one at time).");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_004) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- proper proposal deletion check (two at one time).");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_005) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- proposal with votes removal (only one).");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_006) {
  try {
    BOOST_TEST_MESSAGE(
        "Testing: remove proposal: basic verification operation - remove "
        "proposal with votes and one voteless at same time.");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_007) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: basic verification operation "
                       "- remove proposals with votes at same time.");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob)(carol))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_008) {
  try {
    BOOST_TEST_MESSAGE(
        "Testing: remove proposal: opration arguments validation - all ok");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
...
BOOST_AUTO_TEST_CASE(remove_proposal_009) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: opration arguments "
                       "validation - invalid deleter");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
    ...
    flat_set<int64_t> proposals = {proposal_1};
    BLURT_REQUIRE_THROW(
        remove_proposal(cpd.receiver, proposals, bob_private_key),
        fc::exception);
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(remove_proposal_010) {
  try {
    BOOST_TEST_MESSAGE("Testing: remove proposal: opration arguments "
                       "validation - invalid array(empty array)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
    ...
    BLURT_REQUIRE_THROW(
        remove_proposal(cpd.creator, proposals, bob_private_key),
        fc::exception);
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE(remove_proposal_011) {
  try {
    BOOST_TEST_MESSAGE(
        "Testing: remove proposal: opration arguments validation - invalid "
        "array(array with greater number of digits than allowed)");
    create_proposal_data cpd(db->head_block_time());
    ACTORS((alice)(bob))
    generate_block();
    FUND(cpd.creator, ASSET("80.000 TESTS"));
    ...
    flat_set<int64_t> proposals;
    for (int i = 0; i <= BLURT_PROPOSAL_MAX_IDS_NUMBER; i++) {
      proposals.insert(create_proposal(cpd.creator, cpd.receiver,
                                       cpd.start_date, cpd.end_date,
                                       cpd.daily_pay, alice_private_key));
    }
    BLURT_REQUIRE_THROW(
...
BOOST_AUTO_TEST_CASE(proposals_maintenance_01) {
  try {
    ...
    for (int32_t i = 0; i < nr_proposals; ++i) {
      auto item = inits[i % inits.size()];
      proposals_id.push_back(create_proposal(item.account, receiver,
                                             start_date_00, end_date_00,
                                             daily_pay, item.key));
      generate_block();
    }
    ...
    BOOST_REQUIRE(calc_proposals(proposal_idx, proposals_id) ==
    ...
    generate_blocks(start_time +
                    (start_time_shift + end_time_shift - block_interval));
...
BOOST_AUTO_TEST_CASE(proposals_maintenance_02) {
  try {
    BOOST_TEST_MESSAGE(
        "Testing: removing of old proposals + votes using threshold");
    ...
    auto current_active_votes =
        current_active_proposals * static_cast<int16_t>(inits.size());
    BOOST_REQUIRE(calc_votes(proposal_vote_idx, proposals_id) ==
    ...
    auto threshold = db->get_sps_remove_threshold();
    auto current_active_anything =
        current_active_proposals + current_active_votes;
...
BOOST_AUTO_TEST_CASE(proposals_removing_with_threshold) {
  try {
    ...
    auto item_creator = inits[0];
    for (auto i = 0; i < nr_proposals; ++i) {
      proposals_id.push_back(create_proposal(item_creator.account, receiver,
                                             start_date_00, end_date_00,
                                             daily_pay, item_creator.key));
      generate_block();
    }
...
BOOST_AUTO_TEST_CASE(proposals_removing_with_threshold_01) {
  try {
    ...
    /*
       ...
       Total = nr_proposals * nr_votes_per_proposal + nr_proposals = 10 * 6 + 10
       = 70
    */
    ...
    */
    {
      remove_proposal(item_creator.account, {5, 6, 7 /*doesn't exist*/, 8, 9},
                      item_creator.key);
      auto found_proposals = calc_proposals(proposal_idx, proposals_id);
      auto found_votes = calc_votes(proposal_vote_idx, proposals_id);
      ...
      int32_t cnt = 0;
      for (auto id : proposals_id)
        cnt += (id == 0 || id == 4 || id == 8 || id == 9)
                   ? calc_proposal_votes(proposal_vote_idx, id)
                   : 0;
      BOOST_REQUIRE(cnt == found_votes);
    }
...
BOOST_AUTO_TEST_CASE(proposals_removing_with_threshold_02) {
  try {
    ...
    /*
       ...
       P0    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18
       v19 v20 v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36
       v37 v38 v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49 P1    v0 v1 v2 v3 v4
       ...
       v42 v43 v44 v45 v46 v47 v48 v49 P2    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
       v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28
       v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46
       v47 v48 v49 P3    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15
       v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33
       v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49 P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
       v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49

       Total number of elements to be removed:
       Total = nr_proposals * nr_votes_per_proposal + nr_proposals = 5 * 50 + 5
       = 255
    */

    /*
       rr    xx xx xx xx xx xx xx xx xx xx rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr P1    v0 v1 v2 v3 v4
       v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23
       v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41
       v42 v43 v44 v45 v46 v47 v48 v49 P2    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
       v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28
       v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46
       v47 v48 v49 rr    xx xx xx xx xx xx xx xx xx xx rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
       v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49
    */
    ...
    /*
       rr    xx xx xx xx xx xx xx xx xx xx xxx xxx xxx xxx xxx xxx xxx xxx xxx
       xxx rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr P1    v0 v1 v2 v3 v4
       v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23
       v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41
       v42 v43 v44 v45 v46 v47 v48 v49 P2    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
       v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28
       v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46
       v47 v48 v49 rr    xx xx xx xx xx xx xx xx xx xx xxx xxx xxx xxx xxx xxx
       xxx xxx xxx xxx rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
       v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49
    */
    ...
    /*
       xx    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx
       xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx P1    v0 v1 v2 v3 v4
       v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23
       v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41
       v42 v43 v44 v45 v46 v47 v48 v49 P2    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
       v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28
       v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46
       v47 v48 v49 xx    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx
       xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
       v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49
    */
    ...
    /*
       ...
       oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo P1    v0 v1 v2 v3 v4
       v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23
       v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41
       v42 v43 v44 v45 v46 v47 v48 v49 P2    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
       v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28
       v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46
       v47 v48 v49 oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
       v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49
    */
    ...
    */
    ...
    /*
       oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo rr    xx xx xx xx xx
       xx xx xx xx xx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr P2    v0 v1 v2 v3 v4 v5 v6 v7 v8 v9 v10
       v11 v12 v13 v14 v15 v16 v17 v18 v19 v20 v21 v22 v23 v24 v25 v26 v27 v28
       v29 v30 v31 v32 v33 v34 v35 v36 v37 v38 v39 v40 v41 v42 v43 v44 v45 v46
       v47 v48 v49 oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
    ...
    */
    ...
    /*
       ...
       xx xx xx xx xx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rr    xx xx xx xx xx xx xx xx xx xx xxx
       xxx xxx xxx xxx xxx xxx xxx xxx xxx rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr rrr
       rrr rrr rrr oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
    ...
    */
    ...
    /*
       oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo oo    oo oo oo oo oo
       oo oo oo oo oo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo rr    oo oo oo oo oo oo oo oo oo oo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo rrr oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
    ...
    */
    ...
    /*
       ...
       oo oo oo oo oo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo xx    oo oo oo oo oo oo oo oo oo oo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo xxx oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo P4    v0
       v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 v11 v12 v13 v14 v15 v16 v17 v18 v19 v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
    ...
    */
    ...
    /*
       ...
       ooo ooo xxx oo    oo oo oo oo oo oo oo oo oo oo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo
       ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo ooo P4    xx
       xx xx xx xx xx xx xx xx xx xxx xxx xxx xxx xxx xxx xxx xxx xxx xxx v20
       v21 v22 v23 v24 v25 v26 v27 v28 v29 v30 v31 v32 v33 v34 v35 v36 v37 v38
       v39 v40 v41 v42 v43 v44 v45 v46 v47 v48 v49
    */
    {
...
BOOST_FIXTURE_TEST_SUITE(proposal_tests_performance,
                         sps_proposal_database_fixture_performance)
...
struct initial_data {
  ...
  initial_data(database_fixture *db, const std::string &_account)
    ...
    db->account_create(
        account, key.get_public_key(),
        db->generate_private_key(account + "_post").get_public_key());
  }
};

std::vector<initial_data> generate_accounts(database_fixture *db,
                                            int32_t number_accounts) {
...
BOOST_AUTO_TEST_CASE(proposals_removing_with_threshold_03) {
  try {
    BOOST_TEST_MESSAGE("Testing: removing of all proposals/votes in one block "
                       "using threshold = -1");
