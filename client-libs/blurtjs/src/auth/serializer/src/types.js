...
function damm_checksum_8digit(value) {
  ...
  const t = [
    0, 30, 10, 70, 50, 90, 80, 60, 40, 20, 70, 0, 90, 20, 10, 50, 40, 80, 60,
    30, 40, 20, 0, 60, 80, 70, 10, 30, 50, 90, 10, 70, 50, 0, 90, 80, 30, 40,
    20, 60, 60, 10, 20, 30, 0, 40, 50, 90, 70, 80, 30, 60, 70, 40, 20, 0, 90,
    50, 80, 10, 50, 80, 60, 90, 70, 20, 0, 10, 30, 40, 80, 90, 40, 50, 30, 60,
    20, 0, 10, 70, 90, 40, 30, 80, 60, 10, 70, 20, 0, 50, 20, 50, 80, 10, 40,
    30, 60, 70, 90, 0,
  ];
...
Types.asset = {
  fromByteBuffer(b) {
    ...
    if (precision >= 16) {
      ...
      symbol = new Buffer(b_copy.toBinary(), "binary")
        .toString()
        .replace(/\x00/g, "");
      b.skip(7);
...
Types.asset_symbol = {
  fromByteBuffer(b) {
    ...
    if (precision >= 16) {
      ...
      const symbol = new Buffer(b_copy.toBinary(), "binary")
        .toString()
        .replace(/\x00/g, "");
      if (symbol == "BLURT" || symbol == "TESTS") nai_string = "@@000000021";
      else if (symbol == "VESTS") nai_string = "@@000000037";
      else {
        throw new Error(
          "Expecting non-smt core asset symbol, instead got '" + symbol + "'"
        );
      }
      b.skip(7);
    }
  ...
  appendByteBuffer(b, object) {
    ...
    if (!object.nai.startsWith("@@")) {
      throw new Error(
        "Asset Symbols NAIs must be prefixed with '@@'. Was " + object.nai
      );
    }
