...
/**
 * This is temporary until nodes allow serialization with BLURT symbols
...
function makeSteemCompatible(type, operation) {
    if (type == 'limit_order_create') {
        ...
        operation.reward_blurt = toSteemSymbols(operation.reward_blurt);
...
export function* recoverAccount({
    ...
    try {
        ...
        const { json_metadata, posting_json_metadata } = account;
        // TODO: Investigate wrapping in a redux-saga call fn, so it can be tested!
        yield broadcast.sendAsync(
            {
                extensions: [],
