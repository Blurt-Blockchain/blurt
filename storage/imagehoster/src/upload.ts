...
if (new URL("http://blä.se").toString() !== "http://xn--bl-wia.se/") {
  throw new Error(
    "Incompatible node.js version, must be compiled with ICU support"
  );
}
...
export async function uploadHsHandler(ctx: KoaContext) {
  ...
  APIError.assert(
    (file.stream as any).truncated !== true,
    APIError.Code.PayloadTooLarge
  );
  ...
  if (
    ...
    signedMessage &&
    signedMessage.type &&
    ["login", "posting", "offline", "code", "refresh"].includes(
      signedMessage.type
    ) &&
    signedMessage.app
  ) {
    ...
    await cl.me(function (err: any, res: any) {
      if (!err && res) {
        ...
        APIError.assert(
          signedMessage.app === UPLOAD_LIMITS.app_account,
          APIError.Code.InvalidSignature
        );
        APIError.assert(
          res.scope.includes("comment"),
          APIError.Code.InvalidSignature
        );
    ...
    APIError.assert(
      !accountBlacklist.includes(account.name),
    ...
    APIError.assert(
      repLog10(account.reputation) >= UPLOAD_LIMITS.reputation,
      APIError.Code.Deplorable
    );

    const key =
      "D" + multihash.toB58String(multihash.encode(imageHash, "sha2-256"));
    ...
    ctx.log.info(
      { uploader: account.name, size: data.byteLength },
      "image uploaded"
    );
...
export async function uploadHandler(ctx: KoaContext) {
  ...
  const [account] = await rpcClient.database.getAccounts([
    ctx.params["username"],
  ]);
  APIError.assert(account, APIError.Code.NoSuchAccount);
